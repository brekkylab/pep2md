...
created: 08-Jan-2024
post_history:
- '`02-Jan-2024 <https://discuss.python.org/t/pre-pep-exposing-trusted-publisher-provenance-on-pypi/42337>`__'
...
- Changes to the currently unstandardized PyPI upload API, allowing
  ...
  `attestation objects <attestation-object>`{.interpreted-text
  role="ref"};
- Changes to the
  `HTML and JSON "simple" APIs <packaging:simple-repository-api>`{.interpreted-text
  role="ref"}, allowing clients to retrieve both digital attestations
  and [Trusted Publishing](https://docs.pypi.org/trusted-publishers/)
  metadata for individual release files as
  `provenance objects <provenance-object>`{.interpreted-text
  role="ref"}.
...
- Index verifiability: in the status quo, the only attestation provided
  by the index is an optional PGP signature per release file (see
  `PGP signatures <pgp-signatures>`{.interpreted-text role="ref"}).
  These signatures are not (and cannot be) checked by the index either
  for well-formedness or for validity, since the index has no mechanism
  for identifying the right public key for the signature. This PEP
  overcomes this limitation by ensuring that
  `provenance objects <provenance-object>`{.interpreted-text role="ref"}
  contain all of the metadata needed by the index to verify an
  attestation\'s validity.

This PEP proposes a generic attestation format, containing an
`attestation statement for signature generation <payload-and-signature-generation>`{.interpreted-text
role="ref"}, with the expectation that index providers adopt the format
...
2.  Verification by the index itself: in addition to enabling
    ...
    *ideally* verifiable in some form by the index itself.
    ...
    UX refinements on the index itself (such as a \"provenance\" view
    for each uploaded package).
...
4.  Metadata support: this PEP refers to \"digital attestations\" rather
    ...
    name and its contents, this PEP uses \'\`Statements
    \<https://github.com/in-toto/attestation/blob/v1.0/spec/v1.0/statement.md\>\`\_\_\'
    from the [in-toto project](https://in-toto.io/) to bind the
    distribution\'s contents (via SHA-256 digest) to its filename.
...
be retrieved by installing clients via the `data-gpg-sig` attribute in
the `503`{.interpreted-text role="pep"} API, the `gpg-sig` key on the
`691`{.interpreted-text role="pep"} API, or via an adjacent
`.asc`-suffixed URL.

PGP signature uploads have been disabled on PyPI since [May
2023](https://blog.pypi.org/posts/2023-05-23-removing-pgp/), after [an
investigation](https://blog.yossarian.net/2023/05/21/PGP-signatures-on-PyPI-worse-than-useless)
determined that the majority of signatures (which, themselves,
constituted a tiny percentage of overall uploads) could not be
associated with a public key or otherwise meaningfully verified.
...
`427`{.interpreted-text role="pep"} (and its
`living PyPA counterpart <packaging:binary-distribution-format>`{.interpreted-text
role="ref"}) specify the
`wheel format <packaging:Wheel>`{.interpreted-text role="term"}.
...
signatures are specified over a `376`{.interpreted-text role="pep"}
RECORD, which is modified to include a cryptographic digest for each
recorded file in the wheel.

While wheel signatures are fully specified, they do not appear to be
broadly used; the official [wheel
tooling](https://github.com/pypa/wheel) deprecated signature generation
and verification support [in
0.32.0](https://wheel.readthedocs.io/en/stable/news.html), which was
released in 2018.
...
## Upload endpoint changes {#upload-endpoint}
...
- Each attestation object **MUST** be verifiable by the index. If the
  ...
  reject the upload. The format of attestation objects is defined under
  `attestation-object`{.interpreted-text role="ref"} and the process for
  verifying attestations is defined under
  `attestation-verification`{.interpreted-text role="ref"}.
...
- When an uploaded file has one or more attestations, the index **MAY**
  ...
  JSON-encoded `provenance object <provenance-object>`{.interpreted-text
  role="ref"}, which **SHALL** contain the file\'s attestations.
...
- When a provenance file is present, the index **MAY** include a
  ...
  signaling the the file\'s provenance can be found at that URL. This
  URL **MUST** represent a [secure
  origin](https://www.chromium.org/Home/chromium-security/prefer-secure-origins-for-powerful-new-features/).
...
- The index **MAY** choose to modify the provenance file. For example,
  the index **MAY** permit adding additional attestations and
  verification materials, such as attestations from third-party auditors
  or other services.

  See `changes-to-provenance-objects`{.interpreted-text role="ref"} for
  an additional discussion of reasons why a file\'s provenance may
  change.

### JSON-based Simple API

The following changes are made to the
`JSON simple API <packaging:simple-repository-api-json>`{.interpreted-text
role="ref"}:
...
- When an uploaded file has one or more attestations, the index **MAY**
  ...
  `null`. If `provenance` is not `null`, it **SHALL** be a URL to the
  associated provenance file.

  See `appendix-3`{.interpreted-text role="ref"} for an explanation of
  the technical decision to embed the SHA-256 digest in the JSON API,
  rather than the full
  `provenance object <provenance-object>`{.interpreted-text role="ref"}.
...
in `appendix-2`{.interpreted-text role="ref"}. Attestation objects
**SHOULD** include one or more transparency log entries, and **MAY**
include additional keys for other sources of signed time (such as an
`3161`{.interpreted-text role="rfc"} Time Stamping Authority or a
[Roughtime](https://blog.cloudflare.com/roughtime) server).
...
- Certificates are specified as X.509 certificates, and comply with the
  profile in `5280`{.interpreted-text role="rfc"}.
...
### Attestation statement and signature generation {#payload-and-signature-generation}

The *attestation statement* is the actual claim that is
cryptographically signed over within the attestation object (i.e., the
`envelope.statement`).

The attestation statement is encoded as a [v1 in-toto Statement
object](https://github.com/in-toto/attestation/blob/v1.0/spec/v1.0/statement.md),
...
JSON-encoded statement is provided in `appendix-4`{.interpreted-text
role="ref"}.
...
- `subject[0].name` is the distribution\'s filename, which **MUST** be a
  valid
  `source distribution <packaging:source-distribution-format>`{.interpreted-text
  role="ref"} or
  `wheel distribution <packaging:binary-distribution-format>`{.interpreted-text
  role="ref"} filename.
...
- The following `predicateType` values are supported:
  ...
  - [PyPI Publish
    Attestation](https://docs.pypi.org/attestations/publish/v1):
    `https://docs.pypi.org/attestations/publish/v1`

The signature over this statement is constructed using the [v1 DSSE
signature
protocol](https://github.com/secure-systems-lab/dsse/blob/v1.0.0/protocol.md),
with a `PAYLOAD_TYPE` of `application/vnd.in-toto+json` and a
`PAYLOAD_BODY` of the JSON-encoded statement above. No other
...
and JSON-based Simple API as described above, and will have the
following layout:
...
- `attestation_bundles` is a **required** JSON array, containing one or
  more \"bundles\" of attestations. Each bundle corresponds to a signing
  identity (such as a Trusted Publishing identity), and contains one or
  more attestation objects.
  ...
  - A `kind` key, which **MUST** be a JSON string that uniquely
    identifies the kind of Trusted Publisher.
  ...
  illustrative example of a publisher object is provided in
  `appendix-1`{.interpreted-text role="ref"}.
  ...
  upload time, as described in `upload-endpoint`{.interpreted-text
  role="ref"} and `changes-to-provenance-objects`{.interpreted-text
  role="ref"}.

### Changes to provenance objects

Provenance objects are *not* immutable, and may change over time.
Reasons for changes to the provenance object include but are not limited
to:

- Addition of new attestations for a pre-existing signing identity: the
  index **MAY** choose to allow additional attestations by pre-existing
  signing identities, such as newer attestation versions for already
  uploaded files.
- Addition of new signing identities and associated attestations: the
  index **MAY** choose to support attestations from sources other than
  the file\'s uploader, such as third-party auditors or the index
  itself. These attestations may be performed asynchronously, requiring
  the index to insert them into the provenance object *post facto*.
...
- `envelope.statement` is a valid in-toto v1 Statement, with a subject
  and digest that **MUST** match the distribution\'s filename and
  contents. For the distribution\'s filename, matching **MUST** be
  performed by parsing using the appropriate source distribution or
  wheel filename format, as the statement\'s subject may be equivalent
  but normalized.
...
entries. When verifying transparency entries, the verifier **MUST**
confirm that the inclusion time for each entry lies within the signing
...
specifying higher level security \"policies\" around attestation
validity, thresholds between attestations, and so forth.
...
- All algorithms are specified in a single suite, rather than a
  ...
  example) for an attacker to select a strong signature algorithm with a
  weak hash function, compromising the scheme as a whole.
- Attestation objects are versioned, and may only contain the
  algorithmic suite specified for their version. If a specific suite is
  considered insecure in the future, clients may choose to blanket
...
the index is still effectively trusted to honestly deliver unmodified
...
such as `458`{.interpreted-text role="pep"} and/or
...
# Appendix 1: Example Trusted Publisher Representation {#appendix-1}
...
# Appendix 2: Data models for Transparency Log Entries {#appendix-2}
...
# Appendix 3: Simple JSON API size considerations {#appendix-3}

A previous draft of this PEP required embedding each
`provenance object <provenance-object>`{.interpreted-text role="ref"}
directly into its appropriate part of the JSON Simple API.
...
# Appendix 4: Example attestation statement {#appendix-4}
