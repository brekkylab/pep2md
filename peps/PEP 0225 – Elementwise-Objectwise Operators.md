...
source_path: https://github.com/python/peps/blob/main/peps/pep-0225.rst
source_commit: 502ee6be2eb98a58f44c17ad28bfb03a7bed829f
---

::: rejected
The approach in the later `465`{.interpreted-text role="pep"} was
eventually accepted in lieu of this PEP. The `Rejected Ideas
<465#rejected-alternatives-to-adding-a-new-operator>`{.interpreted-text
role="pep"} of that PEP explains the rationale in more detail.
:::
...
3.  Use *shadow classes*. For matrix class define a shadow array class
    ...
    - Hard to maintain in current Python because ordinary numbers cannot
      have user defined class methods; i.e. `a.E*b` will fail if a is a
      pure number.
...
- The bracket type operators are most favorable when standing alone, but
  not in formulas, as they interfere with visual parsing of parentheses
  for precedence and function argument. This is so for `(op)` and
  `[op]`, and somewhat less so for `{op}` and `<op>`.
- The `<op>` form has the potential to be confused with `<` `>` and `=`.
- The `@op` is not favored because `@` is visually heavy (dense, more
  like a letter): `a@+b` is more readily read as `a@ + b` than `a @+ b`.
...
- Need for the `~+` `~-` operators. The objectwise `+` `-` are important
  because they provide important sanity checks as per linear algebra.
...
- Left division (solve). For matrix, `a*x` is not necessarily equal to
  ...
  different from the solution of `x*a==b`, denoted `x=div(b,a)`. There
  are discussions about finding a new symbol for solve. \[Background:
  MatLab use `b/a` for `div(b,a)` and `a\b` for `solve(a,b)`.\]
...
- Additional multiplication operators. Several forms of multiplications
  ...
  multiplication in linear algebra sense (such as Kronecker product).
  But two forms appear to be more fundamental: outer product and inner
  product. However, their specification includes indices, which can be
  either
  ...
  cover both outer and inner products, and specialize to linear algebra
  multiplication as well. The index rule can be defined as class
  methods, like:
...
- Bitwise operators.
  ...
  - The proposed new math operators use the symbol \~ that is *bitwise
    not* operator. This poses no compatibility problem but somewhat
  ...
  - The symbol `|` was suggested to be used for matrix solve. But the
    new solution of using delayed `.I` is better in several ways.
...
There is an alternative PEP (officially, `211`{.interpreted-text
role="pep"}) by Greg Wilson, titled \"Adding New Linear Algebra
Operators to Python\".

Its first (and current) version is at:

> <http://www.python.org/pipermail/python-dev/2000-August/014876.html>
> `211`{.interpreted-text role="pep"}
