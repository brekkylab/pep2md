...
source_path: https://github.com/python/peps/blob/main/peps/pep-0242.rst
source_commit: 341a827b78c672dc02988113fe42dea586340ca5
---
...
the Fortran 90 \"kind\" solution, adapted to the Python environment.
With this facility an entire calculation can be switched from one level
of precision to another by changing a single line. If the desired
precision does not exist on a particular machine, the program will fail
rather than get the wrong answer. Since coding in this style would
involve an early call to the routine that will fail, this is the next
best thing to not compiling.
...
floating point numbers as it likes, except that it must support at least
two kinds of integer corresponding to the existing int and long, and
must support at least one kind of floating point number, equivalent to
the present float.
...
string is actually a different \"kind\" of string and that a
...
:   For `nd >= 0` and `n >= 1`, return a callable object whose result is
    ...
    interval `[-n, n]`. The kind object accepts arguments that are
    integer or float.
