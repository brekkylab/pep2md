...
python_version: '3.12'
post_history:
- '`24-May-2022 <https://mail.python.org/archives/list/capi-sig@python.org/thread/SIP3VP7JU4OBWP62KBOYGOYCVIOTXEFH/>`__'
...
source_path: https://github.com/python/peps/blob/main/peps/pep-0697.rst
source_commit: c14edbc565ea5b6f616bc4f49ee3cb0727a88876
---

::: canonical-doc
`PyType_Spec.basicsize`{.interpreted-text
role="external+py3.12:c:member"},
`PyObject_GetTypeData`{.interpreted-text role="external+py3.12:c:func"},
`Py_TPFLAGS_ITEMS_AT_END`{.interpreted-text
role="external+py3.12:c:macro"}, `Py_RELATIVE_OFFSET`{.interpreted-text
role="external+py3.12:c:macro"},
`PyObject_GetItemData`{.interpreted-text role="external+py3.12:c:func"}
:::

# Abstract

Add [Limited C
API](https://docs.python.org/3.11/c-api/stable.html#stable-application-binary-interface)
support for extending some types with opaque data by allowing code to
only deal with data specific to a particular (sub)class.
...
`python:type`{.interpreted-text role="py:class"}).
...
(See `652`{.interpreted-text role="pep"} for the benefits of providing a
stable ABI.)
...
illustrate with code, the [example from the
tutorial](https://docs.python.org/3.11/extending/newtypes_tutorial.html#subclassing-other-types)
extends `PyListObject`{.interpreted-text role="external+python:c:type"}
(`python:list`{.interpreted-text role="py:class"}) using the following
`struct`:
...
- Variable-size objects are not handled (see [Extending variable-size
  objects](#extending-variable-size-objects) below).
...
2.  Given an instance, and the subclass `PyTypeObject*`, get a pointer
    to the `SubListState`. A new function, `PyObject_GetTypeData`, will
    be added for this.
...
`python:type`{.interpreted-text role="py:class"} (`PyHeapTypeObject`) or
third-party extensions (for example, NumPy arrays[^1]).
...
`variable-sized objects <PyVarObject>`{.interpreted-text
role="external+python:c:type"} while maintaining loose coupling: the
variable-sized data can collide with subclass data (`SubListState` in
the example above).
...
fixed offset. If subclasses need additional space, it must be added
after any variable-sized data:
...
[reference implementation](#reference-implementation) for draft docs).
::::
...
- `spec->basicsize == 0`: (Inheriting the basicsize)
  ...
  - `base->tp_itemsize > 0`: (Extending a variable-size class)
    - `spec->itemsize == 0`: The item size is inherited. (No change to
      status quo.)
...
- `spec->basicsize < 0`: (Extending the basicsize)
  - `base->tp_itemsize == 0`: (Extending a fixed-size class)
    ...
    - `spec->itemsize > 0`: Fail. (We\'d need to add an `ob_size`, which
      is only possible for trivial types \-- and the trivial layout must
      be known.)
  - `base->tp_itemsize > 0`: (Extending a variable-size class)
...
use a subclass-specific `struct` (`SubListState` above) will get a way
to specify "relative" offsets (offsets based from this `struct`) rather
than "absolute" ones (based off the `PyObject` struct).
...
objects](#extending-variable-size-objects) for a full explanation.)
...
`type->tp_base->tp_basicsize` and `type->tp_basicsize` offsets "belongs"
to `type` (except variable-length types). This is not documented
explicitly, but CPython up to version 3.11 relied on it when adding
`__dict__` to subclasses, so it should be safe.
...
point 2 in [this list](https://discuss.python.org/t/15993)), and [has
been verifying the
implementation](https://discuss.python.org/t/19743/14).

Florian from the HPy project
[said](https://discuss.python.org/t/19743/3) that the API looks good in
general. (See [below](#alignment-performance) for a possible solution to
...
A reference implementation is in the [extend-opaque
branch](https://github.com/python/cpython/compare/main...encukou:cpython:extend-opaque)
...
"tuple-like" layout described in [Extending variable-size
objects](#extending-variable-size-objects), and all mechanisms this PEP
proposes could be adapted to support it. Other layouts could be added as
well. However, it seems there\'d be very little practical benefit, so
...
[^1]: This PEP does not make it "safe" to subclass NumPy arrays
    specifically. NumPy publishes [an extensive list of
    caveats](https://numpy.org/doc/1.23/user/basics.subclassing.html)
    for subclassing its arrays from Python, and extending in C might
    need a similar list.
