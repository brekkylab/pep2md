...
author:
- Mark Mendoza <mendoza.mark.a@gmail.com>
...
::: canonical-doc
`typing:typevartuple`{.interpreted-text role="ref"} and
`typing.TypeVarTuple`{.interpreted-text role="py:class"}
:::

# Abstract

`484`{.interpreted-text role="pep"} introduced `TypeVar`, enabling
creation of generics parameterised with a single type. In this PEP, we
introduce `TypeVarTuple`, enabling parameterisation with an *arbitrary*
number of types - that is, a *variadic* type variable, enabling
...
computing libraries such as NumPy and TensorFlow to be parameterised
with the array *shape*, enabling static type checkers to catch
shape-related bugs in code that uses these libraries.
...
should pass for the `videos` argument. Possibilities include, for
example,
...
- **Documentation**. Without the required shape being clear in the
  signature, the user must hunt in the docstring or the code in question
  to determine what the input/output shape requirements are.
- **Catching shape bugs before runtime**. Ideally, use of incorrect
  shapes should be an error we can catch ahead of time using static
  analysis. (This is particularly important for machine learning code,
  where iteration times can be slow.)
- **Preventing subtle shape bugs**. In the worst case, use of the wrong
  ...
  subtle bug that can take days to track down. (See [this
  exercise](https://spinningup.openai.com/en/latest/spinningup/exercise2_2_soln.html)
  in a popular machine learning tutorial for a particularly pernicious
  example.)

Ideally, we should have some way of making shape requirements explicit
in type signatures. Multiple proposals[^5][^6] [^7] have suggested the
use of the standard generics syntax for this purpose. We would write:
...
This serves as a placeholder not for a single type but for a *tuple* of
types.
...
parameters of `Array`, we must *unpack* the type variable tuple using
the star operator: `*Shape`. The signature of `Array` then behaves as if
we had simply written `class Array(Generic[T1, T2]): ...`.
...
`Array[Height, Width]`, we could also define rank-3 arrays, rank-4
arrays, and so on:
...
- To avoid potential confusion about whether to use a type variable
  tuple in a packed or unpacked form (\"Hmm, should I write
  \'`-> Shape`\', or \'`-> Tuple[Shape]`\', or
  \'`-> Tuple[*Shape]`\'\...?\")
...
Python, we introduce the `Unpack` type operator that can be used in
place of the star operator:
...
values at runtime. `Tuple[int, *Tuple[bool, bool], str]` is equivalent
to `Tuple[int, bool, bool, str]`.
...
simpler form. This enables us to specify types such as
`Tuple[int, *Tuple[str, ...], str]` - a tuple type where the first
element is guaranteed to be of type `int`, the last element is
guaranteed to be of type `str`, and the elements in the middle are zero
or more elements of type `str`. Note that `Tuple[*Tuple[int, ...]]` is
...
`Array[*Tuple[Any, ...]]` stands for an array with an arbitrary number
of dimensions of type `Any`. This means that, in the call to
`expect_variadic_array`, `Batch` is bound to `Any` and `Shape` is bound
to `Tuple[Any, ...]`. In the call to `expect_precise_array`, the
variables `Batch`, `Height`, `Width`, and `Channels` are all bound to
`Any`.
...
from the type checker every time they tried to use the variable `y`,
which would hinder them when migrating a legacy code base to use
`TypeVarTuple`.
...
`484`{.interpreted-text role="pep"} states that when a type annotation
is provided for `*args`, every argument must be of the type annotated.
That is, if we specify `*args` to be type `int`, then *all* arguments
must be of type `int`. This limits our ability to specify the type
signatures of functions that take heterogeneous argument types.
...
`484#arbitrary-argument-lists-and-default-argument-values`{.interpreted-text
role="pep"} behavior of `*args: int`, which accepts zero or more values
of type `int`:
...
always be used unpacked, annotating `*args` as being a plain type
variable tuple instance is *not* allowed:
...
else, e.g. `Tuple[*Ts]`. If `*args` itself is annotated as `Tuple[*Ts]`,
...
Finally, note that a type variable tuple may *not* be used as the type
of `**kwargs`. (We do not yet know of a use case for this feature, so we
...
(For details, see the section on [Unpacking Unbounded Tuple
Types](#unpacking-unbounded-tuple-types).)

This way, even if libraries are updated to use types like
`Array[Height, Width]`, users of those libraries won\'t be forced to
also apply type annotations to all of their code; users still have a
choice about what parts of their code to type and which parts to not.
...
Importantly for our original `Array` example (see [Summary
Examples](#summary-examples)), this allows us to define convenience
aliases for arrays of a fixed shape or datatype:
...
when Type Parameters are not
Specified](#behaviour-when-type-parameters-are-not-specified)):
...
Here, `IntTuple` is a *non*-variadic generic alias that takes exactly
one type argument. Hence, it cannot accept `*Ts` or `*Tuple[float, ...]`
as type arguments, because they represent an arbitrary number of types.

### Aliases with Both TypeVars and TypeVarTuples

In [Aliases](#aliases), we briefly mentioned that aliases can be generic
in both `TypeVar`s and `TypeVarTuple`s:
...
In accordance with [Multiple Type Variable Tuples: Not
Allowed](#multiple-type-variable-tuples-not-allowed), at most one
...
used as a type argument to an alias consisting of both `TypeVar`s and a
`TypeVarTuple`:
...
`TypeVar`s and the `TypeVarTuple`. We assume the arbitrary-length tuple
contains at least as many items as there are `TypeVar`s, such that
...
Here, therefore, `Hamster` is equivalent to
`Tuple[*Tuple[int, ...], int]`: a tuple consisting of zero or more
`int`s, then a final `int`.
...
`Tuple[str, *Tuple[int, ...]]`. This expression is therefore equivalent
to `Tuple[str, *Tuple[int, ...], int]`: a tuple consisting of a `str`,
then zero or more `int`s, ending with an `int`.
...
same is not true of `TypeVarTuple`s in the argument list:
...
As noted in the introduction, it *is* possible to avoid variadic
generics by simply defining aliases for each possible number of type
parameters:
...
arguments to `TypeVar`. However, we realised this may constrain us in
the future, if for example we want type bounds or variance to function
...
the first two parts of the shape are often \'batch\' and \'time\' - but
we plan to support these cases explicitly in a future PEP with a syntax
...
issue of shape checking in numerical libraries is *not* the only
approach possible. Examples of lighter-weight alternatives based on
*runtime* checking include ShapeGuard[^9], tsanley[^10], and
PyContracts[^11].
...
verbose assert statements, none of them enable *static* analysis of
shape correctness. As mentioned in [Motivation](#motivation), this is
particularly desirable for machine learning applications where, due to
library and infrastructure complexity, even relatively simple programs
must suffer long startup times; iterating by running the program until
it crashes, as is necessary with these existing runtime-based
approaches, can be a tedious and frustrating experience.
...
`foo[a, *b, c]`, and `b.__iter__` produces an iterator yielding `d` and
`e`, `foo.__getitem__` would receive `(a, d, e, c)`.
...
`iter()` on the starred object, and inserting the results of the
resulting iterator into the list at the appropriate place. For
...
that the iterator is exhausted, and set that value as the annotation
value. This results in the unpacked `TypeVarTuple` being set directly as
the runtime annotation for `*args`:
...
representation that uses a `Starred` node for the annotations of
`args` - in turn important for tools that rely on the AST such as mypy
to correctly recognise the construction:
...
The second alternative would be to **use \`\`Unpack\`\` instead**,
requiring no grammar changes. However, we regard this as a suboptimal
solution for two reasons:

- **Readability**. `class Array(Generic[DType, Unpack[Shape]])` is a bit
  of a mouthful; the flow of reading is interrupted by length of
  `Unpack` and the extra set of square brackets.
...
- **Intuitiveness**. We think a user is more likely to intuitively
  ...
  is a TypeVar\*\*Tuple\*\* - than the meaning of `Unpack[Ts]`. (This
  assumes the user is familiar with star-unpacking in other contexts; if
  the user is reading or writing code that uses variadic generics, this
  seems reasonable.)
...
CPython is available in
[cpython/23527](https://github.com/python/cpython/pull/24527). A
preliminary version of the version using the star operator, based on an
early implementation of `637`{.interpreted-text role="pep"}, is also
available at
[mrahtz/cpython/pep637+646](https://github.com/mrahtz/cpython/tree/pep637%2B646).
...
another way, there\'s no relationship between the value of `K` in
...
the problem mentioned in [Motivation](#motivation): if one function
returns an array with leading dimensions \'Time × Batch\', and another
function takes the same array assuming leading dimensions \'Batch ×
Time\', we have no way of detecting this.
...
Note that in this case, names are *global* (to the extent that we use
the same `Batch` type in different place). However, because names refer
only to axis *types*, this doesn\'t constrain the *value* of certain
axes to be the same through (that is, this doesn\'t constrain all axes
named `Height` to have a value of, say, 480 throughout).
...
*selection*. For example, if we have an image stored in an array of
shape 64×64x3, we might wish to convert to black-and-white by computing
the mean over the third axis, `mean(image, axis=2)`. Unfortunately, the
simple typo `axis=1` is difficult to spot and will produce a result that
means something completely different (all while likely allowing the
...
axes are selected not by index but by label - e.g.
...
- **Static checking** of shape correctness is not possible. As mentioned
  in [Motivation](#motivation), this is a highly desirable feature in
  machine learning code where iteration times are slow by default.
- **Interface documentation** is still not possible with this approach.
  If a function should *only* be willing to take array arguments that
  have image-like shapes, this cannot be stipulated with named tensors.
...
the fact that axis ordering conventions are often strong enough that
axis names provide little benefit (e.g. when working with images, 3D
tensors are basically *always* height × width × channels). However,
ultimately we are still uncertain why this is the case.
...
`im: Array[Height=64, Width=64, Channels=3]` - but this won\'t be
possible in the short term, due to the rejection of
`637`{.interpreted-text role="pep"}. In any case, our attitude towards
this is mostly \"Wait and see what happens before taking any further
steps\".
...
> clarifications to the broader context of `646`{.interpreted-text
> role="pep"} for array typing in
...
> integrating the new `646`{.interpreted-text role="pep"} variadics into
> numpy.
>
...
> shape manipulation.
>
> All in all, I\'m very interested in where both `646`{.interpreted-text
> role="pep"} and future PEPs will take us and look forward to further
> developments.
...
System](https://github.com/tensorflow/community/pull/208):[^13]
...
> annotations (rather than the custom `input_signature` mechanism we
> have today - see this issue:
...
third popular numerical computing library, PyTorch, but did *not*
receive a statement of endorsement from them. Our understanding is that
although they are interested in some of the same issues - e.g. static
shape inference - they are currently focusing on enabling this through a
DSL rather than the Python type system.)
...
Inspired by this discussion, **Ivan Levkivskyi** made a concrete
proposal at PyCon 2019, summarised in notes on \'Type system
improvements\'[^15] and \'Static typing of Python numeric stack\'[^16].

Expanding on these ideas, **Mark Mendoza** and **Vincent Siles** gave a
presentation on \'Variadic Type Variables for Decorators and
Tensors\'[^17] at the 2019 Python Typing Summit.

Discussion over how type substitution in generic aliases should behave
took place in
[cpython#91162](https://github.com/python/cpython/issues/91162).
...
[^3]: We use the term \'array\' to refer to a matrix with an arbitrary
    number of dimensions. In NumPy, the corresponding class is the
    `ndarray`; in TensorFlow, the `Tensor`; and so on.

[^4]: If the shape begins with \'batch × time\', then
    `videos_batch[0][1]` would select the second frame of the first
    video. If the shape begins with \'time × batch\', then
    `videos_batch[1][0]` would select the same frame.
...
[^17]: Mark Mendoza, \'Variadic Type Variables for Decorators and
    Tensors\', Python Typing Summit 2019:
