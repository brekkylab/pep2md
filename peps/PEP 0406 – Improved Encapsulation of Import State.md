...
- Greg Slodkowicz <jergosh@gmail.com>
status: Withdrawn
...
source_path: https://github.com/python/peps/blob/main/peps/pep-0406.rst
source_commit: 2964b5af667074e676bbfc02990a8078fb71408e
---

# Abstract

This PEP proposes the introduction of a new \'ImportEngine\' class as
part of `importlib` which would encapsulate all state related to
importing modules into a single object. Creating new instances of this
object would then provide an alternative to completely replacing the
built-in implementation of the import statement, by overriding the
...
originally written, as part of `420`{.interpreted-text role="pep"} in
Python 3.3 and `451`{.interpreted-text role="pep"} in Python 3.4.

While providing an encapsulation of the import state is still highly
desirable, it is better tackled in a new PEP using
`451`{.interpreted-text role="pep"} as a foundation, and permitting only
the use of `451`{.interpreted-text role="pep"} compatible finders and
loaders (as those avoid many of the issues of direct manipulation of
...
The namespace PEPs (especially `402`{.interpreted-text role="pep"})
raise a potential need for *additional* process global state, in order
to correctly update package paths as `sys.path` is modified.
...
> > called on `importlib engine.sysengine`, `from_engine()` can be used
> > to create an `ImportEngine` object with a **copy** of the global
> > import state.
>
> `__import__(self, name, globals={}, locals={}, fromlist=[], level=0)`
>
> > Reimplementation of the builtin `__import__()` function. The import
> > of a module will proceed using the state stored in the ImportEngine
> > instance rather than the global import state. For full documentation
> > of `__import__` functionality, see[^2] . `__import__()` from
...
> Provides `__import__()`, `import_module()` and `from_engine()` methods
> like `ImportEngine` but writes through to the global state in `sys`.
...
Rather than attempting to update the `302`{.interpreted-text role="pep"}
APIs to accept additional state, this PEP proposes that `ImportEngine`
support the content management protocol (similar to the context
substitution mechanisms in the `decimal` module).
...
subclasses that blur the boundaries between the engine instance state
and the process global state in various ways.
...
A reference implementation[^4] for an earlier draft of this PEP, based
on Brett Cannon\'s importlib has been developed by Greg Slodkowicz as
part of the 2011 Google Summer of Code. Note that the current
...
That earlier draft of the PEP proposed change the
`302`{.interpreted-text role="pep"} APIs to support passing in an
optional engine instance. This had the (serious) downside of not
correctly affecting further imports from the imported module, hence the
change to the context management based proposal for substituting the
global state.
...
[^2]: \_\_import\_\_() builtin function, The Python Standard Library
    documentation
