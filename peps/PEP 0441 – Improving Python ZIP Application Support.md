...
promoted as part of Python 2.6[^2], so that it is relatively unknown,
but also because the Windows installer does not register a file
...
Zip Application\" were registered (in the mime types database?).
...
- The name of a directory, in which case a new application archive will
  be created from the content of that directory.
...
interpreter values? For example, `-p 3` meaning the same as
...
It was suggested[^5] that the `.pyz` extension should be registered in
...
extension is not listed in the media types database[^6]. It doesn\'t
seem reasonable to register `.pyz` without `.py`, so this idea has been
omitted from this PEP. An interested party could arrange for *both*
`.py` and `.pyz` to be registered at a future date.
...
[^4]: pex - The PEX packaging toolchain
