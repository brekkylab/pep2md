...
bdfl_delegate: Paul Moore <p.f.moore@gmail.com>
...
functionality added by `438`{.interpreted-text role="pep"}, particularly
rel information to classify different types of links and the meta-tag to
...
`438`{.interpreted-text role="pep"} attempted to solve this issue by
allowing projects to explicitly declare if they were using the
repository features or not, and if they were not, it had the installers
classify the links it found as either \"internal\", \"verifiable
external\" or \"unverifiable external\". `438`{.interpreted-text
role="pep"} was accepted and implemented in pip 1.4 (released on Jul 23,
2013) with the final transition implemented in pip 1.5 (released on Jan
2, 2014).

`438`{.interpreted-text role="pep"} was successful in bringing about
more people to utilize PyPI\'s repository features, an altogether good
thing given the global CDN powering PyPI providing speed ups for a lot
of people, however it did so by introducing a new point of confusion and
pain for both the end users and the authors.
...
setuptools for quite some time support for `438`{.interpreted-text
role="pep"} has only existed in pip since the 1.4 version, and still has
yet to be implemented in setuptools. The design of
`438`{.interpreted-text role="pep"} did mean that users still benefited
...
Additionally, the classification system proposed by
`438`{.interpreted-text role="pep"} has, in practice, turned out to be
extremely confusing to end users, so much so that it is a position of
this PEP that the situation as it stands is completely untenable. The
common pattern for a user with this system is to attempt to install a
project possibly get an error message (or maybe not if the project ever
uploaded something to PyPI but later switched without removing old
files), see that the error message suggests `--allow-external`, they
reissue the command adding that flag most likely getting another error
message, see that this time the error message suggests also adding
`--allow-unverified`, and again issue the command a third time, this
time finally getting the thing they wish to install.
...
1.  If pip can locate files at all for a project on the Simple API it
    ...
    erase a large part of the benefit of `438`{.interpreted-text
    role="pep"}. This means that if a project *ever* uploaded a file
    that matches what the user has requested for install that will be
    used regardless of how old it is.

2.  `438`{.interpreted-text role="pep"} makes an implicit assumption
    that most projects would either upload themselves to PyPI or would
    update themselves to directly linking to release files. While a
    large number of projects did ultimately decide to upload to PyPI,
    some of them did so only because the UX around what
    `438`{.interpreted-text role="pep"} was so bad that they felt forced
...
4.  `438`{.interpreted-text role="pep"} takes a security centric view
    and disallows any form of a global opt in for unverified projects.
    While this is generally a good thing, it creates extremely verbose
    and repetitive command invocations such as:
...
The [Python packaging guide](https://packaging.python.org/) MUST be
...
called `pypi-only` and will be in addition to the three that
`438`{.interpreted-text role="pep"} has already given us which are
`pypi-explicit`, `pypi-scrape`, `pypi-scrape-crawl`. This new hosting
mode will modify a project\'s simple api page so that it only lists the
files which are directly hosted on PyPI and will not link to anything
else.
...
1.  Deprecate and remove the hosting modes as defined by
    `438`{.interpreted-text role="pep"}.
...
2.  Implement some mechanism for removing/disabling the default
    repository.
3.  Deprecate / Remove `438`{.interpreted-text role="pep"}
...
affected are using the features provided by `438`{.interpreted-text
role="pep"} to safely host outside of PyPI while 95% of them are
exposing their users to Remote Code Execution via a Man In The Middle
attack.
...
First you should decide if \<X\> is something inherent to PyPI, or if
PyPI could grow a feature to solve \<X\> for you. If PyPI can add a
feature to enable you to host your project on PyPI then you should
propose that feature. However, if \<X\> is something inherent to PyPI,
such as wanting to maintain control over your own files, then you should
setup your own package repository and instruct your users in your
project\'s description to add it to the list of repositories their
...
in their installer\'s default list of repositories. However, part of
this answer will also be explaining that the previous behavior of
...
the end users machine) and a reliability concern and that
`438`{.interpreted-text role="pep"} attempted to resolve this by making
them explicitly opt in, but that `438`{.interpreted-text role="pep"}
brought along with it a number of serious usability issues.
`470`{.interpreted-text role="pep"} represents a simplification of the
model to a model that many users will be familiar with, which is common
amongst Linux distributions.
...
[LetsEncrypt](https://letsencrypt.org/) or they may be gotten cheap from
...
similar to those that caused so many problems with the
`438`{.interpreted-text role="pep"} solution. If needed, a future PEP
...
the general gist of `438`{.interpreted-text role="pep"}.
...
- Continue on the suggested path of `438`{.interpreted-text role="pep"}
  and remove the option to unsafely host externally but continue to
  allow the option to safely host externally.

These proposals are rejected because:

- The classification system introduced in `438`{.interpreted-text
  role="pep"} in an entirely unique concept to PyPI which is not
  generically applicable even in the context of Python packaging. Adding
...
- The ability to host externally while still being linked for automatic
  discovery is mostly a historic relic which causes a fair amount of
  pain and complexity for little reward.
- The installer\'s ability to optimize or clean up the user interface is
  limited due to the nature of the implicit link scraping which would
  need to be done. This extends to the `--allow-*` options as well as
  the inability to determine if a link is expected to fail or not.
- The mechanism paints a very broad brush when enabling an option, while
  `438`{.interpreted-text role="pep"} attempts to limit this with per
  package options. However a project that has existed for an extended
  period of time may oftentimes have several different URLs listed in
  their simple index. It is not unusual for at least one of these to no
  longer be under control of the project. While an unregistered domain
  will sit there relatively harmless most of the time, pip will continue
  to attempt to install from it on every discovery phase. This means
  that an attacker simply needs to look at projects which rely on unsafe
