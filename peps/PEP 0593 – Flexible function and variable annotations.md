...
- Konstantin Kashin <kkashin@fb.com>
...
source_path: https://github.com/python/peps/blob/main/peps/pep-0593.rst
source_commit: f6ae0546d4ae1ca4bdf542b60f21bc3a7aa5c388
---

::: canonical-typing-spec
`typing:annotated`{.interpreted-text role="ref"} and
`typing.Annotated`{.interpreted-text role="py:data"}
:::
...
`484`{.interpreted-text role="pep"} provides a standard semantic for the
annotations introduced in `3107`{.interpreted-text role="pep"}.
`484`{.interpreted-text role="pep"} is prescriptive but it is the de
facto standard for most of the consumers of annotations; in many
statically checked code bases, where type annotations are widely used,
they have effectively crowded out any other form of annotation. Some of
the use cases for annotations described in `3107`{.interpreted-text
role="pep"} (database mapping, foreign languages bridge) are not
currently realistic given the prevalence of type annotations.
...
both static typechecking of `T` (e.g., via
[mypy](http://www.mypy-lang.org/) or [Pyre](https://pyre-check.org/),
...
This was originally brought up as [issue
600](https://github.com/python/typing/issues/600) in the typing github
and then discussed in [Python
ideas](https://mail.python.org/pipermail/python-ideas/2019-January/054908.html).
...
to the typing module and change mypy,
[PyCharm](https://www.jetbrains.com/pycharm/), Pyre,
[pytype](https://github.com/google/pytype), etc\... This is particularly
important when working on open-source code that makes use of these
types, seeing as the code would not be immediately transportable to
other developers\' tools without additional logic. As a result, there is
a high cost to developing and trying out new types in a codebase.
Ideally, authors should be able to introduce new types in a manner that
allows for graceful degradation (e.g.: when clients do not have a custom
[mypy plugin](https://github.com/python/mypy_extensions)), which would
lower the barrier to development and ensure some degree of backward
compatibility.

For example, suppose that an author wanted to add support for [tagged
unions](https://en.wikipedia.org/wiki/Tagged_union) to Python. One way
to accomplish would be to
[annotate](https://mypy.readthedocs.io/en/latest/more_types.html#typeddict)
...
annotation would still be able to treat `Currency` as a `TypedDict`,
...
- `Annotated` cannot infer the decorated type. You could imagine that
  ...
  inferred type [anywhere
  else](https://github.com/python/typing/issues/276); it\'s best to not
