...
source_path: https://github.com/python/peps/blob/main/peps/pep-0393.rst
source_commit: c2a42399b43ef825b9aad9ed18b297231f421039
---
...
An implementation of this PEP is available at[^1].
...
computed. Applications are encouraged to phase out reliance on a
...
- interned: interned-state ([SSTATE]()\*) as in 3.2
- kind: form of string
...
exactly to the wchar_t type of the platform (i.e. uses some
...
which can be modified until PyUnicode_READY() is called (explicitly or
...
should use [PyUnicode](){READ\|WRITE}\[[CHAR]{#char}\]:
...
[PyUnicode_AsString]{#pyunicode_asstring}, which is removed. The
function will compute the utf8 representation when first called. Since
this representation will consume memory until the string object is
released, applications should use the existing PyUnicode_AsUTF8String
where possible (which generates a new string object every time). APIs
that implicitly converts a string to a char\* (such as the ParseTuple
functions) will use PyUnicode_AsUTF8 to compute a conversion.
...
- [Py_UCS4](){strlen, strcpy, strcat, strncpy, strcmp, strncpy, strcmp,
  strncmp, strchr, strrchr}

## Stable ABI

The following functions are added to the stable ABI
(`384`{.interpreted-text role="pep"}), as they are independent of the
actual representation of Unicode objects: PyUnicode_New,
PyUnicode_Substring, PyUnicode_GetLength, PyUnicode_ReadChar,
...
- [Py_UNICODE](){strlen, strcat, strcpy, strcmp, strchr, strrchr}
- PyUnicode_AsUnicodeCopy
- PyUnicode_GetMax

[PyUnicode_AsDefaultEncodedString]{#pyunicode_asdefaultencodedstring} is
removed. It previously returned a borrowed reference to an UTF-8-encoded
bytes object. Since the unicode object cannot anymore cache such a
reference, implementing it without leaking memory is not possible. No
deprecation phase is provided, since it was an API for internal use
only.
...
3,340,000 bytes. This reduction in 3.x using the PEP compared to 2.x
only occurs when comparing with a wide unicode build.
...
indexing works well for PyUnicode_READ([CHAR]{#char}) and
PyUnicode_WRITE. Use void\* as the buffer type for characters to let the
compiler detect invalid dereferencing operations. If you do want to use
pointer arithmetics (e.g. when converting existing code), use (unsigned)
...
then allocate the target string with PyUnicode_New and iterate over the
