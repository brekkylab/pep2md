...
hooks described in `302`{.interpreted-text role="pep"}.
...
second item in `sys.path`. The first is the directory of the `main.py`
(thanks Tim).
...
is the time after repeated runs. Times to import from C: after a re-boot
...
basis for the final implementation of `302`{.interpreted-text
role="pep"}. `273`{.interpreted-text role="pep"} has been implemented
using `302`{.interpreted-text role="pep"}\'s import hooks.[^4]
