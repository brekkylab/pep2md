...
author:
- Sean Reifschneider <jafo-pep@tummy.com>
...
source_path: https://github.com/python/peps/blob/main/peps/pep-0243.rst
source_commit: 7f8aa06f244374ef9c6a934a9bc83d8bf5da9e36
---
...
(as specified in `241`{.interpreted-text role="pep"}), the actual
software distribution, and other optional information. This information
will be uploaded as a multi-part form encoded the same as a regular HTML
file upload request. This form is posted using
`ENCTYPE="multipart/form-data"` encoding (`1867`{.interpreted-text
role="rfc"}).
...
- `distmd5sum` \-- The MD5 hash of the uploaded distribution, encoded in
  ASCII representing the hexadecimal representation of the digest
  (`for byte in digest: s = s + ('%02x' % ord(byte))`).
- `pkginfo` (optional) \-- The file containing the distribution
  meta-data (as specified in `241`{.interpreted-text role="pep"}). Note
  that if this is not included, the distribution file is expected to be
  in `.tar` format (gzipped and bzipped compressed are allowed) or
  `.zip` format, with a `PKG-INFO` file in the top-level directory it
...
- `signature` (optional) \-- A
  `OpenPGP-compatible <2440>`{.interpreted-text role="rfc"} signature of
  the uploaded distribution as signed by the author. This may be used by
  the cataloging system to automate acceptance of uploads.
...
- `TRYAGAIN` \-- The server is unable to accept the upload at this time,
  but the client should try again at a later time. Potential causes of
  this are resource shortages on the server, administrative down-time,
  etc\...
...
with Andrew\'s `241`{.interpreted-text role="pep"} for specifying
distribution meta-data, I hope to have a platform which will allow us to
gather real-world data for finalizing the catalog system for the 2.2
release.
