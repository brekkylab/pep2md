...
source_path: https://github.com/python/peps/blob/main/peps/pep-0420.rst
source_commit: 4ad8f7ad8dd2299eb66276897b1a343a6691c9e0
---
...
builds upon previous work, documented in `382`{.interpreted-text
role="pep"} and `402`{.interpreted-text role="pep"}. Those PEPs have
...
See `402`{.interpreted-text role="pep"}\'s
`"The Problem" <402#the-problem>`{.interpreted-text role="pep"} section
for additional motivations for namespace packages. Note that
`402`{.interpreted-text role="pep"} has been rejected, but the
...
`302`{.interpreted-text role="pep"} defines \"finders\" that are called
to search path elements. These finders\' `find_module` methods return
either a \"loader\" object or `None`.
...
The specification expands `302`{.interpreted-text role="pep"} loaders to
include an optional method called `module_repr()` which if present, is
used to generate module object reprs. See the section below for further
details.
...
recognize `420`{.interpreted-text role="pep"} namespace packages. This
will allow some portions of a namespace to be legacy portions while
others are migrated to `420`{.interpreted-text role="pep"}. These hybrid
namespace packages will not have the dynamic path computation that
normal namespace packages have, since `extend_path()` never provided
...
two directories, \"bar\" and \"baz\". If \"foo.bar\" is removed (perhaps
...
`382`{.interpreted-text role="pep"} and `402`{.interpreted-text
role="pep"} were rejected, to be replaced by this PEP[^2].
...
Alyssa later gave a detailed response to her own objections[^4], which
is summarized here:
...
2.  Minor backward compatibility issues are okay, as long as they are
    properly documented.
3.  This will be addressed in `395`{.interpreted-text role="pep"}.
4.  This will also be addressed in `395`{.interpreted-text role="pep"}.
...
become a namespace package[^5]. While this PEP allows for standard
...
string, it was assumed to be a file system path, and the module
object\'s repr would include this in its value. The only exception was
that `302`{.interpreted-text role="pep"} reserved missing `__file__`
attributes to built-in modules, and in CPython, this assumption was
baked into the module object\'s implementation. Because of this
restriction, some modules contained contrived `__file__` values that did
not reflect file system paths, and which could cause unexpected problems
later (e.g. `os.path.join()` on a non-path `__file__` would return
gibberish).
...
new optional protocol is added to `302`{.interpreted-text role="pep"}
loaders. Loaders can implement a `module_repr()` method which takes a
single argument, the module object. This method should return the string
to be used verbatim as the repr of the module. The rules for producing a
module repr are now standardized as:
...
[^5]: Martin v. Löwis\'s suggestion to make `encodings` a namespace
    package
