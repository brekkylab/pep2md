...
source_path: https://github.com/python/peps/blob/main/peps/pep-0308.rst
source_commit: ea3d3563009626b1b67d4b937a883f65adbaece6
---
...
achieve the same effect using \"and\" and \"or\".[^2]
...
in this context, the lambda could already be followed by an \'if\'
keyword that binds less tightly still (for details, consider the grammar
changes shown above).
...
when \<expression1\> is false! See FAQ 4.16 for alternatives that work
...
\"then\" seems unlikely to have been used as a name in existing
programs.
...
expression1 and expression2 must be evaluated before the function is
called. There\'s no way to short-circuit the expression evaluation. It
could work if \'cond\' (or some other name) were made a keyword, but
that has all the disadvantages of adding a new keyword, plus confusing
syntax: it **looks** like a function call so a casual reader might
expect both \<expression1\> and \<expression2\> to be evaluated.
