...
source_path: https://github.com/python/peps/blob/main/peps/pep-0487.rst
source_commit: fa1dd63a2822b7d909902c43796695305c19554a
---
...
The third category is the topic of another PEP, `520`{.interpreted-text
role="pep"}.
...
The reference implementation for this PEP is attached to [issue
27366](http://bugs.python.org/issue27366).
...
itself was the proposed idea of `422`{.interpreted-text role="pep"}.
Most examples work the same way or even better if the hook is called
only on strict subclasses. In general, it is much easier to arrange to
explicitly call the hook on the class in which it is defined (to opt-in
to such a behavior) than to opt-out (by remember to check for
`cls is __class` in the hook body), meaning that one does not want the
hook to be called on the class it is defined in.
...
In `422`{.interpreted-text role="pep"} the hook worked more like the
`__new__` method than the `__init__` method, meaning that it returned a
class instead of modifying one. This allows a bit more flexibility, but
at the cost of much harder implementation and undesired side effects.

## Adding a class attribute with the attribute order

This got its own `520`{.interpreted-text role="pep"}.

# History

This used to be a competing proposal to `422`{.interpreted-text
role="pep"} by Alyssa Coghlan and Daniel Urban. `422`{.interpreted-text
role="pep"} intended to achieve the same goals as this PEP, but with a
different way of implementation. In the meantime,
`422`{.interpreted-text role="pep"} has been withdrawn favouring this
approach.
