...
address storage and download of packages as described in
`243`{.interpreted-text role="pep"}. Nor is it proposing a local
database of packages as described in `262`{.interpreted-text
role="pep"}.
...
such a system. `262`{.interpreted-text role="pep"} proposes such a
system, but as of this writing the PEP is still unfinished.
...
and bandwidth that I am not in a position to make.
`243`{.interpreted-text role="pep"}, which is still being developed, is
tackling these issues and many more. This proposal is considered
compatible with, and adjunct to the proposal in `243`{.interpreted-text
role="pep"}.

# Specification

The specification takes three parts, the [web
interface](#web-interface), the [Distutils register
command](#distutils-register-command) and the [Distutils Trove
classification](#distutils-trove-classification).
...
:   Lists known packages, optionally filtered. An additional HTML page,
    **search**, presents a form to the user which is used to customise
    ...
    the most recent version. The most recent version information will be
...
:   Accepts a POST submission of metadata about a package. The \"name\"
    ...
    entry in the index. **Submit** will automatically determine whether
    to create a new entry or update an existing entry. The metadata is
...
:   Holds information about changes to package information in the index.
    Changes to the **packages**, **releases**, **roles**, and
    **release_discriminators** tables are listed here by package *name*
    and *version* if the change is release-specific.
