...
source_path: https://github.com/python/peps/blob/main/peps/pep-0507.rst
source_commit: 76cfd7653fee8ebd78b68519de619b7c5abc956c
---
...
reviews, and code hosting. It is similar in intent to
`481`{.interpreted-text role="pep"} but proposes an open source
alternative to GitHub and omits the proposal to run Phabricator. As with
`481`{.interpreted-text role="pep"}, this particular PEP is offered as
an alternative to `474`{.interpreted-text role="pep"} and
...
local VCS manipulations. These interactions include as a minimum, code
review with inline comments, branch diffing, CI integration, and
auto-merging.
...
next-version master branch for new features. GitLab\'s merge requests
are similar in form and function to GitHub\'s pull requests, so anybody
who is already familiar with the latter should be able to immediately
utilize the former.
...
the source branch (e.g. to respond to a commenter\'s feedback or to fix
a failing test) results in a new run of the CI, so that the state of the
...
press the \"Merge\" button for GitLab to automatically perform the
merge. The source branch can be optionally rebased, and once the merge
...
Python documentation to have \"Edit on GitLab\" buttons on every page
and people who discover things like typos, inaccuracies, or just want to
...
- What happens to Roundup and do we switch to the GitLab issue tracker?
  ...
  issues. We have way too much invested in Roundup right now and
  migrating the data would be a huge effort. GitLab does support
  webhooks, so we will probably want to use webhooks to integrate merges
  and other events with updates to Roundup (e.g. to include pointers to
  commits, close issues, etc. similar to what is currently done).
...
[^1]: [Open Hub
    Statistics](https://www.openhub.net/repositories/compare)
