...
Android and Windows (`529`{.interpreted-text role="pep"}) for most
functions \-- although `open()` is a notable exception here. UTF-8 is
also the default encoding of Python scripts, XML and JSON file formats.
The Go programming language uses UTF-8 for all strings.
...
the `surrogateescape` error handler (`383`{.interpreted-text
role="pep"}). It allows processing data as if it were bytes, but uses
Unicode in practice; undecodable bytes are stored as surrogate
characters.
...
(`529`{.interpreted-text role="pep"}) has the priority over UTF-8 Mode.
...
The POSIX locale enables the locale coercion (`538`{.interpreted-text
role="pep"}) and the UTF-8 mode (`540`{.interpreted-text role="pep"}).
When the locale coercion is enabled, enabling the UTF-8 mode has no
additional effect.
...
consistent with Python\'s encoding handling. The upside of the UTF-8
Mode approach is that it allows an embedding application to change the
...
`sys.stdout` uses `mbcs` encoding by default rather than UTF-8. But in
...
- [bpo-29240: Implementation of the PEP 540: Add a new UTF-8
  Mode](http://bugs.python.org/issue29240)
- `538`{.interpreted-text role="pep"}: \"Coercing the legacy C locale to
  C.UTF-8\"
- `529`{.interpreted-text role="pep"}: \"Change Windows filesystem
  encoding to UTF-8\"
- `528`{.interpreted-text role="pep"}: \"Change Windows console encoding
  to UTF-8\"
- `383`{.interpreted-text role="pep"}: \"Non-decodable Bytes in System
  Character Interfaces\"

# Post History

- 2017-12: [\[Python-Dev\] PEP 540: Add a new UTF-8
  Mode](https://mail.python.org/pipermail/python-dev/2017-December/151054.html)
- 2017-04: [\[Python-Dev\] Proposed BDFL Delegate update for PEPs 538 &
  540 (assuming UTF-8 for \*nix system
  boundaries)](https://mail.python.org/pipermail/python-dev/2017-April/147795.html)
- 2017-01: [\[Python-ideas\] PEP 540: Add a new UTF-8
  Mode](https://mail.python.org/pipermail/python-ideas/2017-January/044089.html)
- 2017-01: [bpo-28180: Implementation of the PEP 538: coerce C locale to
  C.utf-8 (msg284764)](https://bugs.python.org/issue28180#msg284764)
- 2016-08-17: [bpo-27781: Change sys.getfilesystemencoding() on Windows
  to UTF-8 (msg272916)](https://bugs.python.org/issue27781#msg272916)
  \-- Victor proposed `-X utf8` for the `529`{.interpreted-text
  role="pep"} (Change Windows filesystem encoding to UTF-8)
