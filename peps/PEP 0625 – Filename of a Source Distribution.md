...
pep_delegate: Pradyun Gedam <pradyunsg@gmail.com>
...
source_path: https://github.com/python/peps/blob/main/peps/pep-0625.rst
source_commit: 4b1d509915df1d40c9cf284bbf64c23c8d5643a0
---
...
The content of an sdist is specified in `517`{.interpreted-text
role="pep"} and `643`{.interpreted-text role="pep"}, but currently the
filename of the sdist is incompletely specified, meaning that consumers
of the format must download and process the sdist to confirm the name
and version of the distribution included within.
...
parses the filename of an sdist from a `503`{.interpreted-text
...
correctness of the inferred data, and must verify it at some point by
...
- `distribution` is the name of the distribution as defined in
  `345`{.interpreted-text role="pep"}, and normalised as described in
  [the wheel
...
- `version` is the version of the distribution as defined in
  `440`{.interpreted-text role="pep"}, e.g. `20.2`, and normalised
  according to the rules in that PEP.
...
`517`{.interpreted-text role="pep"} is extended to require this naming
convention.
...
actual metadata (or reading it, if the sdist conforms to
`643`{.interpreted-text role="pep"}). Tools supporting this
...
assumed to conform to this PEP, but in practice the chance of this
...
Since this PEP was first written, `643`{.interpreted-text role="pep"}
has been accepted, defining a trustworthy, standard sdist metadata
format. This allows distribution metadata (and in particular name and
version) to be determined statically.
