...
`8`{.interpreted-text role="pep"}\", and \"I note that the toy examples
are somewhat misleading -- the functionality that may be useful is a
conditional return (or break etc.) inside a finally block.\".
...
Their use in such a location silently cancels any active exception being
raised through the `finally`, leading to unclear code and possible bugs.

`continue` is currently not supported in a `finally` in Python 3.7 (due
to implementation issues) and the proposal is to not add support for it
in Python 3.8. For `return` and `break` the proposal is to deprecate
their use in Python 3.9, emit a compilation warning in Python 3.10 and
then forbid their use after that.

# Motivation

The use of `return`, `break` and `continue` within a `finally` suite
leads to behaviour which is not at all obvious. Consider the following
function:
...
infinite recursion and raises an exception within the `try`. The reason
is that the `return` within the `finally` will silently cancel any
exception that propagates through the `finally` suite. Such behaviour is
unexpected and not at all obvious. This function is equivalent to:
...
`break` and `continue` have similar behaviour (they silence exceptions)
if they jump to code outside the `finally` suite. For example:
...
Independent to the semantics, implementing `return`/`break`/`continue`
within a `finally` suite is non-trivial as it requires to correctly
track any active exceptions at runtime (an executing `finally` suite may
or may not have an active exception) and cancel them as appropriate.
CPython did have a bug in this for the case of `continue` and so
originally disallowed it[^1]. Requiring correct behaviour for
`return`/`break`/`continue` within a `finally` puts an unnecessary
...
be an explicit return. It is discouraged and handled by
linters[^5],[^6].

Like Ruby, JavaScript also allows use of `return`/`break`/`continue`
within a `finally` but it is seen as unsafe and it is handled by
eslint[^7].

C# forbids the use of ending statements like `return`/`goto`/`break`
within a `finally` [^8],[^9].
...
- For `continue`, a `SyntaxError` (this is the current behaviour of
  3.7).
- For `return`/`break`, a `SyntaxWarning` in 3.10, and a `SyntaxError`
  after that.
...
continue is currently handled in a `finally` (raising a `SyntaxError`)
can be extended to `return` and `break`.
