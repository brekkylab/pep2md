...
python_version: '3.13'
post_history:
- '`22-Mar-2022 <https://mail.python.org/archives/list/typing-sig@python.org/thread/7VWBZWXTCX6RAJO6GG67BAXUPFZ24NTC/>`__'
...
`typing:type_parameter_defaults`{.interpreted-text role="ref"} and
`type-params`{.interpreted-text role="external+py3.13:ref"}
:::
...
some common languages has been conducted by the author of
`695`{.interpreted-text role="pep"} and can be found in its
`Appendix A <695#appendix-a-survey-of-type-parameter-syntax>`{.interpreted-text
role="pep"}.
...
up](https://github.com/python/typing/issues/975) is `Generator`. I
...
- [NumPy](https://github.com/numpy/numpy) --- the default for types like
  `ndarray`\'s `dtype` would be `float64`. Currently it\'s `Unknown` or
  `Any`.
- [TensorFlow](https://github.com/tensorflow/tensorflow) --- this could
  be used for Tensor similarly to `numpy.ndarray` and would be useful to
...
`typing._GenericAlias`/`types.GenericAlias`, and a type checker should
...
With the new Python 3.12 syntax for generics (introduced by
`695`{.interpreted-text role="pep"}), this can be enforced at compile
time:
...
but use a `list` of types or an ellipsis literal \"`...`\" or another
in-scope `ParamSpec` (see [Scoping Rules](#scoping-rules)).
...
`TypeVarTuple` defaults are defined using the same syntax as `TypeVar` s
but use an unpacked tuple of types instead of a single type or another
in-scope `TypeVarTuple` (see [Scoping Rules](#scoping-rules)).
...
builtins.slice](https://github.com/python/typing/issues/159) where the
`start` parameter should default to `int`, `stop` default to the type of
...
change if [Higher Kinded
TypeVars](https://github.com/python/typing/issues/548) are implemented.
...
substituted into the `TypeAlias`. However, it can be specialised further
down the line.
...
similarly to `Generic` `TypeAlias`es. That is, subclasses can be further
...
`bound`. Otherwise the type checker should generate an error.
...
## Function Defaults {#696-function-defaults}
...
semantics of this usage unspecified, as ensuring the `default` is
returned in every code path where the type parameter can go unsolved may
be too hard to implement. Type checkers are free to either disallow this
case or experiment with implementing support.
...
have a default, because it would be ambiguous whether a type argument
...
- [Scoping Rules](#scoping-rules) does not allow usage of type
...
- Multiple `TypeVarTuple`s cannot appear in the type parameter list for
  ...
  `646#multiple-type-variable-tuples-not-allowed`{.interpreted-text
  role="pep"}.
...
- The classes `TypeVar`, `ParamSpec`, and `TypeVarTuple` should expose
  the type passed to `default`. This would be available as a
...
The syntax added in `695`{.interpreted-text role="pep"} will be extended
...
`Similarly to the bound for a type parameter <695-scoping-behavior>`{.interpreted-text
role="ref"}, defaults should be lazily evaluated, with the same scoping
rules to avoid the unnecessary usage of quotes around them.

This functionality was included in the initial draft of
`695`{.interpreted-text role="pep"} but was removed due to scope creep.
...
`TypeVar` [unary syntax](https://github.com/python/typing/issues/813),
...
Ideally, if `637`{.interpreted-text role="pep"} wasn\'t rejected, the
...
could have a type parameter with no default follow a type parameter with
a default. Consider:
...
be used in function signatures. This was removed for the reasons
described in [Function Defaults](#function-defaults). Hopefully, this
can be added in the future if a way to get the runtime value of a type
parameter is added.
