...
source_path: https://github.com/python/peps/blob/main/peps/pep-0492.rst
source_commit: 0b40861446fd9cfbe7d69f33636b2622d8f869cc
---
...
1.  Feedback on the initial beta release of Python 3.5 resulted in a
    ...
    distinct type (implemented in[^7]).
    ...
    into the Tornado web server (reported in[^8]).
...
- Support for asynchronous calls is limited to expressions where `yield`
  is allowed syntactically, limiting the usefulness of syntactic
  features, such as `with` and `for` statements.
...
coroutines in Python (`342`{.interpreted-text role="pep"} and
`380`{.interpreted-text role="pep"}). Motivation for the syntax changes
proposed here comes from the asyncio framework (`3156`{.interpreted-text
role="pep"}) and the \"Cofunctions\" proposal (`3152`{.interpreted-text
role="pep"}, now rejected in favor of this specification).
...
- Internally, two new code object flags were introduced:
  ...
  - `CO_ITERABLE_COROUTINE` is used to make *generator-based coroutines*
    ...
    [types.coroutine()](#types.coroutine) function).
...
- `StopIteration` exceptions are not propagated out of coroutines, and
  ...
  behavior requires a future import (see `479`{.interpreted-text
  role="pep"}).
- When a *native coroutine* is garbage collected, a `RuntimeWarning` is
  raised if it was never awaited on (see also [Debugging
...
- An object with an `__await__` method returning an iterator.
  ...
  (please refer to `3156`{.interpreted-text role="pep"} for a detailed
  explanation).
...
See Python Documentation[^11] and [Grammar Updates](#grammar-updates)
section of this proposal for details.
...
And since `479`{.interpreted-text role="pep"} is accepted and enabled by
default for coroutines, the following example will have its
`StopIteration` wrapped into a `RuntimeError`
...
Moreover, with semantics from `479`{.interpreted-text role="pep"}, all
`StopIteration` exceptions raised in coroutines are wrapped in
`RuntimeError`.
...
2.  *Plain generators* cannot `yield from` *native coroutines*: doing so
    will result in a `TypeError`.

3.  *generator-based coroutines* (for asyncio code must be decorated
    with `@asyncio.coroutine`[^12]) can `yield from` *native coroutine
    objects*.
...
`479`{.interpreted-text role="pep"} is enabled by default for
coroutines). See `342`{.interpreted-text role="pep"},
`380`{.interpreted-text role="pep"}, and Python Documentation[^13] for
details.
...
- `types.coroutine(gen)`. See [types.coroutine()](#types.coroutine)
  section for details.
...
- `sys.set_coroutine_wrapper(wrapper)` allows to intercept creation of
  *native coroutine* objects. `wrapper` must be either a callable that
  accepts one argument (a *coroutine* object), or `None`. `None` resets
  the wrapper. If called twice, the new wrapper replaces the previous
  one. The function is thread-specific. See [Debugging
...
- `sys.get_coroutine_wrapper()` returns the current wrapper object.
  ...
  See [Debugging Features](#debugging-features) for more details.

## New Abstract Base Classes

In order to allow better integration with existing frameworks (such as
Tornado, see[^14]) and compilers (such as Cython, see[^15]), two new
...
- `collections.abc.Coroutine` ABC for *coroutine* objects, that
  ...
  do not implement `__await__` method, and therefore are not instances
  of `collections.abc.Coroutine` and `collections.abc.Awaitable` ABCs:
...
:   A coroutine function is declared with `async def`. It uses `await`
    and `return value`; see [New Coroutine Declaration
    Syntax](#new-coroutine-declaration-syntax) for details.

Native coroutine

:   Returned from a native coroutine function. See [Await
...
:   An object with an `__await__` method, or a C object with
    ...
    object\'s `__await__` completes, and returns the result. See [Await
    Expression](#await-expression) for details.
...
:   An asynchronous context manager has `__aenter__` and `__aexit__`
    ...
    Managers and \"async
    with\"](#asynchronous-context-managers-and-async-with) for details.

Asynchronous iterable

:   An object with an `__aiter__` method, which must return an
    *asynchronous iterator* object. Can be used with `async for`. See
    [Asynchronous Iterators and \"async
    for\"](#asynchronous-iterators-and-async-for) for details.
...
- while tokenizing `async def` block, it replaces `'async'` `NAME` token
  with `ASYNC`, and `'await'` `NAME` token with `AWAIT`;
- while tokenizing `def` block, it yields `'async'` and `'await'` `NAME`
  tokens as is.
...
objects* (see [Differences from
generators](#differences-from-generators) section for more details), it
is advised to make sure that all generator-based coroutines are
...
`async()` function to `ensure_future()` (see [asyncio](#asyncio) section
for details).
...
`3152`{.interpreted-text role="pep"} by Gregory Ewing proposes a
different mechanism for coroutines (called \"cofunctions\"). Some key
points:
...
3.  To make asyncio work with `3152`{.interpreted-text role="pep"} it
    would be required to modify `@asyncio.coroutine` decorator to wrap
    all functions in an object with a `__cocall__` method, or to
    implement `__cocall__` on generators. To call *cofunctions* from
    existing generator-based coroutines it would be required to use
...
- proposal to add async/await in ECMAScript 7[^17]; see also Traceur
  project[^18];
...
`492`{.interpreted-text role="pep"} was accepted in CPython 3.5.0 with
`__aiter__` defined as a method, that was expected to return an
awaitable resolving to an asynchronous iterator.

In 3.5.2 (as `492`{.interpreted-text role="pep"} was accepted on a
provisional basis) the `__aiter__` protocol was updated to return
asynchronous iterators directly.

The motivation behind this change is to make it possible to implement
asynchronous generators in Python. See[^23] and[^24] for more details.
...
[Transition Plan](#transition-plan) section explains how tokenizer is
modified to treat `async` and `await` as keywords *only* in `async def`
blocks. Hence `async def` fills the role that a module level compiler
...
- one of the main points of this proposal is to make native coroutines
  as simple and foolproof as possible, hence the clear separation of the
  protocols.
...
Making existing \"for\" and \"with\" statements to recognize
asynchronous iterators and context managers will inevitably create
...
output of python\'s official set of benchmarks[^25]:
...
7.  New `CO_COROUTINE` and `CO_ITERABLE_COROUTINE` bit flags for code
    objects.
8.  New ABCs: `collections.abc.Awaitable`, `collections.abc.Coroutine`,
...
All concepts proposed in this PEP are implemented[^27] and can be
tested.
...
`492`{.interpreted-text role="pep"} was accepted by Guido, Tuesday, May
5, 2015[^28].

# Implementation

The implementation is tracked in issue 24017[^29]. It was committed on
May 11, 2015.
...
[^6]: <http://www.open-std.org/jtc1/sc22/wg21/docs/papers/2013/n3722.pdf>
    (PDF)
...
[^22]: <http://www.open-std.org/jtc1/sc22/wg21/docs/papers/2013/n3722.pdf>
    (PDF)
