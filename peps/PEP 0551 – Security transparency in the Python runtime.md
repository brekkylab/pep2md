...
source_path: https://github.com/python/peps/blob/main/peps/pep-0551.rst
source_commit: 123cbabff71d60a06d07087d4340fcc98d149132
---
...
See `578`{.interpreted-text role="pep"} for the auditing APIs proposed
for addition to the next version of Python.
...
The audit hooks described in `578`{.interpreted-text role="pep"} are an
essential component in detecting, identifying and analyzing misuse of
Python. While the hooks themselves are neutral (in that not every
reported event is inherently misuse), they provide essential context to
those who are responsible for monitoring an overall system or network.
With enough transparency, attackers are no longer able to hide.
...
order to detect abnormal or malicious usage. With
`578`{.interpreted-text role="pep"}, the Python runtime gains the
ability to provide this. The aim of this PEP is to assist system
administrators with deploying a security transparent version of Python
that can integrate with their existing auditing and protection systems.

On Windows, some specific features that may be integrated through the
hooks added by `578`{.interpreted-text role="pep"} include:
...
Sysadmins should prioritize [detection]{#detection}\_ of misuse over
[prevention]{#prevention}\_ of misuse.
...
[^13]: SELinux access decisions
    <http://man7.org/linux/man-pages/man3/avc_entry_ref_init.3.html>
