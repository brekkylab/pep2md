...
generated_at: '2026-04-24T03:27:52+00:00'
---
...
in favour of the more modest proposal in `566`{.interpreted-text
role="pep"}, which retains the basic Key:Value format of previous
metadata versions, but also defines a standardised mechanism for
translating that format to nested JSON-compatible data structures.

Some of the ideas in this PEP (or the related `459`{.interpreted-text
role="pep"}) may still be considered as part of later proposals, but
they will be handled in a more incremental fashion, rather than as a
single large proposed change with no feasible migration plan.
:::
...
Version 1.0 is specified in `241`{.interpreted-text role="pep"}. Version
1.1 is specified in `314`{.interpreted-text role="pep"}. Version 1.2 is
specified in `345`{.interpreted-text role="pep"}.
...
- defining a binary compatibility tagging format in
  `425`{.interpreted-text role="pep"}
- defining a binary archive format (`wheel`) in `427`{.interpreted-text
  role="pep"}
- explicitly defining versioning and version comparison in
  `440`{.interpreted-text role="pep"}
- explicitly defining the PyPI \"simple\" API in `503`{.interpreted-text
  role="pep"}
- explicitly defining dependency specifiers and the extras system in
  `508`{.interpreted-text role="pep"}
- declaring static build system dependencies (`pyproject.toml`) in
  `518`{.interpreted-text role="pep"}
- migrating PyPI hosting to Rackspace, and placing it behind the Fastly
  CDN
- shipping `pip` with CPython by default in `453`{.interpreted-text
  role="pep"}, and backporting that addition to Python 2.7 in
  `477`{.interpreted-text role="pep"}
- establishing [packaging.python.org](https://packaging.python.org/) as
  the common access point for Python packaging ecosystem documentation
- migrating to using the
  [specifications](https://packaging.python.org/specifications/) section
  of packaging.python.org as the central location for tracking packaging
  related PEPs
...
activity of publishing and distributing software to be moved out to
`459`{.interpreted-text role="pep"}, a separate proposal for a number of
standard metadata extensions that provide additional optional
...
- clarification of the additional fields defined in the past few years
  ...
  covered in a [minor spec version
  update](https://mail.python.org/pipermail/distutils-sig/2017-September/031465.html)

Finally, the PEP was withdrawn in February 2018 in favour of
`566`{.interpreted-text role="pep"} (which pursues that more incremental
strategy).
...
The design draws on the Python community\'s nearly 20 years of
experience with distutils based software distribution, and incorporates
ideas and concepts from other distribution systems, including Python\'s
setuptools, pip and other projects, Ruby\'s gems, Perl\'s CPAN,
Node.js\'s npm, PHP\'s composer and Linux packaging systems such as RPM
and APT.
...
The metadata design in this PEP is based on a particular conceptual
...
- Software development: this phase involves working with a source
  checkout for a particular application to add features and fix bugs. It
  is expected that developers in this phase will need to be able to
...
- Software publication: this phase involves taking the developed
  software and making it available for use by software integrators. This
  includes creating the descriptive metadata defined in this PEP, as
  well as making the software available (typically by uploading it to an
  index server).
...
packages\", but are more colloquially known as just \"packages\"
(relying on context to disambiguate them from the \"module with
submodules\" kind of Python package).
...
registered on the [Python Package Index](https://pypi.org/).
...
\"Source archive\" and \"VCS checkout\" both refer to the raw source
code for a release, prior to creation of an sdist or binary archive.
...
uploads from untrusted third parties. The [Python Package
Index](https://pypi.org/) is a public index server.
...
\"Installation tools\" are integration tools specifically intended to
run on deployment targets, consuming source and binary distribution
archives from an index server or other designated location and deploying
...
\"Distro\" is used as the preferred term for Linux distributions, to
help avoid confusion with the Python-specific use of the term
...
a qualified name - it may contain additional dot separated segments
...
Automated tools MUST NOT insert dummy data for missing fields. If a
...
A [jsonschema](https://pypi.org/project/jsonschema/) description of the
distribution metadata is
[available](https://hg.python.org/peps/file/default/pep-0426/pydist-schema.json).
...
with `3986`{.interpreted-text role="rfc"}.
...
The name of the distribution, as defined in `508`{.interpreted-text
role="pep"}.
...
Unicode Consortium in [TR39: Unicode Security
Mechanisms](https://www.unicode.org/reports/tr39/tr39-1.html#Confusable_Detection))
to be equivalent.
...
The distribution\'s public or local version identifier, as defined in
`440`{.interpreted-text role="pep"}. Version identifiers are designed
for consumption by automated tools and support a variety of flexible
version specification mechanisms (see `440`{.interpreted-text
role="pep"} for details).

Version identifiers MUST comply with the format defined in
`440`{.interpreted-text role="pep"}.

Version identifiers MUST be unique within each project.

Index servers MAY place restrictions on the use of local version
identifiers as described in `440`{.interpreted-text role="pep"}.
...
`459`{.interpreted-text role="pep"} for more information.
...
`"https://github.com/pypa/pip/archive/main.zip"`, but instead must be
...
- `test` dependencies: other releases that are needed to run the
  automated test suite for this release, but are not needed just to use
  it (e.g. `nose2` or `pytest`)
...
[Extras](#extras-optional-dependencies). In addition to the four
standard categories, projects may also declare their own custom
categories in the [Extras](#extras) field.
...
and specification scheme defined in `440`{.interpreted-text role="pep"}
and the dependency specification, extra, and environment marker schemes
defined in `508`{.interpreted-text role="pep"}.
...
The notation described in [Extras (optional
dependencies)](#extras-optional-dependencies) SHOULD be used to
determine exactly what gets installed for various operations.
...
dependencies)](#extras-optional-dependencies) for details.
...
- `extra`: the name of a set of optional dependencies that are requested
  and installed together. See [Extras (optional
  dependencies)](#extras-optional-dependencies) for details
- `environment`: an environment marker defining the environment that
  ...
  markers are defined in `508`{.interpreted-text role="pep"}

Individual entries in the `requires` lists are strings using the
dependency declaration format defined in `508`{.interpreted-text
role="pep"}, with the exception that environment markers MUST NOT be
included in the individual dependency declarations, and are instead
supplied in the separate `environment` field.
...
a dependency specifier MUST be named in the [Extras](#extras) field for
this distribution. This helps avoid typographical errors and also makes
it straightforward to identify the available extras without scanning the
...
standard extensions from `459`{.interpreted-text role="pep"}:
...
the highest version they support (as described in
`440`{.interpreted-text role="pep"}, the major version is the value
...
As defined in `508`{.interpreted-text role="pep"}, extras are additional
dependencies that enable an optional aspect of a project release, often
corresponding to a `try: import optional_dependency ...` block in the
code. They are also used to indicate semantic dependencies for
activities other than normal runtime using (such as testing, building,
or working on the component).

To support the use of the release with or without the optional
dependencies, they are listed separately from the release\'s core
runtime dependencies and must be requested explicitly, either in the
dependency specifications of another project, or else when issuing a
command to an installation tool.
...
- the [wheel project](https://bitbucket.org/dholth/wheel/overview),
  which adds the `bdist_wheel` command to `setuptools`
- the [Warehouse project](https://github.com/dstufft/warehouse), which
  will eventually be migrated to the Python Packaging Authority as the
  next generation Python Package Index implementation
- the [distlib project](https://bitbucket.org/pypa/distlib/) which is
...
is published solely through Read the Docs or the project website). This
...
- Explicit permission for in-place clarifications without releasing a
...
- Changed the version scheme to be based on `440`{.interpreted-text
  role="pep"} rather than `386`{.interpreted-text role="pep"}
- Added the source label mechanism as described in
  `440`{.interpreted-text role="pep"}
- Formally defined dependency declarations, extras, and environment
  markers in `508`{.interpreted-text role="pep"}
...
the wheel format in `427`{.interpreted-text role="pep"}: minor version
increments must behave reasonably when processed by a tool that only
understand earlier metadata versions with the same major version, while
major version increments may include changes that are not compatible
with existing tools.

The major version number of the specification has been incremented
accordingly, as interpreting `426`{.interpreted-text role="pep"}
metadata obviously cannot be interpreted in accordance with earlier
...
See `440`{.interpreted-text role="pep"} for a detailed rationale for the
various changes made to the versioning scheme.
...
to be delegated to other projects, while preserving a standard overall
...
moved out to standard extensions defined in `459`{.interpreted-text
role="pep"}.
...
releases. Discussion on distutils-sig came to the conclusion that
wasn\'t a serious enough problem to tackle directly at the
interoperability specification layer, and if it does become a problem in
the future, it would be better tackled at the point where projects are
uploaded to the public Python Package Index.
...
`440`{.interpreted-text role="pep"} defines a rich syntax for version
comparisons that could potentially be useful with `python_version` and
`python_full_version` in environment markers. However, allowing the full
syntax would mean environment markers are no longer a Python subset,
while allowing only some of the comparisons would introduce yet another
...
\"may_provide\" field. However, it isn\'t clear there\'s any use case
for doing that, so the idea is rejected unless someone can present a
...
justifications for needing such a standard can be found in
`386`{.interpreted-text role="pep"}.

- [reStructuredText markup](https://docutils.sourceforge.io/)
