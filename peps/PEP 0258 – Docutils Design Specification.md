...
source_path: https://github.com/python/peps/blob/main/peps/pep-0258.rst
source_commit: 55be28af3c3222e7a95c1acdc5c56bf0a76a0778
---
...
high-level concepts of a DPS are documented in `256`{.interpreted-text
role="pep"}, \"Docstring Processing System Framework\". Also see
...
- Python Source: See [Python Source Reader](#python-source-reader)
  below. This Reader is currently in development in the Docutils
  sandbox.

- Email: `822`{.interpreted-text role="rfc"} headers, quoted excerpts,
  signatures, MIME parts.

- PEP: `822`{.interpreted-text role="rfc"} headers, \"PEP xxxx\" and
  \"RFC xxxx\" conversion to URIs. The \"PEP Reader\" has been
  implemented in module `docutils.readers.pep`; see
  `287`{.interpreted-text role="pep"} and `12`{.interpreted-text
  role="pep"}.
...
tree](#document-tree). They don\'t know or care anything about the
...
attached to every new document tree. The [Publisher](#publisher) calls
...
Reader has finished processing, the [Publisher](#publisher) calls
...
- Store a mapping of component type name (\'reader\', \'writer\', etc.)
  to component objects. These are used by certain transforms (such as
...
translate the internal [document tree](#document-tree) structure into
the final data format, possibly running Writer-specific
...
- Package \"docutils\".
  - Module \"\_\_init\_\_.py\" contains: class \"Component\", a base
    class for Docutils components; class \"SettingsSpec\", a base class
    for specifying runtime settings (used by docutils.frontend); and
    class \"TransformSpec\", a base class for specifying transforms.

  - Module \"docutils.core\" contains facade class \"Publisher\" and
    convenience functions. See [Publisher](#publisher) above.
  ...
  - Module \"docutils.statemachine\" contains a finite state machine
    ...
    The reStructuredText parser implementation is based on this module.
  ...
  - Module \"docutils.utils\" contains utility functions and classes,
    including a logger class (\"Reporter\"; see [Error
    Handling](#error-handling) below).
  ...
  - Package \"docutils.languages\": Language modules contain
    ...
    language identifier (as defined in [Choice of Docstring
    Format](#choice-of-docstring-format) below), converting dashes to
    underscores.
...
processing. See [Docutils Front-End
Tools](http://docutils.sourceforge.net/docs/user/tools.html) for
details.
...
diagram in the [Docutils Project Model](#docutils-project-model) above.
...
condition (a) is the [Python Source Reader](#python-source-reader) (see
below), where a \"stylist\" transform converts custom nodes. The HTML
`<meta>` tag is an example of condition (b); it is supported by the HTML
Writer but not by others. The reStructuredText \"meta\" directive
creates a \"pending\" node, which contains knowledge that the embedded
\"meta\" node can only be handled by HTML-compatible writers. The
\"pending\" node is resolved by the
...
Type Definition), which comes in two parts:

- the Docutils Generic DTD,
  [docutils.dtd](http://docutils.sourceforge.net/docs/ref/docutils.dtd),
  and
- the OASIS Exchange Table Model,
  [soextbl.dtd](http://docutils.sourceforge.net/docs/ref/soextblx.dtd).
...
See [The Docutils Document
Tree](http://docutils.sourceforge.net/docs/ref/doctree.html) for details
(incomplete).
...
- Level-1, \"INFO\": a minor issue that can be ignored. There is little
  or no effect on the processing. Typically level-1 system messages are
  not reported.
...
levels](http://www.openvms.compaq.com:8000/73final/5841/841pro_027.html#error_cond_severity);
the names in quotes for levels 1 through 4 were borrowed from VMS. Error
handling has since been influenced by the [log4j
project](http://logging.apache.org/log4j/docs/index.html).
...
3.  The abstract syntax trees are converted into an internal
    ...
    well as code structure details. See [AST Mining](#ast-mining) below.
...
5.  PySource assembles all the individual docstrings\' doctrees into a
    ...
    internal representation (see the [Docutils Python Source
    DTD](http://docutils.sourceforge.net/docs/dev/pysource.dtd)).
...
6.  Cross-references from docstrings (interpreted text) to Python
    ...
    rules. See [Identifier
    Cross-References](#identifier-cross-references) below.
7.  A \"Stylist\" transform is applied to the custom doctree (by the
    [Transformer](#transformer)), custom nodes are rendered using
    standard nodes as primitives, and a standard document tree is
    emitted. See [Stylist Transforms](#stylist-transforms) below.
...
2.  Where:
    ...
    b)  Immediately following a simple assignment at the top level of a
        ...
        any comments. See [Attribute Docstrings](#attribute-docstrings)
        below.
    c)  Additional string literals found immediately after the
        docstrings in (a) and (b) will be recognized, extracted, and
        concatenated. See [Additional
        Docstrings](#additional-docstrings) below.
...
(This is a simplified version of `224`{.interpreted-text role="pep"}.)
...
3.  The form of the target:
    ...
    b)  For context 1c above, the target must be of the form
        ...
        method\'s first parameter (the instance parameter) and
        \"attrib\" is a simple identifier as in 3a.
...
(This idea was adapted from `216`{.interpreted-text role="pep"}.)
...
after a simple assignment. Only the first strings in definitions will be
...
case-insensitive language identifier as defined in
`1766`{.interpreted-text role="rfc"}. A typical language identifier
consists of a 2-letter language code from [ISO
639](http://lcweb.loc.gov/standards/iso639-2/englangn.html) (3-letter
codes used only if no 2-letter code exists; `1766`{.interpreted-text
role="rfc"} is currently being revised to allow 3-letter codes). If no
...
and methods (even when dynamically assigned), parentheses (\'()\') may
be included:
