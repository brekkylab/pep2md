...
author:
- Kevin D. Smith <Kevin.Smith@theMorgue.org>
- Jim J. Jewett
- Skip Montanaro
- Anthony Baxter
status: Final
...
source_path: https://github.com/python/peps/blob/main/peps/pep-0318.rst
source_commit: dbfcaf5d95e1a2119864bad2529ba2cbacdc89f7
---
...
functions this separates a key component of the function\'s behavior
...
`3129`{.interpreted-text role="pep"} proposes to add class decorators as
of Python 2.6.
...
- Disagreement about where the \"declaration of intent\" belongs. Almost
  everyone agrees that decorating/transforming a function at the end of
  its definition is suboptimal. Beyond that there seems to be no clear
  consensus where to place this information.
- Syntactic constraints. Python is a syntactically simple language with
  fairly strong constraints on what can and can\'t be done without
  \"messing things up\" (both visually and with regards to the language
  parser). There\'s no obvious way to structure this information so that
  people new to the concept will think, \"Oh yeah, I know what you\'re
  doing.\" The best that seems possible is to keep new users from
...
- Overall unfamiliarity with the concept. For people who have a passing
  ...
  least one other programming language, much of Python is intuitive.
  Very few people will have had any experience with the decorator
  concept before encountering it in Python. There\'s just no strong
  preexisting meme that captures the concept.
- Syntax discussions in general appear to cause more contention than
  ...
  discussions that were associated with `308`{.interpreted-text
  role="pep"} for another example of this.
...
decorators](http://www.python.org/doc/essays/ppt/python10/py10keynote.pdf)
in his DevDay keynote presentation at the [10th Python
Conference](http://www.python.org/workshops/2002-02/), though [he later
said](https://mail.python.org/pipermail/python-dev/2002-February/020017.html)
it was only one of several extensions he proposed there
\"semi-jokingly\". [Michael Hudson raised the
topic](https://mail.python.org/pipermail/python-dev/2002-February/020005.html)
on `python-dev` shortly after the conference, attributing the initial
bracketed syntax to an earlier proposal on `comp.lang.python` by [Gareth
McCaughan](http://groups.google.com/groups?hl=en&lr=&ie=UTF-8&oe=UTF-8&selm=slrna40k88.2h9o.Gareth.McCaughan%40g.local).
...
proposals to [EuroPython
2004](http://www.python.org/doc/essays/ppt/euro2004/euro2004.pdf), where
a discussion took place. Subsequent to this, he decided that we\'d have
the [Java-style]() \@decorator syntax, and this appeared for the first
time in 2.4a2. Barry Warsaw named this the \'pie-decorator\' syntax, in
honor of the Pie-thon Parrot shootout which occurred around the same
time as the decorator syntax, and because the @ looks a little like a
pie. Guido [outlined his
case](https://mail.python.org/pipermail/python-dev/2004-August/author.html)
on Python-dev, including [this
piece](https://mail.python.org/pipermail/python-dev/2004-August/046672.html)
...
consistent with its use in the [GoF
book](https://web.archive.org/web/20031204182047/http://patterndigest.com/patterns/Decorator.html).
The name \'decorator\' probably owes more to its use in the compiler
area \-- a syntax tree is walked and annotated. It\'s quite possible
that a better name may turn up.
...
- allow future compilers to optimize for decorators. With the hope of a
  JIT compiler for Python coming into existence at some point this tends
  to require the syntax for decorators to come before the function
  definition
- move from the end of the function, where it\'s currently hidden, to
  the front where it is more [in your
  face](https://mail.python.org/pipermail/python-dev/2004-August/047112.html)

Andrew Kuchling has links to a bunch of the discussions about
motivations and use cases [in his
blog](http://www.amk.ca/diary/archives/cat_python.html#003255).
Particularly notable is [Jim Huginin\'s list of use
cases](https://mail.python.org/pipermail/python-dev/2004-April/044132.html).
...
application](https://mail.python.org/pipermail/python-dev/2004-September/048874.html)
(bottom to top) is that it matches the usual order for
function-application. In mathematics, composition of functions (g o
f)(x) translates to g(f(x)). In Python, `@g @f def foo()` translates to
`foo=g(f(foo)`.
...
expression returns is called. See [declaration
arguments](https://mail.python.org/pipermail/python-dev/2004-September/048874.html).

# Syntax Alternatives

There have been [a large
number](http://www.python.org/moin/PythonDecorators) of different
syntaxes proposed \-- rather than attempting to work through these
individual syntaxes, it\'s worthwhile to break the syntax discussion
down into a number of areas. Attempting to discuss [each possible
syntax](http://ucsu.colorado.edu/~bethard/py/decorators-output.py)
individually would be an act of madness, and produce a completely
unwieldy PEP.
...
primary one is that it\'s the first real Python case where a line of
code has an effect on a following line. The syntax available in 2.4a3
...
Guido decided [he
preferred](https://mail.python.org/pipermail/python-dev/2004-March/043756.html)
having the decorators on the line before the \'def\', because it was
felt that a long argument list would mean that the decorators would be
\'hidden\'
...
breaks easily \'greppability\' of the source \-- you can no longer
search for \'def foo(\' and find the definition of the function. The
second, more serious, objection is that in the case of multiple
decorators, the syntax would be extremely unwieldy.
...
Guido [summarized the
arguments](https://mail.python.org/pipermail/python-dev/2004-August/047112.html)
against this form (many of which also apply to the previous form) as:
...
inside\" the method body to determine the decorators. In addition, even
though the code is inside the method body, it is not executed when the
method is run. Guido felt that docstrings were not a good
...
example, we\'ll use a \'decorate\' keyword, as it makes no sense with
the \@syntax. :
...
- `@decorator`:
  ...
  currently used in Python (and is used in both IPython and Leo), and
  that the @ symbol is not meaningful. Another objection is that this
  \"wastes\" a currently unused character (from a limited set) on
...
- `decorate()`
  ...
  > is that decorators need to be added as a syntactic feature to avoid
  > the problems with the postfix notation used in 2.2 and 2.3.
...
- [new keyword (and block)]{#new-keyword-and-block}
  ...
  this in [Community Consensus](#community-consensus) below.) Robert
  Brewer wrote up a detailed [J2
  proposal](http://www.aminus.org/rbre/python/pydec.html) document
  outlining the arguments in favor of this form. The initial issues with
  this form are:
  ...
  A few days later, Guido [rejected the
  proposal](https://mail.python.org/pipermail/python-dev/2004-September/048518.html)
  on two main grounds, firstly:
  ...
  > attention to it. This is true for \"if\", \"while\", \"for\",
  > \"try\", \"def\" and \"class\". But the \"using\" keyword (or any
  ...
  Readers are invited to read [the full
  response](https://mail.python.org/pipermail/python-dev/2004-September/048518.html).

- Other forms

  There are plenty of other variants and proposals on [the wiki
  page](https://wiki.python.org/moin/PythonDecoratorProposals).

## Why @?

There is some history in Java using @ initially as a marker in [Javadoc
comments](http://java.sun.com/j2se/javadoc/writingdoccomments/) and
later in Java 1.5 for
[annotations](http://java.sun.com/j2se/1.5.0/docs/guide/language/annotations.html),
which are similar to Python decorators. The fact that @ was previously
...
of what is a decorator and what isn\'t is removed. That said, @ is still
a fairly arbitrary choice. Some have suggested using \| instead.
...
Russell stepped up and posted a
[patch](https://bugs.python.org/issue979728) to SF. This new syntax was
available in 2.4a2. :
...
A [previous
patch](http://starship.python.net/crew/mwh/hacks/meth-syntax-sugar-3.diff)
from Michael Hudson which implements the list-after-def syntax is also
...
number of alternatives in the [Python
wiki](http://wiki.python.org/moin/PythonDecorators), a community
consensus emerged (below). Guido [subsequently
rejected](https://mail.python.org/pipermail/python-dev/2004-September/048518.html)
this alternate form, but added:
...
> IPython\'s author doesn\'t think there\'s going to be much impact;
> Leo\'s author has said that Leo will survive (although it will cause
> him and his users some transitional pain). I actually expect that
> picking a character that\'s already used elsewhere in Python\'s syntax
> might be harder for external tools to adapt to, since parsing will
> have to be more subtle in that case. But I\'m frankly undecided, so
> there\'s some wiggle room here. I don\'t want to consider further
...
syntax (the \"J2\" was how it was referenced on the PythonDecorators
wiki page): the new keyword `using` prefixing a block of decorators
before the `def` statement. For example:
...
Robert Brewer wrote a [detailed
proposal](http://www.aminus.org/rbre/python/pydec.html) for this form,
and Michael Sparks produced [a
patch](https://bugs.python.org/issue1013835).

As noted previously, Guido rejected this form, outlining his problems
with it in [a
message](https://mail.python.org/pipermail/python-dev/2004-September/048518.html)
to python-dev and comp.lang.python.
...
1.  It\'s not yet certain that class decorators will be incorporated
    into the language at a future point. Guido expressed skepticism
    about the concept, but various people have made some [strong
    arguments](https://mail.python.org/pipermail/python-dev/2004-March/thread.html)
    (search for `PEP 318 -- posting draft`) on their behalf in
    `python-dev`. It\'s exceedingly unlikely that class decorators will
    be in Python 2.4.

    `3129`{.interpreted-text role="pep"} proposes to add class
    decorators as of Python 2.6.
