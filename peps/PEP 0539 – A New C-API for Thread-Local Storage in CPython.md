...
source_path: https://github.com/python/peps/blob/main/peps/pep-0539.rst
source_commit: 7effa9778f0ed0d45d944e2ac221a9b34cbd6fc2
---
...
Solution](#rationale-for-proposed-solution) for the origin of the name).
...
Storage and should not be confused with \"Transportation Layer
Security\" protocols.
::::
...
|                   | - reinitialize keys    |   instead)[^4]         |
|                   |   (after fork)         | - (unnecessary)[^5]    |
|                   |                        | - dynamically          |
|                   |                        |   (de-)allocate key    |
|                   |                        | - check key\'s         |
...
|                   | platforms where native | allocate keys when     |
|                   | TLS key is defined in  | `Py_LIMITED_API` is    |
|                   | a way that cannot be   | defined.               |
|                   | safely cast to `int`.  |                        |
+-------------------+------------------------+------------------------+
...
A new \"Native Thread Implementation\" section will be added to
...
- As of CPython 3.7, all platforms are required to provide a native
  ...
  TSS API. Any TSS API problems that occur in an implementation without
  native threads will be closed as \"won\'t fix\".
...
standard (as with `Py_tss_t` described above)[^8]. This leaves it up to
the underlying implementation how a `pthread_key_t` value is used to
look up thread-specific data.
...
pthreads TLS was added[^9].

It could be argued that `11`{.interpreted-text role="pep"} makes
specific requirements for supporting a new, not otherwise
officially-support platform (such as CloudABI), and that the status of
Cygwin support is currently dubious. However, this creates a very high
barrier to supporting platforms that are otherwise Linux- and/or
POSIX-compatible and where CPython might otherwise \"just work\" except
for this one hurdle. CPython itself imposes this implementation barrier
by way of an API that is not compatible with POSIX (and in fact makes
invalid assumptions about pthreads).
...
Since the existing TLS API has been available in *the limited API*[^10]
...
the C11 threads API[^11]. However, this is in no way meant to imply
...
as opaque[^12]. So there is no unsigned integer value that can be safely
...
type contains an explicit `._is_initialized` that can indicate the
key\'s initialization state independent of the underlying
implementation.
...
- Do nothing: The status quo is fine because it works on Linux, and
  ...
  requirements of `11`{.interpreted-text role="pep"}. As explained
  above, while this would be a fair argument if CPython were being to
  asked to make changes to support particular quirks or features of a
  specific platform, in this case it is a quirk of CPython that prevents
  it from being used to its full potential on otherwise POSIX-compliant
  platforms. The fact that the current implementation happens to work on
  Linux is a happy accident, and there\'s no guarantee that this will
  never change.
...
- Affected platforms should use CPython\'s built-in TLS implementation
  ...
  to do just that[^14]. However, the built-in implementation being
  ...
  a native TLS implementation. This idea also cannot be adopted because
  the built-in implementation has since been removed.
- Keep the existing API, but work around the issue by providing a
  ...
  were made at this ([^15],[^16]), but this injects needless complexity
  and overhead into performance-critical code on platforms that are not
...
An initial version of a patch[^17] is available on the bug tracker for
this issue. Since the migration to GitHub, its development has continued
in the `pep539-tss-api` feature branch[^18] in Masayuki Yamamoto\'s fork
...
[^7]: <http://bugs.python.org/issue30832>

[^8]: <http://pubs.opengroup.org/onlinepubs/009695399/functions/pthread_key_create.html>

[^9]: <https://bugs.python.org/msg116292>

[^10]: It is also called as \"stable ABI\" (`384`{.interpreted-text
    role="pep"})

[^11]: <http://www.open-std.org/jtc1/sc22/wg14/www/docs/n1570.pdf#page=404>

[^12]: <https://msdn.microsoft.com/en-us/library/windows/desktop/ms686801(v=vs.85>).aspx

[^13]: <https://bugs.python.org/issue31370>

[^14]: <http://bugs.python.org/file45548/configure-pthread_key_t.patch>

[^15]: <http://bugs.python.org/file44269/issue25658-1.patch>

[^16]: <http://bugs.python.org/file44303/key-constant-time.diff>

[^17]: <http://bugs.python.org/file46379/pythread-tss-3.patch>

[^18]: <https://github.com/python/cpython/compare/master>\...ma8ma:pep539-tss-api

[^19]: <https://github.com/python/cpython/pull/1362>
