...
source_path: https://github.com/python/peps/blob/main/peps/pep-0627.rst
source_commit: 0f54ef5d2d016f9c907b022ae6c5e7356ce757be
---
...
`RECORD` (which `376`{.interpreted-text role="pep"} lists as mandatory,
but suggests it can be left out for \"system packages\"), and
`INSTALLER`.
...
`376`{.interpreted-text role="pep"} is not written as an
interoperability standard. It describes implementation details of
specific tools and libraries, and is underspecified, leaving much room
for implementation-defined behavior.

This is a proposal to "distill" the standard from
`376`{.interpreted-text role="pep"}, clarify it, and rewrite it to be
tool-agnostic.
...
`376`{.interpreted-text role="pep"}\'s rationale focuses on two
problems:
...
term. The PyPA links to `376`{.interpreted-text role="pep"} under the
heading *Recording installed distributions*.
...
`*.pyc` and `RECORD` itself. (Note that `376`{.interpreted-text
role="pep"} is unclear on what was optional; when taken literally, its
text and examples contradict. Despite that, "both fields are optional"
is a reasonable interpretation of `376`{.interpreted-text role="pep"}.
The alternative would be to mandate---rather than recommend---which
files can be recorded without hash and size, and to update that list
over time as new use cases come up.)
...
`376`{.interpreted-text role="pep"} was unclear on this: it mandates
forward slashes in one place, but shows backslackes in a
