...
source_path: https://github.com/python/peps/blob/main/peps/pep-3105.rst
source_commit: b145625e17d1159bddfd73bf94aca4ef196915db
---
...
- `print` is the only application-level functionality that has a
  ...
  from the compiler. Print doesn\'t qualify for such an exception.
...
- There\'s no easy way to convert `print` statements into another call
  if one needs a different separator, not spaces, or none at all. Also,
...
- If `print()` is a function, it would be much easier to replace it
  ...
  and assigning that to `sys.stdout` \-- that\'s not bad, but definitely
  a much larger conceptual leap, and it works at a different level than
  print.
