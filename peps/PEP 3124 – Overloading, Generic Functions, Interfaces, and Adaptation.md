...
See
<https://mail.python.org/pipermail/python-3000/2007-July/008784.html>.
...
3.  does not allow dispatching on multiple argument types (except in a
    limited form for arithmetic operators, where \"right-hand\"
...
and related issues, using decorators and argument annotations
(`3107`{.interpreted-text role="pep"}). The primary features to be
provided are:

- a dynamic overloading facility, similar to the static overloading
  found in languages such as Java and C++, but including optional method
...
(Note also that, although `3119`{.interpreted-text role="pep"} proposes
that it should be possible for abstract base classes like `Iterable` to
allow classes like `MyString` to claim subclass-hood, such a claim is
*global*, throughout the application. In contrast, adding a specific
overload or copying a rule is specific to an individual function, and
therefore less likely to have undesired side effects.)
...
the [Extension API](#extension-api), and will then be usable with
`@when` and other decorators created by this module (like `@before`,
`@after`, and `@around`).
...
[Interfaces and Adaptation](#interfaces-and-adaptation) section),
including user-defined interface types. (They can also be other objects
whose types are appropriately registered via the [Extension
API](#extension-api).)
...
methods cannot have `__proceed__` arguments, as they are not responsible
for calling any other methods. They are simply called as a notification
...
described in more detail under the [Extension API](#extension-api)
section.
...
is also described in more detail under the [Extension
API](#extension-api) section.
...
`3115`{.interpreted-text role="pep"}, however, requires that a class\'
metaclass be determined *before* the class body has executed, making it
impossible to use this technique for class decoration any more.
...
- Be notified of an action in order to cause some related operation to
  be performed, either before the original operation is performed, after
  it, or both. This can include general-purpose operations like adding
...
that is currently eliminated in `3115`{.interpreted-text role="pep"}.
