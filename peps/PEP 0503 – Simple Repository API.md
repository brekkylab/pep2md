...
URL must respond with a valid HTML5 page with a single anchor element
per file for the project. The href attribute **MUST** be a URL that
links to the location of the file for download, and the text of the
anchor tag **MUST** match the final path component (the filename) of the
URL. The URL **SHOULD** include a hash in the form of a URL fragment
with the following syntax: `#<hashname>=<hashvalue>`, where `<hashname>`
is the lowercase name of the hash function (such as `sha256`) and
...
- A repository **MAY** include a `data-gpg-sig` attribute on a file link
  ...
  there is a GPG signature. Repositories that do this **SHOULD** include
  it on every link.

- A repository **MAY** include a `data-requires-python` attribute on a
  file link. This exposes the *Requires-Python* metadata field,
  specified in `345`{.interpreted-text role="pep"}, for the
  corresponding release. Where this is present, installer tools
  **SHOULD** ignore the download when installing to a Python version
...
`426`{.interpreted-text role="pep"} the only valid characters in a name
are the ASCII alphabet, ASCII numbers, `.`, `-`, and `_`. The name
should be lowercased with all runs of the characters `.`, `-`, or `_`
replaced with a single `-` character. This can be implemented in Python
with the `re` module:
