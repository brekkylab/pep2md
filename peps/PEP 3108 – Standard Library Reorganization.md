...
The merging of profile/cProfile as of Python 3.3 did not occur, and thus
is considered abandoned (although it would be acceptable to do in the
future).
::::
...
introduction of a naming convention for modules since Python\'s
...
that they follow the `Python style guide <8>`{.interpreted-text
role="pep"}. This PEP lists modules that should not be included in
Python 3.0 or which need to be renamed.

# Modules to Remove

Guido pronounced that \"silly old stuff\" is to be deleted from the
stdlib for Py3K[^1]. This is open-ended on purpose. Each module to be
removed needs to have a justification as to why it should no longer be
distributed with Python. This can range from the module being deprecated
in Python 2.x to being for a platform that is no longer widely used.
...
`4`{.interpreted-text role="pep"} lists all modules that have been
deprecated in the stdlib. The specified motivations mirror those listed
in `4`{.interpreted-text role="pep"}. All modules listed in the PEP at
the time of the first alpha release of Python 3.0 will be removed.
...
The IRIX operating system is no longer produced[^2]. Removing all
modules from the plat-irix\[56\] directory has been deemed reasonable
...
- DEVICE/GL/gl/cgen/cgensuport
  ...
  - Third-party libraries provide better support (PyOpenGL[^3]).
- ERRNO
  - Undocumented.
- FILE
  - Undocumented.
- FL/fl/flp
  - Wrapper for the FORMS library[^4]
...
- imgfile
  - Wrapper for SGI libimage library for imglib image files (`.rgb`
    files).
  - Python Imaging Library provides read-only support[^5].
...
- user
  - Easily handled by allowing the application specify its own module
    name, check for existence, and import if found.
- new
...
- bsddb185 \[done\]
  ...
  - Available externally from [PyPI](http://pypi.python.org/).
- Canvas \[done\]
...
- commands \[done\]
  - subprocess module replaces it (`324`{.interpreted-text role="pep"}).
  - Remove getstatus(), move rest to subprocess.
- compiler \[done\]
  - Having to maintain both the built-in compiler and the stdlib package
    is redundant[^8].
  - The AST created by the compiler is available[^9].
...
- imageop \[done\]
  - Better support by third-party libraries (Python Imaging
    Library[^10]).
...
- thread \[done\]
  - People should use \'threading\' instead.
    - Rename \'thread\' to [thread]{#thread}.
    - Deprecate dummy_thread and rename [dummy_thread]{#dummy_thread}.
    - Move thread.get_ident over to threading.
  - Guido has previously supported the deprecation [^11].
- urllib \[done\]
...
Many modules existed in the stdlib before `8`{.interpreted-text
role="pep"} came into existence. This has led to some naming
inconsistencies and namespace bloat that should be addressed.

## PEP 8 violations \[done\]

`8`{.interpreted-text role="pep"} specifies that modules \"should have
short, all-lowercase names\" where \"underscores can be used \... if it
improves readability\". The use of underscores is discouraged in package
names. The following modules violate `8`{.interpreted-text role="pep"}
and are not somehow being renamed by being moved to a package.
...
of the Python development team for renaming. See [Open
Issues](#open-issues) for a discussion on this.

- pickle/cPickle \[done\]
  - Rename cPickle to [pickle]{#pickle}.
  - Semantic completeness of C implementation *not* verified.
- profile/cProfile \[TODO\]
  - Rename cProfile to [profile]{#profile}.
  - Semantic completeness of C implementation *not* verified.
...
### dbm package
  ...
  Current Name   Replacement Name
  -------------- -----------------------
  anydbm         dbm.\_\_init\_\_[^12]
  dbhash         dbm.bsd
  dbm            dbm.ndbm
  dumbdm         dbm.dumb
  gdbm           dbm.gnu
  whichdb        dbm.\_\_init\_\_[^13]
...
### http package
  ...
  ------------------ ------------------
  httplib            http.client
  BaseHTTPServer     http.server[^14]
  CGIHTTPServer      http.server[^15]
  SimpleHTTPServer   http.server[^16]
  Cookie             http.cookies
...
### tkinter package
  ...
  FileDialog       tkinter.filedialog[^17]
  FixTk            tkinter.\_fix
  ScrolledText     tkinter.scrolledtext
  SimpleDialog     tkinter.simpledialog[^18]
  Tix              tkinter.tix
  Tkconstants      tkinter.constants
  Tkdnd            tkinter.dnd
  Tkinter          tkinter.\_\_init\_\_
  tkColorChooser   tkinter.colorchooser
  tkCommonDialog   tkinter.commondialog
  tkFileDialog     tkinter.filedialog[^19]
  tkFont           tkinter.font
  tkMessageBox     tkinter.messagebox
  tkSimpleDialog   tkinter.simpledialog[^20]
...
package.
  ...
  urllib         urllib.parse, urllib.request, urllib.error[^21]
...
### xmlrpc package
  ...
  DocXMLRPCServer      xmlrpc.server[^22]
  SimpleXMLRPCServer   xmlrpc.server[^23]
...
- [Issue 2775](http://bugs.python.org/issue2775): Master tracking issue
- [Issue 2828](http://bugs.python.org/issue2828): clean up undoc.rst
...
people transition to new module names [^24]. Import statements will be
rewritten so that only the import statement and none of the rest of the
code needs to be touched. This will be accomplished by using the `as`
keyword in import statements to bind in the module namespace to the old
name while importing based on the new name (when the keyword is not
...
1.  In the module\'s documentation, add a note mentioning that the
    module is renamed in Python 3.0:
...
xml.etree.ElementTree not only does not meet `8`{.interpreted-text
role="pep"} naming standards but it also has an exposed C
implementation. It is an externally maintained package, though
`360`{.interpreted-text role="pep"}. A request will be made for the
maintainer to change the name so that it matches `8`{.interpreted-text
role="pep"} and hides the C implementation.
...
[^7]: appscript (<http://appscript.sourceforge.net/>)

[^8]: python-dev email: getting compiler package failures
    (<https://mail.python.org/pipermail/python-3000/2007-May/007615.html>)

[^9]: [ast]{#ast} module (<http://docs.python.org/library/ast.html>)

[^10]: Python Imaging Library (PIL)
    (<http://www.pythonware.com/products/pil/>)

[^11]: Python-Dev email: Autoloading?
    (<https://mail.python.org/pipermail/python-dev/2005-October/057244.html>)

[^12]: `dbm.__init__` can combine `anybdbm` and `whichdb` since the
    public API for both modules has no name conflict and the two modules
    have closely related usage.

[^13]: `dbm.__init__` can combine `anybdbm` and `whichdb` since the
    public API for both modules has no name conflict and the two modules
    have closely related usage.

[^14]: The `http.server` module can combine the specified modules safely
    as they have no naming conflicts.

[^15]: The `http.server` module can combine the specified modules safely
    as they have no naming conflicts.

[^16]: The `http.server` module can combine the specified modules safely
    as they have no naming conflicts.

[^17]: `tkinter.filedialog` can safely combine `FileDialog` and
    `tkFileDialog` as there are no naming conflicts.

[^18]: `tkinter.simpledialog` can safely combine `SimpleDialog` and
    `tkSimpleDialog` have no naming conflicts.

[^19]: `tkinter.filedialog` can safely combine `FileDialog` and
    `tkFileDialog` as there are no naming conflicts.

[^20]: `tkinter.simpledialog` can safely combine `SimpleDialog` and
    `tkSimpleDialog` have no naming conflicts.

[^21]: The quoting-related functions from `urllib` will be added to
    `urllib.parse`. `urllib.URLOpener` and `urllib.FancyUrlOpener` will
    be added to `urllib.request` as long as the documentation for both
    modules is updated.

[^22]: The modules being combined into `xmlrpc.server` have no naming
    conflicts and thus can safely be merged.

[^23]: The modules being combined into `xmlrpc.server` have no naming
    conflicts and thus can safely be merged.

[^24]: 2to3 refactoring tool
