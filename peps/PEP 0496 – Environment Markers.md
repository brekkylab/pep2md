...
source_path: https://github.com/python/peps/blob/main/peps/pep-0496.rst
source_commit: 70c410dcb083f54ff30ae44c40ba41bd100b18d4
---

# PEP Status

After this PEP was initially drafted, `508`{.interpreted-text
role="pep"} was developed and submitted to fully specify the dependency
declaration syntax, including environment markers. As a result, this PEP
ended up being rejected in favour of the more comprehensive
...
Environment markers were first specified in `345`{.interpreted-text
role="pep"}. `426`{.interpreted-text role="pep"} (which would replace
`345`{.interpreted-text role="pep"}) proposed extensions to the markers.
When 2.7.10 was released, even these extensions became insufficient due
to their reliance on simple lexical comparisons, and thus this PEP has
been born.
...
`VEREXPR` is a `440`{.interpreted-text role="pep"} version identifier,
or one of the `Version number` marker variables listed below.
Comparisons between version numbers are done using
`440`{.interpreted-text role="pep"} semantics.
