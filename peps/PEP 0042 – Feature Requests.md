...
source_path: https://github.com/python/peps/blob/main/peps/pep-0042.rst
source_commit: 1c791149a2f9e0605d59088dc58e8be7222e8e5a
---

::: withdrawn
It is
[obsolete](https://github.com/python/peps/pull/108#issuecomment-249603204).
All new feature requests should either go to the [Python bug
tracker](https://github.com/python/cpython/issues) for very simple
requests or the [Ideas Discourse
category](https://discuss.python.org/c/ideas/6) for everything else. The
...
PEP is created. See `0`{.interpreted-text role="pep"} for details.
...
2.  BDFL will put in if somebody writes the code. (Or at any rate, BDFL
    ...
    > b)  BDFL is never going to be enthusiastic about this, but will
    >     work it in when it\'s easy.
...
- signal handling doesn\'t always work as expected. E.g. if
  ...
  to applied to all places that can do blocking interruptible I/O. So
  it\'s a big project.
...
- rfc822.py should be more lenient than the spec in the types of address
  ...
  Amazon.com \<delivers-<news2@amazon.com>\>\" should be parsed
  correctly.
...
- urlparse should be updated to comply with `2396`{.interpreted-text
  role="rfc"}, which defines optional parameters for each segment of the
  path.

  <https://bugs.python.org/issue210834>

- The exceptions raised by pickle and cPickle are currently different;
  these should be unified (probably the exceptions should be defined in
  a helper module that\'s imported by both). \[No bug report; I just
  thought of this.\]
...
- For a class defined inside another class, the \_\_name\_\_ should be
...
- Use pending deprecation for the types and string modules. This
  ...
  (e.g. string.whitespace and types.TracebackType). It seems we can\'t
  get consensus on this.
...
- Modules/makesetup should make sure the \'config.c\' file it generates
  from the various Setup files, is valid C. It currently accepts module
  names with characters that are not allowable in Python or C
  identifiers.
