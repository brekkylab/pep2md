...
source_path: https://github.com/python/peps/blob/main/peps/pep-0333.rst
source_commit: 29e5d65330ced48c8ad68ff01f8f97e3a008aebb
---

# Preface

Note: For an updated version of this spec that supports Python 3.x and
includes community errata, addenda, and clarifications, please see
`3333`{.interpreted-text role="pep"} instead.
...
their own buffering. See the [Buffering and
...
the application). This protocol is intended to complement
`325`{.interpreted-text role="pep"}\'s generator support, and other
common iterables with `close()` methods.
...
variables, as defined by the Common Gateway Interface specification[^2].
...
variables[^3] as are applicable, such as `HTTPS=on` and `SSL_PROTOCOL`.
...
It is a violation of this specification for a CGI variable\'s value to
be of any type other than `str`.
...
2.  The optional \"size\" argument to `readline()` is not supported, as
    it may be complex for server authors to implement, and is not often
    used in practice.
...
be used, for any value referred to in this specification as a
\"string\".
...
that transmission begins, and continue until the end is reached.
