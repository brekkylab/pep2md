...
source_path: https://github.com/python/peps/blob/main/peps/pep-3099.rst
source_commit: da1fc62e6cb5413ab4a760ad2e4ef826a4245b56
---
...
- Slices and extended slices won\'t go away (even if the
  \_\_getslice\_\_ and \_\_setslice\_\_ APIs may be replaced) nor will
  they return views for the standard object types.
...
- The parser won\'t be more complex than LL(1).
  ...
  > Restricting Python\'s grammar to an LL(1) parser is a blessing, not
  > a curse. It puts us in handcuffs that prevent us from going
...
- No more backticks.
  ...
  > ignoring the backwards compatibility confusion, the character itself
  > causes too many problems (in some fonts, on some keyboards, when
...
- Referencing the global name `foo` will not be spelled `globals.foo`.
  ...
  > Threads: \"replace globals() and global statement with global
  > builtin object\",
