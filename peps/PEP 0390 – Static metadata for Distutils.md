...
A replacement PEP based on `426`{.interpreted-text role="pep"} (metadata
2.0) will be created that defines the minimum amount of information
needed to generate an sdist archive given a source tarball or VCS
checkout.

# Rationale

Today, if you want to list all the Metadata of a distribution (see
`314`{.interpreted-text role="pep"}) that is not installed, you need to
use the `setup.py` command line interface.
...
compatible with `ConfigParser` and the `822`{.interpreted-text
role="rfc"} `LONG HEADER FIELDS` (see section 3.1.1), these are
expressed with `,`-separated values:
...
(see `314`{.interpreted-text role="pep"}).
...
`314`{.interpreted-text role="pep"} is changed accordingly, meaning that
each field will be able to have that extra condition marker.
...
Although, if `386`{.interpreted-text role="pep"} is accepted,
`python_version` could be changed internally into something comparable
with strings, and `<` and `>` operators introduced.
