...
bdfl_delegate: Victor Stinner <vstinner@python.org>
status: Final
...
source_path: https://github.com/python/peps/blob/main/peps/pep-0471.rst
source_commit: 0f9a04bb7c440e380c3358e708c438bbf5e111ba
---
...
more [benchmarks here](https://github.com/benhoyt/scandir#benchmarks).

Somewhat relatedly, many people (see Python [Issue
11406](http://bugs.python.org/issue11406)) are also keen on a version of
`os.listdir()` that yields filenames as it iterates instead of returning
them as one big list. This improves memory efficiency for iterating very
large directories.
...
It lives on GitHub at
[benhoyt/scandir](https://github.com/benhoyt/scandir). (The
implementation may lag behind the updates to this PEP a little.)
...
- Instead of returning bare filename strings, it returns lightweight
  ...
  methods that allow access to the additional data the operating system
  may have returned.
...
call the regular `os.stat()` or `os.path.getsize()` functions which get
fresh data from the operating system every call.
...
- **python-dev**: a good number of +1\'s and very few negatives for
  scandir and `471`{.interpreted-text role="pep"} on [this June 2014
  python-dev
  thread](https://mail.python.org/pipermail/python-dev/2014-June/135217.html)
- **Alyssa Coghlan**, a core Python developer: \"I\'ve had the local Red
  ...
  \[[source1](http://bugs.python.org/issue11406)\]
- **Tim Golden**, a core Python developer, supports scandir enough to
  ...
  extension module. \[[source2](https://github.com/tjguk/scandir)\]
- **Christian Heimes**, a core Python developer: \"+1 for something like
  yielddir()\"
  \[[source3](https://mail.python.org/pipermail/python-ideas/2012-November/017772.html)\]
  and \"Indeed! I\'d like to see the feature in 3.4 so I can remove my
  own hack from our code base.\"
  \[[source4](http://bugs.python.org/issue11406)\]
- **Gregory P. Smith**, a core Python developer: \"As 3.4beta1 happens
  ...
  \[[source5](http://bugs.python.org/issue11406)\]
- **Guido van Rossum** on the possibility of adding scandir to Python
  ...
  it to 3.4.\"
  \[[source6](https://mail.python.org/pipermail/python-dev/2013-November/130583.html)\]

Support for this PEP itself (meta-support?) was given by Alyssa (Nick)
Coghlan on python-dev: \"A PEP reviewing all this for 3.5 and proposing
a specific os.scandir API would be a good thing.\"
\[[source7](https://mail.python.org/pipermail/python-dev/2013-November/130588.html)\]

# Use in the wild

To date, the `scandir` implementation is definitely useful, but has been
clearly marked \"beta\", so it\'s uncertain how much use of it there is
in the wild. Ben Hoyt has had several reports from people using it. For
example:
...
- bschollnick: \"I wanted to let you know about this, since I am using
  ...
  \[[source8](https://github.com/benhoyt/scandir/issues/19)\]
...
Others have [requested a PyPI
package](https://github.com/benhoyt/scandir/issues/12) for it, which has
been created. See [PyPI package](https://pypi.python.org/pypi/scandir).
...
See some [relevant discussion on
python-dev](https://mail.python.org/pipermail/python-dev/2014-June/135228.html).
...
[blog
post](http://blogs.msdn.com/b/oldnewthing/archive/2007/12/17/6785519.aspx)),
...
would be possible to add at a later date if there\'s a cross-platform
way to achieve it, but not for the initial version.

Read more on the [this Nov 2012 python-ideas
thread](https://mail.python.org/pipermail/python-ideas/2012-November/017770.html)
and this [June 2014 python-dev thread on PEP
471](https://mail.python.org/pipermail/python-dev/2014-June/135217.html).

## Methods not following symlinks by default

There was much debate on python-dev (see messages in [this
thread](https://mail.python.org/pipermail/python-dev/2014-July/135485.html))
...
(see [Issue #4 here](https://github.com/benhoyt/scandir/issues/4)).
...
free. However, this isn\'t quite the case, as `stat()` will require an
OS call on POSIX-based systems but not on Windows. Even `is_dir()` and
...
See [this May 2013 python-dev
thread](https://mail.python.org/pipermail/python-dev/2013-May/126184.html)
...
In [this July 2014 python-dev
message](https://mail.python.org/pipermail/python-dev/2014-July/135303.html),
...
See [this July 2014 python-dev
response](https://mail.python.org/pipermail/python-dev/2014-July/135312.html)
...
Coghlan in this [June 2014 python-dev
message](https://mail.python.org/pipermail/python-dev/2014-June/135261.html):
...
Or it means that `scandir()` would have to accept an `onerror` argument
...
See [Ben Hoyt\'s July 2014
reply](https://mail.python.org/pipermail/python-dev/2014-July/135312.html)
to the discussion summarizing this and detailing why he thinks the
original `471`{.interpreted-text role="pep"} proposal is \"the right
one\" after all.
...
- [May 2013 python-dev
  thread](https://mail.python.org/pipermail/python-dev/2013-May/126148.html)
  where Alyssa Coghlan makes the original case for a `DirEntry`-style
  object.
- [June 2014 python-dev
  thread](https://mail.python.org/pipermail/python-dev/2014-June/135244.html)
...
context of scandir
[here](https://mail.python.org/pipermail/python-dev/2013-November/130583.html),
...
- scandir could potentially be further sped up by calling `readdir` /
  ...
  suggested by on Issue 11406 by Antoine Pitrou.
  \[[source9](http://bugs.python.org/msg130125)\]
- scandir could use a free list to avoid the cost of memory allocation
  ...
  help. Suggested by Victor Stinner on a [python-dev thread on June
  27](https://mail.python.org/pipermail/python-dev/2014-June/135232.html).

# Previous discussion

- [Original November 2012 thread Ben Hoyt started on
  python-ideas](https://mail.python.org/pipermail/python-ideas/2012-November/017770.html)
  about speeding up `os.walk()`
- Python [Issue 11406](http://bugs.python.org/issue11406), which
  includes the original proposal for a scandir-like function
- [Further May 2013 thread Ben Hoyt started on
  python-dev](https://mail.python.org/pipermail/python-dev/2013-May/126119.html)
  that refined the `scandir()` API, including Alyssa Coghlan\'s
  suggestion of scandir yielding `DirEntry`-like objects
- [November 2013 thread Ben Hoyt started on
  python-dev](https://mail.python.org/pipermail/python-dev/2013-November/130572.html)
  to discuss the interaction between scandir and the new `pathlib`
  module
- [June 2014 thread Ben Hoyt started on
  python-dev](https://mail.python.org/pipermail/python-dev/2014-June/135215.html)
  to discuss the first version of this PEP, with extensive discussion
  about the API
- [First July 2014 thread Ben Hoyt started on
  python-dev](https://mail.python.org/pipermail/python-dev/2014-July/135377.html)
  to discuss his updates to `471`{.interpreted-text role="pep"}
- [Second July 2014 thread Ben Hoyt started on
  python-dev](https://mail.python.org/pipermail/python-dev/2014-July/135485.html)
  to discuss the remaining decisions needed to finalize
  `471`{.interpreted-text role="pep"}, specifically whether the
  `DirEntry` methods should follow symlinks by default
- [Question on
  StackOverflow](http://stackoverflow.com/questions/2485719/very-quickly-getting-total-size-of-folder)
  about why `os.walk()` is slow and pointers on how to fix it (this
  inspired the author of this PEP early on)
- [BetterWalk](https://github.com/benhoyt/betterwalk), this PEP\'s
