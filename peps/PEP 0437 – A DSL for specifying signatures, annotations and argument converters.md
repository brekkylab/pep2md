...
source_path: https://github.com/python/peps/blob/main/peps/pep-0437.rst
source_commit: 0136565c33382b92d45591b81cf117178c61c046
---
...
`436`{.interpreted-text role="pep"} proposes a domain specific language
(DSL) enclosed in C comments that largely resembles a per-parameter
configuration file. A preprocessor reads the comment and emits an
argument parsing function, docstrings and a header for the function that
utilizes the results of the parsing step.
...
account when designing the [second iteration of the PEP 436
DSL](http://hg.python.org/peps/rev/a2fa10b2424b).

# Rationale

Opinions differ regarding the suitability of the `436`{.interpreted-text
role="pep"} DSL in the context of a C file. This PEP proposes an
alternative DSL. The specific issues with `436`{.interpreted-text
role="pep"} that spurred the counter proposal will be explained in the
final section of this PEP.
...
custom converters in addition to the well-known default converters
\"i\", \"f\", etc.
...
The \'def\' keyword and the function body are redundant, but the author
of this PEP finds the definition more readable if they are present.
...
The preprocessor translates this definition to a PyArg_ParseTuple()
...
A reference implementation is available at [issue
16612](http://bugs.python.org/issue16612). Since this PEP was written
under time constraints and the author is unfamiliar with the PLY
toolchain, the software is written in Standard ML and utilizes the
...
- C declarations are parsed in a primitive way. The final implementation
  should utilize \'declarator\' and \'init-declarator\' from the C
  grammar.
- The *preprocess* tool does not emit code for the left-and-right
  ...
  case.
- Since the *preprocess* tool generates the output from the parse tree,
  the original indentation of the define block is lost.

# Grammar

> TBD: The grammar exists in ml-yacc readable form, but should probably
> be included here in EBNF notation.

# Comparison with PEP 436

The author of this PEP has the following concerns about the DSL proposed
in `436`{.interpreted-text role="pep"}:
...
- The structure of the function definition gets lost in the
  per-parameter specifications. Keywords like positional-only, required
  and keyword-only are scattered across too many different places.

  By contrast, in the alternative DSL the structure of the function
  definition can be understood at a single glance.

- The `436`{.interpreted-text role="pep"} DSL has 14 documented flags
  and at least one undocumented (allow_fd) flag. Figuring out which of
  the 2\*\*15 possible combinations are valid places an unnecessary
  burden on the user.

  Experience with the `3118`{.interpreted-text role="pep"} buffer flags
  has shown that sorting out (and exhaustively testing!) valid
  combinations is an extremely tedious task. The
  `3118`{.interpreted-text role="pep"} flags are still not well
...
- The `436`{.interpreted-text role="pep"} DSL allows too much freedom.
  Types can apparently be omitted, the preprocessor accepts (and
  ignores) unknown keywords, sometimes adding white space after a
  docstring results in an assertion error.
...
This document is licensed under the [Open Publication
