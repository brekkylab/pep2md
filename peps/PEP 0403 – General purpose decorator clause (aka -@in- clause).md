...
source_path: https://github.com/python/peps/blob/main/peps/pep-0403.rst
source_commit: 1c21ddd061ee2fd9f4d911251daa500cc4762301
---
...
`3150`{.interpreted-text role="pep"} (Statement Local Namespaces) so
some elements of the rationale will be familiar to readers of that PEP.
Both PEPs remain deferred for the time being, primarily due to the lack
of compelling real world use cases in either PEP.
...
sense\" would also be possible (see `3150`{.interpreted-text role="pep"}
...
override the implicit \"name = \<defined function or class\>\" name
binding operation that is part of every function or class definition,
...
the operation that needs it breaks the developer\'s flow of thought.
They get to a point where they go \"I need a one-shot operation that
does \<X\>\", and instead of being able to just *say* that directly,
they instead have to back up, name a function to do \<X\>, then call
that function from the operation they actually wanted to do in the first
...
`3150`{.interpreted-text role="pep"} (Statement Local Namespaces)
describes its primary motivation as being to elevate ordinary assignment
...
details of how the value of that item is calculated. This PEP achieves
the same goal in a different way, by allowing the simple name binding of
a standard function definition to be replaced with something else (like
assigning the result of the function to a value).
...
However, the introduction of qualified names in `3155`{.interpreted-text
role="pep"} means that even anonymous classes and functions will now
have different representations if they occur in different scopes. For
example:
...
This proposal has at least one titanic advantage over
`3150`{.interpreted-text role="pep"}: implementation should be
...
using `@`. Feedback on this variant was almost universally negative, as
it was considered both ugly and excessively magical:
...
`3150`{.interpreted-text role="pep"}. It\'s comparatively difficult to
implement properly, the scoping semantics are harder to explain and it
creates quite a few situations where there are two ways to do it without
clear guidelines for choosing between them (as almost any construct that
can be expressed with ordinary imperative code could instead be
expressed using a given statement). While the PEP does propose some new
`8`{.interpreted-text role="pep"} guidelines to help address that last
problem, the difficulties in implementation are not so easily dealt
with.
...
*semantics* of this PEP, while adopting the prettier syntax from
`3150`{.interpreted-text role="pep"}:
...
latest version of `3150`{.interpreted-text role="pep"} uses explicit
forward reference and name binding schemes that greatly simplify the
semantics of the statement, and it does offer the advantage of allowing
the definition of arbitrary subexpressions rather than being restricted
to a single function or class definition.
