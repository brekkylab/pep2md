...
source_path: https://github.com/python/peps/blob/main/peps/pep-0421.rst
source_commit: 86f66e1d6a4ec29fadf8b6be882c17bc4ede23b6
---
...
Attributes](#required-attributes) section below. Those attribute names
will never start with an underscore. The standard library and the
language definition will rely only on those required attributes.
...
discretion, as described in [Adding New Required
Attributes](#adding-new-required-attributes).
...
:   The version of the implementation, as opposed to the version of the
    ...
    in [Version Format](#version-format).
...
:   A string used for the `3147`{.interpreted-text role="pep"} cache
    tag. It would normally be a composite of the name and version (e.g.
    \'cpython-33\' for CPython 3.3). However, an implementation may
    explicitly use a different cache tag. If `cache_tag` is set to None,
...
`metadata` that held any non-required, per-implementation data [^1].
However, this proved to be an unnecessary addition considering the
purpose of `sys.implementation`.
...
As already noted in [Version Format](#version-format), values in
...
otherwise implemented. However, care should be taken to not over-specify
the constraints.

# Discussion

The topic of `sys.implementation` came up on the python-ideas list in
2009, where the reception was broadly positive[^2]. I revived the
discussion recently while working on a pure-python `imp.get_tag()`[^3].
Discussion has been ongoing [^4]. The messages in [issue
#14673](http://bugs.python.org/issue14673) are also relevant.
...
for clues in a couple different `sys` variables[^5]. However, this
approach is fragile, requiring changes to the standard library each time
an implementation changes. Beyond that, support in `platform` is limited
to those implementations that core developers have blessed by
...
`3147`{.interpreted-text role="pep"} defined the use of a module cache
and cache tags for file names. The importlib bootstrap code, frozen into
the Python binary as of 3.3, uses the cache tags during the import
process. Part of the project to bootstrap importlib has been to clean
code out of
[Python/import.c](https://hg.python.org/cpython/file/2f563908ebc5/Python/import.c)
that did not need to be there any longer.

The cache tag defined in `Python/import.c` was
`hard-coded <3147#proposal>`{.interpreted-text role="pep"} to
`"cpython" MAJOR MINOR`. For importlib the options are either
...
([Lib/test/support.py](https://hg.python.org/cpython/file/2f563908ebc5/Lib/test/support.py))
...
of the java environment in the standard library[^6] [^7]. Unfortunately
it masks the os name that would otherwise go there. `sys.implementation`
would help obviate the need for this special case. Currently Jython sets
`os._name` for the normal `os.name` value.
...
sufficiently squishy in the past\"[^8]. With `sys.implementation` we
have the opportunity to improve this situation by first establishing an
...
Jeff Hardy responded to a request for feedback[^9]. He said, \"I\'ll
probably add it the day after it\'s approved\" [^10]. He also gave
useful feedback on both the type of `sys.implementation` and on the
`metadata` attribute (which has since been removed from the PEP).
...
[^12]. Armin Rigo said the following[^13]:
...
`3139`{.interpreted-text role="pep"}, from 2008, recommended a clean-up
of the `sys` module in part by extracting implementation-specific
variables and functions into a separate module. `421`{.interpreted-text
role="pep"} is less ambitious version of that idea. While
`3139`{.interpreted-text role="pep"} was rejected, its goals are
reflected in `421`{.interpreted-text role="pep"} to a large extent,
though with a much lighter approach.

## PEP 399

`399`{.interpreted-text role="pep"} dictates policy regarding the
standard library, helping to make it friendlier to alternate
implementations. `421`{.interpreted-text role="pep"} is proposed in that
same spirit.
...
it is a `3137`{.interpreted-text role="pep"}-lite, but starting as small
as possible.
...
this PEP. (See [Adding New Required
Attributes](#adding-new-required-attributes) if they get you excited.)
...
The implementation of this PEP is covered in [issue
#14673](http://bugs.python.org/issue14673).
