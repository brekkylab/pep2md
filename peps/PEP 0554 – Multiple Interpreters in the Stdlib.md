...
python_version: '3.13'
post_history:
- '`07-Sep-2017 <https://mail.python.org/archives/list/python-ideas@python.org/thread/HQQWEE527HG3ILJVKQTXVSJIQO6NUSIA/>`__'
...
`734`{.interpreted-text role="pep"}. This PEP is kept as-is for the sake
of the various sections of background information and deferred/rejected
ideas that have been stripped from `734`{.interpreted-text role="pep"}.
::::
...
available via the C-API. [\[c-api\]](#c-api){.citation} Multiple
interpreters operate in [relative isolation from one
...
This module is especially relevant since `684`{.interpreted-text
role="pep"} introduced a per-interpreter GIL in Python 3.12.
...
[Examples](#examples) section for concrete usage and use cases.
...
Data](#shared-data) and [API For Communication](#api-for-communication)
sections for more details about sharing/communicating between
interpreters.
...
+-----------------------------+------------------------------------------+
| `.set_main_attrs(**kwargs)` | Bind \"shareable\" objects in            |
|                             | `__main__`.                              |
+-----------------------------+------------------------------------------+
| `.get_main_attr(name)`      | Get a \"shareable\" object from          |
|                             | `__main__`.                              |
+-----------------------------+------------------------------------------+
...
+--------------------------------------------------+------------------------------+
| `create_channel() -> (RecvChannel, SendChannel)` | | Create a new channel for   |
|                                                  |   passing                    |
...
(`489`{.interpreted-text role="pep"}) is considered isolated and thus
compatible with multiple interpreters. Otherwise it is \"incompatible\".
...
which such isolation may extended. (See `684`{.interpreted-text
role="pep"}.)

Alyssa (Nick) Coghlan explained some of the benefits through a
comparison with multi-processing [\[benefits\]](#benefits){.citation}:
...
GIL\--see `684`{.interpreted-text role="pep"}).
...
store internal state. (`3121`{.interpreted-text role="pep"} and
`489`{.interpreted-text role="pep"} provide a solution to that problem,
followed by some extra APIs that improve efficiency, e.g.
`573`{.interpreted-text role="pep"}).
...
(See [Nathaniel\'s
post](https://mail.python.org/archives/list/python-dev@python.org/message/TUEAZNZHVJGGLL4OFD32OW6JJDKM6FAS/)
on python-dev.)
...
is Communicating Sequential Processes [\[CSP\]](#CSP){.citation} (upon
which Go\'s concurrency is roughly based). The intended isolation
inherent to CPython\'s interpreters makes them well-suited to this
approach.
...
*objects* may be shared safely between interpreters; effectively,
objects are bound to the interpreter in which they were created.
Furthermore, the complexity of *object* sharing increases as
interpreters become more isolated, e.g. after GIL removal (though this
is mitigated somewhat for some \"immortal\" objects (see
`683`{.interpreted-text role="pep"}).
...
a basic interim solution using pipes (`os.pipe()`), as well as providing
a dedicated capability (\"channels\"). See [API For
Communication](#api-for-communication) below.
...
usage, including
[mod_wsgi](https://github.com/GrahamDumpleton/mod_wsgi), [OpenStack
...
relatively stable. On the other hand, there isn\'t much of a sample size
from which to judge the utility of the feature.
...
# \"interpreters\" Module API[]{#interpreters-Interpreter}[]{#interpreters-create}[]{#interpreters-get-current}[]{#interpreters-list-all} {#interpreters-is-shareable}
...
channels. See [Channels](#channels) below.
...
(see `traceback.TracebackException`{.interpreted-text role="class"}),
set it to `__cause__` on a new `interpreters.RunFailedError`, and raise
that.
...
As discussed in [Shared Data](#shared-data) above, multiple interpreter
support is less useful without a mechanism for sharing data
(communicating) between them. Sharing actual Python objects between
interpreters, however, has enough potential problems that we are
avoiding support for that in this proposal. Nor, as mentioned earlier,
are we adding anything more than a basic mechanism for communication.
...
\"shareable\". See [Shareable Types](#shareable-types) below.
...
object on the other side. Also see
[is_shareable()](#interpreters-is-shareable) above.
...
- `memoryview`, to allow sharing `3118`{.interpreted-text role="pep"}
  buffers
- [channels](#channels)
...
using an OS pipe](#synchronize-using-an-os-pipe).

## Channels[]{#interpreters-RecvChannel}[]{#interpreters-create-channel} {#interpreters-SendChannel}
...
channels. [\[fifo\]](#fifo){.citation} The main difference from pipes is
that channels can be associated with zero or more interpreters on either
end. Like queues, which are also many-to-many, channels are buffered
(though they also offer methods with unbuffered semantics).
...
copied. One example might be `3118`{.interpreted-text role="pep"}
buffers.
...
[Isolating Extension
Modules](https://docs.python.org/3/howto/isolating-extensions.html)
howto page. Any extra help will be added there. For example, it may
prove helpful to discuss strategies for dealing with linked libraries
that keep their own subinterpreter-incompatible global state.
...
At the very least, it would allow Python types to convert their
instances to some other type that implements `tp_xid`.
...
By using the `PyModuleDef_Slot` introduced by `489`{.interpreted-text
role="pep"}, we could easily add a mechanism by which C-extension
modules could opt out of multiple interpreter support. Then the import
machinery, when operating in a subinterpreter, would need to check the
module for support. It would raise an ImportError if unsupported.

Alternately we could support opting in to multiple interpreters support.
However, that would probably exclude many more modules (unnecessarily)
than the opt-out approach. Also, note that `489`{.interpreted-text
role="pep"} defined that an extension\'s use of the PEP\'s machinery
implies multiple interpreters support.
...
- `importlib.util.reset_globals()`
  [\[reset_globals\]](#reset_globals){.citation}
...
but more generic). See below in [Rejected Ideas](#rejected-ideas) for
more information.
...
(i.e. a script). This is equivalent to `PyRun_StringFlags()`, `exec()`,
or a module body. None of those \"return\" anything. We can revisit this
once `interp.exec()` supports functions, etc.
...
sense to treat them specially when it comes to propagation from
...
if it supports the `3118`{.interpreted-text role="pep"} buffer protocol
(e.g. memoryview).
...
would mean that exception objects would leak across the
inter-interpreter boundary. Likewise, the frames in the traceback would
potentially leak.
...
`Interpreter.exec()` calls in a new `threading.Thread`. Nor would they
be in a position to accidentally pause the current interpreter (in the
current thread) while their interpreter executes.
...
The implementation effort for `554`{.interpreted-text role="pep"} is
being tracked as part of a larger project aimed at improving multi-core
support in CPython.
...
[reset_globals]{#reset_globals .citation-label}
