...
source_path: https://github.com/python/peps/blob/main/peps/pep-0324.rst
source_commit: b5deec81e8804b9175f030d7dee31126a4dcb65a
---
...
- Cross-process exceptions: Exceptions happening in the child before the
  ...
  popen2, for example, it\'s impossible to detect if the execution
  failed.
...
- The factory functions in popen2 have been removed, because I consider
  the class constructor equally easy to work with.
...
- popen2 contains several factory functions and classes for different
  ...
  class. Since the subprocess module supports 12 different combinations
  of redirection, providing a class or function for each of them would
  be cumbersome and not very intuitive. Even with popen2, this is a
  readability problem. For example, many people cannot tell the
  difference between popen2.popen2 and popen2.popen4 without using the
  documentation.
...
- `args` should be a string, or a sequence of program arguments. The
  ...
  uses `os.execvp()` to execute the child program. `args` should
...
- `bufsize`, if given, has the same meaning as the corresponding
  ...
  system default, which usually means fully buffered. The default value
  for `bufsize` is 0 (unbuffered).
...
previous modules[^5] with the same name, but the name \"subprocess\"
