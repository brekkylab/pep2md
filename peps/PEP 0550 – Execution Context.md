...
source_path: https://github.com/python/peps/blob/main/peps/pep-0550.rst
source_commit: 531623bc3fbdb98b7804e5cdc83a4c222eb9b38b
---
...
this PEP has been withdrawn and superseded by a simpler
...
- High-Level Specification (this section): the description of the
  overall solution. We show how it applies to generators and coroutines
  in user code, without delving into implementation details.
...
- changes to context variables in outer context (where the generator is
  being iterated) are visible to the generator, unless these variables
  were also modified inside the generator.
...
- `delete()`: removes the variable from the topmost logical context.
  ...
  restoring context variables](#setting-and-restoring-context-variables)
...
- any changes in the original execution context do not affect the copy,
  and
...
- The `contextvars.run_with_logical_context()` is called, in which case
  the passed logical context object is appended to the execution
  context:
...
- Either `genobj.send()`, `genobj.throw()`, `genobj.close()` are called
  on a `genobj` generator, in which case the logical context recorded in
  `genobj` is pushed onto the stack:
...
- `ContextVar` objects are strongly-referenced only from the application
  ...
  extend their lifespan longer than necessary, or prevent their
  collection by the GC.
...
4.  The new `contextvars.get_execution_context()` function, which
    ...
    current execution context.
5.  The
    `contextvars.run_with_execution_context(ec: ExecutionContext, func, *args, **kwargs)`
...
An equivalent implementation with `521`{.interpreted-text role="pep"}:
...
That said we think it is possible to design new APIs that will be
...
<img src="pep-0550-hamt_vs_dict-v2.png" class="invert-in-dark-mode"
style="width:100.0%"
alt="Figure 1. Benchmark code can be found here:." />
<figcaption aria-hidden="true">Figure 1. Benchmark code can be found
here:<a href="#fn1" class="footnote-ref" id="fnref1"
...
<ol>
<li id="fn1"><p><a
href="https://gist.github.com/1st1/9004813d5576c96529527d44c5457dcd">https://gist.github.com/1st1/9004813d5576c96529527d44c5457dcd</a><a
...
<figure class="invert-in-dark-modealign-center">
<img src="pep-0550-lookup_hamt.png" class="invert-in-dark-mode"
style="width:100.0%"
alt="Figure 2. Benchmark code can be found here:." />
<figcaption aria-hidden="true">Figure 2. Benchmark code can be found
here:<a href="#fn1" class="footnote-ref" id="fnref1"
role="doc-noteref"><sup>1</sup></a>.</figcaption>
</figure>
<section id="footnotes" class="footnotes footnotes-end-of-document"
...
<ol>
<li id="fn1"><p><a
href="https://gist.github.com/1st1/dbe27f2e14c30cce6f0b5fddfc8c437e">https://gist.github.com/1st1/dbe27f2e14c30cce6f0b5fddfc8c437e</a><a
href="#fnref1" class="footnote-back" role="doc-backlink">↩︎</a></p></li>
</ol>
</section>
...
There is research[^11] showing that there are further possible
...
[^13][^14], for pushing the PEP towards a more complete design, and
coming up with the idea of having a stack of contexts in the thread
state.

Thanks to Alyssa (Nick) Coghlan for numerous suggestions and ideas on
the mailing list, and for coming up with a case that cause the complete
rewrite of the initial PEP version[^15].
...
3.  V3 posted on 18-Aug-2017[^19].
    ...
    - Context Item was renamed to Context Key, see the thread with
      Alyssa Coghlan, Stefan Krah, and Yury Selivanov[^20] for details.
    ...
    - Coroutines are created without a Logical Context; ceval loop no
      ...
      Alyssa Coghlan in[^22].)

4.  V4 posted on 25-Aug-2017[^23].

    - The specification section has been completely rewritten.
    - Coroutines now have their own Logical Context. This means there is
      no difference between coroutines, generators, and asynchronous
      generators w.r.t. interaction with the Execution Context.
...
5.  V5 posted on 01-Sep-2017: the current version.
    ...
    - Coroutines have no logical context by default (a revert to the V3
      semantics). Read about the motivation in the [Coroutines not
      leaking context changes by
      default](#coroutines-not-leaking-context-changes-by-default)
      section.

      The [High-Level Specification](#high-level-specification) section
      was also updated (specifically Generators and Coroutines
      subsections).
    ...
    - New [Rejected Ideas](#rejected-ideas), [Should \"yield from\" leak
      context changes?](#should-yield-from-leak-context-changes),
      [Alternative Designs for ContextVar
      API](#alternative-designs-for-contextvar-api), [Setting and
      restoring context
      variables](#setting-and-restoring-context-variables), and [Context
      manager as the interface for
