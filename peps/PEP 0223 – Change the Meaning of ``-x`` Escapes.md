...
source_path: https://github.com/python/peps/blob/main/peps/pep-0223.rst
source_commit: ce209c7e0c9423a991dade487493b65ed1f1f7cd
---
...
followed by more than two hex digits, only the first two are
...
in Python (up to and including 1.6) has been treated the same way across
all platforms: all *except* the last two hex digits were silently
...
for Python 2.0 is in fact what Perl has done from the start (although
you need to run in Perl -w mode to get warned about `\x` escapes with
fewer than 2 hex digits following \-- it\'s clearly more Pythonic to
insist on 2 all the time).
...
tries very hard to allow mixing 8-bit and Unicode patterns and strings
in intuitive ways, and it no longer had any way to guess what, for
example, `r"\x123456"` should mean as a pattern: is it asking to match
...
support that from the start. But then what are `\x` escapes supposed to
mean? Do they ignore all but the last *eight* hex digits then? And if
...
`\xabcdef...` with fewer or more than 2 hex digits following, in either
the Python CVS development tree, or in assorted Python packages sitting
on his machine.
...
backslash, swallow the next character\". Tim Peters checked
`python-mode.el`, the std `tokenize.py` and `pyclbr.py`, and the IDLE
syntax coloring subsystem, and believes there\'s no need to change any
of them. Tools like `tabnanny.py` and `checkappend.py` inherit their
