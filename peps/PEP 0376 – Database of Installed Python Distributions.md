...
post_history:
- '`22-Jun-2009 <https://mail.python.org/archives/list/python-dev@python.org/thread/ILLTIOZAULMDY5CAS6GOITEYJ4HNFATQ/>`__'
python_status: Final
url: https://peps.python.org/pep-0376/
source_path: https://github.com/python/peps/blob/main/peps/pep-0376.rst
source_commit: 2fe3aff814fb878a828738518ce0341cdba58b86
---
...
`262`{.interpreted-text role="pep"}).

Combined with `345`{.interpreted-text role="pep"}, the current proposal
supersedes `262`{.interpreted-text role="pep"}.
...
- `docutils-0.5-py2.6.egg-info`: A file containing the distribution
  metadata as described in `314`{.interpreted-text role="pep"}. This
  file corresponds to the file called `PKG-INFO`, built by the `sdist`
  command.
...
- `METADATA`: contains metadata, as described in `345`{.interpreted-text
  role="pep"}, `314`{.interpreted-text role="pep"} and
  `241`{.interpreted-text role="pep"}.
...
This RECORD file is inspired from `262`{.interpreted-text role="pep"}
FILES.
...
support (see `278`{.interpreted-text role="pep"}) is activated, avoiding
any trouble reading a file produced on a platform that uses a different
new line terminator.
...
- `distinfo_dirname(name, version)` -\> directory name
  ...
  > i.e. any \'-\' characters are replaced with \'\_\' other than the
...
see `273`{.interpreted-text role="pep"} for more details). These classes
are described in the documentation of the prototype implementation for
...
`Distutils2 <262>`{.interpreted-text role="pep"} will provide a very
basic `uninstall` function, that is added in `distutils2.util` and takes
the name of the distribution to uninstall as its argument. `uninstall`
uses the APIs described earlier and remove all unique files, as long as
their hash didn\'t change. Then it removes empty directories left
behind.
...
[^1]: <https://peak.telecommunity.com/DevCenter/setuptools>

[^2]: <https://peak.telecommunity.com/DevCenter/EggFormats>

[^3]: <https://peak.telecommunity.com/DevCenter/EasyInstall>

[^4]: <https://pypi.org/project/pip/>

[^5]: <https://fedoraproject.org/wiki/Packaging/Python/Eggs#Providing_Eggs_using_Setuptools>

[^6]: <https://wiki.debian.org/DebianPython/NewPolicy>

[^7]: <https://web.archive.org/web/20090726092550/http://bitbucket.org/tarek/pep376/>
