...
author:
- Vikram Jayanthi <vikramjayanthi@google.com>
...
source_path: https://github.com/python/peps/blob/main/peps/pep-0650.rst
source_commit: 2a35a1b0ad81a0b9b3fb4d6225f8d4b61df4dbbd
---
...
standard, other installers such as [Poetry](https://python-poetry.org/)
or [Pipenv](https://pipenv-fork.readthedocs.io/en/latest/) are popular
as well due to offering unique features which are optimal for certain
...
:   An installer that can invoke an *installer backend* by calling the
    ...
    also be thought of as the installer frontend, à la the
    [build](https://github.com/pypa/build) project for
    `517`{.interpreted-text role="pep"}.

Installer backend

:   An installer that implements the *installer interface*, allowing it
    to be invoked by a *universal installer*. An *installer backend* may
    also be a *universal installer* as well, but it is not required. In
    comparison to `517`{.interpreted-text role="pep"}, this would be
    [Flit](https://flit.readthedocs.io). *Installer backends* may be
...
:   A set of dependencies that are related and required to be installed
    ...
    How dependency groups are specified is up to the *installer
    backend*.
...
installer interface (*installer backends*) could be invoked by the IDE
by it acting as a *universal installer*.
...
[pip](https://pip.pypa.io). (Some exceptions include Heroku\'s Python
[buildpack](https://elements.heroku.com/buildpacks/heroku/heroku-buildpack-python),
which supports pip and
[Pipenv](https://pipenv-fork.readthedocs.io/en/latest/)). This dictates
the installers that developers can use while working with these
providers, which might not be optimal for their application or workflow.
...
[Dependabot](https://dependabot.com/), [PyUP](https://pyup.io/), etc.
currently support a few installers. They work by parsing and editing the
installer-specific dependency files directly (such as `requirements.txt`
or `poetry.lock`) with relevant package information such as upgrades,
downgrades, or new hashes. Similar to Platform and IDE providers, most
of these providers do not want to support N different Python package
installers as that would require supporting N different file types.
...
Similar to how `517`{.interpreted-text role="pep"} specifies build
systems, the install system information will live in the
`pyproject.toml` file under the `install-system` table.
...
installed by the *universal installer*. The `install-backend` key holds
the name of the install backend's entry point. This will allow the
*universal installer* to install the requirements for the *installer
...
`508`{.interpreted-text role="pep"}\'s \"Dependency specification for
Python Software Packages\" format.
...
constraints specified by `517`{.interpreted-text role="pep"}.
Specifically, that dependency cycles are not permitted and the
*universal installer* SHOULD refuse to install the dependencies if a
cycle is detected.
...
table as specified by `518`{.interpreted-text role="pep"}. For example,
if the *installer backend* is Poetry and you wanted to specify multiple
...
MAY implement the optional hooks. A *universal installer* MAY implement
any of the *installer backend* hooks itself, to act as both a *universal
...
`517`{.interpreted-text role="pep"}. The hooks may be called with
keyword arguments, so *installer backends* implementing them should be
careful to make sure that their signatures match both the order and the
names of the arguments above.

All hooks MAY print arbitrary informational text to `stdout` and
`stderr`. They MUST NOT read from `stdin`, and the *universal installer*
MAY close `stdin` before invoking the hooks.
...
terminal, the *installer backend* is responsible for presenting its
output accurately, as for any program running in a terminal.
...
- `**kwargs` : Arbitrary parameters that a *installer backend* may
  require that are not already specified, allows for backwards
  compatibility.
- Returns: A list of dependencies (`508`{.interpreted-text role="pep"}
  strings) to install.
...
by the *installer backend*, the *installer backend* MUST raise an error.
...
- `dependency_specifiers` : An iterable of dependencies as
  `508`{.interpreted-text role="pep"} strings that are being updated,
  for example : `["requests==2.8.1", ...]`. Optionally for a specific
  dependency group.
...
for tool specific *installer backend* functionality which requires a
user to provide additional information not required by the hook.
...
This also means this PEP takes no stance on OS packaging as that would
be an *installer backend*\'s domain.
...
*installer backends* from being used, though (e.g.
[mamba](https://github.com/mamba-org/mamba)), as they could be executed
as a subprocess from Python code.
