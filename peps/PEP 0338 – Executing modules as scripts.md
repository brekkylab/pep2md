...
source_path: https://github.com/python/peps/blob/main/peps/pep-0338.rst
source_commit: b1b14c6670ede89affcfd9e80364a25ebb7bb524
---
...
proposes making use of the `302`{.interpreted-text role="pep"} import
hooks to allow any module which provides access to its code object to be
executed.
...
scripts (see `299`{.interpreted-text role="pep"}). That issue is
considered orthogonal to the specific feature addressed by this PEP.
...
module the `302`{.interpreted-text role="pep"} import mechanisms are
used to locate the module and retrieve its compiled code, before
executing the module in accordance with the semantics for a top-level
module. The interpreter does this by invoking a new standard library
function `runpy.run_module`.

This is necessary due to the way Python\'s import machinery locates
modules inside packages. A package may modify its own \_\_path\_\_
...
`sys.metapath`. Accordingly, the only way for Python to reliably locate
the module is by importing the containing package and using the
`302`{.interpreted-text role="pep"} import hooks to gain access to the
Python code.
...
A reference implementation is available on SourceForge ([^1]), along
with documentation for the library reference ([^2]). There are two parts
to this implementation. The first is a proposed standard library module
`runpy`. The second is a modification to the code implementing the `-m`
switch to always delegate to `runpy.run_module` instead of trying to run
the module directly. The delegation has the form:
...
> the standard import mechanism (refer to `302`{.interpreted-text
> role="pep"} for details) and then executed in a fresh module
> namespace.
>
> The optional dictionary argument `init_globals` may be used to
> pre-populate the globals dictionary before the code is executed. The
> supplied dictionary will not be modified. If any of the special global
...
> `__builtins__` are set in the globals dictionary before the module
> code is executed.
>
> `__name__` is set to `run_name` if this optional argument is supplied,
> and the original `mod_name` argument otherwise.
>
> `__loader__` is set to the `302`{.interpreted-text role="pep"} module
> loader used to retrieve the code for the module (This loader may be a
> wrapper around the standard import mechanism).
>
> `__file__` is set to the name provided by the module loader. If the
> loader does not make filename information available, this argument is
> set to `None`.
>
> `__builtins__` is automatically initialised with a reference to the
> top level namespace of the `__builtin__` module.
>
...
retrospect) interaction between this PEP and `328`{.interpreted-text
role="pep"} - explicit relative imports don\'t work from a main module.
This is due to the fact that relative imports rely on `__name__` to
determine the current module\'s position in the package hierarchy. In a
main module, the value of `__name__` is always `'__main__'`, so explicit
relative imports will always fail (as they only work for a module inside
a package).
...
`import pkg moduleA` to retrieve the module under test,
`338`{.interpreted-text role="pep"} allows the tests to be run as:
...
- The special variables `__name__`, `__file__` and `__loader__` are set
  in a module\'s global namespace before the module is executed. As
...
- There is no special protection on the alter_sys argument. This may
...
\_\_path\_\_ variables, and looked only in the main package directory. A
Python script with this behaviour can be found in the discussion of the
`execmodule` cookbook recipe[^3].

The `execmodule` cookbook recipe itself was the proposed mechanism in an
earlier version of this PEP (before the PEP\'s author read
`302`{.interpreted-text role="pep"}).
...
[^1]: `338`{.interpreted-text role="pep"} implementation (runpy module
    and `-m` update) (<https://bugs.python.org/issue1429601>)

[^2]: `338`{.interpreted-text role="pep"} documentation (for runpy
    module) (<https://bugs.python.org/issue1429605>)
