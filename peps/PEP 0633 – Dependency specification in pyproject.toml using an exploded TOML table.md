...
author:
- Laurie Opperman <laurie_opperman@hotmail.com>
...
source_path: https://github.com/python/peps/blob/main/peps/pep-0633.rst
source_commit: faa7b1791e8dd3f3d58c4d75c575c9a2438e0570
---

# Rejection Notice

This PEP has been rejected in favour of `631`{.interpreted-text
role="pep"} due to its popularity, consistency with the existing usage
of `508`{.interpreted-text role="pep"} strings, and compatibility with
existing packaging tool suites.
...
represent requirements rather than `508`{.interpreted-text role="pep"}
strings:
...
- Potential for users to guess the keys of given features, rather than
  memorising a syntax.
- Users of multiple other popular languages may already be familiar with
  the TOML syntax.
...
`610`{.interpreted-text role="pep"} and `440`{.interpreted-text
role="pep"} as to reduce differences in the packaging ecosystem and rely
on previous work in specification.
...
- string: the requirement is defined only by a version requirement, with
  same specification as `version` in the requirement table, except
...
- `extras` (array of strings): a list of `508`{.interpreted-text
  role="pep"} extras declarations for the distribution. The list MUST be
  non-empty.
- `markers` (string): a `508`{.interpreted-text role="pep"} environment
  marker expression. The string MUST be non-empty.
...
- `git`, `hg`, `bzr` or `svn` (string): the URL of a VCS repository (as
  specified in `440`{.interpreted-text role="pep"}) to clone, whose tree
  will be installed to satisfy the requirement. Further VCS keys will be
  added via amendments to `610`{.interpreted-text role="pep"}, however
  tools MAY opt to support other VCS\'s using their command-line command
  prior to the acceptance of the amendment.
- `revision` (string): the identifier for a specific revision of the
  specified VCS repository to check-out before installation. Users MUST
...
above](#requirement-table), with the addition of the following required
key:
