...
source_path: https://github.com/python/peps/blob/main/peps/pep-0207.rst
source_commit: 2f50a368cf3379f94bd750ba065e35fc74254c70
---
...
3.  If a class overrides x==y but nothing else, should x!=y be computed
...
1.  Full backwards compatibility can be achieved as follows. When an
    ...
    changed again, I\'m not so sure that it makes a difference any
    more.)
...
5.  In the current proposal, when A\<B returns an array of elementwise
    comparisons, this outcome is considered non-Boolean, and its
    interpretation as Boolean by the shortcut operators raises an
    exception. David Ascher\'s proposal tries to deal with this; I
    don\'t think this is worth the additional complexity in the code
    generator. Instead of A\<B\<C, you can write (A\<B)&(B\<C).

6.  The `min()` and `list.sort()` operations will only use the \<
    operator; max() will only use the \> operator. The \'in\' and \'not
    in\' operators and dictionary lookup will only use the == operator.
...
- New functions:
  ...
  This performs the requested rich comparison, returning a Python object
  or raising an exception. The 3rd argument must be one of Py_LT, Py_LE,
  ...
  of Py_LT, Py_LE, Py_EQ, Py_NE, Py_GT or Py_GE. Note that when
...
- Whenever `PyObject_Compare()` is called with the intent of getting the
  ...
  course for the comparison operators in ceval.c), the code is changed
  to call `PyObject_RichCompare()` or `PyObject_RichCompareBool()`
...
- Most built-in types that currently define a comparison will be
  modified to define a rich comparison instead. (This is optional; I\'ve
  converted lists, tuples, complex numbers, and arrays so far, and am
  not sure whether I will convert others.)

## Classes

- Classes can define new special methods `__lt__`, `__le__`, `__eq__`,
  `__ne__`, `__gt__`, `__ge__` to override the corresponding operators.
  (I.e., \<, \<=, ==, !=, \>, \>=. You gotta love the Fortran heritage.)
  If a class defines `__cmp__` as well, it is only used when `__lt__`
  etc. have been tried and return `NotImplemented`.
...
depending on whether the comparison is true or not for each element
pair. Thus, for example, using the arrays x and y defined above:
...
3.  Addition of an optional argument to the builtin `cmp()`
    ...
    \"\<=\", \"\>\", \"\>=\", \"==\", \"!=\", \"\<\>\" (the last two
    have the same meaning). When called with one of these strings as the
    third argument, `cmp()` can return any Python object. Otherwise, it
    can only return -1, 0 or 1 as before.
...
comparison testings. In other words, the truth-value of the result of
the result of the comparison determines the result of a chained
operation. This is problematic in the case of arrays, since if x, y and
z are three arrays, then the user expects:
