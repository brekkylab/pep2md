...
source_path: https://github.com/python/peps/blob/main/peps/pep-0008.rst
source_commit: d14c9e97b120daecadcd4afe742af69ddc07a34c
---
...
- Derive exceptions from `Exception` rather than `BaseException`. Direct
  ...
  *catching* the exceptions is likely to need, rather than the locations
  ...
  problem occurred\" (see `3151`{.interpreted-text role="pep"} for an
...
- However, outside the stdlib, experiments within the rules of
  ...
  `484`{.interpreted-text role="pep"} style type annotations, reviewing
...
- For code that wants to make a different use of function annotations it
  ...
  checkers can be found in `484`{.interpreted-text role="pep"}.)
...
- Users who don\'t want to use type checkers are free to ignore them.
  ...
  `484`{.interpreted-text role="pep"} recommends the use of stub files:
...
- Although the `526`{.interpreted-text role="pep"} is accepted for
  ...
  stub files on all versions of Python (see `484`{.interpreted-text
