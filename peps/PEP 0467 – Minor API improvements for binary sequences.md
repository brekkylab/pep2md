...
source_path: https://github.com/python/peps/blob/main/peps/pep-0467.rst
source_commit: dda559e67e0f14c7bb8adb9a84bc361a31c4ea84
---
...
The last two (`getbyte` and `iterbytes`) will also be added to
`memoryview`.
...
Python 3 series, for example with `461`{.interpreted-text role="pep"}.
...
in writing new wire format code, and in updating existing code.
...
first argument is the count, and whose second argument is the fill byte
to use (defaults to `\x00`):
...
feature when it was introduced in `3137`{.interpreted-text role="pep"}.
...
like the `bytes.fromint(x)` proposal in this PEP. Providing both
...
workarounds for byte iteration, as demonstrated by [Get single-byte
bytes objects from bytes
objects](https://discuss.python.org/t/get-single-byte-bytes-objects-from-a-bytes-object/41709).
...
`bytes.fromint`), recreating the `ord`/`chr`/`unichr` trio from Python 2
under a different naming scheme (`ord`/`bchr`/`chr`).
...
- [Initial March 2014 discussion thread on
  python-ideas](https://mail.python.org/pipermail/python-ideas/2014-March/027295.html)
- [Guido\'s initial feedback in that
  thread](https://mail.python.org/pipermail/python-ideas/2014-March/027376.html)
- [Issue proposing moving zero-initialised sequences to a dedicated
  API](https://github.com/python/cpython/issues/65094)
- [Issue proposing to use calloc() for zero-initialised binary
  sequences](https://github.com/python/cpython/issues/65843)
- [August 2014 discussion thread on
  python-dev](https://mail.python.org/pipermail/python-ideas/2014-March/027295.html)
- [June 2016 discussion thread on
  python-dev](https://mail.python.org/pipermail/python-dev/2016-June/144875.html)
- [Get single-byte bytes objects from bytes
  objects](https://discuss.python.org/t/get-single-byte-bytes-objects-from-a-bytes-object/41709)
