...
This is an updated version of `333`{.interpreted-text role="pep"},
modified slightly to improve usability under Python 3, and to
incorporate several long-standing de facto amendments to the WSGI
protocol. (Its code samples have also been ported to Python 3.)

While for procedural reasons[^1], this must be a distinct PEP, no
...
outlined in the sections below titled, [A Note On String
...
Unicode \"under the hood\", the *content* of native strings must still
be translatable to bytes via the Latin-1 encoding! (See the section on
[Unicode Issues](#unicode-issues) later in this document for more
details.)
...
`piglatin.py`. (Note: a \"real\" middleware component would probably use
a more robust way of checking the content type, and should also check
...
sections on [The start_response()
Callable](#the-start_response-callable) and [Error
Handling](#error-handling). It is used only when the application has
trapped an error and is attempting to display an error message to the
browser.
...
avoided. See the [Buffering and Streaming](#buffering-and-streaming)
...
as byte-range transmission. See [Other HTTP
...
accurate result. (See the [Handling the Content-Length
Header](#handling-the-content-length-header) section for information on
how this would normally be used.)
...
to complement `342`{.interpreted-text role="pep"}\'s generator support,
...
Handling](#optional-platform-specific-file-handling)). In the general
case, only attributes specified here, or accessed via e.g. the
`234`{.interpreted-text role="pep"} iteration APIs are acceptable.
...
:   The contents of any `Content-Type` fields in the HTTP request. May
    be empty or absent.
...
:   When `HTTP_HOST` is not set, these variables can be combined to
    determine a default. See the [URL
    Reconstruction](#url-reconstruction) section below for more detail.
    `SERVER_NAME` and `SERVER_PORT` are required strings and must never
    be empty.
...
variables[^5] as are applicable, such as `HTTPS=on` and `SSL_PROTOCOL`.
...
at all. It is a violation of this specification for *any* CGI
...
|                     | 1.0.                                           |
+---------------------+------------------------------------------------+
| `wsgi.url_scheme`   | A string representing the \"scheme\" portion   |
...
|                     | error output can be written, for the purpose   |
|                     | of recording program or other errors in a      |
...
[Buffering and Streaming](#buffering-and-streaming) section, below).
...
no surrounding whitespace or other characters. (See
`2616`{.interpreted-text role="rfc"}, Section 6.1.1 for more
...
header field-name (as defined by `2616`{.interpreted-text role="rfc"},
...
Features](#other-http-features) section below.)
...
newly-supplied ones, thus allowing the application to \"change its
mind\" about the output when an error has occurred.
...
back to the server or gateway. See [Error Handling](#error-handling)
...
`"chunked encoding"<2616#section-3.6.1>`{.interpreted-text role="rfc"},
then the server **may** use chunked encoding to send a chunk for each
`write()` call or bytestring yielded by the iterable, thus generating a
`Content-Length` header for each chunk. This allows the server to keep
the client connection alive, if it wishes to do so. Note that the server
**must** comply fully with `2616`{.interpreted-text role="rfc"} when
doing this, or else fall back to one of the other strategies for dealing
with the absence of `Content-Length`.
...
actual web server/gateway. See [Other HTTP
Features](#other-http-features) below, for more details.)
...
else they provide a buffered \"write\" function and a \"flush\"
mechanism to flush the buffer.
...
response headers **must** follow `2616`{.interpreted-text role="rfc"}
with respect to encoding. That is, they must either be ISO-8859-1
characters, or use `2047`{.interpreted-text role="rfc"} MIME encoding.
...
headers do not apply to WSGI internal communications. WSGI applications
...
\"hop-by-hop\" headers in the `environ` dictionary. WSGI servers
...
- You may not return a file object and expect it to work as an iterable,
  ...
  of the time!) Use `wsgi.file_wrapper` or an application-specific file
  wrapper class. (See [Optional Platform-Specific File
  Handling](#optional-platform-specific-file-handling) for more on
...
Python built-in file objects (e.g. `fileno()`), the `wsgi.file_wrapper`
**may** assume that these methods or attributes have the same semantics
as those of a built-in file object.
...
should begin at the current position within the \"file\" at the time
that transmission begins, and continue until the end is reached, or
until `Content-Length` bytes have been written. (If the application
doesn\'t supply a `Content-Length`, the server **may** generate one from
the file using its knowledge of the underlying file implementation.)

Of course, platform-specific file transmission APIs don\'t usually
accept arbitrary \"file-like\" objects. Therefore, a `wsgi.file_wrapper`
has to introspect the supplied object for things such as a `fileno()`
...
3.  What\'s the `close()` for?
    ...
    that\'s proposed by `325`{.interpreted-text role="pep"}.
...
[^2]: SVN revision history for PEP 3333, showing differences from PEP
    333
