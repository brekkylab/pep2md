...
::: canonical-doc
`match`{.interpreted-text role="external+python:ref"}
:::

# Abstract

This PEP provides the technical specification for the match statement.
It replaces `622`{.interpreted-text role="pep"}, which is hereby split
in three parts:

- `634`{.interpreted-text role="pep"}: Specification
- `635`{.interpreted-text role="pep"}: Motivation and Rationale
- `636`{.interpreted-text role="pep"}: Tutorial

This PEP is intentionally devoid of commentary; the motivation and all
explanations of our design choices are in `635`{.interpreted-text
role="pep"}. First-time readers are encouraged to start with
`636`{.interpreted-text role="pep"}, which provides a gentler
introduction to the concepts, syntax and semantics of patterns.

# Syntax and Semantics

See [Appendix A](#634-appendix-a) for the complete grammar.
...
include \"the pattern is matched with (or against) the subject value\"
and \"we match the pattern against (or with) the subject value\".
...
example, while matching the pattern `(0, x, 1)` with the value
`[0, 1, 2]`, the subpattern `x` may succeed if the list elements are
...
### Literal Patterns {#634-literal_pattern}
...
only permitted to express complex numbers; they require a real number on
the left and an imaginary number on the right.
...
- The singleton literals `None`, `True` and `False` are compared using
  the `is` operator.

### Capture Patterns {#634-capture_pattern}
...
`!"_"` expresses). It is treated as a [wildcard
...
operator in `572`{.interpreted-text role="pep"}. (Summary: the name
...
### Wildcard Pattern {#634-wildcard_pattern}
...
(For the syntax of `pattern`, see Patterns above. Note that it contains
no comma \-- a parenthesized series of items with at least one comma is
a sequence pattern, as is `()`.)
...
### Sequence Patterns {#634-sequence_pattern}
...
### Mapping Patterns {#634-mapping_pattern}
...
the `==` operator. If a `'**' NAME` form is present, that name is bound
to a `dict` containing remaining key-value pairs from the subject
mapping.
...
`__missing__` or `__getitem__`. For example, `collections.defaultdict`
instances will only be matched by patterns with keys that were already
present when the match statement was entered.

### Class Patterns {#634-class_pattern}
...
- If only keyword patterns are present, they are processed as follows,
  ...
  - The keyword is looked up as an attribute on the subject.
    ...
    - Otherwise, the subpattern associated with the keyword is matched
      against the attribute value. If this fails, the class pattern
      fails. If it succeeds, the match proceeds to the next keyword.
...
`dict`, `float`, `frozenset`, `int`, `list`, `set`, `str`, and `tuple`.
...
name of class patterns. While none of those typically create any
...
# Appendix A \-- Full Grammar {#634-appendix-a}

Here is the full grammar for `match_stmt`. This is an additional
alternative for `compound_stmt`. Remember that `match` and `case` are
soft keywords, i.e. they are not reserved words in other grammatical
