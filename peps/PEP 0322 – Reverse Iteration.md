...
source_path: https://github.com/python/peps/blob/main/peps/pep-0322.rst
source_commit: 5996ce919b0cf10c5be7174e58c9a864d81dd27c
---
...
does arise regularly in practice. See [Real World Use
Cases](#real-world-use-cases) below.

# Proposal

Add a builtin function called *reversed()* that makes a reverse iterator
over sequence objects that support \_\_getitem\_\_() and \_\_len\_\_().
...
- heapq.heapify() uses `for i in xrange(n//2 - 1, -1, -1)` because
  ...
  which iterates over the underlying list in the opposite direction. The
  replacement code `for i in reversed(xrange(n//2))` makes clear the
  range covered and how many iterations it takes.
...
- random.shuffle() uses `for i in xrange(len(x)-1, 0, -1)` because the
  ...
  from an ever diminishing pool. In fact, the algorithm can be run in a
  forward direction but is less intuitive and rarely presented that way
  in literature. The replacement code
