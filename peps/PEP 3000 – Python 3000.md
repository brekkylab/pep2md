...
source_path: https://github.com/python/peps/blob/main/peps/pep-3000.rst
source_commit: 13bbbffbbebc224d5cc069bf417664b4222c23b9
---
...
process has been decided and specified. In practice, we\'ll be
discussing features and process simultaneously; often the debate about a
particular feature will prompt a process discussion.
...
Python 3000 PEPs are numbered starting at `3000`{.interpreted-text
role="pep"}. PEPs 3000-3099 are meta-PEPs \-- these can be either
process or informational PEPs. PEPs 3100-3999 are feature PEPs.
`3000`{.interpreted-text role="pep"} itself (this PEP) is special; it is
the meta-PEP for Python 3000 meta-PEPs (IOW it describe the process to
define processes). `3100`{.interpreted-text role="pep"} is also special;
it\'s a laundry list of features that were selected for (hopeful)
inclusion in Python 3000 before we started the Python 3000 process for
real. PEP 3099, finally, is a list of features that will *not* change.

# Timeline

See `361`{.interpreted-text role="pep"}, which contains the release
schedule for Python 2.6 and 3.0. These versions will be released in
lockstep.
...
- It will contain backported versions of many Py3k features, either
  enabled through \_\_future\_\_ statements or simply by allowing old
...
there will be a separate source code conversion tool[^1]. This tool can
