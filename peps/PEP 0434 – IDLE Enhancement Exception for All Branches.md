...
author:
- Todd Rovito <rovitotv@gmail.com>
...
source_path: https://github.com/python/peps/blob/main/peps/pep-0434.rst
source_commit: 5914a703d11d82ce0c0f4312255f9fc0ab0eef5a
---
...
patches would not necessarily have to be split into \'bugfix\' changes
...
before the new options were added (Issue 10405[^3]).
...
being an easy computer language for beginners[^4]. A major Python
...
included with other programming languages[^5]. IDLE is an important
...
not included in the 2.7 patch because of issues with old-style
classes[^6].) On the other hand, bike-shedding can be an energy drain.
If the obvious fix for a bug looks like an enhancement, writing a
separate bugfix-only patch is more work. And making the code diverge
between versions makes future multi-version patches more difficult.

These issue are illustrated by the search-and-replace dialog box. It
used to raise an exception for certain user entries[^7]. The uncaught
exception caused IDLE to exit. At least on Windows, the exit was silent
(no visible traceback) and looked like a crash if IDLE was started
normally, from an icon.
...
something useful with the user\'s entry? Would replacing a \'crash\'
with useful behavior be an enhancement, limited to future Python
releases? Should IDLE developers have to ask that?
...
because the patch a) added a new item to standard rmenu_specs and b)
expected every rmenu_spec to be lengthened. It is not clear whether this
violates the guarantee, but there is a second patch that fixes
...
[^3]: IDLE breakpoint facility undocumented, Deily, Ned
