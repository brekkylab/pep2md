...
:   This read/write attribute specifies the number of rows to fetch at a
    ...
    params value to `execute()`). This attribute will default to a
    single row.
...
:   This read-only attribute is a tuple of 7-tuples. Each 7-tuple
    ...
    The \'type_code\' is one of the \'dbi\' values specified in the
    section below.
...
:   Execute (prepare) a database operation (query or command).
    ...
    the `setinputsizes()` method to specify the parameter types and
...
:   Fetch the next set of rows of a query result, returning as a list of
    ...
    `None`, then the cursor\'s arraysize determines the number of rows
    to be fetched.
...
:   (Note: this method is not well-defined yet.) This can be used before
    ...
    parameters. sizes is specified as a tuple \-- one item for each
    ...
    column (this is useful to avoid predefined areas for large inputs).
...
each of the result columns of a query. The \'type_code\' is defined to
be one of five types exported by this module: `STRING`, `RAW`, `NUMBER`,
`DATE`, or `ROWID`.
