...
source_path: https://github.com/python/peps/blob/main/peps/pep-0378.rst
source_commit: b408e3e5760b75f28685ca4a91d8cae34c8c0c7b
---
...
program\'s output, improving its professional appearance and
readability.
...
other
challenges](https://docs.python.org/2.6/library/locale.html#background-details-hints-tips-and-caveats)
in detail.
...
Such tasks are better suited to robust tools like
[Babel](http://babel.edgewall.org/). Instead, the goal is to make a
common, everyday task easier for many users.
...
The \',\' option indicates that commas should be included in the output
as a thousands separator. As with locales which do not use a period as
the decimal point, locales which use a different convention for digit
...
\'f\', \'g\', \'E\', \'G\', \'%\', \'F\' and \'\'. To allow future
extensions, it is undefined for other types: binary, octal, hex,
character, etc.
...
- [Python 2.6
  docs](https://docs.python.org/2.6/library/string.html#formatstrings)
...
[C-Sharp](http://blog.stevex.net/index.php/string-formatting-in-csharp/)
provides both styles (picture formatting and type specifiers). The type
specifier approach is locale aware. The picture formatting only offers a
...
[Common
Lisp](http://www.cs.cmu.edu/Groups/AI/html/cltl/clm/node200.html) uses a
COLON before the `~D` decimal type specifier to emit a COMMA as a
thousands separator. The general form of `~D` is
...
- The [ADA
  language](http://archive.adaic.com/standards/83lrm/html/lrm-02-04.html)
  allows UNDERSCORES in its numeric literals.

Visual Basic and its brethren (like [MS
Excel](http://www.brainbell.com/tutorials/ms-office/excel/Create_Custom_Number_Formats.htm))
use a completely different style and have ultra-flexible custom format
specifiers like:

    "_($* #,##0_)".

[COBOL](http://en.wikipedia.org/wiki/Cobol#Syntactic_features) uses
picture clauses like:

    PICTURE $***,**9.99CR

Java offers a [Decimal.Format
Class](http://java.sun.com/javase/6/docs/api/java/text/DecimalFormat.html)
that uses picture patterns (one for positive numbers and an optional one
for negatives) such as: `"#,##0.00;(#,##0.00)"`. It allows arbitrary
groupings including hundreds and ten-thousands and uneven groupings. The
special pattern characters are non-localized (using a DOT for a decimal
separator and a COMMA for a grouping separator). The user can supply an
alternate set of symbols using the formatter\'s *DecimalFormatSymbols*
object.
...
The thousands separator is defined as shown above for types \'d\',
\'e\', \'f\', \'g\', \'%\', \'E\', \'G\' and \'F\'. To allow future
extensions, it is undefined for other types: binary, octal, hex,
character, etc.
...
- James Knight observed that Indian/Pakistani numbering systems group by
  ...
  Smith pointed-out that these are already handled by the \"n\"
  specifier in the locale module (albeit only for integers). This PEP
  does not attempt to support all of those possibilities. It focuses on
  a single, relatively common grouping convention that offers a quick
  way to improve readability in many (though not all) contexts.
