...
source_path: https://github.com/python/peps/blob/main/peps/pep-0320.rst
source_commit: 804432ef479c6d8583289098f08f5b3b4723990b
---
...
- `218`{.interpreted-text role="pep"} Builtin Set Objects.
- `289`{.interpreted-text role="pep"} Generator expressions.
- `292`{.interpreted-text role="pep"} Simpler String Substitutions to be
  implemented as a module.
- `318`{.interpreted-text role="pep"}: Function/method decorator syntax,
  using \@syntax
- `322`{.interpreted-text role="pep"} Reverse Iteration.
- `327`{.interpreted-text role="pep"}: A Decimal package for fixed
  precision arithmetic.
- `328`{.interpreted-text role="pep"}: Multi-line Imports
...
- Deprecate and/or remove the modules listed in `4`{.interpreted-text
  role="pep"} (`posixfile`, `gopherlib`, `pre`, `others`)
- Remove support for platforms as described in `11`{.interpreted-text
  role="pep"}.
...
- It would be nice if the built-in SSL socket type could be used for
  ...
  implement async servers using SSL have to require third-party packages
  such as pyopenssl.
- AST-based compiler: this branch was not completed in time for 2.4, but
  will land on the trunk some time after 2.4 final is out, for inclusion
  in 2.5.
...
- For a class defined inside another class, the `__name__` should be
  \"outer.inner\", and pickling should work. ([^3]. I\'m no longer
  certain this is easy or even right.)

- Decide on a clearer deprecation policy (especially for modules) and
  act on it. For a start, see this message from Neal Norwitz[^4]. There
...
- Provide alternatives for common uses of the `types` module; Skip
  Montanaro has posted a proto-PEP for this idea[^5]. There hasn\'t been
  any progress on this, AFAICT.
...
- `262`{.interpreted-text role="pep"} Database of Installed Python
  Packages (Kuchling)
...
- `266`{.interpreted-text role="pep"} Optimizing Global
  ...
  made a little progress with a new compiler, but it\'s going slowly and
...
[^3]: Nested class \_\_name\_\_ <https://bugs.python.org/issue633930>
