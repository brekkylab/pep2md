...
source_path: https://github.com/python/peps/blob/main/peps/pep-0597.rst
source_commit: b272c12fdeb12558c248528fb7d165ff2020ac83
---
...
we start emitting `DeprecationWarning` everywhere the `encoding`
argument is omitted, it will be too noisy and painful.

Although this PEP doesn\'t propose changing the default encoding, it
will help enable that change by:
...
Add a new `EncodingWarning` warning class as a subclass of `Warning`. It
is emitted when the `encoding` argument is omitted and the default
locale-specific encoding is used.
...
When the flag is set, `io.TextIOWrapper()`, `open()` and other modules
using them will emit `EncodingWarning` when the `encoding` argument is
omitted.
...
`DeprecationWarning` when the `encoding` argument is omitted would be
too noisy.
...
You can use `-X warn_default_encoding` or `PYTHONWARNDEFAULTENCODING=1`
...
[^6]: pypa/pip: \"Installing packages fails if Python 3 installed into
