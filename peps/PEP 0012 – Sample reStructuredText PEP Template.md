...
created: 05-Aug-2002
post_history:
- '`30-Aug-2002 <https://mail.python.org/archives/list/python-dev@python.org/thread/KX3AS7QAY26QH3WIUAEOCCNXQ4V2TGGV/>`__'
...
source_commit: 4ff2527f7946262a2bdb326435421b08f4572cf3
---
...
[template](#template) (which is included as a file in the [PEPs
...
`1`{.interpreted-text role="pep"}, this should make it easy for you to
...
The source for this (or any) PEP can be found in the [PEPs
repository](https://github.com/python/peps/), as well as via a link at
the bottom of each PEP.
...
standard library. When in doubt, read `1`{.interpreted-text role="pep"}
for details, or open a tracker issue on the PEPs repo to ask for
assistance.
...
- Make a copy of this file (the `.rst` file, **not** the HTML!) and
  perform the following edits. Name the new file
  `pep-{NNNN}.rst`{.interpreted-text role="file"}, using the next
  available number (not used by a published or in-PR PEP).
...
- Add the direct URL of the PEP\'s canonical discussion thread (on e.g.
  ...
  to the PEPs repository and you create the corresponding discussion
  thread. See `PEP 1 <1#discussing-a-pep>`{.interpreted-text role="pep"}
  for more details.
...
- Change the Created header to today\'s date. Be sure to follow the
  ...
  is the three-letter English month abbreviation, i.e. one of Jan, Feb,
  Mar, Apr, May, Jun, Jul, Aug, Sep, Oct, Nov, Dec.

- For Standards Track PEPs, after the Created header, add a
  Python-Version header and set the value to the next planned version of
  Python, i.e. the one your new feature will hopefully make its first
  appearance in. Do not use an alpha or beta release designation here.
  Thus, if the last version of Python was 2.2 alpha 1 and you\'re hoping
  to get your new feature into Python 2.2, set the header to:
...
- Add a Topic header if the PEP belongs under one shown at the
  `topic-index`{.interpreted-text role="ref"}. Most PEPs don\'t.

- Post-History can be \'Pending\' for now; you\'ll add dates and
  corresponding links to this header each time you post your PEP to the
  designated discussion forum (and update the Discussions-To header with
  said link, as above). For each thread, use the date (in the
  `DD-MMM-YYYY` format) as the linked text, and insert the URLs inline
  as anonymous reST [hyperlinks](#hyperlinks), with commas in between
  each posting.
...
- Add a Replaces header if your PEP obsoletes an earlier PEP. The value
  of this header is the number of the PEP that your new PEP is
...
- Now write your Abstract, Rationale, and other content for your PEP,
  ...
  characters and the indentation requirements. See \"Suggested
  Sections\" below for a template of sections to include.
...
- Run `./build.py` to ensure the PEP is rendered without errors, and
  check that the output in `build/pep-{NNNN}.html`{.interpreted-text
  role="file"} looks as you intend.

- Create a pull request against the [PEPs
  repository](https://github.com/python/peps/).
...
substituting the \"short name\" of the appropriate [Pygments
lexer](https://pygments.org/docs/lexers/) (or `text` to disable
...
appropriate `language` at the top of the PEP body (below the headers and
above the Abstract). All literal blocks will then be treated as that
...
inline with the text, insert the link into angle brackets (`<>`) after
the text you want to link, but before the closing backtick, with a space
...
> See `1`{.interpreted-text role="pep"} for more information on how to
> write a PEP, and
> `the Hyperlink section of PEP 12 <12#hyperlinks>`{.interpreted-text
> role="pep"} for how to link.
...
Footnotes begin with \".. \" (the explicit markup start), followed by
the footnote marker (no underscores), followed by the footnote body. For
example:
...
preferred for graphics, JPEG for photos and GIF for animations. Images
should be optimised to reduce their file size, and should be legible in
both light and dark mode in the browser.
...
an explicit markup start: two periods and whitespace. Leave the \"..\"
on a line by itself to ensure that the comment is not misinterpreted as
...
You can use [Intersphinx
references](https://www.sphinx-doc.org/en/master/usage/extensions/intersphinx.html)
to other Sphinx sites, such as the [Python
documentation](https://docs.python.org/)
[packaging.python.org](https://packaging.python.org/), and
[typing.python.org](https://typing.python.org/), to easily
cross-reference pages, sections and Python/C objects.
...
As `PEP 1 describes <1#pep-maintenance>`{.interpreted-text role="pep"},
PEPs are considered historical documents once marked Final, and their
canonical documentation/specification should be moved elsewhere. To
indicate this, use the `canonical-doc` directive or an appropriate
subclass:
...
site, a [reST hyperlink](#hyperlinks)).

For example, to create a banner pointing to the
`python:sqlite3`{.interpreted-text role="mod"} docs, you would write the
following:
...
> ::: canonical-doc
> `python:sqlite3`{.interpreted-text role="mod"}
> :::

Or for a PyPA spec, such as the
`packaging:core-metadata`{.interpreted-text role="ref"}, you would use:
...
> `typing:packaging-typed-libraries`{.interpreted-text role="ref"}
> :::
>
> ::: canonical-typing-spec
> `typing:literal-types`{.interpreted-text role="ref"} and
> `typing.Literal`{.interpreted-text role="py:data"}
> :::
...
> ::: canonical-doc
> the `python:sqlite3-connection-objects`{.interpreted-text role="ref"}
> and `python:sqlite3.DataError`{.interpreted-text role="exc"} docs
>
> Also, see the `Data Persistence docs <persistence>`{.interpreted-text
> role="ref"} for other examples.
> :::
...
`1`{.interpreted-text role="pep"}. Those sections are provided here for
convenience.
...
by basic [Docutils](https://docutils.sourceforge.io/) and the extensions
added by [Sphinx](https://www.sphinx-doc.org/).
...
- [reStructuredText Markup
  ...
  the authoritative, comprehensive documentation of the basic reST
  syntax, directives, roles and more.
- [Sphinx
  ...
  the extended constructs added by the Sphinx documentation system used
  to render the PEPs to HTML.
...
open an [issue on the PEPs
repository](https://github.com/python/peps/issues) or reach out to a PEP
editor directly.

# Change History

- [22-Feb-2026](https://discuss.python.org/t/small-but-useful-change-to-pep-1-12/106250)
  - Suggest a new section, headed `Change History` to the PEP template,
    and add that section to this PEP.
