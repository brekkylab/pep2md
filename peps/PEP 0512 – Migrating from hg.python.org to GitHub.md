...
source_path: https://github.com/python/peps/blob/main/peps/pep-0512.rst
source_commit: 2e3392945435493329771828ed9f63559460eab9
---
...
<https://github.com/python/cpython> on 2017-02-10.
::::

# Abstract

This PEP outlines the steps required to migrate Python\'s development
process from Mercurial[^1] as hosted at hg.python.org[^2] to Git[^3] on
GitHub[^4]. Meeting the minimum goals of this PEP should allow for the
development process of Python to be as productive as it currently is,
and meeting its extended goals should improve the development process
from its status quo.
...
2.  Checkout out the CPython source code from hg.python.org[^6].
3.  Make the fix.
4.  Upload a patch.
5.  Have a core developer review the patch using our fork of the
    Rietveld code review tool[^7].
...
allowing patches to be uploaded to bugs.python.org[^8] is potentially
simple for an external contributor, it is as slow and burdensome as it
gets for a core developer to work with.
...
workflows was made, in the end leading to two: `481`{.interpreted-text
role="pep"} and `507`{.interpreted-text role="pep"} proposing
GitHub[#github](##NOTE###github) and GitLab[^9], respectively.
...
core-workflow mailing list[^10]. PyCon US 2015 also showed that the
community was a bit frustrated with our process due to both cognitive
overhead for new contributors and how long it was taking for core
developers to look at a patch (see the end of Guido van Rossum\'s
keynote at PyCon US 2015[^11] as an example of the frustration).
...
- Maintaining custom infrastructure has been a burden on volunteers
  (e.g., an unmaintained, custom fork of Rietveld[^13] is currently
  being used).
...
- The custom workflow is a hindrance to external contributors (acts as a
  barrier of entry due to time required to ramp up on development
...
- Our BDFL prefers GitHub (who would be the first person to tell you
  that his opinion shouldn\'t matter, but the person making the decision
  felt it was important that the BDFL feel comfortable with the workflow
  of his own programming language to encourage his continued
  participation).
...
While hg.python.org[^15] hosts many repositories, there are only five
key repositories that need to move:

1.  devinabox[^16] (done)
2.  benchmarks[^17] (skipped)
3.  peps[^18] (done)
4.  devguide[^19] (done)
5.  cpython[^20]

The devinabox repository is code-only. The peps and devguide
repositories involve the generation of webpages. And the cpython
repository has special requirements for integration with
bugs.python.org[^21].

# Migration Plan

The migration plan is separated into sections based on what is required
to migrate the repositories listed in the [Repositories to
Migrate](#repositories-to-migrate) section. Completion of requirements
...
the python organization[^22]. Any repository that is moved will have the
\'Python core\' team added to it with write permissions[^23]. Anyone who
previously had rights to manage SSH keys on hg.python.org will become a
team maintainer for the \'Python core\' team.

### Define commands to move a Mercurial repository to Git

Since moving to GitHub also entails moving to Git[^24], we must decide
what tools and commands we will run to translate a Mercurial repository
to Git. The tools developed specifically for this migration are hosted
at <https://github.com/orsenthil/cpython-hg-to-git> .
...
(CLA)[^25]. Up until now, enforcement of CLA signing of contributed code
has been enforced by core developers checking whether someone had an `*`
by their username on bugs.python.org[^26]. With this migration, the plan
is to start off with automated checking and enforcement of contributors
signing the CLA.
...
is that an association will be needed between a person\'s
bugs.python.org[^27] account and their GitHub account, which will be
done through a new field in a user\'s profile. This does implicitly
require that contributors will need both a
GitHub[#github](##NOTE###github) and bugs.python.org account in order to
sign the CLA and contribute through GitHub.
...
bugs.python.org[^28] account, which has the data as to whether someone
has signed the CLA, a bot can monitor pull requests on GitHub and denote
whether the contributor has signed the CLA.
...
GitHub\'s status API (e.g., a red label stating, \"CLA not signed\"). If
a contributor lacks a bugs.python.org account, that will lead to the
...
CLA-related label (this is in contrast to using a GitHub status
check[^29] which is only triggered on code changes).

As no pre-existing bot exists to meet our needs, it will be hosted on
Heroku[^30] and written to target Python 3.5 to act as a showcase for
asynchronous programming. The code for the bot is hosted in the Knights
Who Say Ni project[^31].
...
Due to their use for generating webpages, the devguide[^32] and
peps[^33] repositories need their respective processes updated to pull
from their new Git repositories.

## Requirements for the cpython Repository

Obviously the most active and important repository currently hosted at
hg.python.org[^34] is the cpython repository[^35]. Because of its
importance and high-frequency use, it requires more tooling before being
moved to GitHub compared to the other repositories mentioned in this
PEP.
...
a contribution from a patch file uploaded to bugs.python.org[^36]. This
will obviously help keep the linear history, but it will need to be made
to have attribution to the patch author.
...
request to
cpython](#git-cli-commands-for-committing-a-pull-request-to-cpython).

### Linking pull requests to issues

Historically, external contributions were attached to an issue on
bugs.python.org[^37] thanks to the fact that all external contributions
were uploaded as a file. For changes committed by a core developer who
committed a change directly, the specifying of an issue number in the
commit message of the format `Issue #` at the start of the message led
to a comment being posted to the issue linking to the commit.
...
will be made on bugs.python.org[^38]. Some visible notification \-- e.g.
...
from either the Subversion or Mercurial copies of the cpython repo[^39]
to get redirected to the URL for that revision in the Mercurial
repository. The URL rewriter will need to be updated to redirect to the
Git repository and to support the new revision IDs created for the Git
repository.
...
The [bugs.python.org commit number
rewriter](https://hg.python.org/tracker/python-dev/file/tip/extensions/local_replace.py#l76)
...
Once the cpython repository[^40] is migrated, all repositories will have
been moved to GitHub[#github](##NOTE###github) and the development
process should be on equal footing as before the move. But a key reason
for this migration is to improve the development process, making it
better than it has ever been. This section outlines some plans on how to
improve things.

It should be mentioned that overall feature planning for
bugs.python.org[^41] \-- which includes plans independent of this
migration \-- are tracked on their own wiki page[^42].

### Handling Misc/NEWS

Traditionally the `Misc/NEWS` file[^43] has been problematic for changes
which spanned Python releases. Oftentimes there will be merge conflicts
when committing a change between e.g., 3.5 and 3.6 only in the
...
in the `Misc/NEWS` file [^44]. As part of our tool modernization,
working with the `Misc/NEWS` file will be simplified.
...
entries could be dropped since critical information is captured by the
...
Traditionally the `Misc/ACKS` file[^45] has been managed by hand. But
thanks to Git supporting an `author` value as well as a `committer`
value per commit, authorship of a commit can be part of the history of
the code itself.
...
Just as hg.python.org[^46] currently points to the Mercurial repository
for Python, git.python.org should do the equivalent for the Git
repository.

### Backup of pull request data

Since GitHub[#github](##NOTE###github) is going to be used for code
hosting and code review, those two things need to be backed up. In the
case of code hosting, the backup is implicit as all non-shallow Git[^47]
...
such as Homu[^48] or Zuul[^49].

The name given to this bot in order to give it commands is an open
issue: [Naming the bots](#naming-the-bots).

### A CI service

There are various CI services that provide free support for open source
projects hosted on GitHub[#github](##NOTE###github). After experimenting
with a couple CI services, the decision was made to go with Travis[^50].

The current CI service for Python is Pypatcher[^51]. A request can be
made in IRC to try a patch from bugs.python.org[^52]. The results can be
viewed at <https://ci.centos.org/job/cPython-build-patch/> .
...
for open source projects. In the end, Codecov[^53] was chosen as the
best option.
...
bugs.python.org[^54] when a review comment is left on Rietveld[^55]. It
would be nice to fix this so that people can subscribe only to comments
at bugs.python.org and not GitHub[#github](##NOTE###github) and yet
still know when something occurs on GitHub in terms of review comments
on relevant pull requests. Current thinking is to post a comment to
...
although with GitHub now supporting
[reviews](https://help.github.com/articles/reviewing-changes-in-pull-requests/)
...
As of right now, bugs.python.org[^56] allows people to log in using
Google, Launchpad, or OpenID credentials. It would be good to expand
this to GitHub credentials.
...
one of the repositories to be moved as part of this migration. As such,
it would be nice to set up appropriate webhooks to trigger rebuilding
the appropriate web content when the files they are based on change
instead of having to wait for, e.g., a cronjob to trigger.

This can partially be solved if the documentation is a Sphinx project as
then the site can have an unofficial mirror on [Read the
Docs](https://readthedocs.org/), e.g.
...
which points back to the file on GitHub[#github](##NOTE###github) that
stores the content of the page. That would allow for quick pull requests
to fix simple things such as spelling mistakes.
...
- [Installing Python
  Modules](https://docs.python.org/3/installing/index.html)
- [Distributing Python
  Modules](https://docs.python.org/3/distributing/index.html)
- [Extending and
  Embedding](https://docs.python.org/3/extending/index.html)
...
It has also been suggested to split out the [What\'s
New](https://docs.python.org/3/whatsnew/index.html) documents. That
would require deciding whether a workflow could be developed where it
would be difficult to forget to update What\'s New (potentially through
a label added to PRs, like \"What\'s New needed\").
...
Requirements for migrating the devinabox[^57] repository:

- Completed
  - [Adding GitHub username support to
    bugs.python.org](#adding-github-username-support-to-bugs.python.org)
    (Maciej Szulik and Ezio Melotti)
  - [A bot to enforce CLA signing](#a-bot-to-enforce-cla-signing):
    <https://github.com/python/the-knights-who-say-ni> (Brett Cannon)
  - [Create a \'Python core\' team](#create-a-python-core-team)
  - [Define commands to move a Mercurial repository to
    Git](#define-commands-to-move-a-mercurial-repository-to-git):
...
## cpython repo[^60] {#cpython-repocpython-repo}

Required:

- Not started
  - [Update PEP 101](#update-pep-101) (commitment from Ned Deily to do
    this; **non-blocker**)
- In progress
  - [Deprecate sys.\_mercurial](#deprecate-sys._mercurial)
    (<http://bugs.python.org/issue27593>; review committal from Ned
    Deily; **non-blocker**)
  - [Update the linking service for mapping commit IDs to
    URLs](#update-the-linking-service-for-mapping-commit-ids-to-urls)
    ...
    **post-migration**)
- Completed
  - [Notify the issue if a commit is
    made](#notify-the-issue-if-a-commit-is-made)
  ...
  - [Update the devguide](#update-the-devguide), including [Document
    steps to commit a pull
    request](#document-steps-to-commit-a-pull-request)
  ...
  - [Linking a pull request to an
    issue](#linking-a-pull-request-to-an-issue)
...
- Not started
  ...
  - [Create https://git.python.org](#create-httpsgit.python.org)
  - [Backup of pull request data](#backup-of-pull-request-data)
  - [Handling Misc/ACKS](#handling-miscacks)
  - [Pull request commit queue](#pull-request-commit-queue)
  - [Allow bugs.python.org to use GitHub as a login
    provider](#allow-bugs.python.org-to-use-github-as-a-login-provider)
  - [Web hooks for re-generating web
    content](#web-hooks-for-re-generating-web-content)
  - [Splitting out parts of the documentation into their own
    repositories](#splitting-out-parts-of-the-documentation-into-their-own-repositories)
  - [Backup of Git repositories](#backup-of-git-repositories)
- In progress
  - [Notifying issues of pull request
    comments](#notifying-issues-of-pull-request-comments)
...
- Completed
  - [A CI Service](#a-ci-service)
  - [Test coverage report](#test-coverage-report)
  - [Link web content back to files that it is generated
    from](#link-web-content-back-to-files-that-it-is-generated-from)
  - [Handling Misc/NEWS](#handling-miscnews)
  - [Bot to generate cherry-pick pull
    requests](#bot-to-generate-cherry-pick-pull-requests)
...
With the code repositories moving over to Git[^61], there is no
technical need to keep hg.python.org[^62] running. Having said that,
some in the community would like to have it stay functioning as a
Mercurial[^63] mirror of the Git repositories. Others have said that
...
Because Git[^64] may be a new version control system for core
...
patch file uploaded to bugs.python.org[^65]. Here the linear history
...
As part of the discussion surrounding [Handling
Misc/NEWS](#handling-miscnews), the suggestion has come up of deriving
the file from the commit logs itself. In this scenario, the first line
of a commit message would be taken to represent the news entry for the
change. Some heuristic to tie in whether a change warranted a news entry
would be used, e.g., whether an issue number is listed.
...
on bugs.python.org[^67]. This would mean an issue that is marked as
\"resolved\" could not be closed until a news entry is added in the
...
[^6]: <https://hg.python.org>

[^7]: Rietveld (<https://github.com/rietveld-codereview/rietveld>)

[^8]: <https://bugs.python.org>

[^9]: GitLab (<https://about.gitlab.com/>)

[^10]: core-workflow mailing list
    (<https://mail.python.org/mailman/listinfo/core-workflow>)

[^11]: Guido van Rossum\'s keynote at PyCon US
    (<https://www.youtube.com/watch?v=G-uKNd5TSBw>)

[^12]: Email to core-workflow outlining reasons why GitHub was selected
    (<https://mail.python.org/pipermail/core-workflow/2016-January/000345.html>)

[^13]: Rietveld (<https://github.com/rietveld-codereview/rietveld>)

[^14]: Pythocat (<https://octodex.github.com/pythocat>)

[^15]: <https://hg.python.org>

[^16]: Mercurial repository for devinabox
    (<https://hg.python.org/devinabox/>)

[^17]: Mercurial repository for the Unified Benchmark Suite
    (<https://hg.python.org/benchmarks/>)

[^18]: Mercurial repository of the Python Enhancement Proposals
    (<https://hg.python.org/peps/>)

[^19]: Mercurial repository for the Python Developer\'s Guide
    (<https://hg.python.org/devguide/>)

[^20]: Mercurial repository for CPython
    (<https://hg.python.org/cpython/>)

[^21]: <https://bugs.python.org>

[^22]: Python organization on GitHub (<https://github.com/python>)

[^23]: GitHub repository permission levels
...
[^25]: Python Software Foundation Contributor Agreement
    (<https://www.python.org/psf/contrib/contrib-form/>)

[^26]: <https://bugs.python.org>

[^27]: <https://bugs.python.org>

[^28]: <https://bugs.python.org>

[^29]: GitHub status checks
    (<https://developer.github.com/v3/repos/statuses/>)

[^30]: Heroku (<https://www.heroku.com/>)

[^31]: The Knights Who Say Ni project
    (<https://github.com/python/the-knights-who-say-ni>)

[^32]: Mercurial repository for the Python Developer\'s Guide
    (<https://hg.python.org/devguide/>)

[^33]: Mercurial repository of the Python Enhancement Proposals
    (<https://hg.python.org/peps/>)

[^34]: <https://hg.python.org>

[^35]: Mercurial repository for CPython
    (<https://hg.python.org/cpython/>)

[^36]: <https://bugs.python.org>

[^37]: <https://bugs.python.org>

[^38]: <https://bugs.python.org>

[^39]: Mercurial repository for CPython
    (<https://hg.python.org/cpython/>)

[^40]: Mercurial repository for CPython
    (<https://hg.python.org/cpython/>)

[^41]: <https://bugs.python.org>

[^42]: Wiki page for bugs.python.org feature development
    (<https://wiki.python.org/moin/TrackerDevelopmentPlanning>)

[^43]: `Misc/NEWS`
    (<https://hg.python.org/cpython/file/default/Misc/NEWS>)

[^44]: Devguide instructions on how to merge across branches
...
[^46]: <https://hg.python.org>

[^47]: Git (<https://git-scm.com/>)

[^48]: Homu (<http://homu.io/>)

[^49]: Zuul (<http://docs.openstack.org/infra/zuul/>)

[^50]: Travis (<https://travis-ci.org/>)

[^51]: Pypatcher (<https://github.com/kushaldas/pypatcher>)

[^52]: <https://bugs.python.org>

[^53]: Codecov (<https://codecov.io/>)

[^54]: <https://bugs.python.org>

[^55]: Rietveld (<https://github.com/rietveld-codereview/rietveld>)

[^56]: <https://bugs.python.org>

[^57]: Mercurial repository for devinabox
    (<https://hg.python.org/devinabox/>)

[^58]: Mercurial repository of the Python Enhancement Proposals
    (<https://hg.python.org/peps/>)

[^59]: Mercurial repository for the Python Developer\'s Guide
    (<https://hg.python.org/devguide/>)

[^60]: Mercurial repository for CPython
    (<https://hg.python.org/cpython/>)

[^61]: Git (<https://git-scm.com/>)

[^62]: <https://hg.python.org>

[^63]: Mercurial (<https://www.mercurial-scm.org/>)
