...
source_path: https://github.com/python/peps/blob/main/peps/pep-0413.rst
source_commit: 9a4b6cb8fa3fcebd97ea94dd66a92ccc2b00bb54
---

# PEP Withdrawal

With the acceptance of `453`{.interpreted-text role="pep"} meaning that
`pip` will be available to most new Python users by default, this will
hopefully reduce the pressure to add new modules to the standard library
before they are sufficiently mature.
...
Like `407`{.interpreted-text role="pep"}, it aims to adjust the current
balance between measured change that allows the broader community time
to adapt and being able to keep pace with external influences that
evolve more rapidly than the current release cycle can handle (this
problem is particularly notable for standard library elements that
relate to web technologies).

However, it\'s more conservative in its aims than
`407`{.interpreted-text role="pep"}, seeking to restrict the increased
pace of development to builtin and standard library interfaces, without
affecting the rate of change for other elements such as the language
syntax and version numbering as well as the CPython binary API and
bytecode format.

# Rationale

To quote the `407`{.interpreted-text role="pep"} abstract:
...
I agree with the `407`{.interpreted-text role="pep"} authors that the
current release cycle of the *standard library* is too slow to
effectively cope with the pace of change in some key programming areas
(specifically, web protocols and related technologies, including
databases, templating and serialisation formats).

However, I have written this competing PEP because I believe that the
approach proposed in `407`{.interpreted-text role="pep"} of offering
full, potentially binary incompatible releases of CPython every 6 months
places too great a burden on the wider Python ecosystem.
...
branches enter \"security fix only\" mode (for example, Twisted
currently ships binaries for 2.5, 2.6 and 2.7, NumPy and SciPy support
those 3 along with 3.1 and 3.2, PyGame adds a 2.4 binary release,
wxPython provides both 32-bit and 64-bit binaries for 2.6 and 2.7, etc).
...
deprecation periods, support periods, all sorts of things.
`407`{.interpreted-text role="pep"} proposes that collective knowledge
all be swept aside, without offering a compelling rationale for why such
a course of action is actually *necessary* (aside from, perhaps, making
the lives of the CPython core developers a little easier at the expense
of everyone else).
...
\"standard library releases\". As with `407`{.interpreted-text
role="pep"}, this will give CPython 3 kinds of release:
...
- binary incompatible changes to the C ABI (although the
  `384`{.interpreted-text role="pep"} stable ABI must still be
  preserved)
- bug fixes
...
- new features in C extension modules (subject to
  `399`{.interpreted-text role="pep"} compatibility requirements)
...
In each case, the scenario is described for both the status quo (i.e.
slow release cycle) the versioning scheme in this PEP and the free
wheeling minor version number scheme proposed in `407`{.interpreted-text
role="pep"}.
...
version, not the standard library version. Most users won\'t even need
to care that the standard library version number exists. In the two
...
addition of new features to the standard library, I\'m calling this a
win for the scheme in this PEP.
...
**Status quo:** unless using the `384`{.interpreted-text role="pep"}
stable ABI, a new binary release is needed every time the minor version
number changes.
...
\"LTS\" tag to an otherwise ordinary release number. Tagging a
particular version as being special is great for communicating with
version control systems and associated automated tools, but it\'s a
lousy way to communicate information to other humans.

## Python developer, deciding their minimum version dependency

**Status quo:** look for \"version added\" or \"version changed\"
markers in the documentation, check against `sys.version_info`
...
since `407`{.interpreted-text role="pep"} does have the advantage of
preserving the status quo, I\'m calling this one for
`407`{.interpreted-text role="pep"} (albeit with a slim margin).
...
**Verdict:** another marginal win for `407`{.interpreted-text
role="pep"}. The new standard library version *is* an extra piece of
information that users may need to pass back to developers when
reporting issues with Python libraries (or Python itself, on our own
tracker). However, by including it in `sys.version`, many fault reports
will already include it, and it is easy to request if needed.
...
**Verdict:** until `407`{.interpreted-text role="pep"} is updated to
actually address this scenario, a clear win for this PEP.

# Effects

## Effect on development cycle

Similar to `407`{.interpreted-text role="pep"}, this PEP will break up
the delivery of new features into more discrete chunks. Instead of a
whole raft of changes landing all at once in a language release, each
language release will be limited to 6 months worth of standard library
changes, as well as any changes associated with new syntax.
...
`407`{.interpreted-text role="pep"} has this to say about the effects on
the community:
...
they\'re only covering an \"old\" version of Python (\"You\'re only
using 3.3, the latest is 3.5!\"), etc.

As the minor version number starts climbing 3 times faster than it has
...
The community benefits listed in `407`{.interpreted-text role="pep"} are
equally applicable to this PEP, at least as far as the standard library
is concerned:
...
> quickly available to normal users.
...
separate-files-per-change approach, similar to that used by Twisted[^1].
...
separate \"\<branch\>-release\" trees that only receive versions that
have been marked as good by the stable buildbots, it\'s still a useful
...
version number approach proposed in `407`{.interpreted-text role="pep"}
could not (at least, not without a lot of user confusion and
uncertainty).
...
require a little tidying up of the What\'s New document before making
the release.

## Why isn\'t PEP 384 enough?

`384`{.interpreted-text role="pep"} introduced the notion of a \"Stable
ABI\" for CPython, a limited subset of the full C ABI that is guaranteed
to remain stable. Extensions built against the stable ABI should be able
to support all subsequent Python versions with the same binary.
...
- Interpreter implementation updates
  ...
  - changes to the compilation toolchain that do not affect the AST or
    alter the bytecode magic number
  - changes to the core interpreter eval loop
- bug fixes from the corresponding maintenance release
...
either, because too many \"standard library modules\" are actually
tightly linked in to the internal details of their respective
interpreters (for example, the builtins, `weakref`, `gc`, `sys`,
`inspect`, `ast`).
...
Thanks go to the `407`{.interpreted-text role="pep"} authors for
starting this discussion, as well as to those authors and Larry Hastings
for initial discussions of the proposal made in this PEP.
...
[^1]: Twisted\'s \"topfiles\" approach to NEWS generation
