...
display a user\'s Python installations. For example, the
`397`{.interpreted-text role="pep"} `py.exe` launcher and editors such
as PyCharm and Visual Studio already make use of this information.
...
\"values\" (named and typed attributes attached to a key). These are
used on Windows to store settings in much the same way that directories
...
available from the MSDN Library[^1].
...
The company name `PyLauncher` is reserved for the
`397`{.interpreted-text role="pep"} launcher (`py.exe`). It does not
follow this convention and should be ignored by tools.
