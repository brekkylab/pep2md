...
created: 18-Jun-2009
post_history:
- '`19-Jun-2009 <https://mail.python.org/archives/list/python-dev@python.org/thread/YAZQHFUCYEYIX5MIG6LFJLCVQ5ORVUM6/>`__'
...
source_path: https://github.com/python/peps/blob/main/peps/pep-0387.rst
source_commit: 2b6f1391f559924aee5a5a6b724655248b2c440b
---
...
- Anything documented publicly as being private. Note that if something
  is not documented at all, it is *not* automatically considered
  private.
...
- Backward compatibility rules do not apply to any module or API that is
  explicitly documented as **Provisional** per `411`{.interpreted-text
  role="pep"}.
...
- In general, incompatibilities should have a large benefit to breakage
  ...
  code. For example, adding an stdlib module with the same name as a
  third party package is generally not acceptable. Adding a method or
...
- Unless it is going through the deprecation process below, the behavior
  of an API *must* not change in an incompatible fashion between any two
  consecutive releases. Python\'s yearly release process
  (`602`{.interpreted-text role="pep"}) means that the deprecation
...
# Soft Deprecation {#pep387-soft-deprecation}
...
[Backwards Compatibility Rules](#backwards-compatibility-rules) (i.e.,
there is no exception because the feature is already soft deprecated).
...
1.  Discuss the change. Depending on the degree of incompatibility, this
    could be on [Discourse](https://discuss.python.org/), the [issue
    tracker](https://github.com/python/cpython/issues/), or in an
    appropriate workgroup or SIG. If the discussion reaches consensus a
    `PEP <1>`{.interpreted-text role="pep"} or similar document may be
    written. Hopefully users of the affected API will pipe up to
    comment.
...
2.  Add a warning to the current `main` branch. If behavior is changing,
    ...
    the usual warning category to use, but `PendingDeprecationWarning`
    may be used in special cases where the old and new versions of the
    API will coexist for many releases[^2]. The warning message should
    ...
    feasible, also change [typeshed](https://github.com/python/typeshed)
    to add the `@deprecated` decorator (see `702`{.interpreted-text
    role="pep"}) to the deprecated API, so that users of static type
...
3.  Wait for the warning to appear in at least two minor Python versions
    ...
    at least Python 3.12 or Python 4.0 to make the change). It is
    preferred, though, to wait 5 years before removal (e.g., warn
    starting in Python 3.10, removal in 3.15; this happens to coincide
    with the current lifetime of a minor release of Python).
...
- 2025-Jan-27: Updated to prefer a 5-year deprecation period before
  removal.
- 2023-Nov-14: Added `@deprecated` decorator per `702`{.interpreted-text
  role="pep"}.
...
- 2023-Jun-26: Multiple smaller updates and clarifications, discussed in
  <https://discuss.python.org/t/22042>.
