...
source_path: https://github.com/python/peps/blob/main/peps/pep-0355.rst
source_commit: cca89384b3af7b41922ec6f3de2c1c2d22d7e2e8
---
...
discussion flares up from time to time, it\'s time to put this PEP out
...
for handling paths in an object oriented fashion. The \"weak\"
...
- Almost every program uses paths to access files. It makes sense that a
  task, that is so often performed, should be as intuitive and as easy
...
deprecated.
  ...
  \_\_new\_\_()          os.path.join(), os.curdir
...
- The `__div__()` method was removed. Overloading the / (division)
  ...
  to be division. The method can always be re-added later if the BDFL so
...
- The methods and the properties `getatime()/atime`, `getctime()/ctime`,
  ...
  methods and properties have been merged to `atime()`, `ctime()`,
  `mtime()` and `size()`. The reason they are not properties instead, is
  because there is a possibility that they may change unexpectedly. The
...
- Due to `Path` subclassing either `str` or `unicode`, the following
  ...
  not. Most persons debating said that most string methods doesn\'t make
  sense in the context of filesystem paths \-- they are just dead
