...
source_path: https://github.com/python/peps/blob/main/peps/pep-0204.rst
source_commit: eb0ce6100f5b545409cfa69a6c8bdcb6b638657c
---
...
- Clarity of purpose: Adding another function call, possibly with extra
  arithmetic to determine the desired length and step of the list, does
  not improve readability of the code. Also, it is possible to
  \"shadow\" the builtin `range` function by supplying a local or global
  variable with the same name, effectively replacing it. This may or may
  not be a desired effect.
...
- Consistency: There already is a syntax that is used to denote ranges,
  as shown below. This syntax uses the exact same arguments, though all
  optional, in the exact same way. It seems logical to extend this
  syntax to ranges, to form \"range literals\".
...
- Because it is built-in syntax, instead of a builtin function, it
  ...
  about what the code does, and that an optimizer will not have to worry
  about `range()` being \"shadowed\".
...
- One possible solution to the discrepancy of requiring the *end*
  ...
  function does. However, a generator would not be a list, and it would
  be impossible, for instance, to assign to items in the generator, or
  append to it.
...
- How should range literals interact with another proposed new feature,
  `"list comprehensions" <202>`{.interpreted-text role="pep"}?
  Specifically, should it be possible to create lists in list
  comprehensions? E.g.:
...
- Range literals accept objects other than integers: it performs
  ...
  present problems with user-defined classes: even if the base class can
  be found and a new instance created, the instance may require
