...
source_path: https://github.com/python/peps/blob/main/peps/pep-0296.rst
source_commit: c8bbd29f1911cba9dbf3ab0a61bce74d1e009ea3
---

# Notice

This PEP is withdrawn by the author (in favor of `358`{.interpreted-text
role="pep"}).
...
1.  Efficient underlying array storage via the standard C type
    \"unsigned char\". This allows fine grain control over how much
    memory is allocated. With the alignment restrictions designated in
    the next item, it is trivial for low level extensions to cast the
    pointer to a different type as needed.
...
3.  The bytes object implements a subset of the sequence operations
    ...
    \"view\". Additionally, repetition and concatenation are undefined
    for bytes objects and will raise an exception.
...
- Guido van Rossum wondered whether it would make sense to be able to
  ...
  object. (It doesn\'t resize, and the pointer is valid for the lifetime
  of the object.) As such, a method could be added to the mmap module
  such that a bytes object could be created directly from a mmap object.
  An initial stab at how this would be implemented would be to use the
  `PyBytes_FromPointer()` function described above and pass the
  `mmap_object` as the user pointer. The destructor function would
  decref the `mmap_object` for cleanup.
...
- Todd Miller notes that it may be useful to have two new functions:
  ...
  are similar to `PyObject_AsReadBuffer()` and
  ...
  support `LONG_LONG` lengths) and most other buffer like objects (which
  only support int lengths). These functions could be in lieu of, or in
  addition to, creating a specific `PyByte_GetReadBuffer()` and
  ...
  it should only affect abstract.c and abstract.h. Should this be added
  above?
...
- It was generally agreed that abusing the segment count of the
  ...
  presumably in many third party extensions, punt when the segment count
  is not 1.
