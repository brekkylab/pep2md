...
source_path: https://github.com/python/peps/blob/main/peps/pep-0408.rst
source_commit: 347ce3eab334d84e03ebad4f74d2db74f4344a76
---
...
module\'s eventual full inclusion into the standard library, or to the
stability of its API, which may change for the next release.
...
Barnett\'s \'regex\' module[^2] as a provisional addition to the
standard library for Python 3.3 (using the \'regex\' name, rather than
as a drop-in replacement for the existing \'re\' module).

# Proposal - the \_\_preview\_\_ package
...
about whether the module\'s API is optimal, the module can be placed in
a special package named `__preview__` for a single minor release.
...
# Candidates for inclusion into \_\_preview\_\_
...
- Encapsulation of the import state (`368`{.interpreted-text
  role="pep"})
...
`407`{.interpreted-text role="pep"} proposes a change to the core Python
release cycle to permit interim releases every 6 months (perhaps limited
to standard library updates). If such a change to the release cycle is
made, the following policy for the `__preview__` namespace is suggested:
...
`__future__` module, so it\'s reasonable to ask why that can\'t be
re-used for this new purpose.
...
have become the compiler\'s default behaviour. Again, this is precisely
the opposite of what is intended for the preview package - it is almost
...
It was proposed[^4] to use a package name like `preview` or `exp`,
instead of `__preview__`. This was rejected in the discussion due to the
special meaning a \"dunder\" package name (that is, a name *with*
...
package in Python[^5]. Although his original proposal uses the name
...
[^2]: Proposal for inclusion of regex:
