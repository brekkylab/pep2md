...
source_path: https://github.com/python/peps/blob/main/peps/pep-3140.rst
source_commit: dfbec8b4e6fff525b4446dbd9e40f9367f7bfd87
---
...
One of the motivations for `3138`{.interpreted-text role="pep"} is that
neither `repr` nor `str` will allow the sensible printing of dicts whose
keys are non-ASCII text strings. Now that Unicode identifiers are
allowed, it includes Python\'s own attribute dicts. This also includes
JSON serialization (and caused some hoops for the json lib).

`3138`{.interpreted-text role="pep"} proposes to fix this by breaking
the \"repr is safe ASCII\" invariant, and changing the way `repr` (which
is used for persistence) outputs some objects, with system-dependent
failures.
