...
source_path: https://github.com/python/peps/blob/main/peps/pep-0461.rst
source_commit: 18b84ac3f7d47b307bec33f81dc207408441b440
---
...
`-`, space, and `+` (plus any added to Python 3)). The only non-numeric
codes allowed are `c`, `b`, `a`, and `s` (which is a synonym for b).
...
- input type supports `Py_buffer`[^4]? use it to collect the necessary
  bytes
- input type is something else? use its `__bytes__` method[^5] ; if
...
2/3 code bases easier to maintain. Python 3 only code use `%a`[^7].
...
- Rejected as this would lead to intermittent failures. Better to have
  the operation always fail so the trouble-spot can be correctly fixed.
...
A competing PEP,
`PEP 460 Add binary interpolation and formatting <460>`{.interpreted-text
role="pep"}, also exists.
...
[^4]: <http://docs.python.org/3/c-api/buffer.html> examples:
    `memoryview`, `array.array`, `bytearray`, `bytes`
...
[^8]: <http://bugs.python.org/issue23467> \-- originally `%r` was not
    allowed, but was added for consistency during the 3.5 alpha stage.
