...
author:
- Samuele Pedroni <pedronis@python.org>
status: Rejected
...
source_path: https://github.com/python/peps/blob/main/peps/pep-0325.rst
source_commit: a6129053cdc84fec3dc62170217c4a8f2a24b779
---
...
Rejected in favor of `342`{.interpreted-text role="pep"} which includes
substantially all of the requested behavior in a more refined form.
...
Currently `255`{.interpreted-text role="pep"} disallows yield inside a
try clause of a try-finally statement, because the execution of the
finally clause cannot be guaranteed as required by try-finally
semantics.
...
`288`{.interpreted-text role="pep"} proposes a more general solution,
allowing custom exception passing to generators. The proposal in this
PEP addresses more directly the problem of resource release. Were
`288`{.interpreted-text role="pep"} implemented, Exceptions Semantics
for close could be layered on top of it, on the other hand
`288`{.interpreted-text role="pep"} should make a separate case for the
