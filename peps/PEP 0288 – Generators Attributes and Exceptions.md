...
source_path: https://github.com/python/peps/blob/main/peps/pep-0288.rst
source_commit: 0a06b27a9803249d02f3e8a6bea8b2437fccdcd0
---
...
subsumed into `343`{.interpreted-text role="pep"}. The attribute passing
capability never built a following, did not have a clear implementation,
and did not have a clean way for the running generator to access its own
namespace.
...
generator. Most of the underlying machinery is already in place, only
the access function needs to be added.
...
trigger clean-up code (`255`{.interpreted-text role="pep"}).
...
the raise syntax would be supported (no variants for \"`raise string`\"
or \"`raise class, instance`\").
