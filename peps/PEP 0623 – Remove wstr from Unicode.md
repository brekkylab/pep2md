...
source_path: https://github.com/python/peps/blob/main/peps/pep-0623.rst
source_commit: 9404ff6f9def04965b7d44186d17eed98a745b52
---

# Abstract

`393`{.interpreted-text role="pep"} deprecated some unicode APIs, and
introduced `wchar_t *wstr`, and `Py_ssize_t wstr_length` in the Unicode
structure to support these deprecated APIs.
...
`393`{.interpreted-text role="pep"} introduced efficient internal
representation of Unicode and removed border between \"narrow\" and
\"wide\" build of Python.

`393`{.interpreted-text role="pep"} was implemented in Python 3.3 which
is released in 2012. Old APIs were deprecated since then, and the
removal was scheduled in Python 4.0.

Python 4.0 was expected as next version of Python 3.9 when
`393`{.interpreted-text role="pep"} was accepted. But the next version
of Python 3.9 is Python 3.10, not 4.0. This is why this PEP schedule the
removal plan again.

## Python 2 reached EOL

Since Python 2 didn\'t have `393`{.interpreted-text role="pep"} Unicode
implementation, legacy APIs might help C extension modules supporting
...
- [When can we remove wchar_t\* cache from
...
- [bpo-38604: Schedule Py_UNICODE API
...
- [bpo-30863: Rewrite PyUnicode_AsWideChar() and
  PyUnicode_AsWideCharString()](https://bugs.python.org/issue30863):
