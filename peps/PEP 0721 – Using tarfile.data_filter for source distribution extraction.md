...
python_version: '3.12'
post_history:
- '`04-Jul-2023 <https://discuss.python.org/t/28928>`__'
resolution: '`02-Aug-2023 <https://discuss.python.org/t/28928/13>`__'
python_status: Final
url: https://peps.python.org/pep-0721/
source_path: https://github.com/python/peps/blob/main/peps/pep-0721.rst
source_commit: 5a13ad965d3ddeca69507de8f830b331aa00b57c
---

::: canonical-pypa-spec
`packaging:sdist-archive-features`{.interpreted-text role="ref"}
:::

# Abstract

Extracting a source distribution archive should normally use the `data`
filter added in `706`{.interpreted-text role="pep"}. We clarify details,
and specify the behaviour for tools that cannot use the filter directly.
...
platform-dependent. As explained in `706`{.interpreted-text role="pep"},
when extracting a tarball, one should always either limit the allowed
features, or explicitly give the tarball total control.

# Rationale

For source distributions, the `data` filter introduced in
`706`{.interpreted-text role="pep"} is enough. It allows slightly more
features than `git` and `zip` (both commonly used in packaging
workflows).
...
of `pip download` and `setuptools.archive_util.unpack_tarfile` is valid,
except cases deemed too dangerous to allow. Another consideration is
ease of implementation for non-Python tools.
...
The following will be added to [the PyPA source distribution format
spec](https://packaging.python.org/en/latest/specifications/source-distribution-format/)
...
`tarfile.data_filter` (e.g. `TarFile.extractall(..., filter='data')`),
OR follow the *Unpacking without the data filter* section below.

As an exception, on Python interpreters without
`hasattr(tarfile, 'data_filter')` (`706`{.interpreted-text role="pep"}),
tools that normally use that filter (directly on indirectly) MAY warn
the user and ignore this specification. The trade-off between usability
(e.g. fully trusting the archive) and security (e.g. refusing to unpack)
is left up to the tool in this case.
...
out there probably does rely on details that aren\'t guaranteed. This
PEP bans the most dangerous of those features, and the rest is made
tool-specific.
