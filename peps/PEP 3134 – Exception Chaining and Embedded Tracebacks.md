...
source_path: https://github.com/python/peps/blob/main/peps/pep-3134.rst
source_commit: c54db2e5a0faf182c265bd8c1d9b5a62da547eb2
---

# Numbering Note

This PEP started its life as `344`{.interpreted-text role="pep"}. Since
...
exception with more information. Brett Cannon[^2] brought up chained
...
instances. This is noted in `3000`{.interpreted-text role="pep"}.
...
Python-Dev[^8], including `cause`, `antecedent`, `reason`, `original`,
...
has expressed concerns with making such changes[^9].
...
that implicitly retains chained exceptions in an array named `@@`. In
that RFC, the most recently raised exception is exposed for matching, as
in this PEP; also, arbitrary expressions (possibly involving `@@`) can
be evaluated for exception matching.
...
the client doesn\'t have to be aware of the storage-specific details,
but doesn\'t want to lose the underlying error information.
...
traceback from `sys.exc_traceback` or `sys.exc_info()`[^12] and pass
...
into a weak reference when the variable goes out of scope[^15].
...
- If `340`{.interpreted-text role="pep"} or `343`{.interpreted-text
  role="pep"} is accepted, replace the three (`type`, `value`,
...
were implemented in revision 57783[^16].
...
[^11]: MSDN .NET Framework Library, \"Exception.InnerException
    Property\"
