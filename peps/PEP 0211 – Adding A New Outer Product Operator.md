...
source_path: https://github.com/python/peps/blob/main/peps/pep-0211.rst
source_commit: b437601a8033b9abd08792d373519b204ede4c66
---
...
Numeric module (`209`{.interpreted-text role="pep"}) will overload this
...
therefore provide two forms of the common arithmetic operators. One form
works element-by-element, e.g. multiplies corresponding elements of its
matrix arguments. The other implements the \"mathematical\" definition
of that operation, e.g. performs row-column matrix multiplication.

Zhu and Lielens have `proposed <225>`{.interpreted-text role="pep"}
doubling up Python\'s operators in this way. Their proposal would create
six new binary infix operators, and six new in-place operators.

The original version of this proposal was much more conservative. The
author consulted the developers of GNU Octave[^1], an open source clone
of MATLAB. Its developers agreed that providing an infix operator for
matrix multiplication was important: numerical programmers really do
...
> I DON\'T think it\'s a must have, and I do a lot of matrix inversion.
> I cannot remember if its `A\b` or `b\A` so I always write `inv(A)*b`
> instead. I recommend dropping `\`.
...
for this proposal. As part of the discussion of `201`{.interpreted-text
role="pep"}, Lockstep Iteration, the author of this proposal conducted
an informal usability experiment[^3]. The results showed that users are
...
2.  `@` would have be chainable in the same way as comparison operators,
    ...
    support from the parser, as the outer iterator created by the first
...
3.  Introduce prefixed forms of all existing operators, such as `~*` and
    `~+`, as proposed in `225`{.interpreted-text role="pep"}.

    Our objections to this are that there isn\'t enough demand to
    justify the additional complexity (see Rawlings\' comments[^4]), and
    that the proposed syntax fails the \"low toner\" readability test.
