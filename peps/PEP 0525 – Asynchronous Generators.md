...
source_path: https://github.com/python/peps/blob/main/peps/pep-0525.rst
source_commit: 58eb1b527e1184023b2f481ffdb0ac44d6f78a24
---

# Abstract

`492`{.interpreted-text role="pep"} introduced support for native
coroutines and `async`/`await` syntax to Python 3.5. It is proposed here
to extend Python\'s asynchronous capabilities by adding support for
*asynchronous generators*.

# Rationale and Goals

Regular generators (introduced in `255`{.interpreted-text role="pep"})
enabled an elegant way of writing complex *data producers* and have them
...
Essentially, the goals and rationale for `255`{.interpreted-text
role="pep"}, applied to the asynchronous execution case, hold true for
...
generators and coroutines in Python (`342`{.interpreted-text
role="pep"}, `380`{.interpreted-text role="pep"} and
`492`{.interpreted-text role="pep"}).
...
iteration protocol defined in `492`{.interpreted-text role="pep"}.
...
`492`{.interpreted-text role="pep"} requires an event loop or a
scheduler to run coroutines. Because asynchronous generators are meant
to be used from coroutines, they also require an event loop to run and
finalize them.
...
makes it possible to implement \"shutdown\" mechanisms to safely
...
![image](pep-0525-1.png){.invert-in-dark-mode
.invert-in-dark-modealign-center width="80.0%"}
...
Originally, `492`{.interpreted-text role="pep"} defined `__aiter__` as a
method that should return an *awaitable* object, resulting in an
...
`525`{.interpreted-text role="pep"} was accepted by Guido, September 6,
2016[^1].

# Implementation

The implementation is tracked in issue 28003[^2]. The reference
implementation git repository is available at[^3].
