...
author:
- Peter Harris <scav@blueyonder.co.uk>
status: Final
...
and comp.lang.python revealed a desire for several tools that operated
on function objects, but were not related to functional programming.
Rather than create a new module for these tools, it was agreed[^1] that
...
partial implementation with a \_\_get\_\_ method to more closely emulate
...
such as `(+1)` to mean the equivalent of Python\'s `(lambda x: x + 1)`.
...
- A curry class would indeed be a useful addition to the standard
  library.
- It isn\'t function currying, but partial application. Hence the name
  is now proposed to be partial().
...
by special punctuation \-- a way of directly expressing
partially-applied function literals is not such a stretch. However, not
one single person has said they like it, so as far as I\'m concerned
it\'s a dead parrot.
...
good implementation and non-confusing semantics, I don\'t think it
should be ruled out.
...
I also coded the class in Pyrex, to estimate how the performance might
be improved by coding it in C:
...
implementation, since to be fully general it has to operate by Python
API calls. For the same reason, a C implementation will be unlikely to
be much faster, so the case for a built-in coded in C is not very
strong.
...
[931005](https://bugs.python.org/issue931005),
[931007](https://bugs.python.org/issue931007), and
[931010](https://bugs.python.org/issue931010) respectively) have been
submitted but not yet checked in.

A C implementation by Hye-Shik Chang has also been submitted, although
it is not expected to be included until after the Python implementation
has proven itself useful enough to be worth optimising.
...
[^2]: Patches [931005](##REF##931005), [931007](##REF##931007), and
    [931010](##REF##931010).
