...
source_path: https://github.com/python/peps/blob/main/peps/pep-0391.rst
source_commit: be89ca4c0e2afe45cd6a1c3bf53143e751be0e8f
---
...
logging\'s configuration) as \'crufty\' or \'ugly\', in some cases
apparently on purely aesthetic grounds.
...
Historically, the logging package has not been `8`{.interpreted-text
role="pep"} conformant. At some future time, this will be corrected by
changing method and function names in the package in order to conform
with `8`{.interpreted-text role="pep"}. However, in the interests of
uniformity, the proposed additions to the API use a naming scheme which
is consistent with the present scheme used by logging.
...
- A function, called `dictConfig()`, which takes a single argument
  \- the dictionary holding the configuration. Exceptions will be raised
  if there are errors while processing the dictionary.

It will be possible to customize this API - see the section on [API
Customization](#api-customization). [Incremental
configuration](#incremental-configuration) is covered in its own
section.
...
Handler ids are treated specially, see the section on [Handler
Ids](#handler-ids), below.
...
configuration](#incremental-configuration) for more details.
...
made for being able to configure these incrementally. On the basis that
...
- `incremental` - whether the configuration is to be interpreted as
  ...
  described in the section on [Incremental
  Configuration](#incremental-configuration), below.
