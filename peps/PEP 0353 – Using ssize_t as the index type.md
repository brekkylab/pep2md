...
source_path: https://github.com/python/peps/blob/main/peps/pep-0353.rst
source_commit: 3917622ab90332f1e9a602c37d6aabff36ce2de3
---
...
this, introducing a platform-specific index type Py_ssize_t. An
...
to use Py_ssize_t, including the sequence slots in type objects, and the
buffer interface.
...
The conversion codes \'s#\' and \'t#\' will output Py_ssize_t if the
macro PY_SSIZE_T_CLEAN is defined before Python.h is included, and
...
when calling functions that return Py_ssize_t or size_t, in particular,
for functions that return the length of an object (this includes the
strlen function and the sizeof operator). A good compiler will warn when
a Py_ssize_t/size_t value is truncated into an int. In these cases,
...
- statically determine that the value shouldn\'t overflow an int unless
  ...
  smaller than INT_MAX, and raise an InternalError if it isn\'t.

- otherwise, check whether the value fits an int, and raise a ValueError
  if it doesn\'t.
...
and then using Py_ssize_t in the rest of the code. For the
tp_as_sequence slots, additional typedefs might be necessary;
...
to Py_ssize_t. The compiler will help in finding the spots, but a manual
review is still necessary.

Particular care must be taken for PyArg_ParseTuple calls: they need all
be checked for s# and t# converters, and PY_SSIZE_T_CLEAN must be
defined before including Python.h if the calls have been updated
accordingly.

Fredrik Lundh has written a
[scanner](http://svn.effbot.python-hosting.com/stuff/sandbox/python/ssizecheck.py)
which checks the code of a C module for usage of APIs whose signature
has changed.
...
places where Py_ssize_t is output through a pointer (rather than a
...
One might think that using Py_ssize_t in all tuples, strings, lists,
...
- Marc-Andre Lemburg commented that complete backwards compatibility
  ...
  functions that have Py_ssize_t\* output arguments should continue to
  run correctly even if the callers pass int\*.
