...
source_path: https://github.com/python/peps/blob/main/peps/pep-0440.rst
source_commit: 256bf232dcc7ee648b76ea3cfadf77fbfb012333
---

::: canonical-pypa-spec
`version-specifiers`{.interpreted-text role="ref"}
:::
...
`345`{.interpreted-text role="pep"} and `386`{.interpreted-text
role="pep"}.
...
\"post-release\" or \"developmental release\" as defined in the
following sections.
...
`python.integrator` extension metadata (as defined in
`459`{.interpreted-text role="pep"}).
...
cycle to support testing by their users prior to a final release.
...
Leading and trailing whitespace must be silently ignored and removed
...
Metadata v1.0 (`241`{.interpreted-text role="pep"}) and metadata v1.1
(`314`{.interpreted-text role="pep"}) do not specify a standard version
identification or ordering scheme. However metadata v1.2
(`345`{.interpreted-text role="pep"}) does specify a scheme which is
defined in `386`{.interpreted-text role="pep"}.
...
supersedes `386`{.interpreted-text role="pep"} even for metadata v1.2.
Tools SHOULD ignore any versions which cannot be parsed by the rules in
this PEP, but MAY fall back to implementation defined version parsing
and ordering schemes if no versions complying with this PEP are
available.
...
[Semantic versioning](https://semver.org/) is a popular version
identification scheme that is more prescriptive than this PEP regarding
the significance of different elements of a release number. Even if a
project chooses not to abide by the details of semantic versioning, the
scheme is worth understanding as it covers many of the issues that can
arise when depending on other distributions, and when publishing a
...
- `<=`, `>=`: [Inclusive ordered
  comparison](#inclusive-ordered-comparison) clause
- `<`, `>`: [Exclusive ordered
  comparison](#exclusive-ordered-comparison) clause
...
consistent ordering defined by the standard [Version
scheme](#version-scheme). Whether or not pre-releases are considered as
candidate versions SHOULD be handled as described in [Handling of
pre-releases](#handling-of-pre-releases).
...
described in [Version scheme](#version-scheme). Local version
identifiers are NOT permitted in this version specifier.
...
The specified version identifier must be in the standard format
described in [Version scheme](#version-scheme), but a trailing `.*` is
permitted on public version identifiers as described below.
...
those of the [Version matching](#version-matching) operator, except that
the sense of any match is inverted.
...
by the standard [Version scheme](#version-scheme).
...
However, they specifically exclude pre-releases, post-releases, and
local versions of the specified version.
...
- excluding pre-releases for all version specifiers (reporting an error
  or warning if a pre-release is already installed locally, or if a
...
- Note: this comparison is to `pkg_resourses.parse_version` as it
  existed at the time the PEP was written. After the PEP was accepted,
  setuptools 6.0 and later versions adopted the behaviour described in
  this PEP.
...
`386`{.interpreted-text role="pep"} is to sort top level developmental
releases like `X.Y.devN` ahead of alpha releases like `X.Ya1`. This is a
far more logical sort order, as projects already using both development
releases and alphas/betas/release candidates do not want their
developmental releases sorted in between their release candidates and
their final releases. There is no rationale for using `dev` releases in
that position rather than merely creating additional release candidates.
...
As in `386`{.interpreted-text role="pep"}, the primary focus is on
codifying existing practices to make them more amenable to automation,
rather than demanding that existing projects make non-trivial changes to
their workflow. However, the standard scheme allows significantly more
flexibility than is needed for the vast majority of simple Python
packages (which often don\'t even need maintenance releases - many users
are happy with needing to upgrade to a new feature release to get bug
fixes).
...
from the Ruby community\'s \"pessimistic version constraint\"
operator[^2] to allow projects to take a cautious approach to forward
...
upstream bug fixes to older versions. It\'s one of the services that
gets Linux distro vendors paid, and application developers may also
apply patches they need to bundled dependencies.
...
All possible normalization rules were weighed against whether or not
...
weighed against the kinds of additional versions it allowed, how
\"ugly\" those versions looked, how hard there were to parse (both
mentally and mechanically) and how much additional compatibility it
would bring.
...
replace style transforms increase the likelihood of ambiguous or
\"junk\" versions.

For an extended discussion on the various types of normalizations that
were considered, please see the proof of concept for
`440`{.interpreted-text role="pep"} within pip[^3].
...
- The exclusive ordered comparisons were updated to no longer imply a
  ...
  simply disallow matching pre-releases, post-releases, and local
  versions of the specified version (unless the specified version is
  itself a pre-release, post-release or local version). For an extended
  discussion see the threads on distutils-sig[^4][^5].
- The normalized form for release candidates was updated from \'c\' to
  \'rc\'. This change was based on user feedback received when
  setuptools 8.0 started applying normalisation to the release metadata
  generated when preparing packages for publication on PyPI[^6].
- The PEP text and the `is_canonical` regex were updated to be explicit
  ...
  sequences of ASCII digits, not arbitrary Unicode \[Nd\] code points.
  This was previously implied by the version parsing regex in Appendix
  B, but not stated explicitly[^7].
...
- [Reference Implementation of PEP 440 Versions and
  Specifiers](https://github.com/pypa/packaging/pull/1)
- [Pessimistic version
  constraint](https://web.archive.org/web/20130509214125/http://docs.rubygems.org/read/chapter/16)
- [Changing the status of PEP 440 to
  Provisional](https://mail.python.org/pipermail/distutils-sig/2014-December/025412.html)
...
is already in that form, and if it\'s not, extract the various
...
[packaging](https://github.com/pypa/packaging) project):
...
[^1]: [File URIs in
    Windows](https://web.archive.org/web/20130321051043/http://blogs.msdn.com/b/ie/archive/2006/12/06/file-uris-in-windows.aspx)

[^2]: [Version compatibility analysis
    script](https://github.com/pypa/packaging/blob/master/tasks/check.py)

[^3]: [Proof of Concept: PEP 440 within
    pip](https://github.com/pypa/pip/pull/1894)

[^4]: [PEP440: foo-X.Y.Z does not satisfy
    \"foo\>X.Y\"](https://mail.python.org/pipermail/distutils-sig/2014-December/025451.html)

[^5]: [PEP440: \>1.7 vs
    \>=1.7](https://mail.python.org/pipermail/distutils-sig/2014-December/025507.html)

[^6]: [Amend PEP 440 with Wider Feedback on Release
    Candidates](https://mail.python.org/pipermail/distutils-sig/2014-December/025409.html)

[^7]: [PEP 440: regex should not permit Unicode \[Nd\]
    characters](https://github.com/python/peps/pull/966)
