...
source_path: https://github.com/python/peps/blob/main/peps/pep-0231.rst
source_commit: 5e69b65920864dcaffd01dc278f02be8d10c9d42
---
...
`213`{.interpreted-text role="pep"} describes a different approach to
hooking into attribute access and modification. The semantics proposed
in `213`{.interpreted-text role="pep"} can be implemented using the
`__findattr__()` hook described here, with one caveat. The current
reference implementation of `__findattr__()` does not support hooking on
attribute deletion. This could be added if it\'s found desirable. See
example below.
