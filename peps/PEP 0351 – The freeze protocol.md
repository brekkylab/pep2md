...
source_path: https://github.com/python/peps/blob/main/peps/pep-0351.rst
source_commit: 038e0a1a00ca2ed46374a0f20d5d2e6fe5ebbeec
---
...
This PEP was rejected. For a rationale, see [this thread on
python-dev](https://mail.python.org/pipermail/python-dev/2006-February/060793.html).
...
immutable\" so that you can create `sets.Set`\'s of `sets.Set`\'s.
`218`{.interpreted-text role="pep"} deliberately dropped this feature
from built-in sets. This PEP advances that the feature is still useful
and proposes a standard mechanism for its support.

# Proposal

It is proposed that a new built-in function called `freeze()` is added.

If `freeze()` is passed an immutable object, as determined by `hash()`
on that object not raising a `TypeError`, then the object is returned
directly.
...
Patch
[1335812](http://sourceforge.net/tracker/index.php?func=detail&aid=1335812&group_id=5470&atid=305470)
provides the C implementation of this feature. It adds the `freeze()`
built-in, along with implementations of the `__freeze__()` method for
lists and sets. Dictionaries are not easily freezable in current Python,
so an implementation of `dict.__freeze__()` is not provided yet.
