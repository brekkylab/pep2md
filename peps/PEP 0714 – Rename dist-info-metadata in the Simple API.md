...
created: 06-Jun-2023
post_history:
- '`06-Jun-2023 <https://discuss.python.org/t/27471>`__'
...
This PEP renames the metadata provided by `658`{.interpreted-text
role="pep"} in both HTML and JSON formats of the Simple API and provides
guidelines for both clients and servers in how to handle the renaming.

# Motivation

`658`{.interpreted-text role="pep"} specified a mechanism to host the
core metadata files from an artifact available through the Simple API
such that a client could fetch the metadata and use it without having to
download the entire artifact. Later `691`{.interpreted-text role="pep"}
was written to add the ability to use JSON rather than HTML on the
Simple API, which included support for the `658`{.interpreted-text
role="pep"} metadata.

Unfortunately, PyPI did not support `658`{.interpreted-text role="pep"}
until just [recently](https://github.com/pypi/warehouse/pull/13649),
which released with a
[bug](https://github.com/pypi/warehouse/issues/13705) where the
`dist-info-metadata` key from `658`{.interpreted-text role="pep"} was
incorrectly named in the JSON representation, to be
`data-dist-info-metadata`. However, when attempting to fix that bug, it
was discovered that pip *also* had a
[bug](https://github.com/pypa/pip/issues/12042), where any use of
`dist-info-metadata` in the JSON representation would cause pip to hard
fail with an exception.

The bug in pip has existed since at least `v22.3`, which means that it
has been released for approximately 8 months, long enough to have been
...
2.  Do the same as (1), but special case PyPI so it does not emit the
    `658`{.interpreted-text role="pep"} metadata for pip, even if it is
    available. This allows people to upgrade pip if they\'re on a broken
...
isn\'t the case here: the spec is fine and these are just genuine bugs
in pip and PyPI.
...
The `658`{.interpreted-text role="pep"} metadata, when used in the HTML
representation of the Simple API, **MUST** be emitted using the
attribute name `data-core-metadata`, with the supported values remaining
the same.

The `658`{.interpreted-text role="pep"} metadata, when used in the
`691`{.interpreted-text role="pep"} JSON representation of the Simple
API, **MUST** be emitted using the key `core-metadata`, with the
supported values remaining the same.

To support clients that used the previous key names, the HTML
representation **MAY** also be emitted using the
`data-dist-info-metadata`, and if it does so it **MUST** match the value
of `data-core-metadata`.

## Clients

Clients consuming any of the HTML representations of the Simple API
**MUST** read the `658`{.interpreted-text role="pep"} metadata from the
key `data-core-metadata` if it is present. They **MAY** optionally use
the legacy `data-dist-info-metadata` if it is present but
`data-core-metadata` is not.

Clients consuming the JSON representation of the Simple API **MUST**
read the `658`{.interpreted-text role="pep"} metadata from the key
`core-metadata` if it is present. They **MAY** optionally use the legacy
`dist-info-metadata` key if it is present but `core-metadata` is not.
...
degrade gracefully, and simply act as if the `658`{.interpreted-text
role="pep"} metadata does not exist.
...
We believe that the improvements brought by `658`{.interpreted-text
role="pep"} are very important to improving the performance of resolving
dependencies from PyPI, and would like to be able to deploy it as
quickly as we can.
...
the `658`{.interpreted-text role="pep"} metadata and the above still
works.
...
with PyPI, it is really a bug that would happen with any
`691`{.interpreted-text role="pep"} repository that correctly exposed
the `658`{.interpreted-text role="pep"} metadata. This would mean that
every repository would have to carry this special case for pip.
...
based on that version number so that we don\'t serve the
`658`{.interpreted-text role="pep"} metadata to versions of pip that are
broken.
...
2.  PyPI *could* support the `User-Agent` detection idea by mutating the
    ...
    cached for those requests and we wouldn\'t emit a `Vary` on
    `User-Agent` for them to know that it isn\'t acceptable to share
    those caches, and adding a `Vary: User-Agent` for downstream caches
    would have the same problem as (1), but for downstream caches
    instead of our CDN cache.
3.  The pip bug ultimately isn\'t PyPI specific, it affects any
    repository that implements `691`{.interpreted-text role="pep"} and
    `658`{.interpreted-text role="pep"} together. This would mean that
    workarounds that rely on implementation specific fixes have to be
    replicated for each repository that implements both, which may not
    be easy or possible in all cases (static mirrors may not be able to
    do this `User-Agent` detection for instance).
...
lose `658`{.interpreted-text role="pep"} support in any HTML only
clients or repositories that might already support it. This PEP
mitigates that breakage by allowing both clients and servers to continue
to support both keys, with a recommendation of when and how to do that.
...
We recommend that servers *only* emit the newer keys, particularly for
the JSON representation of the Simple API since the bug itself only
affected JSON.

Servers that wish to support `658`{.interpreted-text role="pep"} in
clients that use HTML and have it implemented, can safely emit both keys
*only* in HTML.
...
to support repositories that already have correctly implemented
`658`{.interpreted-text role="pep"} and `691`{.interpreted-text
role="pep"} but have not implemented this PEP.
