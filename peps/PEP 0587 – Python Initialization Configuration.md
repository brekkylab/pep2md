...
bdfl_delegate: Thomas Wouters <thomas@python.org>
...
source_path: https://github.com/python/peps/blob/main/peps/pep-0587.rst
source_commit: cb072ff46ac8078fe9488e001b8827ebbd99da42
---
...
This extracts a subset of the API design from the
`432`{.interpreted-text role="pep"} development and refactoring work
that is now considered sufficiently stable to make public (allowing 3rd
party embedding applications access to the same configuration APIs that
the native CPython CLI is now using).
...
Implementing the `540`{.interpreted-text role="pep"} (UTF-8 Mode) and
the new `-X dev` correctly was almost impossible in Python 3.6. The code
base has been deeply reworked in Python 3.7 and then in Python 3.8 to
read the configuration into a structure with no side effect. It becomes
possible to clear the configuration (release memory) and read again the
configuration if the encoding changed . It is required to implement
properly the UTF-8 which changes the encoding using `-X utf8` command
line option. Internally, bytes `argv` strings are decoded from the
filesystem encoding. The `-X dev` changes the memory allocator (behaves
as `PYTHONMALLOC=debug`), whereas it was not possible to change the
...
Rules](#priority-and-rules).

This PEP is a partial implementation of `432`{.interpreted-text
role="pep"} which is the overall design. New fields can be added later
to `PyConfig` structure to finish the implementation of the
`432`{.interpreted-text role="pep"} (e.g. by adding a new partial
...
- `Py_ExitStatusException(status)`: Call `exit(exitcode)` if *status* is
  an exit. Print the error messageand exit with a non-zero exit code if
...
- `void PyConfig_InitIsolatedConfig(PyConfig *config)`: Initialize
  configuration with [Isolated Configuration](#isolated-configuration).
...
- `PyStatus PyConfig_SetArgv(PyConfig *config, int argc, wchar_t * const *argv)`:
  Set command line arguments from wide character strings. Preinitialize
  Python if needed.
...
- `bytes_warning` (`int`): If equals to 1, issue a warning when
  comparing `bytes` or `bytearray` with `str`, or comparing `bytes` with
  `int`. If equal or greater to 2, raise a `BytesWarning` exception.
- `check_hash_pycs_mode` (`wchar_t*`): `--check-hash-based-pycs` command
  line option value (see `552`{.interpreted-text role="pep"}). Valid
  values: `always`, `never` and `default`. The default value is
  `default`.
- `configure_c_stdio` (`int`): If non-zero, configure C standard streams
  (`stdio`, `stdout`, `stdout`). For example, set their mode to
  `O_BINARY` on Windows.
...
- `parse_argv` (`int`): If non-zero, parse `argv` the same way the
  ...
  `argv`: see [Command Line Arguments](#command-line-arguments).
...
- `_init_main` (`int`): If equal to 0, stop Python initialization before
  the \"main\" phase (see `432`{.interpreted-text role="pep"}).
...
This function enables C locale coercion (`538`{.interpreted-text
role="pep"}) and UTF-8 Mode (PEP 540) depending on the LC_CTYPE locale,
`PYTHONUTF8` and `PYTHONCOERCECLOCALE` environment variables.
...
Executable\" discussed in `432`{.interpreted-text role="pep"}.
...
initialization, the core feature of the `432`{.interpreted-text
role="pep"}:
...
coercion (`538`{.interpreted-text role="pep"}) and the UTF-8 Mode
(`540`{.interpreted-text role="pep"}) by default to prevent mojibake.
The new API using the [Python Configuration](#python-configuration) is
needed to enable them automatically.
...
- If `isolated` is non-zero, `use_environment` and `user_site_directory`
  are set to 0.
...
- [bpo-1195571](https://bugs.python.org/issue1195571): \"simple callback
  system for Py_FatalError\"
- [bpo-11320](https://bugs.python.org/issue11320): \"Usage of API method
...
- [bpo-29778](https://bugs.python.org/issue29778): \"\_Py_CheckPython3
...
- [bpo-32573](https://bugs.python.org/issue32573): \"All sys attributes
...
- [bpo-36204](https://bugs.python.org/issue36204): \"Deprecate calling
  Py_Main() after Py_Initialize()? Add Py_InitializeFromArgv()?\"

Issues of the PEP implementation:

- [bpo-16961](https://bugs.python.org/issue16961): \"No regression tests
...
- [bpo-32030](https://bugs.python.org/issue32030): \"PEP 432: Rewrite
  Py_Main()\"
- [bpo-32124](https://bugs.python.org/issue32124): \"Document functions
...
- [bpo-34170](https://bugs.python.org/issue34170): \"Py_Initialize():
  computing path configuration must not have side effect (PEP 432)\"
- [bpo-34589](https://bugs.python.org/issue34589): \"Py_Initialize() and
  Py_Main() should not enable C locale coercion\"
- [bpo-34639](https://bugs.python.org/issue34639): \"PYTHONCOERCECLOCALE
  is ignored when using -E or -I option\"
- [bpo-36142](https://bugs.python.org/issue36142): \"Add a new
  [PyPreConfig]{#pypreconfig} step to Python initialization to setup
  memory allocator and encodings\"
- [bpo-36202](https://bugs.python.org/issue36202): \"Calling
  Py_DecodeLocale() before [PyPreConfig_Write]{#pypreconfig_write}() can
  produce mojibake\"
- [bpo-36301](https://bugs.python.org/issue36301): \"Add
  [Py_PreInitialize]{#py_preinitialize}() function\"
- [bpo-36443](https://bugs.python.org/issue36443): \"Disable
  coerce_c_locale and utf8_mode by default in
  [PyPreConfig]{#pypreconfig}?\"
- [bpo-36444](https://bugs.python.org/issue36444): \"Python
  initialization: remove
  [PyMainInterpreterConfig]{#pymaininterpreterconfig}\"
- [bpo-36471](https://bugs.python.org/issue36471): \"PEP 432, PEP 587:
  Add [Py_RunMain]{#py_runmain}()\"
...
- [bpo-36900](https://bugs.python.org/issue36900): \"Use
  [PyCoreConfig]{#pycoreconfig} rather than global configuration
  variables\"
...
- [bpo-15577](https://bugs.python.org/issue15577): \"Real argc and argv
...
- [bpo-25631](https://bugs.python.org/issue25631): \"Segmentation fault
...
- [bpo-33919](https://bugs.python.org/issue33919): \"Expose
  [PyCoreConfig]{#pycoreconfig} structure to Python\"
...
- July-August 2018:
  - July: [\[Python-Dev\] New \_Py_InitializeFromConfig() function (PEP
    432)](https://mail.python.org/pipermail/python-dev/2018-July/154882.html)
  - August: [\[Python-Dev\] New \_Py_InitializeFromConfig() function
    (PEP
    432)](https://mail.python.org/pipermail/python-dev/2018-August/154896.html)
...
- Version 4:
  - Introduce \"Python Configuration\" and \"Isolated Configuration\"
    which are well better defined. Replace all macros with functions.
...
`587`{.interpreted-text role="pep"} was [accepted by Thomas Wouters on
May 26,
