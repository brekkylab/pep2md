...
source_path: https://github.com/python/peps/blob/main/peps/pep-0283.rst
source_commit: c5bb1f0810923af71612c39899c7970051275b2e
---
...
- The `bool` type and its constants, `True` and `False`
  (`285`{.interpreted-text role="pep"}).

- `PyMalloc` was greatly enhanced and is enabled by default.

- Universal newline support (`278`{.interpreted-text role="pep"}).

- `263`{.interpreted-text role="pep"} Defining Python Source Code
  Encodings, Lemburg
...
- Stage B0 of the `int`/`long` integration (`237`{.interpreted-text
  role="pep"}). This means issuing a `FutureWarning` about situations
  where `hex` or `oct` conversions or left shifts returns a different
  value for an `int` than for a `long` with the same value. The
  semantics do *not* change in Python 2.3; that will happen in Python
  2.4.
...
- `218`{.interpreted-text role="pep"} Adding a Built-In Set Object Type,
  Wilson
  ...
  as the module `sets`, although some details may still change until the
  first beta release. (There are no plans to make this a built-in type,
  for now.)

- `293`{.interpreted-text role="pep"} Codec error handling callbacks,
  Dörwald

  Fully implemented. Error handling in `unicode.encode` or `str.decode`
  can now be customized.

- `282`{.interpreted-text role="pep"} A Logging System, Mick
...
- A modified MRO (Method Resolution Order) algorithm. Consensus is that
  we should adopt C3. Samuele Pedroni has contributed a draft
...
- `273`{.interpreted-text role="pep"} Import Modules from Zip Archives,
  Ahlstrom

  Implemented as a part of the `302`{.interpreted-text role="pep"}
  implementation work.

- `302`{.interpreted-text role="pep"} New Import Hooks, JvR

  Implemented (though the 2.3a1 release contained some bugs that have
  been fixed post-release).

- A new pickling protocol. See `307`{.interpreted-text role="pep"}.

- `305`{.interpreted-text role="pep"} (CSV File API, by Skip Montanaro
  et al.) is in; this is the csv module.

- Raymond Hettinger\'s `itertools` module is in.

- `311`{.interpreted-text role="pep"} (Simplified GIL Acquisition for
  Extensions, by Mark Hammond) has been included in beta 1.
...
- Two new `PyArg_Parse*()` format codes, \'k\' returns an unsigned C
  ...
  truncating without range checking. (SF 595026; Thomas Heller did this
  work.)

- A new version of IDLE was imported from the IDLEfork project
  (<http://idlefork.sf.net>). The code now lives in the `idlelib`
  package in the standard library and the `idle` script is installed by
  `setup.py`.
...
- `304`{.interpreted-text role="pep"} (Controlling Generation of
  Bytecode Files by Montanaro) seems to have lost steam.
...
- `269`{.interpreted-text role="pep"} Pgen Module for Python, Riehl

  (Some necessary changes are in; the `pgen` module itself needs to
  mature more.)

- Add support for the long-awaited Python catalog. Kapil Thangavelu has
  a Zope-based implementation that he demoed at OSCON 2002. Now all we
  need is a place to host it and a person to champion it. (Some changes
  to distutils to support this are in, at least.)

- `266`{.interpreted-text role="pep"} Optimizing Global
  Variable/Attribute Access, Montanaro

  `267`{.interpreted-text role="pep"} Optimized Access to Module
  Namespaces, Hylton

  `280`{.interpreted-text role="pep"} Optimizing access to globals, van
  Rossum
  ...
  breath. In the meantime, Oren Tirosh has a much simpler idea that may
  give a serious boost to the performance of accessing globals and
...
- `286`{.interpreted-text role="pep"} Enhanced Argument Tuples, von
  Loewis
