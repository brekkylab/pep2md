...
source_path: https://github.com/python/peps/blob/main/peps/pep-0477.rst
source_commit: b2e786da2e2b6f5a2595ec40898d03a44618915d
---
...
1.  The Python 2.x to 3.x migration has a number of painpoints that are
    eased by a number of third party modules such as six[^1],
    modernize[^2], or future[^3]. However relying on these tools
    requires that everyone who uses the project have a tool to install
    these packages.
...
3.  Users also will need a number of tools in order to create python
    ...
    proposed. Things like setuptools[^4], Wheel[^5], and twine[^6] are
    enabling a safer, faster, and more reliable packaging tool chain.
    These tools can be difficult for people to use if first they must be
    told how to go out and install the package manager.
...
benefits of `453`{.interpreted-text role="pep"} and both PyPy and Jython
have plans to backport ensurepip to their 2.7 runtimes.

# Automatic Invocation

`453`{.interpreted-text role="pep"} has `ensurepip` automatically
invoked by default in the `Makefile` and the Windows and OSX Installers.
This allowed it to ensure that, by default, all users would get Python
with pip already installed. This PEP however believes that while this is
fine for the Python 2.7 Windows and Mac OS X installers it is *not* ok
for the Python 2.7 `Makefile` in general.
...
[^1]: [six.py](https://pypi.python.org/pypi/six)

[^2]: [modernize](https://pypi.python.org/pypi/modernize)

[^3]: [python-future](https://pypi.python.org/pypi/future)

[^4]: [setuptools](https://pypi.python.org/pypi/setuptools)

[^5]: [Wheel](https://pypi.python.org/pypi/wheel)

[^6]: [twine](https://pypi.python.org/pypi/twine)
