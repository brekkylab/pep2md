...
source_path: https://github.com/python/peps/blob/main/peps/pep-0564.rst
source_commit: f2f55195898613dad634ff96abb1f41b44505dea
---
...
Five years ago, the `410`{.interpreted-text role="pep"} proposed a large
and complex change in all Python functions returning time to support
nanosecond resolution using the `decimal.Decimal` type.
...
Since the `410`{.interpreted-text role="pep"} was rejected:

- The `os.stat_result` structure got 3 new fields for timestamps as
  nanoseconds (Python `int`): `st_atime_ns`, `st_ctime_ns` and
  `st_mtime_ns`.
- The `418`{.interpreted-text role="pep"} was accepted, Python 3.3 got 3
  new clocks: `time.monotonic()`, `time.perf_counter()` and
  `time.process_time()`.
...
The `410`{.interpreted-text role="pep"} proposed to return
`decimal.Decimal` which already exists and supports arbitrary precision,
but it was rejected. Apart from `decimal.Decimal`, no portable real
number type with better precision is currently available in Python.
...
See also the `410`{.interpreted-text role="pep"} for a previous long
discussion on other types.
...
Internally, Python starts `monotonic()` and `perf_counter()` clocks at
