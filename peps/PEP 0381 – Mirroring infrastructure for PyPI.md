...
source_path: https://github.com/python/peps/blob/main/peps/pep-0381.rst
source_commit: 73401851308d355c2cf46aac51b973258ddc945a
---
...
format as generated by \"openssl dsa -pubout\" (i.e.
`3280`{.interpreted-text role="rfc"} SubjectPublicKeyInfo, with the
algorithm 1.3.14.3.2.12). This URL must *not* be mirrored, and clients
must fetch the official serverkey from PyPI directly, or use the copy
that came with the PyPI client software. Mirrors should still download
the key, to detect a key rollover.
...
`3279`{.interpreted-text role="rfc"} Dsa-Sig-Value, created by algorithm
1.2.840.10040.4.3)
...
It needs to obey `305`{.interpreted-text role="pep"}. Basically, it
should be readable by Python\'s `csv` module.
