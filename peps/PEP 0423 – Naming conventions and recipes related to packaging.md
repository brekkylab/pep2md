...
source_path: https://github.com/python/peps/blob/main/peps/pep-0423.rst
source_commit: fe016c246681e51d8bd833285e3fb60c8e3cdd31
---
...
- new projects should follow the [guidelines](#overview) below.
- existing projects should be aware of these guidelines and can follow
  [specific recipes for existing
  projects](#how-to-apply-naming-guidelines-on-existing-projects).

# PEP Deferral

Further consideration of this PEP has been deferred at least until after
`426`{.interpreted-text role="pep"} (package metadata 2.0) and related
updates have been resolved.

# Terminology

Reference is [packaging terminology in Python
documentation](https://packaging.python.org/glossary/).

# Relationship with other PEPs

- `8#package-and-module-names`{.interpreted-text role="pep"} deals with
  code style guide, including names of Python packages and modules. It
  covers syntax of package/modules names.
- `345`{.interpreted-text role="pep"} deals with packaging metadata, and
  defines name argument of the `packaging.core.setup()` function.
- `420`{.interpreted-text role="pep"} deals with namespace packages. It
  brings support of namespace packages to Python core. Before,
  namespaces packages were implemented by external libraries.
- `3108`{.interpreted-text role="pep"} deals with transition between
  Python 2.x and Python 3.x applied to standard library: some modules to
  be deleted, some to be renamed. It points out that naming conventions
  matter and is an example of transition plan.
...
- [understand and respect namespace ownership](#respect-ownership).
- if your project is related to another project or community:
  - search for conventions in main project\'s documentation, because
    [projects should organize community
    contributions](#organize-community-contributions).
  - [follow specific project or related community
    conventions](#follow-community-or-related-project-conventions-if-any),
    if any.
  - if there is no convention, [follow a standard naming
    pattern](#use-standard-pattern-for-community-contributions).
- make sure your project name is unique, i.e. avoid duplicates:
  - [use top-level namespace for
    ownership](#top-level-namespace-relates-to-code-ownership),
  - [check for name availability](#how-to-check-for-name-availability),
  - [register names with PyPI](#register-names-with-pypi).
- make sure distributed packages and modules names are unique, unless
  you explicitly want to distribute alternatives to existing packages or
  modules. [Using the same value for package/module name and project
  name](#use-a-single-name) is the recommended way to achieve this.
- [distribute only one package or module at a
  time](#multiple-packages-modules-should-be-rare), unless you know what
...
- make it easy to discover and remember your project:
  - [use as much memorable names as possible](#pick-memorable-names),
  - [use as much meaningful names as possible](#pick-meaningful-names),
  - [use other packaging metadata](#use-packaging-metadata).
- [avoid deep nesting](#avoid-deep-nesting). Flat things are easier to
  use and remember than nested ones:
...
- [follow PEP 8](#follow-pep-8-for-syntax-of-package-and-module-names)
...
- if you followed specific conventions, or if your project is intended
  ...
  contributions](#organize-community-contributions).
- [if still in doubt, ask](#if-in-doubt-ask).

# If in doubt, ask

If you feel unsure after reading this document, ask [Python
community](http://www.python.org/community/) on IRC or on a mailing
list.
...
- an individual. Example:
  [gp.fileupload](http://pypi.python.org/pypi/gp.fileupload/) is owned
  and maintained by Gael Pasgrimaud.
- an organization. Examples:
  - [zest.releaser](http://pypi.python.org/pypi/zest.releaser/) is owned
    and maintained by Zest Software.
  - [Django](http://djangoproject.com/) is owned and maintained by the
    Django Software Foundation.
- a group or community. Example: [sphinx](http://sphinx.pocoo.org) is
  maintained by developers of the Sphinx project, not only by its
  author, Georg Brandl.
- a group or community related to another package. Example:
  ...
  is owned by its author: David Glick, Groundwire. But the
  \"collective\" namespace is owned by Plone community.
...
contributions](#organize-community-contributions) below.

Also, this rule applies to non-Python projects.

As an example, don\'t use \"apache\" as top-level namespace: \"Apache\"
is the name of an existing project (in the case of \"Apache\", it is
also a trademark).

## Private (including closed-source) projects use a namespace

\... because private projects are owned by somebody. So apply the
[ownership rule](#top-level-namespace-relates-to-code-ownership).
...
\... because they are owned by individuals. So apply the [ownership
rule](#top-level-namespace-relates-to-code-ownership).
...
mind [it is easy to rename the project](#how-to-rename-a-project).
...
As an example, [sphinx](http://sphinx.pocoo.org) project belongs to the
Sphinx development team. There is no need to have some \"sphinx\"
namespace package with only one \"sphinx.sphinx\" project inside.
...
- it doesn\'t block future changes. When a project becomes mature and
  ...
  to rename the project](#how-to-rename-a-project).
...
- It also limits implicit clashes between package/module names. By using
  a single name, when you register a project name to
  [PyPI](http://pypi.python.org), you also perform a basic
  package/module name availability verification.

  As an example, [pipeline](http://pypi.python.org/pypi/pipeline/),
  [python-pipeline](http://pypi.python.org/pypi/python-pipeline/) and
  [django-pipeline](http://pypi.python.org/pypi/django-pipeline/) all
...
For historical reasons, [PyPI](http://pypi.python.org) contains many
distributions where project and distributed package/module names differ.
::::

## Multiple packages/modules should be rare

Technically, Python distributions can provide multiple packages and/or
modules. See [setup script
reference](http://docs.python.org/dev/packaging/setupscript.html) for
details.

Some distributions actually do. As an example,
[setuptools](http://pypi.python.org/pypi/setuptools) and
[distribute](http://packages.python.org/distribute/) are both declaring
...
A notable exception to the [Use a single name](#use-a-single-name) rule
is when you explicitly need distinct names.

As an example, the [Pillow](http://pypi.python.org/pypi/Pillow/) project
provides an alternative to the original
[PIL](http://pypi.python.org/pypi/PIL/) distribution. Both projects
distribute a \"PIL\" package.
...
`PEP 8 <8#package-and-module-names>`{.interpreted-text role="pep"}
applies to names of Python packages and modules.

If you [Use a single name](#use-a-single-name),
`PEP 8 <8#package-and-module-names>`{.interpreted-text role="pep"} also
...
As an example, [celery](http://pypi.python.org/pypi/celery/) is not a
meaningful name. At first, it is not obvious that it deals with message
queuing. But it is memorable, partly because it can be used to feed a
...
As an example, [DateUtils](http://pypi.python.org/pypi/DateUtils/) is a
meaningful name. It is obvious that it deals with utilities for dates.
...
As an example, there is a \"\`Framework :: Twisted\`\_\" classifier.
Even if names are quite heterogeneous (they don\'t follow a particular
pattern), we get the list.

In order to [Organize community
contributions](#organize-community-contributions), conventions about
...
`The Zen of Python <20>`{.interpreted-text role="pep"} says \"Flat is
better than nested\".
...
[Use packaging metadata](#use-packaging-metadata) instead.
...
documentation](#organize-community-contributions).
...
As an example, [Plone community](http://plone.org/community/develop)
releases community contributions in the \"collective\" namespace
package. It differs from the [standard namespace for
contributions](#use-standard-pattern-for-community-contributions)
...
It is the simplest way to [Organize community
contributions](#organize-community-contributions).
...
- `${MAINPROJECT}.contrib.*` is inside \${MAINPROJECT} namespace, i.e.
  it is owned by \${MAINPROJECT} authors. It breaks the [Top-level
  namespace relates to code
  ownership](#top-level-namespace-relates-to-code-ownership) rule.
- `${MAINPROJECT}.contrib.*` breaks the [Avoid deep
  nesting](#avoid-deep-nesting) rule.
...
This is the counterpart of the [follow community
conventions](#follow-community-or-related-project-conventions-if-any)
and [standard pattern for
contributions](#use-standard-pattern-for-community-contributions) rules.

Actions:

- Choose a naming convention for community contributions.
- If it is not [the
  default](#use-standard-pattern-for-community-contributions), then
  document it.
  - if you use the [default
    convention](#use-standard-pattern-for-community-contributions), then
    this document should be enough. Don\'t repeat it. You may reference
    it.
...
- Also recommend the use of additional metadata, such as [classifiers
  and keywords](#use-packaging-metadata).

About convention choices:

- New projects should choose the [default contrib
  pattern](#use-standard-pattern-for-community-contributions).

- Existing projects with community contributions should start with
  custom conventions. Then they can [Promote
  migrations](#promote-migrations).
...
[PyPI](http://pypi.python.org) is the central place for distributions in
Python community. So, it is also the place where to register project and
package names.

See [Registering with the Package
Index](https://docs.python.org/3/distutils/packageindex.html) for
details.
...
That\'s why it is important you [register names with
PyPI](#register-names-with-pypi).
...
- inside projects at `PyPI`. There is currently no helper for that.
  Notice that the more projects follow the [use a single
  name](#use-a-single-name) rule, the easier is the verification.
- you may [ask the community](#if-in-doubt-ask).

The [use a single name](#use-a-single-name) rule also helps you avoid
clashes with package names: if a project name is available, then the
package name has good chances to be available too.
...
3.  Assign `Obsoletes-Dist` metadata to new distribution in setup.cfg
    file. See
    `PEP 345 about Obsolete-Dist <345#obsoletes-dist-multiple-use>`{.interpreted-text
    role="pep"} and [setup.cfg
    specification](http://docs.python.org/dev/packaging/setupcfg.html).
...
If many projects follow [Renaming howto](#how-to-rename-a-project)
...
- at least, [state about current naming](#state-about-current-naming).
- then [plan and promote migration](#promote-migrations).
- optionally actually [rename existing project or distributed
  packages/modules](#how-to-rename-a-project).
...
3.x, new standard library\'s packaging or namespace
...
- many projects are not Python 3.x compatible. It includes \"big\"
  products or frameworks. It means that many projects will have to do a
  migration to support Python 3.x.
...
- `420`{.interpreted-text role="pep"} brings official support of
  namespace packages to Python.
...
- [Martin Aspeli\'s article about
  names](http://www.martinaspeli.net/articles/the-naming-of-things-package-names-and-namespaces).
  Some parts of this document are quotes from this article.
- [in development official packaging
  documentation](http://docs.python.org/dev/packaging/).
- [The Hitchhiker\'s Guide to
  Packaging](https://the-hitchhikers-guide-to-packaging.readthedocs.io/en/latest/),
  which has an empty placeholder for \"naming specification\".
