...
source_path: https://github.com/python/peps/blob/main/peps/pep-0459.rst
source_commit: 5b4d1def6ba6390109cbb134de14a455bfbc02fa
---

# PEP Withdrawal

This PEP depends on `426`{.interpreted-text role="pep"}, which has
itself been withdrawn. See the PEP Withdrawal section in that PEP for
details.
...
applicable license Trove classifiers in the [Classifiers](#classifiers)
field. Even when an appropriate Trove classifier is available, the
license summary can be a good way to specify a particular version of
that license, or to indicate any variations or exception to the license.
...
source archive for the distribution. See [Document
names](#document-names) for details.
...
the distribution. Classifiers are described in `301`{.interpreted-text
role="pep"} \[2\].
...
- `description`: a file containing a long description of the
  distribution
...
format is in `5322`{.interpreted-text role="rfc"} (sections 3.2.3 and
3.4.1) and `5321`{.interpreted-text role="rfc"}, with a more readable
form given in the informational `3696`{.interpreted-text role="rfc"} and
...
For names that contain dots, the portion of the name before the final
...
with a \_\_main\_\_ submodule (if no `name` subfield is given in the
export specifier) or else to a callable inside the named module.
