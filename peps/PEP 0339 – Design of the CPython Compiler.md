...
source_path: https://github.com/python/peps/blob/main/peps/pep-0339.rst
source_commit: efaa150e8898498e11c8a55321d3d0ebf374b776
---
...
laid out in the Dragon Book [\[Aho86\]](#Aho86){.citation}.
...
\'?\' means it is optional, \'\*\' means 0 or more, no modifier means
only one value for the argument and it is required. FunctionDef, for
instance, takes an identifier for the name, \'arguments\' for args, zero
or more stmt arguments for \'body\', and zero or more expr arguments for
\'decorators\'.
...
All three kinds also have an \'attributes\' argument; this is shown by
the fact that \'attributes\' lacks a \'\|\' before it.
...
\'kind\' field specifies which component of the union is initialized.
The FunctionDef() constructor function sets \'kind\' to FunctionDef_kind
and initializes the \'name\', \'args\', \'body\', and \'attributes\'
fields.
...
of the \'if\' and the \'else\' body (which may be NULL), each of which
are their own basic blocks. Both of those blocks in turn point to the
...
of a large \'switch\' statement, branching based on the kind of node
type passed to it. Simple things are handled inline in the \'switch\'
statement with more complex transformations farmed out to other
...
change. But you will also need to change the \'compiler\' package. The
key files to do that are Lib/compiler/pyassem.py and
...
- Parser/
  ...
  - 
    ...
    :   \"An implementation of the Zephyr Abstract Syntax Definition
        Language.\" Uses
        [SPARK](http://pages.cpsc.ucalgary.ca/~aycock/spark/) to parse
        the ASDL files.
  ...
  - 
    ...
    :   [SPARK](http://pages.cpsc.ucalgary.ca/~aycock/spark/) parser
        generator
- Python/
  - 
    ...
    :   Creates C structs corresponding to the ASDL types. Also contains
        code for marshaling AST nodes (core ASDL types have marshaling
        code in asdl.c). \"File automatically generated by
        Parser/asdl_c.py\". This file must be committed separately after
        every grammar change is committed since the \_\_version\_\_
  ...
  - 
    ...
    :   Contains code to handle the ASDL sequence type. Also has code to
        handle marshalling the core ASDL types, such as number and
...
- Include/
  - 
    ...
    :   Contains the actual definitions of the C structs as generated by
        Python/Python-ast.c . \"Automatically generated by
        Parser/asdl_c.py\".
...
created named CALL_ATTR[^3]. Currently only works for classic classes
and for new-style classes rough benchmarking showed an actual slowdown
