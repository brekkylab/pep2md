...
source_path: https://github.com/python/peps/blob/main/peps/pep-0279.rst
source_commit: f5415b46d5c00898a894c58bcafd294b4e879721
---
...
in `234`{.interpreted-text role="pep"}. The `iter()` factory function
was provided as common calling convention and deep changes were made to
use iterators as a unifying theme throughout Python. The unification
came in the form of establishing a common iterable interface for
mappings, sequences, and file objects.

Generators, as proposed in `255`{.interpreted-text role="pep"}, were
introduced as a means for making it easier to create iterators,
especially ones with complex internal execution or variable states. The
availability of generators makes it possible to improve on the loop
counter ideas in `212`{.interpreted-text role="pep"}. Those ideas
provided a clean syntax for iteration with indices and values, but did
not apply to all iterable objects. Also, that approach did not have the
memory friendly benefit provided by generators which do not evaluate the
...
notation. Like `zip()`, it is expected to become a commonly used looping
idiom.
...
Note A: `212`{.interpreted-text role="pep"} Loop Counter Iteration
discussed several proposals for achieving indexing. Some of the
proposals only work for lists unlike the above function which works for
any generator, xrange, sequence, or iterable object. Also, those
proposals were presented and evaluated in the world prior to Python 2.2
which did not include generators. As a result, the non-generator version
in `212`{.interpreted-text role="pep"} had the disadvantage of consuming
memory with a giant list of tuples. The generator version presented here
is fast and light, works with all iterables, and allows users to abandon
the sequence in mid-stream with no loss of computation effort.
...
:   I\'m also quite happy with everything you proposed \... and the
    extra built-ins (really \'indexed\' in particular) are things I have
...
:   I think indexed would be a useful and natural built-in function. I
    would certainly use it a lot. I like `indexed()` a lot; +1. I\'m
    quite happy to have it make PEP 281 obsolete. Adding a separate
