...
source_path: https://github.com/python/peps/blob/main/peps/pep-0250.rst
source_commit: 4b0e13a2d7b0f101bad19ce14546f83beba50b08
---

# Abstract

The standard Python distribution includes a directory
`Lib/site-packages`, which is used on Unix platforms to hold locally
installed modules and packages. The `site.py` module distributed with
Python includes support for locating other modules in the site-packages
directory.
...
modules. The \"expected\" location appears to be the directory
containing the Python executable itself. This is also the location where
distutils (and distutils-generated installers) installs packages.
Including locally developed code in the same directory as installed
...
- Both the current location (`sys.prefix`) and the new directory
  (site-packages) are included in sitedirs, so that `.pth` files will be
  recognised in either location.
- This proposal adds a single additional site-packages directory to
  ...
  version-independent files (Python code) and one for version-dependent
  code (C extensions). This is necessary on Unix, as the sitedirs
  include a common (across Python versions) package location, in
  `/usr/local` by default. As there is no such common location available
  on Windows, there is also no need for having two separate package
  directories.
...
- Comments from Unix users indicate that there may be issues with the
  ...
  Windows platform, leaving changes for other platforms to be covered in
  other PEPs.
