...
source_path: https://github.com/python/peps/blob/main/peps/pep-8012.rst
source_commit: ac90a81f0697873b560785810452e7c2e867b5ac
---

# PEP Rejection

`8012`{.interpreted-text role="pep"} was rejected [by a core developer
...
Should this model be adopted, it will be codified in
...
This model favors a small group, between three and five people. That way
...
`8002`{.interpreted-text role="pep"}). ECMAScript and C++ are similarly
developed.
...
developers should avoid pushing their changes directly to the cpython
...
clarifications over information already present in `1`{.interpreted-text
role="pep"}:
...
- at some point, one of the named experts posts a \"summary comment\"
  that lays out the current state of discussion, especially major points
  of disagreement and tradeoffs; at the same time the expert proposes a
  \"motion for final comment period\" (**FCP**), along with a proposed
...
If any existing core developer does not feel comfortable with the
...
Thank you to the authors of `8002`{.interpreted-text role="pep"} which
was a helpful resource in shaping this document.
