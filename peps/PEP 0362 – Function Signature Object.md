...
source_path: https://github.com/python/peps/blob/main/peps/pep-0362.rst
source_commit: b26900d0d8cfd7c607db17473b8beca1905afb20
---
...
a [Parameter object](#parameter-object) in its `parameters` collection.
...
- 
  ...
  :   Creates a new Signature instance based on the instance `replace`
      ...
      Note that the \'=\<optional\>\' notation, means that the argument
      is optional. This notation applies to the rest of this PEP.
...
- 
  ...
  :   Default Signature constructor. Accepts an optional sequence of
      ...
      parameters with duplicate names, and that the parameters are in
...
- If the object is a bound method, construct and return a new
  ...
  removed. (`classmethod` and `staticmethod` are supported too. Since
  both are descriptors, the former returns a bound method, and the
  latter returns its wrapped function.)
...
`362`{.interpreted-text role="pep"} was accepted by Guido, Friday, June
22, 2012[^3] . The reference implementation was committed to trunk later
that day.
