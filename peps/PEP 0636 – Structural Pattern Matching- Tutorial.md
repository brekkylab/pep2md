...
source_path: https://github.com/python/peps/blob/main/peps/pep-0636.rst
source_commit: 6c28c072e70113f4c14108b52cd3df46426461c6
---

# Abstract

This PEP is a tutorial for the pattern matching introduced by
`634`{.interpreted-text role="pep"}.

`622`{.interpreted-text role="pep"} proposed syntax for pattern
matching, which received detailed discussion both from the community and
the Steering Council. A frequent concern was about how easy it would be
to explain (and learn) this feature. This PEP addresses that concern
providing the kind of document which developers could use to learn about
pattern matching in Python.

This is considered supporting material for `634`{.interpreted-text
role="pep"} (the technical specification for pattern matching) and
`635`{.interpreted-text role="pep"} (the motivation and rationale for
having pattern matching and design considerations).
...
Let\'s say that you would actually like to allow a \"go\" command only
in a restricted set of directions based on the possible exits from the
...
next case as if the pattern hadn\'t matched (with the possible
side-effect of having already bound some variables).
...
following the same order that you\'d use when constructing an object.
For example, if the UI framework above defines their class like this:
...
`634`{.interpreted-text role="pep"} for the whole list), you can use a
...
# Appendix A \-- Quick Intro {#PEP 636 Appendix A}
...
- Sequence patterns support wildcards: `[x, y, *rest]` and
  ...
  The name after `*` may also be `_`, so `(x, y, *_)` matches a sequence
