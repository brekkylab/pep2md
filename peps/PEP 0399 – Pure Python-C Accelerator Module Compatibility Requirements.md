...
source_path: https://github.com/python/peps/blob/main/peps/pep-0399.rst
source_commit: 4b849eb723df2014694dbe26fcc6b53a5b38e13b
---
...
Python has grown beyond the CPython virtual machine (VM).
[IronPython](http://ironpython.net/), [Jython](http://www.jython.org/),
and [PyPy](http://pypy.org/) are all currently viable alternatives to
the CPython VM. The VM ecosystem that has sprung up around the Python
...
Since other VMs do not typically support the entire [C API of
CPython](http://docs.python.org/py3k/c-api/index.html) they are unable
to use the code used to create the module. Oftentimes this leads these
other VMs to either re-implement the modules in pure Python or in the
programming language used to implement the VM itself (e.g., in C# for
...
specific C-based library (e.g.,
[sqlite3](http://docs.python.org/py3k/library/sqlite3.html)). In
granting an exemption it will be recognized that the module will be
considered exclusive to CPython and not part of Python\'s standard
library that other VMs are expected to support. Usage of `ctypes` to
provide an API for a C library will continue to be frowned upon as
`ctypes` lacks compiler guarantees that C code typically relies upon to
prevent certain errors from occurring (e.g., API changes).
