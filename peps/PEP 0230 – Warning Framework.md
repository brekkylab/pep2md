...
source_path: https://github.com/python/peps/blob/main/peps/pep-0230.rst
source_commit: 55e0ff4daffd379a010a704968da7316ddeae9dc
---
...
Therefore, I propose to introduce a flexible \"warning filter\" which
can filter out warnings or change them into exceptions, based on:
...
- To issue a warning from Python:
  ...
  below); it defaults to warnings.UserWarning. This may raise an
  exception if the particular warning issued is changed into an error by
  the warnings filter. The stacklevel can be used by wrapper functions
...
- To issue a warning from C:
  ...
  warning was transformed into an exception, or because of a malfunction
  ...
  which case it defaults to `PyExc_RuntimeWarning`. When `PyErr_Warn()`
  function returns 1, the caller should do normal exception handling.
...
- \'action\' is an abbreviation of one of the allowed actions
  (\"error\", \"default\", \"ignore\", \"always\", \"once\", or
  \"module\")
...
- Paul: What if I have a speed-critical module that triggers warnings in
  an inner loop. It should be possible to disable the overhead for
...
- Fred: why do the Warning category classes have to be in `__builtin__`?
  ...
  call, which imports the warnings module. I see no problem with making
  them available as built-ins.
