...
source_path: https://github.com/python/peps/blob/main/peps/pep-3151.rst
source_commit: b60fba47dea98e41d83ac1b7c0f5a69ca0a880b5
---
...
[Appendix B](#PEP 3151 Appendix B) surveys the use of the various
...
(moreover, the use of certain of these exceptions is irregular;
[Appendix B](#PEP 3151 Appendix B) exposes the case of the
[select](#select) module, which raises different exceptions depending on
the implementation)
...
helped, but at least code which \"works\" won\'t suddenly raise errors
and crash. This is important since such code is likely to be present in
...
# Step 1: coalesce exception types {#Step 1}
...
explained in the [rationale](#rationale) section, but it also allows for
a better and more complete resolution of [Step 2](#Step 2) (see
...
The survey in [Appendix B](#PEP 3151 Appendix B) shows that IOError is
...
than OSError [^1]. However, with no intention to deprecate IOError in
...
A solution is to recognize these names at compilation time, and then
...
# Step 2: define additional subclasses {#Step 2}
...
of existing exception matching practices (see [Appendix
A](#PEP 3151 Appendix A)) helps us propose a reasonable subset of all
values. Trying to map all errno mnemonics, indeed, seems foolish,
pointless, and would pollute the root namespace.
...
[Step 1](#Step 1) is a loose prerequisite for this.
...
and IOError, depending on the context. If we don\'t want to break
*useful compatibility*, we can\'t make an `except OSError` (or IOError)
fail to match an exception where it would succeed today.
...
sub-hierarchy (OSError, assuming [Step 1](#Step 1) is accepted in full)
is not shown:
...
concision (especially with long names such as `ConnectionAbortedError`).
...
Making the exception hierarchy finer-grained makes the root (or
...
has received informal support in the past[^2]; both the introduction of
...
part of `another PEP <348#removing-windowserror>`{.interpreted-text
role="pep"}, but there seemed to be a consensus that the distinction
with OSError wasn\'t meaningful. This supports at least its aliasing
with OSError.

# Implementation

The reference implementation has been integrated into Python 3.3. It was
formerly developed in <http://hg.python.org/features/pep-3151/> in
branch `pep-3151`, and also tracked on the bug tracker at
<http://bugs.python.org/issue12555>. It has been successfully tested on
a variety of systems: Linux, Windows, OpenIndiana and FreeBSD buildbots.
...
module is numerically equal to the [Windows Socket error
code](http://msdn.microsoft.com/en-us/library/ms740668%28v=vs.85%29.aspx)
returned by the `WSAGetLastError` system call:

    >>> errno.EWOULDBLOCK

10035 \>\>\> errno.WSAEWOULDBLOCK 10035
::::
...
`socket.timeout` when it signals a socket timeout (see [issue
10272](http://bugs.python.org/issue10272)).
...
# Appendix A: Survey of common errnos {#PEP 3151 Appendix A}
...
# Appendix B: Survey of raised OS and IO errors {#PEP 3151 Appendix B}
...
by Jean-François Piéronne[^3]; the motivation for including VMSError was
...
[dbm.error]{#dbm.error} and [gdbm.error]{#gdbm.error} inherit from
IOError:
...
[^1]: Google Code Search of `IOError` in Python code: [around 40000
    results](http://www.google.com/codesearch?q=lang%3Apython%20IOError);
    `OSError`: [around 15200
    results](http://www.google.com/codesearch?q=lang%3Apython%20OSError);
    `EnvironmentError`: [around 3000
    results](http://www.google.com/codesearch?q=lang%3Apython%20EnvironmentError)

[^2]: \"IO module precisions and exception hierarchy\":
    <https://mail.python.org/pipermail/python-dev/2009-September/092130.html>

[^3]: <http://bugs.python.org/issue614055>
