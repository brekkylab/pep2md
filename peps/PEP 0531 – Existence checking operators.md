...
source_path: https://github.com/python/peps/blob/main/peps/pep-0531.rst
source_commit: 85c015847df6dc4cd93cd9d4beb395dd333361e3
---

# Abstract

Inspired by `505`{.interpreted-text role="pep"} and the related
discussions, this PEP proposes the addition of two new control flow
operators to Python:
...
general idea is that when checking for \"missing data\" sentinels,
we\'re almost always looking for a *specific* sentinel value, rather
than *any* sentinel value.
...
`if-else` operators more flexible[^3] without introducing any new
syntax, so I\'ll be writing that up as another possible alternative to
`505`{.interpreted-text role="pep"}.

# Relationship with other PEPs

While this PEP was inspired by and builds on Mark Haase\'s excellent
work in putting together `505`{.interpreted-text role="pep"}, it
ultimately competes with that PEP due to significant differences in the
specifics of the proposed syntax and semantics for the feature.
...
the decimal module) and use the appropriate operation (e.g.
`math.isnan`, `cmath.isnan`, `decimal.getcontext().is_nan()`,
respectively)
...
Drawing from the existing summary in `505`{.interpreted-text role="pep"}
and the Wikipedia articles on the \"safe navigation operator[^5] and the
\"null coalescing operator\"[^6], we see:
...
of \"?\"?) and also hard to *describe* (given reused keywords, there\'s
no obvious shorthand terms for \"existence checking precondition
(?and)\" and \"existence checking fallback (?or)\" that would
distinguish them from \"logical conjunction (and)\" and \"logical
disjunction (or)\").
...
\"existence checking\" and elevates it to the status of being a
syntactic language feature with a clearly defined operator protocol.
...
invariant in place saying that values which indicate they don\'t exist
in an existence checking context should also report themselves as being
...
been deferred pending in-principle interest in the idea of adding these
operators - the implementation isn\'t the hard part of these proposals,
the hard part is deciding whether or not this is a change where the long
term benefits for new and existing Python users outweigh the short term
costs involved in the wider ecosystem (including developers of other
implementations, language curriculum developers, and authors of other
Python related educational material) adjusting to the change.
