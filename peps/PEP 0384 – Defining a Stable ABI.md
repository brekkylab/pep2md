...
source_path: https://github.com/python/peps/blob/main/peps/pep-0384.rst
source_commit: fa068a7594f3410386d469957987ae81ec62cf53
---
...
- tp_dealloc, tp_getattr, tp_setattr, tp_repr, tp_hash, tp_call, tp_str,
  tp_getattro, tp_setattro, tp_doc, tp_traverse, tp_clear,
...
All functions starting with [Py]{#py} are not available to applications.
...
(PyByteArray_Init, PyOS_FiniInterrupts and all functions ending in
[Fini]{#fini} or [ClearFreeList]{#clearfreelist}).
...
Also on Unix, the `3149`{.interpreted-text role="pep"} tag
abi\<PYTHON_ABI_VERSION\> is accepted in file names of extension
modules. No checking is performed that files named in this way are
actually restricted to the limited API, and no support for building such
files will be added to distutils due to the distutils code freeze.
