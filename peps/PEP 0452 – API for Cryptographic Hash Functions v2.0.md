...
source_path: https://github.com/python/peps/blob/main/peps/pep-0452.rst
source_commit: 669da330f8a52f84596640c798afa90706e34b93
---
...
:   Create a new hashing object and return it. The first form is for
    ...
    object using its `update()` method, and the hash value can be
    obtained at any time by calling the object\'s `digest()` method.
...
:   An integer value; the size of the digest produced by the hashing
    ...
    \'digest_size\' attribute, but it can be convenient to have this
    value available from the module. Hashes with a variable output size
    will set this variable to None.
...
:   An integer value or `NotImplemented`; the internal block size of the
    ...
    to pad the secret key to `digest_size` or to hash the secret key if
    it is longer than `digest_size`. If no HMAC algorithm is
    standardized for the hash algorithm, return `NotImplemented`
    instead.
...
:   Return the hash value of this hashing object as a string containing
    ...
    \'a\' through \'f\'. Like the `.digest()` method, this method
    mustn\'t alter the object.
...
It\'s a required parameter, and the usual convention is to place
required parameters first, but that also means that the \'string\'
parameter moves from the first position to the second. It would be
possible to get confused and pass a single argument to a keyed hash,
thinking that you\'re passing an initial string to an unkeyed hash, but
it doesn\'t seem worth making the interface for keyed hashes more
...
:   The name property was made mandatory by [issue
    18532](http://bugs.python.org/issue18532).
