...
source_path: https://github.com/python/peps/blob/main/peps/pep-0200.rst
source_commit: 56196e89a394718709d34585805c5b66209c9703
---
...
good sense or we wouldn\'t have given you commit privileges \<0.5
...
- Discuss any controversial changes on python-dev first. If you get a
  lot of +1 votes and no -1 votes, make the change. If you get a some -1
  votes, think twice; consider asking Guido what he thinks.
- If the change is to code you contributed, it probably makes sense for
  you to fix it.
- If the change affects code someone else wrote, it probably makes sense
  to ask him or her first.
...
- Change meaning of x escapes - `223`{.interpreted-text role="pep"} -
  Fredrik Lundh
...
- SRE - Fredrik Lundh \[at least I **think** it\'s done, as of
  15-Aug-2000 - tim\]
- Fix xrange printing behavior - Fred Drake Remove the tp_print handler
  for the xrange type; it produced a list display instead of
  \'xrange(\...)\'. The new code produces a minimal call to xrange(),
  enclosed in (`... * N`) when N != 1. This makes the repr() more human
  readable while making it do what reprs are advertised as doing. It
  also makes the xrange objects obvious when working in the interactive
  interpreter.
- Extended print statement - Barry Warsaw `214`{.interpreted-text
  role="pep"} SF Patch #100970
...
- Eliminated SET_LINENO opcode - Vladimir Marangozov Small optimization
  ...
  instruction. Uses code rewriting technique (that Guido\'s frowns on)
  to support debugger, which uses SET_LINENO.
