...
bdfl_delegate: Pradyun Gedam <pradyunsg@gmail.com>
...
source_path: https://github.com/python/peps/blob/main/peps/pep-0610.rst
source_commit: feff35d7349093ffe0793e640884096dc126112e
---
...
Following `440`{.interpreted-text role="pep"}, a distribution can be
identified by a name and either a version, or a direct URL reference
(see
`PEP440 Direct References <440#direct-references>`{.interpreted-text
role="pep"}). After installation, the name and version are captured in
the project metadata, but currently there is no way to obtain details of
the URL used when the distribution was identified by a direct URL
reference.
...
packages (see `376`{.interpreted-text role="pep"}).
...
> temporary directory, invoke the `517`{.interpreted-text role="pep"}
> build backend to produce a wheel if needed, install the wheel, and
...
know whether a `440`{.interpreted-text role="pep"} direct reference URL
can be constructed using the tag form, as only tags have the semantics
of immutability.
...
with other `517`{.interpreted-text role="pep"} build backends).
...
They are also sufficient to reproduce
`PEP440 Direct References <440#direct-references>`{.interpreted-text
role="pep"}, as well as [Pipfile and
Pipfile.lock](https://github.com/pypa/pipfile) entries. Finally, they
are sufficient to record the branch, tag, and/or Git ref origin of the
installed version that is already available for editable installs by
virtue of a VCS checkout being present.
...
This JSON file MUST be a dictionary, compliant with
...
- A `vcs` key (type `string`) MUST be present, containing the name of
  ...
  be registered by writing a PEP to amend this specification. The `url`
  value MUST be compatible with the corresponding VCS, so an installer
  can hand it off without transformation to a checkout/download command
  of the VCS.
...
- A `commit_id` key (type `string`) MUST be present, containing the
  ...
  commit-hash based revision identifiers, such commit-hash MUST be used
  as `commit_id` in order to reference the immutable version of the
...
- A `hash` key (type `string`) SHOULD be present, with value
  ...
  standard library\'s `hashlib` module be used for source archive
...
and be compliant with `8089`{.interpreted-text role="rfc"}. In
particular, the path component must be absolute. Symbolic links SHOULD
be preserved when making relative paths absolute.
...
The now withdrawn `426`{.interpreted-text role="pep"} specifies a
`source_url` metadata entry. It is also implemented in
[distlib](https://distlib.readthedocs.io).
...
requirement URLs. The same limitation is present in
`PEP440 Direct References <440#direct-references>`{.interpreted-text
role="pep"}.
...
environment variables for user:password in `440`{.interpreted-text
role="pep"}, would be necessary for use in this PEP.
