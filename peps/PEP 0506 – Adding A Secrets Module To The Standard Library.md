...
source_path: https://github.com/python/peps/blob/main/peps/pep-0506.rst
source_commit: cdf8c8f8c14721f5e5bc0ac5c50b21d2e22a3d5a
---
...
- CSPRNG:
  ...
  used to produce random-looking numbers which are resistant to
  prediction.

- MT:

  Mersenne Twister. An extensively studied PRNG which is currently used
  by the `random` module as the default.
...
that the default is not suitable for security purposes[^2], it is
...
The first[^3] hit when searching for \"python how to generate
passwords\" on Google is a tutorial that uses the default functions from
the `random` module[^4]. Although it is not intended for use in web
...
module, with the aim of providing \"secure by default\"
cryptographically strong primitives that developers can build upon
without thinking about security. (See Alternatives below.) This proposes
...
- Instead of requiring crypto-naive users to write secure code, the
  ...
  for the most common needs, such as generating secure tokens. This code
  will both directly satisfy a need (\"How do I generate a password
  reset token?\"), and act as an example of acceptable practises which
...
- A limited interface to the system CSPRNG, using either `os.urandom`
  ...
  - A function for returning a random integer in the half-open range 0
    to the given upper limit, `secrets.randbelow`[^9].
...
been discussed on the issue tracker for the \"cryptography\"
module[^12]. The following pseudo-code should be taken as the starting
...
expected to change in the future, possibly even in a maintenance
release[^14].
...
Alyssa Coghlan made an `earlier suggestion <504>`{.interpreted-text
role="pep"} for a globally configurable PRNG which uses the system
CSPRNG by default, but has since withdrawn it in favour of this
proposal.
...
- Q: What about a password generator?
  ...
  A: The consensus is that the requirements for password generators are
  too variable for it to be a good match for the standard library[^35].
  No password generator will be included in the initial release of the
  module, instead it will be given in the documentation as a recipe (à
  la the recipes in the `itertools` module)[^36].
...
- Q: Will `secrets` use /dev/random (which blocks) or /dev/urandom
  ...
  cryptographic randomness. On Linux, that may be `/dev/urandom`[^37],
...
[^9]: After considerable discussion, Guido ruled that the module need
    only provide `randbelow`, and not similar functions `randrange` or
...
[^17]: By default PHP seeds the MT PRNG with the time (citation needed),
    ...
    output from the system CSPRNG, which is believed to be much harder
    to exploit.
