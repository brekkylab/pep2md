...
source_path: https://github.com/python/peps/blob/main/peps/pep-0598.rst
source_commit: b3f6df6da4f70bff0336e9fbe9a6407ab2836e8a
---

# Abstract

`602`{.interpreted-text role="pep"} proposes reducing the feature
...
every 18-24 months to instead occur every 9-12 months.
...
proposal in `605`{.interpreted-text role="pep"}.
...
releases (such as the EL Python draft at[^1]), so the ideas presented
here may provide useful design suggestions for such proposals.
...
that would qualify as a \"feature release\" in the
`13`{.interpreted-text role="pep"} sense (incremental feature releases
wouldn\'t count).
...
- an installation within a feature release series can be updated to a
  ...
  reinstallation or any other changes to previously installed components
...
- baseline feature releases may contain higher risk changes to the
  ...
  potentially invasive feature additions that carry a risk of unintended
  side effects on other existing functionality
...
- many (most?) public projects will only add a new release series to
  ...
  already been published, which can make it difficult to resolve issues
  that require providing new flags or APIs to explicitly opt-in to old
  behaviour after a default behaviour changed
...
- with the approval of the release manager, backwards compatible
  ...
  constructs
- with the approval of the release manager, performance improvements can
  be incorporated for existing APIs and syntactic constructs
...
- all API additions made in an incremental feature release must support
  ...
  `sys.version_info` or runtime code object introspection. In most
  cases, a simple `hasattr` check on the affected module will serve this
...
- to maintain pickle compatibility in mixed version environments, and to
  enable easier compatibility testing across multiple API versions
...
- public Python projects that start relying on features added in an
  ...
  metadata appropriately (projects already do this when necessary - e.g.
...
motivation for change in `596`{.interpreted-text role="pep"}: the
current 18-24 month gap between feature releases has a lot of
undesirable consequences, especially for the standard library (see
`596`{.interpreted-text role="pep"} for further articulation of the
details).

This PEP\'s concern with the specific proposal in
`596`{.interpreted-text role="pep"} is that it doubles the number of
actively supported Python branches, increasing the complexity of
compatibility testing matrices for the entire Python community,
increasing the number of binary Python wheels to be uploaded to PyPI
when not using the stable ABI, and just generally having a high chance
of inflicting a relatively high level of additional cost across the
...
This is an area where this PEP could adopt part of the proposal in
`596`{.interpreted-text role="pep"}, by instead making that split \~9
months of pre-release development, and \~15 months of incremental
feature releases:
...
addition period (similar to the way that `596`{.interpreted-text
role="pep"} proposes to starting publishing Python 3.9.0 alpha releases
during the Python 3.8.0 release candidate period).
...
[^3]: Anthony Sottile created a pseudo \"Python 3.10\" to find and fix
    such issues (<https://github.com/asottile-archive/python3.10>)
