...
source_path: https://github.com/python/peps/blob/main/peps/pep-0342.rst
source_commit: ce2cf3326a898655c8568669d605ae9eb7b1c72b
---
...
- `288`{.interpreted-text role="pep"}, Generators Attributes and
  Exceptions. The current PEP covers its second half, generator
  exceptions (in fact the `throw()` method name was taken from
  `288`{.interpreted-text role="pep"}). `342`{.interpreted-text
  role="pep"} replaces generator attributes, however, with a concept
  from an earlier revision of `288`{.interpreted-text role="pep"}, the
  *yield expression*.
- `325`{.interpreted-text role="pep"}, Resource-Release Support for
  Generators. `342`{.interpreted-text role="pep"} ties up a few loose
  ends in the `325`{.interpreted-text role="pep"} spec, to make it
...
`try/finally` or `with` block (per `343`{.interpreted-text role="pep"})
to ensure that a given generator is properly finalized.
...
use in for-loops (carried over from `340`{.interpreted-text
role="pep"}), that would pass the value of *EXPR* into the iterator
being looped over. This feature has been withdrawn for the time being,
because the scope of this PEP has been narrowed to focus only on passing
values into generator-iterators, and not other kinds of iterators. It
was also felt by some on the Python-Dev list that adding new syntax for
this particular feature would be premature at best.
...
1.  What exception should be raised by `close()` when the generator
    ...
    `343`{.interpreted-text role="pep"} uses `RuntimeError` for similar
    offenses. Arguably they should all use the same exception. I\'d
    rather not introduce a new exception class just for this purpose,
    since it\'s not an exception that I want people to catch: I want it
    to turn into a traceback which is seen by the programmer who then
...
Raymond Hettinger (`288`{.interpreted-text role="pep"}) and Samuele
Pedroni (`325`{.interpreted-text role="pep"}) first formally proposed
the ideas of communicating values or exceptions into generators, and the
ability to *close* generators. Timothy Delaney suggested the title of
this PEP, and Steven Bethard helped edit a previous version. See also
the Acknowledgements section of `340`{.interpreted-text role="pep"}.
