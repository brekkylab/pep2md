...
source_path: https://github.com/python/peps/blob/main/peps/pep-0348.rst
source_commit: 203115203c6c3a06e85a6bf7af43422d6cafeabc
---
...
interface to raised objects to be made (see `344`{.interpreted-text
role="pep"}). A discussion about all of this has occurred before on
python-dev[^2].
...
:   
    ...
    :   +\-- GeneratorExit (defined in `342`{.interpreted-text
        role="pep"}) +\-- StandardError +\-- ArithmeticError +\--
        DivideByZeroError +\-- FloatingPointError +\-- OverflowError
        +\-- AssertionError +\-- AttributeError +\-- EnvironmentError
        +\-- IOError +\-- EOFError +\-- OSError +\-- ImportError +\--
        LookupError +\-- IndexError +\-- KeyError +\-- MemoryError +\--
        NameError +\-- UnboundLocalError +\-- NotImplementedError
        (stricter inheritance) +\-- SyntaxError +\-- IndentationError
        +\-- TabError +\-- TypeError +\-- RuntimeError +\-- UnicodeError
        +\-- UnicodeDecodeError +\-- UnicodeEncodeError +\--
        UnicodeTranslateError +\-- ValueError +\-- ReferenceError +\--
        StopIteration +\-- SystemError +\-- Warning +\--
        DeprecationWarning +\-- FutureWarning +\--
        PendingDeprecationWarning +\-- RuntimeWarning +\-- SyntaxWarning
        +\-- UserWarning + \-- WindowsError
...
it was passed on because its name did not properly reflect the fact that
it is an exception itself.
...
attributes. If `344`{.interpreted-text role="pep"} is accepted, the
attributes outlined there will be guaranteed to be on all exceptions
raised. This should help facilitate debugging by making the querying of
...
`Zen of Python <20>`{.interpreted-text role="pep"}. But Practicality
Beats Purity (PBP), also in the Zen of Python, trumps both of these in
this case. The BDFL has stated that bare `except` clauses will work this
way [^10].
...
becoming the same as `except Exception`) will go into affect. In order
to make all of this work in a backwards-compatible way in Python 2.5
would require very deep hacks in the exception machinery which could be
...
The documentation for the \'exceptions\' module[^11], tutorial[^12], and
`290`{.interpreted-text role="pep"} will all require updating.
...
used enough[^14]. The idea of then removing WindowsError was proposed
and accepted as reasonable, thus completely negating the idea of adding
these exceptions.
...
the exception is already used throughout the interpreter [^16].
Rejection of SimpleError was founded on the thought that people should
be free to use whatever exception they choose and not have one so
blatantly suggested[^17].
...
[^2]: python-dev Summary (An exception is an exception, unless it
...
[^4]: python-dev thread (Pre-PEP: Exception Reorganization for Python
    3.0)
...
[^5]: python-dev thread (PEP, take 2: Exception Reorganization for
    Python 3.0)
...
[^13]: python-dev email (Pre-PEP: Exception Reorganization for Python
    3.0)
    <https://mail.python.org/pipermail/python-dev/2005-July/055019.html>

[^14]: python-dev email (Pre-PEP: Exception Reorganization for Python
    3.0)
    <https://mail.python.org/pipermail/python-dev/2005-July/055019.html>

[^15]: python-dev email (PEP, take 2: Exception Reorganization for
    Python 3.0)
