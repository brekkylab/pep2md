...
source_path: https://github.com/python/peps/blob/main/peps/pep-0219.rst
source_commit: f6ca4073d1be24aaa3c839e64ede413964d4f944
---
...
related to `220`{.interpreted-text role="pep"}, which describes how
Python should be extended to support these facilities. The focus of this
PEP is strictly on the changes required to allow these extensions to
work.
...
microthreads (as has been done by Will Ware[^2]) and generators. But in
...
implementations of generators[^3] and coroutines[^4] using threads (and
...
involve transfers of control that are difficult to accommodate in a
language implementation based on a single stack. (Generators can be done
on a single stack, but they can also be regarded as a very simple case
of coroutines.)
...
Another justification for this PEP (explored in `220`{.interpreted-text
role="pep"}) is that coroutines and generators often allow a more direct
expression of an algorithm than is possible in today\'s Python.
...
separate C\'s stack usage from Python\'s stack usage. Secondly,
associate with each frame enough stack space to handle that frame\'s
execution.
...
compromise: Python\'s internal usage (and C extension writers who want
to go to the effort) will no longer use a nested invocation of the
...
\"stack\" of arguments and results since the real stack can now only
hold the most recent. The code will be more verbose, because it\'s not
quite as obvious when we\'re done. While Stackless is not implemented
this way, it has to deal with the same issues.
...
fix these situations because of the amount of code changes required.
