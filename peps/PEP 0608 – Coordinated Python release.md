...
module: `565`{.interpreted-text role="pep"}).
...
<602>`{.interpreted-text role="pep"} and the `PEP 605: A
rolling feature release stream for CPython
<605>`{.interpreted-text role="pep"} would like to release Python more
often to ship new features more quickly.
...
<606>`{.interpreted-text role="pep"} for more examples of incompatible
changes.
...
- Structure made opaque in the C API. For example, PyInterpreterState
  ...
  `interp->modules` (`PyImport_GetModuleDict()` should be used instead).
