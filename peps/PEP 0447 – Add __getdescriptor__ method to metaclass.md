...
source_path: https://github.com/python/peps/blob/main/peps/pep-0447.rst
source_commit: b1c0fde82ce077e3d7594e2e242738c05e54072f
---
...
attribute lookup, especially when using a
[super](http://docs.python.org/3/library/functions.html#super) object.
...
It is currently not possible to influence how the [super
class](http://docs.python.org/3/library/functions.html#super) looks up
...
attributes even when looking them up using the [super
class](http://docs.python.org/3/library/functions.html#super).

The new method affects `object.__getattribute__` (and
[PyObject_GenericGetAttr](http://docs.python.org/3/c-api/object.html#PyObject_GenericGetAttr))
as well for consistency and to have a single place to implement dynamic
attribute resolution for classes.
...
types, an example of which is [PyObjC](http://pyobjc.sourceforge.net/).
PyObjC creates a Python class for every class in the Objective-C
runtime, and looks up methods in the Objective-C runtime when they are
used. This works fine for normal access, but doesn\'t work for access
with [super](http://docs.python.org/3/library/functions.html#super)
objects. Because of this PyObjC currently includes a custom
[super](http://docs.python.org/3/library/functions.html#super) that must
be used with its classes, as well as completely reimplementing
[PyObject_GenericGetAttr](http://docs.python.org/3/c-api/object.html#PyObject_GenericGetAttr)
for normal attribute access.

The API in this PEP makes it possible to remove the custom
[super](http://docs.python.org/3/library/functions.html#super) and
...
[PyObjC](http://pyobjc.sourceforge.net/) cannot precalculate the
contents of the class `__dict__` because Objective-C classes can grow
new methods at runtime. Furthermore, Objective-C classes tend to contain
a lot of methods while most Python code will only use a small subset of
them, this makes precalculating unnecessarily expensive.
::::
...
[PyObject_GenericGetAttr](http://docs.python.org/3/c-api/object.html#PyObject_GenericGetAttr)
and in particular `_PyType_Lookup` in C code) walk an object\'s MRO and
currently peek in the class\' `__dict__` to look up attributes.
...
447\" with a method call to perform the actual lookup, making is
possible to affect that lookup both for normal attribute access and
access through the [super
proxy](http://docs.python.org/3/library/functions.html#super).
...
descriptors, and should raise
[AttributeError](http://docs.python.org/3/library/exceptions.html#AttributeError)
when the name cannot be found.

The [type](http://docs.python.org/3/library/functions.html#type) class
provides a default implementation for `__getdescriptor__`, that looks up
the name in the class dictionary.
...
this slot corresponds to the `__getdescriptor__` method on
[type](http://docs.python.org/3/library/functions.html#type).
...
The change for
[PyObject_GenericGetAttr](http://docs.python.org/3/c-api/object.html#PyObject_GenericGetAttr)
will be done by changing the private function `_PyType_Lookup`. This
currently returns a borrowed reference, but must return a new reference
when the `__getdescriptor__` method is present. Because of this
`_PyType_Lookup` will be renamed to `_PyType_LookupName`, this will
cause compile-time errors for all out-of-tree users of this private API.
...
- `dir` might not show all attributes
  ...
  As with a custom `__getattribute__` method
  [dir()](http://docs.python.org/3/library/functions.html#dir) might not
  see all (instance) attributes when using the `__getdescriptor__()`
  method to dynamically resolve attributes.

  The solution for that is quite simple: classes using
  `__getdescriptor__` should also implement [\_\_dir\_\_()]() if they
  want full support for the builtin
  [dir()](http://docs.python.org/3/library/functions.html#dir) function.
...
- `inspect.getmembers` and `inspect.classify_class_attrs`
  ...
  Both of these functions directly access the class \_\_dict\_\_ of
...
updated when I\'ve ported the patch to the current trunk. I don\'t
expect significant changes to the results in this section.

## Micro benchmarks

[Issue 18181](http://bugs.python.org/issue18181) has a micro benchmark
as one of its attachments
([pep447-micro-bench.py](http://bugs.python.org/file40013/pep447-micro-bench.py))
that specifically tests the speed of attribute lookup, both directly and
through super.
...
runs, I\'ve seen difference in \"minimum time\" vary from -0.1% to
+1.5%, with similar (but slightly smaller) differences in the \"average
time\" difference.
...
simpler and easier to understand. A comment on [Issue
18181](http://bugs.python.org/issue18181) asked about reusing the
`tp_getattro` slot, that is super could call the `tp_getattro` slot of
all methods along the MRO.
...
class dictionaries changes the semantics of the [super
class](http://docs.python.org/3/library/functions.html#super).
...
class itself (similar to how `__new__` is a
[staticmethod](http://docs.python.org/3/library/functions.html#staticmethod)
of the class and not a method of the metaclass).
...
- The initial version of the PEP was send with Message-ID
  [75030FAC-6918-4E94-95DA-67A88D53E6F5@mac.com](75030FAC-6918-4E94-95DA-67A88D53E6F5@mac.com)
- Further discussion starting at a message with Message-ID
  [5BB87CC4-F31B-4213-AAAC-0C0CE738460C@mac.com](5BB87CC4-F31B-4213-AAAC-0C0CE738460C@mac.com)
- And more discussion starting at message with Message-ID
  [00AA7433-C853-4101-9718-060468EBAC54@mac.com](00AA7433-C853-4101-9718-060468EBAC54@mac.com)

# References

- [Issue 18181](http://bugs.python.org/issue18181) contains an out of
