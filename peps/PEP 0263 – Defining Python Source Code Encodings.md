...
source_path: https://github.com/python/peps/blob/main/peps/pep-0263.rst
source_commit: 8c505d111901973421d0dc9e5d4e1a033982272b
---
...
non-Latin-1 locales such as many of the Asian countries. Programmers can
write their 8-bit strings using the favorite encoding, but are bound to
the \"unicode-escape\" encoding for Unicode literals.
...
encoding comment, the only allowed encoding for the comment is
\'utf-8\'. Any other encoding will cause an error.
...
1.  The complete Python source file should use a single encoding.
    ...
    such as Shift_JIS. It does not include encodings which use two or
...
2.  Handling of escape sequences should continue to work as it does now,
...
3.  Python\'s tokenizer/compiler combo will need to be updated to work
    as follows:
...
SUZUKI Hisao is working on a patch; see[^1] for details. A patch
implementing only phase 1 is available at[^2].
...
[^1]: Phase 2 implementation: <https://bugs.python.org/issue534304>

[^2]: Phase 1 implementation: <https://bugs.python.org/issue526840>
