...
modules to be executed in the `__main__` namespace using the
`489`{.interpreted-text role="pep"} multi-phase initialization.
...
The technical groundwork to make this possible has been done for
`489`{.interpreted-text role="pep"}, and enabling the `-m` option is
listed in that PEP\'s "Possible Future Extensions" section. Technically,
the additional changes proposed here are relatively small.
...
Since Python 3.5, extension modules can use `489`{.interpreted-text
role="pep"} multi-phase initialization. In this scenario, the `PyInit_*`
entry point returns a `PyModuleDef` structure: a description of how the
module should be created and initialized. The extension can choose to
customize creation of the module object using the `Py_mod_create`
callback, or opt to use a normal module object by not specifying
`Py_mod_create`. Another callback, `Py_mod_exec`, is then called to
initialize the module object, e.g. by populating it with methods and
classes.
...
While `489`{.interpreted-text role="pep"} was designed to make these
changes generally possible, it\'s necessary to decouple module
discovery, creation, and initialization steps for extension modules, so
that another module can be used instead of a newly initialized one, and
the functionality needs to be added to `runpy` and `importlib`.
...
(single-phase initialization) or a `PyModuleDef` (for
`489`{.interpreted-text role="pep"} multi-phase initialization).
