...
python_version: '3.11'
post_history:
- '`24-Apr-2021 <https://mail.python.org/archives/list/typing-sig@python.org/thread/TXL5LEHYX5ZJAZPZ7YHZU7MVFXMVUVWL/>`__'
...
source_path: https://github.com/python/peps/blob/main/peps/pep-0681.rst
source_commit: 1fba9cb928daff00d78fcaa32495b89d7b0b2199
---

::: canonical-typing-spec
`typing:dataclass-transform`{.interpreted-text role="ref"} and
`@typing.dataclass_transform <typing.dataclass_transform>`{.interpreted-text
role="py:func"}
:::

# Abstract

`557`{.interpreted-text role="pep"} introduced the dataclass to the
Python stdlib. Several popular libraries have behaviors that are similar
to dataclasses, but these behaviors cannot be described using standard
type annotations. Such projects include attrs, pydantic, and object
...
- Supporting \"field specifiers\", which describe attributes of
  individual fields that a static type checker must be aware of, such as
  whether a default value is provided for the field.

The full behavior of the stdlib dataclass is described in the [Python
documentation](https://docs.python.org/3.11/library/dataclasses.html).
...
- `eq`, `order`, `frozen`, `init` and `unsafe_hash` are parameters
  ...
  `PEP 557 <557#id7>`{.interpreted-text role="pep"}.
...
- `field_specifiers` specifies a static list of supported classes that
  ...
  standard dataclass behavior supports only one type of field specifier
  called `Field` plus a helper function (`field`) that instantiates this
  class, so if we were describing the stdlib dataclass behavior, we
  would provide the tuple argument
...
- Frozen dataclasses cannot inherit from non-frozen dataclasses. A class
  ...
  from it. Similarly, a class that directly specifies a metaclass that
  is decorated with `dataclass_transform` is considered neither frozen
  nor non-frozen.
...
- Field ordering and inheritance is assumed to follow the rules
  specified in `557 <557#inheritance>`{.interpreted-text role="pep"}.
  This includes the effects of overrides (redefining a field in a child
  class that has already been defined in a parent class).

- `PEP 557 indicates <557#post-init-parameters>`{.interpreted-text
  role="pep"} that all fields without default values must appear before
  fields with default values. Although not explicitly stated in PEP 557,
  this rule is ignored when `init=False`, and this specification
  likewise ignores this requirement in that situation. Likewise, there
  is no need to enforce this ordering when keyword-only parameters are
  used for `__init__`, so the rule is not enforced if `kw_only`
...
- KW_ONLY sentinel values are supported as described in [the Python
  docs](https://docs.python.org/3/library/dataclasses.html#dataclasses.KW_ONLY)
  and [bpo-43532](https://bugs.python.org/issue43532).

- ClassVar attributes are not considered dataclass fields and are
  [ignored by dataclass
  mechanisms](https://docs.python.org/3/library/dataclasses.html#class-variables).
...
[Pyright](https://github.com/Microsoft/pyright) contains the reference
implementation of type checker support for `dataclass_transform`.
Pyright\'s `dataClasses.ts` [source
file](https://github.com/microsoft/pyright/blob/main/packages/pyright-internal/src/analyzer/dataClasses.ts)
would be a good starting point for understanding the implementation.

The [attrs](https://github.com/python-attrs/attrs/pull/796) and
[pydantic](https://github.com/samuelcolvin/pydantic/pull/2721) libraries
...
whether class members decorated with `526`{.interpreted-text role="pep"}
variable annotations but with no assignment should be treated as data
fields.
...
The attrs library performs [automatic
aliasing](https://www.attrs.org/en/stable/init.html#private-attributes)
of field names that start with a single underscore, stripping the
...
Django applies [additional logic for primary and foreign
keys](https://docs.djangoproject.com/en/4.0/topics/db/models/#automatic-primary-key-fields).
For example, it automatically adds an `id` field (and `__init__`
parameter) if there is no field designated as a primary key.
...
to our misunderstanding. For more details, see the [Pyright
bug](https://github.com/microsoft/pyright/issues/3245).

## `converter` field specifier parameter

The attrs library supports a `converter` field specifier parameter,
which is a `Callable` that is called by the generated `__init__` method
to convert the supplied value to some other desired value. This is
tricky to support since the parameter type in the synthesized `__init__`
method needs to accept uncovered values, but the resulting field is
typed according to the output of the converter.

Some aspects of this issue are detailed in a [Pyright
discussion](https://github.com/microsoft/pyright/discussions/1782?sort=old#discussioncomment-653909).
