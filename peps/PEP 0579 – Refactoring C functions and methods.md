...
source_path: https://github.com/python/peps/blob/main/peps/pep-0579.rst
source_commit: 0ff9e555bc71ca85324f38121e4cc40389b78a35
---

# Approval Notice

This PEP describes design issues addressed in `575`{.interpreted-text
role="pep"}, `580`{.interpreted-text role="pep"},
`590`{.interpreted-text role="pep"} (and possibly later proposals).

As noted in `PEP 1 <1#pep-types>`{.interpreted-text role="pep"}:
...
**Reference**: `580`{.interpreted-text role="pep"}
...
This is also needed for `573`{.interpreted-text role="pep"} where all
cfunctions require access to their \"parent\" (the module for functions
of a module or the defining class for methods).
...
**References**: `580`{.interpreted-text role="pep"},
...
extension types (see `576`{.interpreted-text role="pep"} for the latter,
keeping in mind that this may have some minor backwards compatibility
issues). This is not a goal by itself but just something to keep in mind
...
ABI (`384`{.interpreted-text role="pep"}), we cannot change the
