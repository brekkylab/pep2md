...
source_path: https://github.com/python/peps/blob/main/peps/pep-3109.rst
source_commit: c749fbf490e3e3de67db4a2781f6976cbcc55173
---
...
preferably only one -- obvious way to do it" <20>`{.interpreted-text
role="pep"}. Python 2.x\'s `raise` statement violates this principle,
permitting multiple ways of expressing the same thought. For example,
...
where T is a traceback. As specified in `344`{.interpreted-text
role="pep"}, exception objects in Python 3.x will possess a
`__traceback__` attribute, admitting this translation of the
...
2.  `raise EXCEPTION` is used to raise a new exception. This form has
    ...
    BaseException and its subclasses (`352`{.interpreted-text
    role="pep"}). If `EXCEPTION` is a subclass, it will be called with
...
second and third parameters. The signature thus changes
(`342`{.interpreted-text role="pep"}) from :
...
Rossum\'s 2to3 utility[^5] using the `raise` and `throw` fixers
([^6],[^7]).
...
4.  Two- and three-expression `raise` statements where `E` is a tuple
    literal can be converted automatically using `2to3`\'s `raise`
    fixer. `raise` statements where `E` is a non-literal tuple, e.g.,
    the result of a function call, will need to be converted manually.
