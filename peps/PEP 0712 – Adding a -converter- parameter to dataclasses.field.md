...
python_version: '3.13'
post_history:
- '`27-Dec-2022 <https://mail.python.org/archives/list/typing-sig@python.org/thread/NWZQIINJQZDOCZGO6TGCUP2PNW4PEKNY/>`__'
...
- We did not find evidence of a strong consensus that this feature was
  ...
  favor in order to reduce their dependence on third party packages. For
  those who need such functionality, we think those existing third party
  libraries such as attrs and Pydantic (which the PEP references) are
  acceptable alternatives.
- This feature seems to us like an accumulation of what could be
  considered more cruft in the standard library, leading us ever farther
  away from the "simple" use cases that dataclasses are ideal for.
- Reading the "How to Teach This" section of the PEP gives us pause that
  the pitfalls and gotchas are significant, with a heightened confusion
  and complexity outweighing any potential benefits.
- The PEP seems more focused toward helping type checkers than people
  using the library.

# Abstract

`557`{.interpreted-text role="pep"} added
`dataclasses`{.interpreted-text role="mod"} to the Python stdlib.
`681`{.interpreted-text role="pep"} added
`~py3.11:typing.dataclass_transform`{.interpreted-text role="func"} to
help type checkers understand several common dataclass-like libraries,
such as attrs, Pydantic, and object relational mapper (ORM) packages
...
`dataclasses.field`{.interpreted-text role="func"} (along with the
requisite changes to `dataclasses.Field`{.interpreted-text role="class"}
and `~py3.11:typing.dataclass_transform`{.interpreted-text role="func"})
to specify the function to use to convert the input value for each field
to the representation to be stored in the dataclass.

# Motivation

There is no existing, standard way for `dataclasses`{.interpreted-text
role="mod"} or third-party dataclass-like libraries to support argument
conversion in a type-checkable way. To work around this limitation,
library authors/users are forced to choose to:
...
- Provide a custom `__init__` which declares \"wider\" parameter types
  and converts them when setting the appropriate attribute. This not
  only duplicates the typing annotations between the converter and
  `__init__`, but also opts the user out of many of the features
  `dataclasses`{.interpreted-text role="mod"} provides.
...
`~py3.11:typing.dataclass_transform`{.interpreted-text role="func"},
...
`dataclasses.field`{.interpreted-text role="func"} function. If
provided, it represents a single-argument callable used to convert all
values when assigning to the associated attribute.
...
- A `converter` attribute will be added to
  `dataclasses.Field`{.interpreted-text role="class"}.
- `converter` will be added to
  `~py3.11:typing.dataclass_transform`{.interpreted-text role="func"}\'s
...
be compatible with `Callable[[T], X]` where `T` is the input type for
...
if an argument for `converter` is provided alongside either `default` or
...
checked using the type of the single argument to the `converter`
callable.
...
`dataclasses`{.interpreted-text role="mod"}) and linked from the
*What\'s New* document.
...
The attrs library [already
includes](https://www.attrs.org/en/21.2.0/examples.html#conversion) a
...
CPython support is implemented on [a branch in the author\'s
fork](https://github.com/thejcannon/cpython/tree/converter).
...
`~py3.11:typing.dataclass_transform`{.interpreted-text role="func"} was
briefly [discussed on
Typing-SIG](https://mail.python.org/archives/list/typing-sig@python.org/thread/NWZQIINJQZDOCZGO6TGCUP2PNW4PEKNY/)
where it was suggested to broaden this to
`dataclasses`{.interpreted-text role="mod"} more generally.

Additionally, adding this to `dataclasses`{.interpreted-text role="mod"}
ensures anyone can reap the benefits without requiring additional
libraries.
...
[Pydantic\'s data
conversion](https://docs.pydantic.dev/usage/models/#data-conversion) has
semantics which appear to be similar to this approach.
...
this would reduce the common repetition this PEP introduces (e.g.
`x: str = field(converter=str)`), it isn\'t clear how to best support
this while maintaining the current dataclass semantics (namely, that the
