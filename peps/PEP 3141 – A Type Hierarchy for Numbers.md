...
source_path: https://github.com/python/peps/blob/main/peps/pep-3141.rst
source_commit: d3718d955cdc446ef460e070bdfcb375dd59bf29
---
...
\"A is a supertype of B\". The hierarchy is inspired by Scheme\'s
numeric tower[^1].
...
terminology from `3119`{.interpreted-text role="pep"}, but the hierarchy
is intended to be meaningful for any systematic method of defining sets
of classes.
...
## Changes to operations and \_\_magic\_\_ methods

To support more precise narrowing from float to int (and more generally,
from Real to Integral), we propose the following new \_\_magic\_\_
methods, to be called from the corresponding library functions. All of
these return Integrals rather than Reals.
...
\_\_add\_\_ and \_\_radd\_\_ should be defined as:
...
1.  If A defines an \_\_add\_\_ which accepts b, all is well.
2.  If A falls back to the boilerplate code, and it were to return a
    value from \_\_add\_\_, we\'d miss the possibility that B defines a
    more intelligent \_\_radd\_\_, so the boilerplate should return
    NotImplemented from \_\_add\_\_. (Or A may not implement \_\_add\_\_
    at all.)
3.  Then B\'s \_\_radd\_\_ gets a chance. If it accepts a, all is well.
...
really wasn\'t interested, and we ran into the issue that even if `x` is
an instance of `X <: MonoidUnderPlus` and `y` is an instance of
...
scaled back the proposal to resemble the Scheme numeric tower much more
closely.
