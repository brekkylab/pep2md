...
- Unparenthesized assignment expressions are prohibited in lambda
  ...
  name binding at the top level inside a lambda function is unlikely to
  be of value, as there is no way to make use of it. In cases where the
  name will be used more than once, the expression is likely to need
...
reference implementation[^1] will ensure that such cases raise
...
`8`{.interpreted-text role="pep"}\'s recommendation for `=` when used
for assignment, whereas the latter disallows spaces around `=` used for
keyword arguments.)
...
- In a dict comprehension `{X: Y for ...}`, `Y` is currently evaluated
  before `X`. We propose to change this so that `X` is evaluated before
  `Y`. (In a dict display like `{X: Y}` this is already the case, and
  also in `dict((X, Y) for ...)` which should clearly be equivalent to
...
- for consistency with other similar exceptions, and to avoid locking in
  an exception name that is not necessarily going to improve clarity for
  end users, the originally proposed `TargetScopeError` subclass of
...
- due to a limitation in CPython\'s symbol table analysis process, the
  ...
  only raising them when the named expression target conflicts with one
  of the iteration variables in the comprehension. This could be
  revisited given sufficiently compelling examples, but the extra
  complexity needed to implement the more selective restriction doesn\'t
  seem worthwhile for purely hypothetical use cases.
...
1.  `EXPR as NAME`:
    ...
    - In all other situations where an `as` clause is allowed, even
      readers with intermediary skills are led to anticipate that clause
      (however optional) by the keyword that starts the line, and the
      grammar ties that keyword closely to the as clause:
      ...
      `if` or `while` that starts the line, and we intentionally allow
...
4.  Adding a `where:` to any statement to create local name bindings:
    ...
    existing keyword is repurposed (most likely `with:`). See
    `3150`{.interpreted-text role="pep"} for prior discussion on this
    subject (with the proposed keyword being `given:`).
...
arise. For the benefit of style guides such as `8`{.interpreted-text
role="pep"}, two recommendations are suggested.
...
get another [line]{#line}\_ of surrounding code on screen. I didn\'t
give much weight to this at first, but it was so very frequent it added
up, and I soon enough became annoyed that I couldn\'t actually run the
