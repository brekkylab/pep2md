...
python_version: '3.13'
post_history:
- '`30-Aug-2023 <https://discuss.python.org/t/32566>`__'
python_status: Withdrawn
url: https://peps.python.org/pep-0727/
source_path: https://github.com/python/peps/blob/main/peps/pep-0727.rst
source_commit: 686a41a47bfd1223978c6a4b8a81ccfbf688ded9
---

# Abstract

This PEP proposes a standardized way to provide documentation strings
for Python symbols defined with `~typing.Annotated`{.interpreted-text
role="py:class"} using a new class `typing.Doc`.

# PEP Withdrawal

The reception of this PEP was mostly negative, with concerns raised
about verbosity and readability. As a result, this PEP has been
withdrawn.

# Motivation

There\'s already a well-defined way to provide documentation for
classes, functions, class methods, and modules: using docstrings.
...
inside of strings, robustly parsing them for **rendering** requires
complex logic that needs to be implemented by the tools supporting them.
...
the docstrings (e.g.
[Sphinx](https://www.sphinx-doc.org/en/master/usage/restructuredtext/domains.html#info-field-lists),
[numpydoc](https://numpydoc.readthedocs.io/en/latest/format.html#parameters))
but now that information doesn\'t need to be in docstrings as there is
now an official `syntax for type annotations <484>`{.interpreted-text
role="pep"}.
...
type annotations with `~typing.Annotated`{.interpreted-text
role="py:class"}, and a new class `Doc` in `typing`.
...
`~typing.Annotated`{.interpreted-text role="py:class"}. Some tool
providers (at least VS Code and PyCharm) have shown they would consider
implementing support for this only if it was a standard.

This doesn\'t deprecate current usage of docstrings, docstrings should
be considered the preferred documentation method when available (not
...
that can be declared with `~typing.Annotated`{.interpreted-text
role="py:class"} (currently only covered by the several available
microsyntax conventions).
...
- **Rendering** would be relatively straightforward to implement by
  static tools (tools that don\'t need runtime execution), as the
  information can be extracted from the AST they normally already
  create.
- Deduplication of information: the name of a parameter would be defined
  in a single place, not duplicated inside of a docstring.
- Elimination of the possibility of having inconsistencies when removing
  a parameter or class variable and forgetting to remove its
  documentation.
...
- A more formalized way to document other symbols, like type aliases,
  that could use `~typing.Annotated`{.interpreted-text role="py:class"}.
...
- Parameter documentation inheritance for functions captured by
  `~typing.ParamSpec`{.interpreted-text role="py:class"}.

# Specification

The main proposal is to introduce a new class, `typing.Doc`. This class
should only be used within `~typing.Annotated`{.interpreted-text
role="py:class"} annotations. It takes a single positional-only string
argument. It should be used to document the intended meaning and use of
the symbol declared using `~typing.Annotated`{.interpreted-text
role="py:class"}.
...
`~typing.Annotated`{.interpreted-text role="py:class"} is normally used
as a type annotation, in those cases, any `typing.Doc` inside of it
would document the symbol being annotated.

When `~typing.Annotated`{.interpreted-text role="py:class"} is used to
declare a type alias, `typing.Doc` would then document the type alias
symbol.
...
`~typing.get_type_hints`{.interpreted-text role="py:func"} with the
parameter `include_extras=True`, and as
`~typing.Annotated`{.interpreted-text role="py:class"} is normalized
(even with type aliases), this would mean they should use the last
`typing.Doc` available, if more than one is used, as that is the last
one used.
...
`~typing.ParamSpec`{.interpreted-text role="py:class"}, any
...
valid docstring. This means that
[f-strings](https://docs.python.org/3/reference/lexical_analysis.html#formatted-string-literals)
and string operations should not be used. As this cannot be enforced by
the Python runtime, tools should not rely on this behavior.

When tools providing **rendering** show the raw signature, they could
allow configuring if the whole raw `~typing.Annotated`{.interpreted-text
role="py:class"} code should be displayed, but they should default to
not include `~typing.Annotated`{.interpreted-text role="py:class"} and
its internal code metadata, only the type of the symbols annotated. When
those tools support `typing.Doc` and rendering in other ways than just a
raw signature, they should show the string value passed to `typing.Doc`
in a convenient way that shows the relation between the documented
symbol and the documentation string.

Tools providing **rendering** could allow ways to configure where to
show the parameter documentation and the prose docstring in different
ways. Otherwise, they could simply show the prose docstring first and
then the parameter documentation second.
...
they can use `typing.Doc` inside of
`~typing.Annotated`{.interpreted-text role="py:class"} annotations for
the symbols that support it.
...
a special format for comments put on top of the function definition.
This would be similar to Python current docstring microsyntax
...
JavaScript uses [JSDoc](https://jsdoc.app/).
...
Rust uses another similar variation of a microsyntax in [Doc
...
Go also uses a form of [Doc Comments](https://go.dev/doc/comment).
...
functionality is covered by `typing.deprecated()` in
`702`{.interpreted-text role="pep"}, so it was dropped from this
proposal.
...
`~typing.Annotated`{.interpreted-text role="py:class"} in a way that
could be composed with other future proposals.
...
`~typing.Annotated`{.interpreted-text role="py:class"} to deprecate
...
This was already proposed and rejected in `224`{.interpreted-text
role="pep"}, mainly due to the ambiguity of how is the string connected
with the symbol it\'s documenting.
...
`~typing.Annotated`{.interpreted-text role="py:class"}:
...
of `~typing.Annotated`{.interpreted-text role="py:class"}, and any tool
that was using plain strings in them for any other purpose, which is
currently allowed, would now be invalid.

Having an explicit `typing.Doc` makes it compatible with current valid
uses of `~typing.Annotated`{.interpreted-text role="py:class"}.

## Another Annotated-Like Type

In the discussion it was suggested to define a new type similar to
`~typing.Annotated`{.interpreted-text role="py:class"}, it would take
the type and a parameter with the documentation string:
...
make it more difficult to combine it with
`~typing.Annotated`{.interpreted-text role="py:class"} for other
...
declared with `~typing.Annotated`{.interpreted-text role="py:class"}
were used, and these type aliases were used in annotations, the
documentation string would be transferred to the annotated symbol.
...
`Doc` class, runtime executing of current versions of Python don\'t
allow it.

At runtime, `~typing.Annotated`{.interpreted-text role="py:class"}
requires at least two arguments, and it requires the first argument to
be type, it crashes if it is a slice.
...
using `~typing.Annotated`{.interpreted-text role="py:class"} and
`typing.Doc`. If `~typing.Annotated`{.interpreted-text role="py:class"}
had more usage, it could make sense to have an improved shorter syntax
for it and for the type of metadata it would carry. But that would only
make sense once `~typing.Annotated`{.interpreted-text role="py:class"}
is more widely used.
...
Still, there\'s a high chance that library authors could receive
pressure to adopt this if it became the blessed solution.
...
that type annotations carry, the same way as `702`{.interpreted-text
role="pep"} extends them to include deprecation information.

`~typing.Annotated`{.interpreted-text role="py:class"} was added to the
standard library precisely to support adding additional metadata to the
annotations, and as the new proposed `Doc` class is tightly coupled to
`~typing.Annotated`{.interpreted-text role="py:class"}, it makes sense
for it to live in the same module. If
`~typing.Annotated`{.interpreted-text role="py:class"} was moved to
another module, it would make sense to move `Doc` with it.
...
[mkdocstrings](https://github.com/mkdocstrings/mkdocstrings), which
