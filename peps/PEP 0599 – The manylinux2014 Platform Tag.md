...
standard, and that a new `425`{.interpreted-text role="pep"}-style
platform tag called `manylinux2014` be derived from CentOS 7 and that
the `manylinux` toolchain, PyPI, and `pip` be updated to support it.
...
- Installers are unable to install a platform tag which predates a given
  release.
...
4.  If a wheel is built for any version of CPython 2 or CPython versions
    ...
    (`3149`{.interpreted-text role="pep"}[^6])
...
wheels.[^7] Its behavior and purpose will be otherwise unchanged from
...
`599`{.interpreted-text role="pep"} was [accepted by Paul Moore on July
31,
