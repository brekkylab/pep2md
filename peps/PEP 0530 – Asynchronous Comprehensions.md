...
source_path: https://github.com/python/peps/blob/main/peps/pep-0530.rst
source_commit: 1dce5f5dafecfb477ed2617b12e7589356a8fdfd
---

# Abstract

`492`{.interpreted-text role="pep"} and `525`{.interpreted-text
role="pep"} introduce support for native coroutines and asynchronous
generators using `async` / `await` syntax. This PEP proposes to add
asynchronous versions of list, set, dict comprehensions and generator
expressions.
...
comprehensions. Pending `525`{.interpreted-text role="pep"} approval, we
can also allow creation of asynchronous generator expressions.
...
`530`{.interpreted-text role="pep"} was accepted by Guido, September 6,
2016[^1].

# Implementation

The implementation is tracked in issue 28008[^2]. The reference
