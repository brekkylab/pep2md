...
title: Literal Types
author:
- Michael Lee <michael.lee.0x2a@gmail.com>
...
This PEP proposes adding *Literal types* to the `484`{.interpreted-text
role="pep"} ecosystem. Literal types indicate that some expression has
literally a specific value. For example, the following function will
accept only expressions that have literally the value \"4\":
...
- `numpy.unique` will return either a single array or a tuple containing
  anywhere from two to four arrays depending on three boolean flag
  values.
...
functions: `484`{.interpreted-text role="pep"} does not include any
mechanism for writing signatures where the return type varies depending
on the value passed in. Note that this problem persists even if we
redesign these APIs to instead accept enums: `MyEnum.FOO` and
`MyEnum.BAR` are both considered to be of type `MyEnum`.
...
`handle NewTypes <484#newtype-helper-function>`{.interpreted-text
role="pep"}.
...
- Arbitrary expressions like `Literal[3 + 4]` or
  ...
  - Rationale: Literal types are meant to be a minimal extension to the
    `484`{.interpreted-text role="pep"} typing ecosystem and requiring
    type checkers to interpret potentially expressions inside types adds
    too much complexity. Also see [Rejected or out-of-scope
    ideas](#rejected-or-out-of-scope-ideas).
  - As a consequence, complex numbers like `Literal[4 + 3j]` and
    `Literal[-4 + 2j]` are also prohibited. For consistency, literals
    like `Literal[4j]` that contain just a single complex number are
    also prohibited.
...
- Tuples containing valid literal types like
  `Literal[(1, "foo", "bar")]`. The user could always express this type
  as `Tuple[Literal[1], Literal["foo"], Literal["bar"]]` instead. Also,
...
- Any other types: for example, `Literal[Path]`, or
  ...
  if `T` is a typevar, `Literal[T]` is not allowed. Typevars can vary
...
`codified within PEP 484 <484#support-for-singleton-types-in-unions>`{.interpreted-text
role="pep"}. However, many type checkers (such as mypy) do not yet
implement this due to the expected complexity of the implementation
work.
...
`591`{.interpreted-text role="pep"} proposes adding a \"Final\"
qualifier to the typing ecosystem. This qualifier can be used to declare
that some variable or attribute cannot be reassigned:
...
If both this PEP and `591`{.interpreted-text role="pep"} are accepted,
type checkers are expected to support this shortcut. Specifically, given
a variable or attribute assignment of the form `var: Final = value`
where `value` is a valid parameter for `Literal[...]`, type checkers
should understand that `var` may be used in any context that expects a
`Literal[value]`.
...
dependent type system to the `484`{.interpreted-text role="pep"}
ecosystem. One obvious extension would be to implement a full-fledged
dependent type system that lets users predicate types based on their
values in arbitrary ways. That would let us write signatures like the
below:
