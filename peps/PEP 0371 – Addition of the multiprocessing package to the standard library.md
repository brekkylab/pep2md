...
post_history:
- '`03-Jun-2008 <https://mail.python.org/pipermail/python-dev/2008-June/080011.html>`__'
python_status: Final
url: https://peps.python.org/pep-0371/
source_path: https://github.com/python/peps/blob/main/peps/pep-0371.rst
source_commit: 96931881714b32cc1943bcd3fa2e3d74a345ed3b
---

# Abstract

This PEP proposes the inclusion of the `pyProcessing`[^1] package into
...
planned[^2], the GIL will remain as-is within the CPython interpreter
for the foreseeable future. While the GIL itself enables clean and easy
to maintain C code for the interpreter and extensions base, it is
frequently an issue for those Python programmers who are leveraging
multi-core machines.
...
albeit in a `8`{.interpreted-text role="pep"} compliant manner, that of
the threading API, with known semantics and easy scalability.
...
Obviously, with the renaming of the API to `8`{.interpreted-text
role="pep"} compliance there would be additional renaming which would
need to occur within user applications, however minor.
...
In the discussion on Python-Dev about the inclusion of this package[^3]
...
communication[^4], CORBA, or other distributed object approaches[^5].
...
target function through a single threaded (linear), multi-threaded (via
...
distribution\'s examples/ directory. The examples will be included in
...
not `8`{.interpreted-text role="pep"} compliant. It has been decided
that instead of adding the package \"as is\" and therefore perpetuating
the non-`8`{.interpreted-text role="pep"} compliant naming, we will
rename all APIs, classes, etc to be fully `8`{.interpreted-text
role="pep"} compliant.
...
APIs for the threading module - the current one, and the
`8`{.interpreted-text role="pep"} compliant one. Warnings about the
upcoming removal of the original java-style API will be issued when -3
is invoked.

In Python 3000, the threading API will become `8`{.interpreted-text
role="pep"} compliant, which means that the multiprocessing module and
the threading module will again have matching APIs.
...
- DONE: Also note that the default behavior of process spawning does not
  make it compatible with use within IDLE as-is, this will be examined
  as a bug-fix or \"setExecutable\" enhancement.
- DONE: Add in \"multiprocessing.setExecutable()\" method to override
  ...
  current executable name rather than the Python interpreter. Note that
  Mark Hammond has suggested a factory-style interface for this[^7].
...
[^1]: The 2008 era PyProcessing project (the pyprocessing name was since
    repurposed)
...
[^6]: The original run_benchmark.py code was published in Python
    ...
    Interpreter Lock\" by Jesse Noller. It has been modified for this
    PEP.
