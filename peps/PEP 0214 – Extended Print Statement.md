...
source_path: https://github.com/python/peps/blob/main/peps/pep-0214.rst
source_commit: 5bea38f3d882dc6590efedf98a4f53c42182bfdc
---

# Introduction

This PEP describes a syntax to extend the standard \'print\' statement
so that it can be used to print to any file-like object, instead of the
...
which simply pop the file like object off the top of the stack and use
it instead of `sys.stdout` as the output stream.
...
- Challenge: Why not one of these?
  ...
  which form of print statement is used. (The Python parser is a simple
  LL(1) or recursive descent parser.)
  ...
  would definitely choose \'\>\>\'. In part because \'in\' would be a
  new invention (I know of no other language that uses it, while
  \'\>\>\' is used in sh, awk, Perl, and C++), in part because \'\>\>\',
  being non-alphabetic, stands out more so is more likely to catch the
  reader\'s attention.

- Challenge: Why does there have to be a comma between the file and the
  rest?

  Response: The comma separating the file from the following expression
  is necessary! Of course you want the file to be an arbitrary
  expression, not just a single word. (You definitely want to be able to
  write `print >>sys.stderr`.) Without the expression the parser
  wouldn\'t be able to distinguish where that expression ends and where
  the next one begins, e.g.
...
- Challenge: Why do you need a syntax extension? Why not writeln(file,
  ...
  to decide between \"print(item, \...)\" and \"print item, \...\". I
  chose to make it a statement because printing needs to be taught very
  early on, and is very important in the programs that beginners write.
  Also, because ABC, which lead the way for so many things, made it a
...
- Challenge: Surely you can design a `println()` function can do all
  ...
  file. With the new syntax, the programmer only needs to learn one new
  thing: `print >> file`, and the answer can be like this:
