...
generated_at: '2026-04-25T02:55:22+00:00'
---
...
Council described in `13`{.interpreted-text role="pep"}, in their role
as the final authorities on whether or not PEPs will be accepted or
rejected.
...
& Workflow](#pep-editor-responsibilities-workflow) for details.
...
workflow can be conducted via the GitHub [PEP
repository](https://github.com/python/peps) issues and pull requests.
...
directly to the [Python issue
tracker](https://github.com/python/cpython/issues). The PEP editors
reserve the right to reject PEP proposals if they appear too unfocused
or too broad. If in doubt, split your PEP into several well-focused
ones.
...
whether the idea is PEP-able. Posting to the [Ideas
category](https://discuss.python.org/c/ideas/) of the [Python
Discourse](https://discuss.python.org/) is usually the best way to go
about this, unless a more specialized venue is appropriate, such as the
[Typing category](https://discuss.python.org/c/typing/) (for static
typing ideas) or [Packaging
category](https://discuss.python.org/c/packaging/) (for packaging ideas)
...
of the GitHub \"PEP editors\" team and members of the Typing Council
(`729`{.interpreted-text role="pep"}) are pre-approved to be sponsors.
The sponsor\'s job is to provide guidance to the PEP author to help them
...
becoming a co-author or PEP-Delegate later on (but not both). The
...
PEP via a [GitHub pull request](https://github.com/python/peps/pulls).
The draft must be written in PEP style as described below, else it will
fail review immediately (although minor errors may be corrected by the
editors).

The standard PEP workflow is:

- You, the PEP author, fork the [PEP
  repository](https://github.com/python/peps), and create a file named
  `pep-{NNNN}.rst`{.interpreted-text role="file"} that contains your new
  PEP. `{NNNN}`{.interpreted-text role="samp"} should be the next
...
- In the \"Type:\" header field, enter \"Standards Track\",
  \"Informational\", or \"Process\" as appropriate, and for the
  \"Status:\" field enter \"Draft\". For full details, see [PEP Header
  Preamble](#pep-header-preamble).

- Update
  [.github/CODEOWNERS](https://docs.github.com/en/repositories/managing-your-repositorys-settings-and-features/customizing-your-repository/about-code-owners)
  such that any co-author(s) or sponsors with write access to the [PEP
  repository](https://github.com/python/peps) are listed for your new
  file. This ensures any future pull requests changing the file will be
  assigned to them.

- Push this to your GitHub fork and submit a pull request.

- The PEP editors review your PR for structure, formatting, and other
  errors. For a reST-formatted PEP, `12`{.interpreted-text role="pep"}
  is provided as a template. It also provides a complete introduction to
  reST markup that is used in PEPs. Approval criteria are:
  ...
  - The PEP\'s language (spelling, grammar, sentence structure, etc.)
    and code style (examples should match `7`{.interpreted-text
    role="pep"} & `8`{.interpreted-text role="pep"}) should be correct
    and conformant. The PEP text will be automatically checked for
    correct reStructuredText formatting when the pull request is
    submitted. PEPs with invalid reST markup will not be approved.
...
Developers with write access to the [PEP
repository](https://github.com/python/peps) may claim PEP numbers
...
the PEP editors (see [PEP Editor Responsibilities &
Workflow](#pep-editor-responsibilities-workflow)). This includes
ensuring the initial version meets the expected standards for submitting
a PEP. Alternately, even developers should submit PEPs via pull request.
When doing so, you are generally expected to handle the process
yourself; if you need assistance from PEP editors, mention
...
repository](https://github.com/python/peps). Getting a PEP number
assigned early can be useful for ease of reference, especially when
multiple draft PEPs are being considered at the same time.
...
to the [PEP repository](https://github.com/python/peps), a discussion
thread for the PEP should be created to provide a central place to
discuss and review its contents, and the PEP should be updated so that
the `Discussions-To` header links to it.
...
- The discussion is subject to the [Python Community Code of
...
The [PEPs category](https://discuss.python.org/c/peps/) of the [Python
Discourse](https://discuss.python.org/) is the preferred choice for most
new PEPs, whereas historically the
[Python-Dev](https://mail.python.org/mailman3/lists/python-dev.python.org/)
mailing list was commonly used. Some specialized topics have specific
venues, such as the [Typing
category](https://discuss.python.org/c/typing/) and the [Packaging
category](https://discuss.python.org/c/packaging/) on the Python
Discourse for typing and packaging PEPs, respectively. If the PEP
authors are unsure of the best venue, the PEP Sponsor and PEP editors
...
occurs, the `Discussions-To` link must be updated and a new
`Post-History` entry added pointing to this new thread.

If it is not chosen as the discussion venue, a brief announcement post
should be made to the [PEPs
category](https://discuss.python.org/c/peps/) with at least a link to
the rendered PEP and the `Discussions-To` thread when the draft PEP is
...
issue](https://github.com/python/steering-council/issues/new/choose)
...
offer to serve as its PEP-Delegate by [notifying the Steering
Council](https://github.com/python/steering-council/issues/new/choose)
of their intent. If the Steering Council approves their offer, the
PEP-Delegate will then have the authority to approve or reject that PEP.
For PEPs related to the Python type system, the Typing Council
(`729`{.interpreted-text role="pep"}) provides a recommendation to the
Steering Council. To request such a recommendation, open an issue on the
[Typing Council issue
tracker](https://github.com/python/typing-council/issues).
...
\"PEP-Delegate\" field in the PEP\'s header. The term \"BDFL-Delegate\"
is a deprecated alias for PEP-Delegate, a legacy of the time when when
Python was led by [a BDFL](#pythons-bdfl). Any legacy references to
...
a [new
issue](https://github.com/python/steering-council/issues/new/choose) ).
Those taking on this responsibility are free to seek additional guidance
from the Steering Council at any time, and are also expected to take the
advice and perspectives of other core developers into account.
...
Steering Council. This logic is intentionally circular.) See
`2`{.interpreted-text role="pep"} for standard library module acceptance
criteria.

Except where otherwise approved by the Steering Council, pronouncements
of PEP resolution will be posted to the [PEPs
category](https://discuss.python.org/c/peps/) on the [Python
...
proposal to avoid the need to rely on the \"Provisional\" status (e.g.
by deferring some features to later PEPs), as this status can lead to
version compatibility challenges in the wider Python ecosystem.
`411`{.interpreted-text role="pep"} provides additional details on
potential use cases for the Provisional status.
...
![PEP process flow diagram](pep-0001/process_flow.svg){.invert-in-dark-mode
.invert-in-dark-mode}

While not shown in the diagram, \"Accepted\" PEPs may technically move
to \"Rejected\" or \"Withdrawn\" even after acceptance. This will only
...
if they are never meant to be completed. E.g. `1`{.interpreted-text
role="pep"} (this PEP).
...
Reference](https://docs.python.org/3/reference/index.html) for core
features, the [Library
Reference](https://docs.python.org/3/library/index.html) for standard
library modules or the [PyPA
...
1.  Preamble \-- `2822`{.interpreted-text role="rfc"} style headers
    containing meta-data about the PEP, including the PEP number, a
    short descriptive title (limited to a maximum of 44 characters), the
    names, and optionally the contact info for each author, etc.
...
9.  Reference Implementation \-- The reference implementation must be
    ...
    be completed before the PEP is accepted. While there is merit to the
    ...
    code\" is still useful when it comes to resolving many discussions
    of API details.
...
12. Acknowledgements \-- Useful to thank and acknowledge people who have
    helped develop, discuss, or draft the PEP, or for any other purpose.
    The section can be used to recognise contributors to the work who
    are not co-authors.
...
:   discussions and feedback. Think of this as a \"changelog\" or
    ...
    discretion, but such a section can be helpful for those following
...
PEPs are UTF-8 encoded text files using the
[reStructuredText](https://www.sphinx-doc.org/en/master/usage/restructuredtext/index.html)
format. reStructuredText allows for rich markup that is still quite easy
to read, but also results in good-looking and functional HTML.
`12`{.interpreted-text role="pep"} contains instructions and a
`PEP template <12#suggested-sections>`{.interpreted-text role="pep"}.

The PEP text files are automatically [converted to
HTML](https://peps.python.org/docs/rendering_system/) (via a
[Sphinx](https://www.sphinx-doc.org/)-based
`build system <676>`{.interpreted-text role="pep"}) for easier [online
reading](https://peps.python.org/).

# PEP Header Preamble

Each PEP must begin with an `2822`{.interpreted-text role="rfc"} style
header preamble. The headers must appear in the following order. Headers
marked with \"\*\" are optional and are described below. All other
...
following `2822`{.interpreted-text role="rfc"} continuation line
conventions. Note that personal email addresses in PEPs will be obscured
as a defense against spam harvesters.
...
link to the thread in the list\'s archives, rather than just a mailto:
or hyperlink to the list itself.
...
belongs under. See the `topic-index`{.interpreted-text role="ref"} for
the existing topics.
...
should generally be first proposed on the PEP\'s discussion thread
listed in its `Discussions-To` header, while copyedits and corrections
can be submitted as a [GitHub
issue](https://github.com/python/peps/issues) or [GitHub pull
request](https://github.com/python/peps/pulls). PEP authors with write
access to the PEP repository can update the PEPs themselves by using
`git push` or a GitHub PR to submit their changes. For guidance on
modifying other PEPs, consult the [PEP Maintenance](#pep-maintenance)
section.

See the [Contributing
Guide](https://github.com/python/peps/blob/main/CONTRIBUTING.rst) for
additional details, and when in doubt, please check first with the PEP
author and/or a PEP editor.
...
the \'net (i.e. is unreachable or not responding to email). A bad reason
to transfer ownership is because the author doesn\'t agree with the
direction of the PEP. One aim of the PEP process is to try to build
consensus around a PEP, but if that\'s not possible, an author can
always submit a competing PEP.

If you are interested in assuming ownership of a PEP, you can also do
this via pull request. Fork the [PEP
repository](https://github.com/python/peps), make your ownership
...
and must watch the [PEP repository](https://github.com/python/peps).
...
- Make sure that the PEP is either co-authored by a core developer, has
  ...
  for this PEP by the Steering Council.
...
- Ensure that everyone listed as a sponsor or co-author of the PEP who
  has write access to the [PEP
  repository](https://github.com/python/peps) is added to
  [.github/CODEOWNERS](https://docs.github.com/en/repositories/managing-your-repositorys-settings-and-features/customizing-your-repository/about-code-owners).
- Skim the PEP for obvious defects in language (spelling, grammar,
  ...
  `7`{.interpreted-text role="pep"} & `8`{.interpreted-text
  role="pep"}). Editors may correct problems themselves, but are not
  required to do so (reStructuredText syntax is checked by the repo\'s
  CI).
- If a project is portrayed as benefiting from or supporting the PEP,
  make sure there is some direct indication from the project included to
  make the support clear. This is to avoid a PEP accidentally portraying
  a project as supporting a PEP when in fact the support is based on
  conjecture.
...
ask the author(s) to use `12`{.interpreted-text role="pep"} as a
template and resubmit.
...
- Check that the author has correctly labeled the PEP\'s type
  (\"Standards Track\", \"Informational\", or \"Process\"), and marked
  its status as \"Draft\".

- Ensure all CI build and lint checks pass without errors, and there are
  no obvious issues in the rendered preview output.
...
Updates to existing PEPs should be submitted as a [GitHub pull
request](https://github.com/python/peps/pulls).
...
- 2026-02-02
  - Added an optional `Change History` section for PEPs, for summarizing
    changes when updating the `Post-History` header.
