...
source_path: https://github.com/python/peps/blob/main/peps/pep-0396.rst
source_commit: 489d0292b96c97e3970663d5a38438b14630ea6e
---

::: withdrawn
Refer to `packaging:runtime-version-access`{.interpreted-text
role="ref"} and `packaging:single-source-version`{.interpreted-text
role="ref"} in the Python Packaging User Guide for up to date
recommendations on accessing package version information at runtime, and
on defining runtime `__version__` attributes which are automatically
...
first written, and APIs such as `importlib.metadata.version()`[^2]
provide for a much better experience.

This rejection was reclassified as a withdrawal on 2024-10-21, as the
previous state was being misinterpreted[^3] as suggesting that *no*
modules should be defining `__version__` attributes, which definitely
isn\'t the case.

Modules are still free to define `__version__` if they choose to.
However, choosing *not* to do so won\'t interfere with looking up the
version information for installed distribution packages, so an
Informational PEP isn\'t the right tool to document community
conventions around the use of module `__version__` attributes (they\'re
better covered as part of the Python Packaging User Guide).
...
users. Bob learns about the Cheeseshop[^4], and adds some simple
...
inspecting an attribute. Classic Python distutils `setup()`
functions[^5] describe a `version` argument where the release\'s version
number can be specified. `8`{.interpreted-text role="pep"} describes the
...
the PEP author\'s own email archives, the earliest example of the use of
an `__version__` module attribute by independent module developers dates
back to 1995.

Another example of version information is the sqlite3[^6] module with
its `sqlite_version_info`, `version`, and `version_info` attributes. It
may not be immediately obvious which attribute contains a version number
for the module, and which contains a version number for the underlying
SQLite3 library.
...
6.  Module version numbers SHOULD conform to the normalized version
    format specified in `386`{.interpreted-text role="pep"}.
...
8.  The `version` attribute in a classic distutils `setup.py` file, or
    the `345`{.interpreted-text role="pep"} `Version` metadata field
    SHOULD be derived from the `__version__` field, or vice versa.
...
In consultation with the distutils-sig[^7], two options are proposed.
...
use the `pkgutil` module[^8] to make it available programmatically. E.g.
in `elle.py`:
...
`376`{.interpreted-text role="pep"} defines a standard for static
metadata, but doesn\'t describe the process by which this metadata gets
created. It is highly desirable for the derived version information to
be placed into the `376`{.interpreted-text role="pep"} `.dist-info`
...
[^3]: Misinterpreting the significance of this PEP\'s rejection
    (<https://discuss.python.org/t/please-make-package-version-go-away/58501>)

[^4]: The Cheeseshop (Python Package Index) (<http://pypi.python.org>)

[^5]: <http://docs.python.org/distutils/setupscript.html>

[^6]: sqlite3 module documentation
    (<http://docs.python.org/library/sqlite3.html>)

[^7]: <https://mail.python.org/pipermail/distutils-sig/2011-June/017862.html>
