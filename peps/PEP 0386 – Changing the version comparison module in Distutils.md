...
dependency specification scheme defined in `440`{.interpreted-text
role="pep"}.
...
metadata field (see `345`{.interpreted-text role="pep"}) and it will
optionally allow use of that field to restrict the dependency to a set
of compatible versions. Notice that this field is replacing `Requires`
that was expressing dependencies on modules and packages.
...
2.  a significant number of projects need special meaning versions for
    \"pre-releases\" (such as \"alpha\", \"beta\", \"rc\"), and these
    have widely used aliases (\"a\" stands for \"alpha\", \"b\" for
    \"beta\" and \"c\" for \"rc\"). And these pre-release versions make
    it impossible to use a simple alphanumerical ordering of the version
    string components. (Example: 3.1a1 \< 3.1)
3.  some projects also need \"post-releases\" of regular versions,
    mainly for installer work which can\'t be clearly expressed
    otherwise.
...
algorithm to convert the strings to sortable keys, with a
...
it doesn\'t enforce any policy and leads to very complex semantics due
to the lack of a clear standard. It just tries to adapt to widely used
conventions.
...
`easy_install`[^5], `pip`[^6] or `zc.buildout` [^7] to install
...
attempt to switch to a more typical \"major.minor\...\" version scheme
is problematic because it will always sort less than \"20090421\".
...
there (e.g. Twisted[^9]). For example, *after* a `1.2.0` release there
might be a `1.2.0-r678` release. We used `post` instead of `r` because
the `r` is ambiguous as to whether it indicates a pre- or post-release.
...
from the original version, it\'s a suggested normalized version. Last,
if it returns the same string, it means that the version matches the
scheme.
...
[^1]: <https://docs.python.org/3.11/distutils/>

[^2]: <https://peak.telecommunity.com/DevCenter/setuptools>

[^3]: <https://peak.telecommunity.com/DevCenter/setuptools#specifying-your-project-s-version>

[^4]: <https://pypi.org/>

[^5]: <https://peak.telecommunity.com/DevCenter/EasyInstall>

[^6]: <https://pypi.org/project/pip/>

[^7]: <https://pypi.org/project/zc.buildout/>

[^8]: <https://web.archive.org/web/20090726093825/http://bitbucket.org/tarek/distutilsversion/>
