...
bdfl_delegate: Eric Snow <ericsnowcurrently@gmail.com>
...
source_commit: f5bcbc04d66dd9d2c3840b4c7cc5e82054a583ad
---

::: canonical-doc
`py3.13:initializing-modules`{.interpreted-text role="ref"}. For Python
3.14+, see `python:extension-modules`{.interpreted-text role="ref"} and
`python:pymoduledef`{.interpreted-text role="ref"}
:::
...
ModuleSpec-based loading mechanism introduced in `451`{.interpreted-text
role="pep"}.

This proposal draws inspiration from `PyType_Spec` of
`384`{.interpreted-text role="pep"} to allow extension authors to only
define features they need, and to allow future additions to extension
module declarations.

Extensions modules are created in a two-step process, fitting better
into the ModuleSpec architecture, with parallels to `__new__` and
`__init__` of classes.
...
Not all problems tackled in `3121`{.interpreted-text role="pep"} are
solved in this proposal. In particular, problems with run-time module
lookup (`PyState_FindModule`) are left to a future PEP.
...
then the module code is being executed (`302`{.interpreted-text
role="pep"}). A ModuleSpec object (`451`{.interpreted-text role="pep"})
is used to hold information about the module, and passed to the relevant
hooks.
...
ModuleSpec, or any information it contains, such as the `__file__` or
...
\"`__init__.py`\" modules, i.e. packages, especially when relative
imports are being used at module init time.
...
the goal of `3121`{.interpreted-text role="pep"}, but many extensions,
including some in the standard library, took the least-effort approach
to porting to Python 3, leaving these issues unresolved. This PEP keeps
backwards compatibility, which should reduce pressure and give extension
authors adequate time to consider these issues when porting.
...
The initialization function (`PyInit_modulename`) will be allowed to
...
similarly to `384`{.interpreted-text role="pep"}\'s `PyType_Spec` for
types. To keep binary compatibility, and avoid needing to introduce a
new structure (which would introduce additional supporting functions and
per-module storage), the currently unused *m_reload* pointer of
`PyModuleDef` will be changed to hold the slots. The structures are
defined as:
...
When using multi-phase initialization, the *m_name* field of
`PyModuleDef` will not be used during importing; the module name will be
taken from the ModuleSpec.

Before it is returned from [PyInit]()*, the \`\`PyModuleDef\`\` object
must be initialized using the newly added \`\`PyModuleDef_Init\`\`
function. This sets the object type (which cannot be done statically on
certain compilers), refcount, and internal bookkeeping data
(*m_index\*). For example, an extension module \"example\" would be
exported as:
...
The `PyModuleDef` object must be available for the lifetime of the
module created from it -- usually, it will be declared statically.
...
`451#how-loading-will-work`{.interpreted-text role="pep"}.
...
`ExecutionLoader.create_module` -- is governed by the `Py_mod_create`
slot.
...
`451`{.interpreted-text role="pep"}, and the `PyModuleDef` structure. It
should return a new module object, or set an error and return NULL.

This function is not responsible for setting import-related attributes
specified in `451#attributes`{.interpreted-text role="pep"} (such as
`__name__` or `__loader__`) on the new module.
...
authors are advised to keep `Py_mod_create` minimal, an in particular to
...
normal module object using `PyModule_New`. The name is taken from
*spec*.

### Post-creation steps

If the `Py_mod_create` function returns an instance of
`types.ModuleType` or a subclass (or if a `Py_mod_create` slot is not
present), the import machinery will associate the `PyModuleDef` with the
module. This also makes the `PyModuleDef` accessible to execution phase,
the `PyModule_GetDef` function, and garbage collection routines
(traverse, clear, free).

If the `Py_mod_create` function does not return a module subclass, then
*m_size* must be 0, and *m_traverse*, *m_clear* and *m_free* must all be
NULL. Otherwise, `SystemError` is raised.

Additionally, initial attributes specified in the `PyModuleDef` are set
on the module object, regardless of its type:
...
This PEP only adds one, `Py_mod_exec`, but others may be added in the
future.

The execution phase is done on the `PyModuleDef` associated with the
module object. For objects that are not a subclass of `PyModule_Type`
(for which `PyModule_GetDef` would fail), the execution phase is
skipped.
...
in `451#attributes`{.interpreted-text role="pep"} (such as `__name__` or
...
setting the module\'s initial attributes. The \"module\" argument
receives the module object to initialize.
...
`PyInit` hook implements the creation phase, and the execution phase is
a no-op.
...
correctly, avoiding the issues mentioned in Python documentation[^1].
The mechanism is designed to make this easy, but care is still required
on the part of the extension author. No user-defined functions, methods,
or instances may leak to different interpreters. To achieve this, all
module-level state should be kept in either the module dict, or in the
module object\'s storage reachable by `PyModule_GetState`. A simple rule
of thumb is: Do not define any static data, except built-in types with
no mutable or user-settable class attributes.
...
The `PyState_FindModule` function will return NULL, and
`PyState_AddModule` and `PyState_RemoveModule` will also fail on modules
with non-NULL *m_slots*. PyState registration is disabled because
multiple module objects may be created from the same `PyModuleDef`.

## Module state and C-level callbacks

Due to the unavailability of `PyState_FindModule`, any function that
...
have been discussed on the import-sig mailing list[^2].

As a rule of thumb, modules that rely on `PyState_FindModule` are, at
the moment, not good candidates for porting to the new mechanism.

## New Functions

A new function and macro implementing the module creation phase will be
added. These are similar to `PyModule_Create` and `PyModule_Create2`,
...
Another function will be introduced to initialize a `PyModuleDef`
object. This idempotent function fills in the type, refcount, and module
index. It returns its argument cast to `PyObject*`, so it can be
returned directly from a `PyInit` function:
...
named `PyInitU_<encodedname>`, where the name is encoded using
CPython\'s \"punycode\" encoding (`Punycode <3492>`{.interpreted-text
role="rfc"} with a lowercase suffix), with hyphens (\"-\") replaced by
underscores (\"\_\").
...
Reloading an extension module using `importlib.reload()` will continue
to have no effect, except re-setting import-related attributes.
...
`384`{.interpreted-text role="pep"}, allows later extensions.
...
this case would need a `Py_mod_exec` function that runs before the slots
...
when the module is given to Python\'s `-m`{.interpreted-text
role="program"} switch. For this to work, the `runpy` module will need
to be modified to take advantage of ModuleSpec-based loading introduced
in `451`{.interpreted-text role="pep"}. Also, it will be necessary to
add a mechanism for setting up a module according to slots it wasn\'t
originally defined with.

# Implementation

Work-in-progress implementation is available in a GitHub repository[^3];
a patchset is at[^4].

# Previous Approaches

Stefan Behnel\'s initial proto-PEP[^5] had a \"`PyInit_modulename`\"
hook that would create a module class, whose `__init__` would be then
called to create the module. This proposal did not correspond to the
(then nonexistent) `451`{.interpreted-text role="pep"}, where module
...
Alyssa (Nick) Coghlan proposed \"`Create`\" and \"`Exec`\" hooks, and
wrote a prototype implementation[^6]. At this time
`451`{.interpreted-text role="pep"} was still not implemented, so the
prototype does not use ModuleSpec.

The original version of this PEP used `Create` and `Exec` hooks, and
allowed loading into arbitrary pre-constructed objects with `Exec` hook.
...
A further iteration used a \"`PyModuleExport`\" hook as an alternative
to `PyInit`, where `PyInit` was used for existing scheme, and
`PyModuleExport` for multi-phase. However, not being able to determine
the hook name based on module name complicated automatic generation of
`PyImport_Inittab` by tools like freeze. Keeping only the `PyInit` hook
