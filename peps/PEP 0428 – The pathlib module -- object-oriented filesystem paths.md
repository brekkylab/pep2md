...
python_version: '3.4'
post_history:
- '`05-Oct-2012 <https://mail.python.org/pipermail/python-ideas/2012-October/016338.html>`__'
resolution: https://mail.python.org/pipermail/python-dev/2013-November/130424.html
python_status: Final
url: https://peps.python.org/pep-0428/
source_path: https://github.com/python/peps/blob/main/peps/pep-0428.rst
source_commit: 449c533a9debe257bbb4cc4d3d1db1fc8f585a53
---

# Abstract

This PEP proposes the inclusion of a third-party module,
[pathlib](http://pypi.python.org/pypi/pathlib/), in the standard
library. The inclusion is proposed under the provisional label, as
described in `411`{.interpreted-text role="pep"}. Therefore, API changes
can be done, either as part of the PEP process, or after acceptance in
the standard library (and until the provisional label is removed).
...
and rejected in `355`{.interpreted-text role="pep"}. Several third-party
implementations of the idea of object-oriented filesystem paths exist in
the wild:

- The historical [path.py module](https://github.com/jaraco/path.py) by
  Jason Orendorff, Jason R. Coombs and others, which provides a
  `str`-subclassing `Path` class;
- Twisted\'s slightly specialized [FilePath
  class](http://twistedmatrix.com/documents/current/api/twisted.python.filepath.FilePath.html);
- An [AlternativePathClass
  proposal](http://wiki.python.org/moin/AlternativePathClass),
  subclassing `tuple` rather than `str`;
- [Unipath](https://bitbucket.org/sluggo/unipath/overview), a variation
  on the str-subclassing approach with two public classes, an
  `AbstractPath` class for operations which don\'t do I/O and a `Path`
  class for all common operations.

This proposal attempts to learn from these previous attempts and the
rejection of `355`{.interpreted-text role="pep"}.

# Implementation

The implementation of this proposal is tracked in the `pep428` branch of
pathlib\'s [Mercurial
repository](https://bitbucket.org/pitrou/pathlib/).
...
The [pathlib](http://pypi.python.org/pypi/pathlib/) module implements a
...
- a path class is of a given flavour according to the kind of operating
  system paths it represents.
  [pathlib](http://pypi.python.org/pypi/pathlib/) implements two
...
operations with strings by not subclassing it. A [blog
post](http://www.snarky.ca/why-pathlib-path-doesn-t-inherit-from-str) by
...
> \>\>\> p = PurePosixPath(\'/usr/bin\') \>\>\> p.match(\'/usr/b\*\')
> True \>\>\> p.match(\'usr/b\*\') True \>\>\> p.match(\'b\*\') True
> \>\>\> p.match(\'/u\*\') False
...
docs](https://pathlib.readthedocs.org/en/pep428/).

# Discussion

## Division operator

The division operator came out first in a
[poll](https://mail.python.org/pipermail/python-ideas/2012-October/016544.html)
about the path joining operator. Initial versions of
[pathlib](http://pypi.python.org/pypi/pathlib/) used square brackets
...
> \"If glob(\"\*.py\") failed to find SETUP.PY on Windows, that would be
