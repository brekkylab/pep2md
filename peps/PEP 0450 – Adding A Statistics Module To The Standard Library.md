...
source_path: https://github.com/python/peps/blob/main/peps/pep-0450.rst
source_commit: 552a343f7641ff59e540e67ba5faff93698f1419
---
...
libraries and ad hoc code.[^1] Statistical functions such as mean,
standard deviation and others are obvious and useful batteries, familiar
to any Secondary School student. Even cheap scientific calculators
typically include multiple statistical functions such as:
...
- It may be overkill for many purposes. The documentation for numpy even
  ...
  > \"It can be hard to know what functions are available in numpy. This
  > is not a complete list, but it does cover most of them.\"[^5]
...
- For many people, installing numpy may be difficult or impossible. For
  ...
  third-party software. For the casual Python user, having to learn
  about installing third-party packages in order to average a list of
  numbers is unfortunate.
...
- Concentrate on data in sequences, allowing two-passes over the data,
  ...
  one-pass algorithm. Functions expect data will be passed as a list or
  other sequence; if given an iterator, they may internally convert to a
  list.
...
- Although functions support data sets of floats, Decimals or Fractions,
  there is no guarantee that *mixed* data sets will be supported. (But
  on the other hand, they aren\'t explicitly rejected either.)
- Plenty of documentation, aimed at readers who understand the basic
  ...
  contradictory/confusing definitions in use, it is only fair that I do
  my best to clarify rather than obfuscate the topic.
...
- 
  ...
  :   sample standard deviation of *data*, optionally using *xbar* as
      the sample mean.
...
`median`, `median.low`, `median.high` etc. Although there is at least
one existing use of this style in the standard library, in
