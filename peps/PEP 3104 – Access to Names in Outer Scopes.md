...
source_path: https://github.com/python/peps/blob/main/peps/pep-3104.rst
source_commit: 53175ceb3ab726e3b5323dd599cccf780b1f54eb
---
...
names bound in all enclosing scopes (see `227`{.interpreted-text
role="pep"}). This change makes the above code example work as expected.
However, because any assignment to a name implicitly declares that name
to be local, it is impossible to rebind a name in an outer scope (except
when a `global` declaration forces the name to be global). Thus, the
following code, intended to display a number that can be incremented and
decremented by clicking buttons, doesn\'t work as someone familiar with
lexical scoping might expect:
...
\"multi-paradigm language\" because it derives so much strength,
practical flexibility, and pedagogical power from its support and
graceful integration of multiple programming paradigms.

A proposal for scoping syntax appeared on Python-Dev as far back as
1994[^1], long before `227`{.interpreted-text role="pep"}\'s support for
nested scopes was adopted. At the time, Guido\'s response was:

> This is dangerously close to introducing CSNS \[classic static nested
> scopes\]. *If* you were to do so, your proposed semantics of scoped
> seem alright. I still think there is not enough need for CSNS to
> warrant this kind of construct \...

After `227`{.interpreted-text role="pep"}, the \"outer name rebinding
discussion\" has reappeared on Python-Dev enough times that it has
become a familiar event, having recurred in its present form since at
least 2003[^2]. Although none of the language changes proposed in these
discussions have yet been adopted, Guido has acknowledged that a
language change is worth considering[^3].
...
`var x = 3` would both declare `x` to belong to the local scope and bind
it, where as `x = 3` would rebind the existing visible `x`. In a context
without an enclosing scope containing a `var x` declaration, the
statement `x = 3` would be statically determined to be illegal.
...
has been suggested for this is `.x`[^10], which would refer to `x`
...
would confuse the reader[^11]. A closely related idea is to use multiple
dots to specify the number of scope levels to ascend[^12], but most
consider this too error-prone [^13].
...
`x = 3` both declares `x` a local variable and binds it to 3, the
statement `x := 3` would change the existing binding of `x` without
declaring it local.

This is a simple solution, but according to `3099`{.interpreted-text
role="pep"} it has been rejected (perhaps because it would be too easy
to miss or to confuse with `=`).
...
to mean a variable with top-level scope[^35]. In C, the keyword `extern`
means that a name refers to a variable in a different compilation unit.
While `nonlocal` is a bit long and less pleasant-sounding than some of
the other options, it does have precisely the correct meaning: it
declares a name not local.
...
errors, including, for example, \_\_future\_\_ import with an unknown
...
This PEP targets Python 3000, as suggested by Guido[^42]. However,
others have noted that some options considered in this PEP may be small
enough changes to be feasible in Python 2.x[^43], in which case this PEP
...
[^40]: Issue 4199: combining assignment with global & nonlocal (Guido
    van Rossum)
