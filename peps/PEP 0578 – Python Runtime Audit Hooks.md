...
bdfl_delegate: Christian Heimes <christian@python.org>
status: Final
...
source_path: https://github.com/python/peps/blob/main/peps/pep-0578.rst
source_commit: 4b8a5ce81e4261d665adccc2893302e889e1e41a
---

::: canonical-doc
`python:audit-events`{.interpreted-text role="ref"}
:::
...
See `551`{.interpreted-text role="pep"} for discussion and
recommendations on enhancing the security of a Python runtime making use
of these auditing APIs.
...
which operations provide audit events.
  ...
  `exec`, `eval`, `run_mod`        `exec`                             `(code_object,)`                                                Detect dynamic execution of code
  ...
  `_ctypes._CData`           `ctypes.cdata`           `(ptr_as_int,)`                        Detect when code is accessing
...
article](https://danielmiessler.com/p/security-by-obscurity/) for
discussion).
...
This API was originally presented as part of `551`{.interpreted-text
role="pep"} Security Transparency in the Python Runtime.
...
`551`{.interpreted-text role="pep"} is an informational PEP discussing
how to integrate Python into a secure or audited environment.
