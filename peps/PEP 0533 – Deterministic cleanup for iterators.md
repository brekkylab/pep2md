...
author:
- Nathaniel J. Smith
bdfl_delegate: Yury Selivanov <yury@edgedb.com>
status: Deferred
...
source_path: https://github.com/python/peps/blob/main/peps/pep-0533.rst
source_commit: a4b83da82b9ffae4df42cd54add514db47b9864e
---
...
<333>`{.interpreted-text role="pep"} adds a `close` method on top of the
regular iterator protocol and demands that consumers call it at the
appropriate time (though forgetting to do so is a [frequent source of
bugs](http://blog.dscpl.com.au/2012/10/obligations-for-calling-close-on.html));
and `342`{.interpreted-text role="pep"} (based on
`325`{.interpreted-text role="pep"}) extended generator objects to add a
...
- Async generators (`525`{.interpreted-text role="pep"}) can only
  perform cleanup under the supervision of the appropriate coroutine
  runner. `__del__` doesn\'t have access to the coroutine runner;
  indeed, the coroutine runner might be garbage collected before the
  generator object. So relying on the garbage collector is effectively
  impossible without some kind of language extension.
  (`525`{.interpreted-text role="pep"} does provide such an extension,
  but it has a number of limitations that this proposal fixes; see the
...
be muddling along okay \-- but async generators make it much more
...
<525#finalization>`{.interpreted-text role="pep"} managed by new
...
generators. In principle, this proposal and `525`{.interpreted-text
role="pep"} are complementary, in the same way that `with` blocks and
`__del__` are complementary: this proposal takes care of ensuring
deterministic cleanup in most cases, while `525`{.interpreted-text
role="pep"}\'s GC hooks clean up anything that gets missed. But
...
- The GC hook semantics aren\'t part of the abstract async iterator
  ...
  generator concrete
  type](https://mail.python.org/pipermail/python-dev/2016-September/146129.html).
  ...
  then you can\'t refactor this into an async generator without changing
  its semantics, and vice-versa. This seems very unpythonic. (It also
  leaves open the question of what exactly class-based async iterators
  are supposed to do, given that they face exactly the same cleanup
  problems as async generators.) `__aiterclose__`, on the other hand, is
  defined at the protocol level, so it\'s duck-type friendly and works
  for all iterators, not just generators.
...
- The WSGI experience suggests that there exist important iterator-based
  APIs that need prompt cleanup and cannot rely on the GC, even in
  CPython. For example, consider a hypothetical WSGI-like API based
  around async/await and async iterators, where a response handler is an
  async generator that takes request headers + an async iterator over
  the request body, and yields response headers + the response body.
  (This is actually the use case that got me interested in async
  generators in the first place, i.e. this isn\'t hypothetical.) If we
  follow WSGI in requiring that child iterators must be closed properly,
  then without `__aiterclose__` the absolute most minimalistic
  middleware in our system looks something like:
...
code so that it calls `__aiterclose__`, e.g. by using a `with` block:
...
hooks arguably aren\'t worth it, given that the only people they help
are those who want to manually call `__anext__` yet don\'t want to
manually call `__aiterclose__`. But Yury disagrees with me on this :-).
And both options are viable.
...
have `__exit__` methods that call `self.close()`, but I\'m not aware of
any common cases that make use of `__exit__`\'s full power. I also
can\'t think of any examples where this would be useful. And it seems
unnecessarily confusing to allow iterators to affect flow control by
swallowing exceptions \-- if you\'re in a situation where you really
want that, then you should probably use a real `with` block anyway.

# Specification

This section describes where we want to eventually end up, though there
are some backwards compatibility issues that mean we can\'t jump
...
- `__del__` calls `self.close()` (same as now), and additionally issues
  a `ResourceWarning` if the generator wasn\'t exhausted. This warning
  is hidden by default, but can be enabled for those who want to make
  sure they aren\'t inadvertently relying on CPython-specific GC
  semantics.
...
- `__del__` issues a `RuntimeWarning` if `aclose` has not been called,
  since this probably indicates a latent bug, similar to the \"coroutine
...
`525`{.interpreted-text role="pep"} was accepted on a provisional basis,
and async generators are by far the biggest beneficiary of this PEP\'s
proposed changes. Therefore, I think we should strongly consider
enabling `__aiterclose__` for `async for` loops and async generators
ASAP, ideally for 3.6.0 or 3.6.1.
...
- Similarly, functions like `list` use stack introspection (!!) to check
  whether their direct caller has `__future__.iterclose` enabled, and
  use this to decide whether to call `__iterclose__` or
...
- For generators (and files, if we decide to do that),
  ...
  methods on the object are modified to check for this flag. If they
  find the flag set, they issue a `PendingDeprecationWarning` to inform
  the user that in the future this sequence would have led to a
