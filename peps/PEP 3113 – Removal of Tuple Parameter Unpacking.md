...
instead stores MSIL[^2] in `func_code.co_code` attribute of the
function. This fact prevents the `inspect.getargspec` function from
working when run under IronPython. It is unknown whether other Python
implementations are affected but is reasonable to assume if the
implementation is not just a re-implementation of the Python virtual
machine.

## No Loss of Abilities If Removed

As mentioned in [Introspection Issues](#introspection-issues), to handle
tuple parameters the function\'s bytecode starts with the bytecode
required to unpack the argument into the proper parameter names. This
means that there is no special support required to implement tuple
parameters and thus there is no loss of abilities if they were to be
removed, only a possible convenience (which is addressed in [Why They
Should (Supposedly) Stay](#why-they-should-supposedly-stay)).
...
Consider `3102`{.interpreted-text role="pep"} (keyword-only arguments)
and `3107`{.interpreted-text role="pep"} (function annotations). Both
PEPs have been accepted and introduce new functionality within a
function\'s signature. And yet for both PEPs the new feature cannot be
applied to tuple parameters as a whole. `3102`{.interpreted-text
role="pep"} has no support for tuple parameters at all (which makes
sense as there is no way to reference a tuple parameter by name).
`3107`{.interpreted-text role="pep"} allows annotations for each item
within the tuple (e.g., `(x:int, y:int)`), but not the whole tuple
(e.g., `(x, y):int`).
...
parameters. And as shown in [No Loss Of Abilities If
Removed](#no-loss-of-abilities-if-removed), their use is purely
practical and in no way provide a unique ability that cannot be handled
in other ways very easily.
...
Use](#practical-use), seeing `(x, y)` as a parameter in a function makes
