...
source_path: https://github.com/python/peps/blob/main/peps/pep-0209.rst
source_commit: 1ef758002854e4fb246a1ff5f6d21addeabe4715
---
...
Array, and ArrayView; and a low-level C-extension module,
[ufunc]{#ufunc}, to handle the array operations efficiently. In
addition, each array type has its own C-extension module which defines
the coercion rules, operations, and methods for that type. This design
enables new types, features, and functionality to be added in a modular
fashion. The new version will introduce some incompatibilities with the
current Numeric.
...
1.  Improved memory usage
    ...
    a.  Use a local coercion model
        ...
        local coercion model as described in `208`{.interpreted-text
        role="pep"} which defers the responsibility of coercion to the
        operator. By using internal buffers, a coercion operation can be
        done for each array (including output arrays), if necessary, at
        the time of the operation. Benchmarks[^1] have shown that
        performance is at most degraded only slightly and is improved in
        cases where the internal buffers are less than the L2 cache size
        and the processor is under load. To avoid array coercion
        altogether, C functions having arguments of mixed type are
        allowed in Numeric 2.
...
4.  Rich comparisons
    ...
    The implementation of `207`{.interpreted-text role="pep"}: Rich
    Comparisons in Python 2.1 provides additional flexibility when
    manipulating arrays. We intend to implement this feature in Numeric
    2.
...
2.  UFunc:
    ...
    The \'func\' argument is a CFuncObject, while the \'swap\' and
    \'conv\' arguments are lists of CFuncObjects for those arrays
    needing pre- or post-processing, otherwise None is used. The data
    argument is a list of buffer objects, and the slice argument gives
    the number of iterations for each dimension along with the buffer
    offset and step size for each array and each dimension.
...
5.  C-extension modules:
    ...
    a.  [ufunc]{#ufunc}:
        ...
        The primary module of this set is the
        [ufuncmodule.c]{#ufuncmodule.c}. The intention of this module is
        to do the bare minimum, i.e. iterate over arrays using a
        specified C function. The interface of these functions is the
        same as Numeric 1, i.e.
    ...
    b.  [int32]{#int32}, [real64]{#real64}, etc.:

        There will also be C-extension modules for each array type, e.g.
        [int32module.c]{#int32module.c},
        [real64module.c]{#real64module.c}, etc. As mentioned previously,
...
1.  Does slicing syntax default to copy or view behavior?
    ...
    some have argued that Numeric\'s slice notation should also have
    copy behavior to be consistent with Python lists. In this case the
...
5.  How should records be implemented?
    ...
    keyword argument should probably be changed to the more descriptive
    \'format\' keyword.
...
1.  Implement basic UFunc capability
    ...
    d.  Minimal C-extension module:
        ...
        [UFunc]{#ufunc}, which does the innermost array loop in C.
...
- `207`{.interpreted-text role="pep"}: Rich Comparisons by Guido van
  Rossum and David Ascher
- `208`{.interpreted-text role="pep"}: Reworking the Coercion Model by
  Neil Schemenauer and Marc-Andre\' Lemburg
- `211`{.interpreted-text role="pep"}: Adding New Linear Algebra
  Operators to Python by Greg Wilson
- `225`{.interpreted-text role="pep"}: Elementwise/Objectwise Operators
  by Huaiyu Zhu
- `228`{.interpreted-text role="pep"}: Reworking Python\'s Numeric Model
  by Moshe Zadka
