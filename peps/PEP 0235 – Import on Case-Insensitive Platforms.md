...
source_commit: 189b2a7a0544f74165a66c13d9acbc23bb9f4fec
---
...
and only `open("fiLe")` will open it (`open("file")` will not, nor will
the 14 other variations on that theme).
...
\-- they had patches of their own pending to do this, but the reviewer
for those balked).
...
multiple times, which is a powerful argument in PythonLand). It can\'t
cause any existing non-exceptional Windows import to fail, because any
existing non-exceptional Windows import finds a case-sensitive match
first in the path \-- and it still will. An exceptional Windows import
currently blows up with a `NameError` or `ImportError`, in which latter
case it still will, or in which former case will continue searching, and
either succeed or blow up with an `ImportError`.
...
I don\'t intend to implement #A for Unix too, but that\'s just because
I\'m not clear on how I *could* do so efficiently (I\'m not going to
slow imports under Unix just for theoretical purity).
...
support for them is ugly. We\'re already supporting (and will continue
to support) `PYTHONCASEOK` for their benefit, but they don\'t deserve
