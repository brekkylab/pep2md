...
that in `101`{.interpreted-text role="pep"}, it turns out not to be
enough justification for the duplication of information, and with it,
the danger of one of the copies to become out of date. Therefore, this
PEP is not maintained anymore, and micro releases are fully covered by
`101`{.interpreted-text role="pep"}.

# Abstract

Making a Python release is an arduous process that takes a minimum of
half a day\'s work even for an experienced releaser. Until recently,
most \-- if not all \-- of that burden was borne by Guido himself. But
several recent releases have been performed by other folks, so this PEP
attempts to collect, in one place, all the steps needed to make a Python
bugfix release.

The major Python release process is covered in `101`{.interpreted-text
role="pep"} - this PEP is just `101`{.interpreted-text role="pep"},
trimmed down to only include the bits that are relevant for micro
releases, a.k.a. patch, or bug fix releases.
...
3.  On the branch, change Include/patchlevel.h in two places, to reflect
    ...
    the PY_VERSION macro, and one or several of the version subpart
    macros just above PY_VERSION, as appropriate.

4.  Change the \"%define version\" line of Misc/RPM/python-2.3.spec to
...
5.  If you\'re changing the version number for Python (e.g. from Python
    ...
    which has a big banner at the top proclaiming its identity. Don\'t
    do this if you\'re just releasing a new alpha or beta release, but
    /do/ do this if you\'re release a new micro, minor or major release.
...
8.  For the Windows build, additional files have to be updated.
    ...
    PC/python_nt.rc, this step is now automated by the build process.)
...
12. Once the release process has started, the documentation needs to be
    ...
    branch to the trunk during the cleaning up phase. Basically, if
    it\'s in Doc/ Fred will take care of it.
...
14. Sean performs his Red Hat magic, generating a set of RPMs. He
...
16. Do a \"cvs update\" in this directory. Do NOT include the -A flag!
    ...
    and/or \"U\" files. I.e. you better not have any uncommitted changes
    in your working directory, but you may pick up some of Fred\'s or
    Thomas\'s last minute changes.
...
24. While you\'re waiting, you can start twiddling the web pages to
    ...
    7.  Similarly, edit the ../index.ht file, i.e. the python.org home
        ...
        X.YaZ is out\". Edit for content, and preview locally, but do
        NOT do a \"make install\" yet!

25. Now we\'re waiting for the scp to creosote to finish. Da de da, da
    de dum, hmm, hmm, dum de dum.
...
30. Send a SourceForge News Item about the release. From the project\'s
    ...
    \"Submit\" link. Type a suitable subject (e.g. \"Python 2.2c1
    released\" :-) in the Subject box, add some text to the Details box
    (at the very least including the release URL at www.python.org and
    the fact that you\'re happy with the release) and click the SUBMIT
    button.
...
1.  Edit the file Include/patchlevel.h so that the PY_VERSION string
    says something like \"X.YaZ+\". Note the trailing \'+\' indicating
    that the trunk is going to be moving forward with development. E.g.
...
he\'s done, he\'ll tag the branch something like \"rX.YaZ-mac\". He\'ll
also be responsible for merging any Mac-related changes back into the
trunk.
...
For this reason we want to have a higher coordination between the three
...
and make sure you can search for a random module (Thomas uses \"random\"
...
the downloaded file against the file he uploaded.
