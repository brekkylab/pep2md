...
for the language. It differs from `8010`{.interpreted-text role="pep"}
by specifically not proposing a central singular leader, but instead a
group of three people as the leaders.
...
adopted, it will be codified in `13`{.interpreted-text role="pep"} along
with the names of all officeholders described in this PEP.
...
`8011`{.interpreted-text role="pep"} was rejected [by a core developer
vote](https://discuss.python.org/t/python-governance-vote-december-2018-results/546/)
described in `8001`{.interpreted-text role="pep"} on Monday, December
17, 2018.

`8016`{.interpreted-text role="pep"} and the governance model it
describes were chosen instead.
...
will generally be the same voting process as described in
`8001`{.interpreted-text role="pep"}.
...
- Pronounce on PEPs, either as a team, or individually if the other trio
  members agree.
- Provide vision and leadership for Python, the programming language and
  the community.
...
- Be a voting member of The PSF (one of Contributing / Manager / Fellow
  / Supporter).[^1]
...
- Set good example of behavior, culture, and tone to Python community.
  ...
  inspiration, other communities will look at Python and learn from us.
...
arise; these should be handled through the process described in
`8001`{.interpreted-text role="pep"}.
...
- Day to day bug reports do not require the trio to intervene. Any core
  devs are able to make decisions, but will defer to the respective
  focused workgroups, and will eventually defer to the trio when there
  are major disagreements among core developers.
...
nominations, voting can begin, and the voting mechanism described in
`8001`{.interpreted-text role="pep"} will be used.
...
- Once the trio has pronounced a decision, core devs will be supportive,
  even if they were not supportive in the beginning (before the trio
  made such decision)
- Continue with day-to-day decision making in the bug tracker, and defer
  to the trio if there is major disagreement
- Python core developers do not handle CoC cases, those are
  responsibilities of the CoC workgroup, but will speak out if they
  witness those cases
- Aware that they are part of the larger Python community, not just the
...
Another difference with the previous \"BDFL-delegate\" role, the group
