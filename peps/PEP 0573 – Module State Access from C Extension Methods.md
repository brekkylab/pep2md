...
source_path: https://github.com/python/peps/blob/main/peps/pep-0573.rst
source_commit: 92210b327804f068f216a5705927c913903c9e74
---
...
This fixes one of the remaining roadblocks for adoption of
`3121`{.interpreted-text role="pep"} (Extension module initialization
and finalization) and `489`{.interpreted-text role="pep"} (Multi-phase
extension module initialization).

While this PEP takes an additional step towards fully solving the
problems that `3121`{.interpreted-text role="pep"} and
`489`{.interpreted-text role="pep"} started tackling, it does not
attempt to resolve *all* remaining concerns. In particular, access to
the module state from slot methods (`nb_add`, etc) is not solved.
...
`489`{.interpreted-text role="pep"} introduced a new way to initialize
extension modules, which brings several advantages to extensions that
implement it:
...
The biggest hurdle for adoption of `489`{.interpreted-text role="pep"}
is allowing access to module state from methods of extension types.
Currently, the way to access this state from extension methods is by
looking up the module via `PyState_FindModule` (in contrast to module
level functions in extension modules, which receive a module reference
as an argument). However, `PyState_FindModule` queries the thread-local
state, making it relatively costly compared to C level process global
access and consequently deterring module authors from using it.
...
`489`{.interpreted-text role="pep"}\'s multi-phase initialization, so
...
`3121`{.interpreted-text role="pep"} attempted to resolve this by
offering the `PyState_FindModule` API, but this still has significant
problems when it comes to extension methods (rather than module level
functions):
...
- Look up the class through walking the MRO. This is potentially
  expensive, but will be usable if performance is not a problem (such as
  when raising a module-level exception).
...
Getting to [per-module state](#per-module-state) from a heap type is a
...
It would be possible to add a `489`{.interpreted-text role="pep"}
execution slot type to make creating heap types significantly easier
than calling `PyType_FromModuleAndSpec`. This is left to a future PEP.
