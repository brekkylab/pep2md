...
are not meaningful for that file. Indexes MUST skip these attributes,
and tools MUST ignore them.
...
2.  An index implementation that has more complete wheel support but
    ...
    The index needs to permit the user to upload said JSON file and it
    needs to account for it in the wheel listings, but it does not need
    to process variant metadata directly.
