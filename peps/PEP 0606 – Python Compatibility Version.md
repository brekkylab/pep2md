...
source_path: https://github.com/python/peps/blob/main/peps/pep-0606.rst
source_commit: 2f0b0f993578756fb0c1e042f90edca29e46b0c4
---
...
Defining a Stable ABI <384>`{.interpreted-text role="pep"}.
...
<20>`{.interpreted-text role="pep"} motto is:
...
module: see the `565`{.interpreted-text role="pep"}). Some developers
simply ignore all warnings when there are too many warnings, thus only
bother with exceptions when the deprecated code is removed.
...
### from \_\_future\_\_ import python38_syntax
...
In 2009, `3003`{.interpreted-text role="pep"} \"Python Language
Moratorium\" proposed a temporary moratorium (suspension) of all changes
to the Python language syntax, semantics, and built-ins for Python 3.1
and Python 3.2.

In May 2018, during the `572`{.interpreted-text role="pep"} discussions,
it was also proposed to slow down Python changes: see the python-dev
thread [Slow
...
<387>`{.interpreted-text role="pep"} proposes a process to make
...
<497>`{.interpreted-text role="pep"} proposes different solutions to
provide backward compatibility.

Except for the `__past__` mechanism idea, `497`{.interpreted-text
role="pep"} does not propose concrete solutions:
...
> for forward compatibility such as new future_statements.
...
- XML attribute order: [bpo-34160](https://bugs.python.org/issue34160).
  Broken projects:
...
- `asyncio` no longer exports the `selectors` and `_overlapped` modules
  as `asyncio.selectors` and `asyncio._overlapped`. Replace
  `from asyncio import selectors` with `import selectors`.
- `479`{.interpreted-text role="pep"} is enabled for all code in Python
  3.7, meaning that `StopIteration` exceptions raised directly or
  indirectly in coroutines and generators are transformed into
...
- Python 3.7.2, `compileall` and `py_compile` module: the
  *invalidation_mode* parameter\'s default value is updated to `None`;
  the `SOURCE_DATE_EPOCH` environment variable no longer overrides the
  value of the *invalidation_mode* argument, and determines its default
  value instead.
...
- Python 3.4.1, `os.makedirs(name, mode=0o777, exist_ok=False)`: Before
  ...
  implement safely, it was removed in Python 3.4.1
...
- See also withdrawn `PEP 598 -- Introducing incremental feature
  releases <598>`{.interpreted-text role="pep"}
