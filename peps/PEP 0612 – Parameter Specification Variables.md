...
source_path: https://github.com/python/peps/blob/main/peps/pep-0612.rst
source_commit: d218284b840c8ff86ca063283275f35a365380c2
---

::: canonical-typing-spec
`typing:paramspec`{.interpreted-text role="ref"} and
...
`Callable[[int, str], bool]` syntax defined in `484`{.interpreted-text
role="pep"}, and callback protocols from `PEP
544 <544#callback-protocols>`{.interpreted-text role="pep"}. Neither of
these support forwarding the parameter types of one callable over to
another callable, making it difficult to annotate function decorators.
This PEP proposes `typing.ParamSpec` and `typing.Concatenate` to support
...
function. `484`{.interpreted-text role="pep"} supports dependencies
between single types, as in
...
Therefore, we need to make sure that these special types are only
brought into the world together, and are used together, so that our
usage is valid for all possible partitions.
...
- A function of type `Callable[P, R]` can be called with
  ...
  `kwargs` has the type `P.kwargs`, and that those types both originated
  from the same function declaration.
...
module. A reference implementation for CPython can be found
...
- The lack of user-defined operators makes common patterns hard to
  ...
  referring to any callable. It's just being used as a container for the
  parameters we wish to propagate. It would read better if we could
  define an operator `RemoveList[List[X]] = X` and then `unwrapping`
