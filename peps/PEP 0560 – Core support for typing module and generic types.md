...
source_path: https://github.com/python/peps/blob/main/peps/pep-0560.rst
source_commit: ea934589d3e74823910993de6fa540948ae68705
---

::: canonical-doc
the documentation for `~object.__class_getitem__`{.interpreted-text
role="external+python:meth"} and
`~object.__mro_entries__`{.interpreted-text role="external+python:meth"}
:::

# Abstract

Initially `484`{.interpreted-text role="pep"} was designed in such way
that it would not introduce *any* changes to the core CPython
interpreter. Now type hints and the `typing` module are extensively used
by the community, e.g. `526`{.interpreted-text role="pep"} and
`557`{.interpreted-text role="pep"} extend the usage of type hints, and
the backport of `typing` on PyPI has 1M downloads/month. Therefore, this
restriction can be removed. It is proposed to add two special methods
`__class_getitem__` and `__mro_entries__` to the core CPython for better
...
because subscripted generic types (see `484`{.interpreted-text
role="pep"} for definition of terms used in this PEP) are class objects
(see also[^1]). There are three main ways how the performance can be
improved with the help of the proposed special methods:
...
- Currently generics do dangerous things with private ABC caches to fix
  large memory consumption that grows at least as O(N^2^), see[^2]. This
  point is also important because it was recently proposed to
...
- Other operations with generic types and instances (like method lookup
  and `isinstance()` checks) are improved by around 10-20%
...
[^1]: Discussion following Mark Shannon\'s presentation at Language
    Summit (<https://github.com/python/typing/issues/432>)
