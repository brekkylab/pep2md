...
source_path: https://github.com/python/peps/blob/main/peps/pep-0203.rst
source_commit: 79a50ca04c455883fde0ddaf8d1e137d5d5083ad
---
...
- If the left-hand object (`x`) is an instance object, and it has a
  ...
  `x` and `y` as arguments. The result of the operation is whatever that
  function returns.
...
- Otherwise, if the left-hand object is not an instance object, but its
  ...
  the second argument; that is something that cannot happen with normal
  binary operations.
...
- Some special casing exists to account for the case of `+` and `*`,
  ...
  is converted to a C integer before calling either `sq_inplace_repeat`
  and `sq_repeat`. This is done even if `y` is an instance, though not
  if `x` is an instance.
...
with, but it is a start. `211`{.interpreted-text role="pep"} is looking
at adding new operators.
