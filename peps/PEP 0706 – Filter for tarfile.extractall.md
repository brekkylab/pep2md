...
python_version: '3.12'
post_history:
- '`25-Jan-2023 <https://discuss.python.org/t/23149>`__'
...
::: canonical-doc
`tarfile documentation <python:tarfile-extraction-filter>`{.interpreted-text
role="ref"}
:::

# Abstract

The extraction methods in `tarfile`{.interpreted-text
role="external+py3.11:mod"} gain a `filter` argument, which allows
...
many cases, it\'s best to ignore or disallow some of them when
extracting an archive.

Python allows extracting `tar` archives using
`tarfile.TarFile.extractall`{.interpreted-text
role="external+py3.11:meth"}, whose docs warn to *never extract archives
from untrusted sources without prior inspection*. However, it's not
clear what kind of inspection should be done. Indeed, it's quite tricky
to do such an inspection correctly. As a result, many people don't
bother, or do the check incorrectly, resulting in security issues such
as [CVE-2007-4559](https://nvd.nist.gov/vuln/detail/CVE-2007-4559).

Since `tarfile`{.interpreted-text role="external+py3.11:mod"} was first
written, it\'s become more accepted that warnings in documentation are
not enough. Whenever possible, an unsafe operation should be *explicitly
requested*; potentially dangerous operations should *look* dangerous.
However, `TarFile.extractall` looks benign in a code review.

Tarfile extraction is also exposed via
`shutil.unpack_archive`{.interpreted-text role="external+py3.11:func"},
which allows the user to not care about the kind of archive they\'re
dealing with. The API is very inviting for extracting archives without
prior inspection, even though the docs again warn against it.

It has been argued that Python is not wrong \-- it behaves exactly as
documented \-- but that\'s beside the point. Let\'s improve the
situation rather than assign/avoid blame. Python and its docs are the
best place to improve things.

# Rationale

How do we improve things? Unfortunately, we will need to change the
defaults, which implies breaking backwards compatibility.
`TarFile.extractall <tarfile.TarFile.extractall>`{.interpreted-text
role="external+py3.11:meth"} is what people reach for when they need to
extract a tarball. Its default behaviour needs to change.
...
- Unpacking a general data archive: the
  `shutil.unpack_archive`{.interpreted-text role="external+py3.11:func"}
  use case, where it\'s not important to preserve details specific to
  `tar` or Unix-like filesystems.

After a deprecation period, the last option \-- the most limited but
most secure one \-- will become the default.
...
- Call
  `TarFile.getmembers <tarfile.TarFile.getmembers>`{.interpreted-text
  role="external+py3.11:meth"}
- Verify or modify each member\'s `~tarfile.TarInfo`{.interpreted-text
  role="external+py3.11:class"}
...
- Calling `getmembers` can be expensive and it [requires a seekable
  archive](https://github.com/python/cpython/issues/45385#issuecomment-1255615199).
...
- Provide a supported way to "clone" and modify `TarInfo` objects. A
  `replace` method, similar to `dataclasses.replace`{.interpreted-text
  role="external+py3.11:func"} or
  `namedtuple._replace <collections.somenamedtuple._replace>`{.interpreted-text
  role="external+py3.11:meth"} should do the trick.
...
`TarFile.add <tarfile.TarFile.add>`{.interpreted-text
role="external+py3.11:meth"}. We\'ll also name it `filter`. (In some
...
`zipfile`{.interpreted-text role="external+py3.11:mod"}, gain similar
functionality, they should mimic this API as much as it\'s reasonable.
...
function that deals with `~tarfile.TarInfo`{.interpreted-text
role="external+py3.11:class"} objects.

The `shutil.unpack_archive`{.interpreted-text
role="external+py3.11:func"} function will get a `filter` argument,
...
The `~tarfile.TarInfo`{.interpreted-text role="external+py3.11:class"}
class will gain a new method, `replace()`, which will work similarly to
...
`None`. When `extract` or `extractall` encounters such a `None`, it will
not set that piece of metadata. (If `uname` or `gname` is `None`, it
will fall back to `uid` or `gid` as if the name wasn\'t found.) When
`addfile` or `tobuf` encounters such a `None`, it will raise a
`ValueError`. When `list` encounters such a `None`, it will print a
placeholder string.

The documentation will mention why the method is there: `TarInfo`
objects retrieved from
`TarFile.getmembers <tarfile.TarFile.getmembers>`{.interpreted-text
role="external+py3.11:meth"} are "live"; modifying them directly will
affect subsequent unrelated operations.

## Filters

`TarFile.extract <tarfile.TarFile.extract>`{.interpreted-text
role="external+py3.11:meth"} and
`TarFile.extractall <tarfile.TarFile.extractall>`{.interpreted-text
role="external+py3.11:meth"} methods will grow a `filter` keyword-only
parameter, which takes a callable that can be called as:
...
- `'tar'`: Roughly follow defaults of the GNU `tar` command (when run as
  a normal user):
  ...
  - Refuse to extract files whose absolute path (after following
    symlinks) would end up outside the destination. (Note that GNU `tar`
    instead delays creating some links.)
  - Clear high mode bits (setuid, setgid, sticky) and group/other write
    bits (`S_IWGRP|S_IWOTH <stat.S_IWGRP>`{.interpreted-text
    role="external+py3.11:data"}). (This is an approximation of GNU
    `tar`\'s default, which limits the mode by the current `umask`
    setting.)
- `'data'`: Extract a \"data\" archive, disallowing common attack
  vectors but limiting functionality. In particular, many features
  specific to UNIX-style filesystems (or equivalently, to the `tar`
  archive format) are ignored, making this a good filter for
  cross-platform archives. In addition to `tar`:
  ...
  - Refuse to extract links (hard or soft) which end up linking to a
    ...
    links, `tarfile` will, in most cases, fall back to creating regular
    files. This proposal doesn\'t change that behaviour.)
  - Refuse to extract device files (including pipes).
  - For regular files and hard links:
    - Set the owner read and write permissions
      (`S_IRUSR|S_IWUSR <stat.S_IRUSR>`{.interpreted-text
      role="external+py3.11:data"}).
    - Remove the group & other *executable* permission
      (`S_IXGRP|S_IXOTH <stat.S_IXGRP>`{.interpreted-text
      role="external+py3.11:data"}) if the owner doesn\'t have it
      (`~stat.S_IXUSR`{.interpreted-text role="external+py3.11:data"}).
...
`tarfile.fully_trusted_filter()`, `tarfile.tar_filter()`, etc., so they
can be easily used in custom policies.
...
`~tarfile.TarFile`{.interpreted-text role="external+py3.11:class"} will
gain a new attribute, `extraction_filter`, to allow configuring the
default filter. By default it will be `None`, but users can set it to a
callable that will be used if the `filter` argument is missing or
`None`.
...
`tarfile`{.interpreted-text role="external+py3.11:mod"} module. It\'ll
...
Currently, `~tarfile.TarFile`{.interpreted-text
role="external+py3.11:class"} has an *errorlevel* argument/attribute,
which specifies how errors are handled:

- With `errorlevel=0`, documentation says that "all errors are ignored
  when using `~tarfile.TarFile.extract`{.interpreted-text
  role="external+py3.11:meth"} and
  `~tarfile.TarFile.extractall`{.interpreted-text
  role="external+py3.11:meth"}". The code only ignores *non-fatal* and
...
- With `errorlevel=1` (the default), all *non-fatal* errors are ignored.
  ...
  - "unable to resolve link inside archive" (raised on systems that do
    not support symlinks)
  - "fifo/special devices not supported by system" (not used for
    failures if the system supports these, e.g. for a `PermissionError`)
  - "could not change owner/mode/modification time"

  Note that, for example, *file name too long* or *out of disk space*
  don\'t qualify. The *non-fatal* errors are not very likely to appear
...
- This PEP does not change existing behavior. (Ideas for improvements
  are welcome in [Discourse topic
  25970](https://discuss.python.org/t/25970).)
...
Even with the proposed changes, `tarfile`{.interpreted-text
role="external+py3.11:mod"} will not be suited for extracting untrusted
files without prior inspection. Among other issues, the proposed
policies don\'t prevent denial-of-service attacks. Users should do
additional checks.
...
- `tarfile` does not protect against issues with "live" data, e.g. an
  ...
  adding) is going on (see the [GNU tar
  manual](https://www.gnu.org/software/tar/manual/html_node/Live-untrusted-data.html#Live-untrusted-data)
  for more info).
...
`zipfile`{.interpreted-text role="external+py3.11:mod"} or
`shutil`{.interpreted-text role="external+py3.11:mod"} (which is out of
scope for this proposal).

## TarInfo identity, and `offset` {#706-offset}
...
If and when other archive libraries, such as `zipfile`{.interpreted-text
role="external+py3.11:mod"}, grow similar functionality, their
...
`shutil.unpack_archive`{.interpreted-text role="external+py3.11:func"}
will gain a `filter` argument. If it\'s given, it will be passed to the
underlying extraction function. Passing it for a `zip` archive will fail
for now (until `zipfile`{.interpreted-text role="external+py3.11:mod"}
...
`'data'`. With such a mechanism, API for (at least) set-up and tear-down
would need to be set in stone.
::::
...
role="external+py3.11:meth"} and
`TarFile.extractall <tarfile.TarFile.extractall>`{.interpreted-text
role="external+py3.11:meth"} will change, after raising
`DeprecationWarning` for 2 releases (shortest deprecation period allowed
in Python\'s `backwards compatibility policy <387>`{.interpreted-text
role="pep"}).

Additionally, code that relies on `tarfile.TarInfo`{.interpreted-text
role="external+py3.11:class"} object identity may break, see
`706-offset`{.interpreted-text role="ref"}.
...
Other than that, *all* of the changes to `tarfile` should be backported,
so `hasattr(tarfile, 'data_filter')` becomes a reliable check for all of
the new functionality.

Note that CPython\'s usual policy is to avoid adding new APIs in
security backports. This feature does not make sense without a new API
(`TarFile.extraction_filter` and the `filter` argument), so we\'ll make
an exception. (See [Discourse comment
23149/16](https://discuss.python.org/t/23149/16) for details.)
...
compatibility. In particular, it will help users avoid
...
See [pull request
#102953](https://github.com/python/cpython/pull/102953) on GitHub.
...
implements security checks (see
[gh-65308](https://github.com/python/cpython/issues/65308)). There are
...
Issue [gh-73974](https://github.com/python/cpython/issues/73974) asks
for adding an `absolute_path` option to extraction methods. This would
be a minimal change to formally resolve
[CVE-2007-4559](https://nvd.nist.gov/vuln/detail/CVE-2007-4559). It
doesn\'t go far enough to protect the unaware, nor to empower the
diligent and curious.

## Other names for the `'tar'` filter

The `'tar'` filter exposes features specific to UNIX-like filesystems,
so it could be named `'unix'`. Or `'unix-like'`, `'nix'`, `'*nix'`,
`'posix'`?
...
For consistency, `zipfile`{.interpreted-text role="external+py3.11:mod"}
and `shutil.unpack_archive`{.interpreted-text
role="external+py3.11:func"} could gain support for a `filter` argument.
However, this would require research that this PEP\'s author can\'t
promise for Python 3.12.
...
`ZipFile.extract <zipfile.ZipFile.extract>`{.interpreted-text
role="external+py3.11:meth"}\'s defaults are already similar to what a
`'data'` filter would do. A `'fully_trusted'` filter, which would *newly
allow* absolute paths and `..` path components, might not be useful for
much except a unified `unpack_archive` API.
...
that works with both `~tarfile.TarInfo`{.interpreted-text
role="external+py3.11:class"} and `~zipfile.ZipInfo`{.interpreted-text
role="external+py3.11:class"}. That is *definitely* out of scope of this
PEP.
