...
source_path: https://github.com/python/peps/blob/main/peps/pep-0652.rst
source_commit: c07b87eedb350dd25d946f20b38ae087e7249c5c
---

::: canonical-doc
`python:stable`{.interpreted-text role="ref"} (user docs) and
`devguide:c-api`{.interpreted-text role="ref"} (development docs)
:::

# Abstract

CPython\'s Limited C-API and Stable ABI, introduced in
`384`{.interpreted-text role="pep"}, will be formalized in a single
definitive file, tested, and documented.

# Motivation

`384`{.interpreted-text role="pep"} defined a Limited API and Stable
ABI, which allow extenders and embedders of CPython to compile extension
modules that are binary-compatible with any subsequent version of 3.x.
...
However, in hindsight, `384`{.interpreted-text role="pep"} and its
implementation has several issues:

- It is ill-defined. According to `384`{.interpreted-text role="pep"},
  functions are *opt-out*: all functions not specially marked are part
  of the Stable ABI. In practice, for Windows there\'s a list that\'s
  *opt-in*. For users there is a `#define` that should make only the
  Stable ABI available, but there is no process that ensures it is kept
  up-to date. Neither is there a process for updating the documentation.
...
- It is incomplete. Some operations are not available in the Stable ABI,
  with little reason except \"we forgot\". (This last point is one the
  PEP will not help with, however.)
...
`384`{.interpreted-text role="pep"} and this document deal with the
*Limited API* and the *Stable ABI*, two related but distinct concepts.
In short:
...
The Limited API is not guaranteed to be *stable*. In the future, parts
of the Limited API may be deprecated. They may even be removed, as long
as the *Stable ABI* is kept stable and Python\'s general backwards
compatibility policy, `387`{.interpreted-text role="pep"}, is followed.
...
hypothetical `#ifndef MACOSX` or
`#if defined(POSIX) && !defined(LINUX)`), a new feature macro will be
derived.
...
- The Stable ABI specified in `384`{.interpreted-text role="pep"}.
...
it and removing existing ones, will be added to the
[Devguide](https://devguide.python.org/). The checklist will 1) mention
best practices and common pitfalls in Python C API design and 2) guide
the developer around the files that need changing and scripts that need
running when the Limited API is changed.
...
- Make sure the new API and its intended use follows standard C, not
  ...
  the C dialect specified in `7`{.interpreted-text role="pep"}.
...
- Think about ease of use for the user. (In C, ease of use itself is not
  very important; what *is* useful is reducing boilerplate code needed
  to use the API. Bugs like to hide in boiler plates.)
...
guidelines, consider discussing the change at the
[capi-sig](https://mail.python.org/mailman3/lists/capi-sig.python.org/)
mailing list.
