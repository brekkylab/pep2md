...
source_path: https://github.com/python/peps/blob/main/peps/pep-0615.rst
source_commit: 308c7d4df11af9f77301b3f555a0eb5ac16efddb
---

::: canonical-doc
`zoneinfo`{.interpreted-text role="external+python:mod"}
:::
...
there are IETF RFCs both for the maintenance procedures
(`6557`{.interpreted-text role="rfc"}) and for the compiled binary
(TZif) format (`8536`{.interpreted-text role="rfc"}). As such, it is
...
same zone only if `dt1.tzinfo is dt2.tzinfo`.[^3] In addition to the
...
invariant for all hashable objects [^5][^6] that if `x == y`, it must
...
(the Unicode Common Locale Data Repository[^7]).
...
with both newer `importlib.resources`[^9] access patterns and older
access patterns like `pkgutil.get_data`[^10] .
...
2.  The raw tzdb format, while stable, is *less* stable than the TZif
    ...
    \"rearguard\" format to ease the transition.[^11]
...
Although `453`{.interpreted-text role="pep"}, which introduced the
`ensurepip` mechanism to CPython, provides a convenient template for a
standard library module maintained on PyPI, a potentially similar
`ensuretzdata` mechanism is somewhat less necessary, and would be
complicated enough that it is considered out of scope for this PEP.
...
A `pytz`-like ([^12]) interface was proposed in `431`{.interpreted-text
role="pep"}, but was ultimately withdrawn / rejected for lack of
ambiguous datetime support. `495`{.interpreted-text role="pep"} added
the `fold` attribute to address this problem, but `fold` obviates the
...
3.  Use only the `PYTHONTZPATH` variable, but provide a custom special
    ...
    native support for `TZDIST <7808>`{.interpreted-text role="rfc"}
    were to be added to the library in the future.
...
shaking\" distributions are in Python\'s future.[^17]
...
1.  In order to avoid forcing all `datetime` users to import `zoneinfo`,
    ...
    module-level `__getattr__` and `__dir__` per `562`{.interpreted-text
    role="pep"}, but this would add some complexity to the
    implementation of the `datetime` module. This sort of behavior in
    modules or classes tends to confuse static analysis tools, which may
    not be desirable for a library as widely used and critical as
    `datetime`.
...
:   The hash value for a given `datetime` is cached on first
...
:   The term \"first party\" here is distinguished from \"third party\"
    in that, although it is distributed via PyPI and is not currently
...
[^3]: Paul Ganssle: \"A curious case of non-transitive datetime
...
[^6]: Hynek Schlawack: \"Python Hashes and Equality\" (Published 20
...
[^13]: Paul Ganssle: \"pytz: The Fastest Footgun in the West\"
