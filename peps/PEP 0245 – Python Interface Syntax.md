...
author:
- Michel Pelletier <michel@users.sourceforge.net>
status: Rejected
...
source_path: https://github.com/python/peps/blob/main/peps/pep-0245.rst
source_commit: ec845d099bd723101becc437cf5e076d0a6257c5
---
...
Wayback Machine
archive](https://web.archive.org/web/20050327013919/http://www.zope.org/Wikis/Interfaces/FrontPage).
Also, the Interface-Dev Zope mailing list on which this PEP was
discussed was shut down, but [its archives remain
available](https://mail.zope.dev/pipermail/interface-dev/).
::::

# Rejection Notice

I\'m rejecting this PEP. It\'s been five years now. While at some point
I expect that Python will have interfaces, it would be naive to expect
it to resemble the syntax in this PEP. Also, `246`{.interpreted-text
role="pep"} is being rejected in favor of something completely
different; interfaces won\'t play a role in adaptation or whatever will
replace it. GvR.
...
Jim\'s \"Scarecrow\" interfaces prototype. Starting with version 2.3,
Zope comes with an Interface package as standard software. Zope\'s
Interface package is used as the reference implementation for this PEP.

The syntax proposed by this PEP relies on syntax enhancements describe
in `232`{.interpreted-text role="pep"} and describes an underlying
framework which `233`{.interpreted-text role="pep"} could be based upon.
There is some work being done with regard to interface objects and Proxy
...
- There are many implied interfaces in Python, commonly referred to as
  ...
  former with sequential, integer keys). There is no way for the
...
- Python is limited, from the developer\'s point of view, by the split
  ...
  code like \'type(foo) == type(\"\")\' to determine if \'foo\' is a
  string. When instances of classes are expected, the consumer uses
  \'isinstance(foo, MyString)\' to determine if \'foo\' is an instance
  of the \'MyString\' class. There is no unified model for determining
  if an object can be used in a certain, valid way.
...
called an \"interface assertion\". An interface assertion can be either
an interface object, or tuple of interface assertions.
...
The interface\'s suite is then executed in a new execution frame (see
the Python Reference Manual, section 4.1), using a newly created local
...
\'implements\' statements are evaluated, if present. The \'inheritance\'
behavior is unchanged as defined in Section 7.6 of the Language
Reference.

The \'implements\', if present, is evaluated after inheritance. This
must evaluate to an interface specification, which is either an
interface, or a tuple of interface specifications. If a valid interface
specification is present, the assertion is assigned to the class
object\'s \'\_\_implements\_\_\' attribute, as a tuple.
...
It\'s also possible to implement one interface with many classes that
mix in pieces the functionality of the interface or, conversely, it\'s
...
The new `from __future__` Python syntax (`236`{.interpreted-text
role="pep"}), and the new warning framework (`230`{.interpreted-text
role="pep"}) is ideal for resolving this backward incompatibility. To
