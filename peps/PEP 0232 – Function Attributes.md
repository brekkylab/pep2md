...
source_path: https://github.com/python/peps/blob/main/peps/pep-0232.rst
source_commit: b34d6b531fd54281c12f3a71e0a12c2276c19e90
---
...
to define parsing rules.[^1] Zope\'s ZPublisher ORB[^2] uses docstrings
...
Directions](#future-directions) discussion below for approaches in
subsequent versions of Python.
...
Paul Prescod enumerated a bunch of other uses on the [python-dev
thread](https://mail.python.org/pipermail/python-dev/2000-April/003364.html).
...
- A previous version of this PEP allowed for both setter and getter of
  ...
  Because a change to `a` bound `c1` also caused a change to `a` bound
  ...
  either bound or unbound methods, but does allow for getting attributes
  on either \-- both return the attribute value on the underlying
  function object.
  ...
  dictionary first, followed by a lookup on the class\'s dictionary, and
  finally a lookup on the function object\'s dictionary.
...
discussion thread starts on
[python-dev](https://mail.python.org/pipermail/python-dev/2000-April/003361.html).
