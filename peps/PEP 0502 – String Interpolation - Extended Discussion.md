...
`498`{.interpreted-text role="pep"}: *Literal String Interpolation*,
which proposed \"formatted strings\" was accepted September 9th, 2015.
Additional background and rationale given during its design phase is
detailed below.
...
more expressions built on [the existing
syntax](https://docs.python.org/3/library/string.html#format-string-syntax)
of `str.format()`.[^1][^2] The formatted string expands at compile-time
into a conventional string format operation, with the given expressions
from its text extracted and passed instead as positional arguments.
...
than a factual one. This PEP was also deemed not critical as
`498`{.interpreted-text role="pep"} was already written and should be
the place to house design decision details.
...
[post to
python-ideas](https://mail.python.org/pipermail/python-ideas/2015-July/034659.html)
that started the snowball (that became `498`{.interpreted-text
role="pep"}) rolling.[^3]

Furthermore, replacement of the print statement with the more consistent
print function of Python 3 (`3105`{.interpreted-text role="pep"}) has
added one additional minor burden, an additional set of parentheses to
type and read. Combined with the verbosity of current string formatting
solutions, this puts an otherwise simple language at an unfortunate
...
the \"Backtick\" (or grave accent ``[) are also \`constrained by
history]{.title-ref}\_ as a shortcut for `repr()`.
...
This [venerable
technique](https://docs.python.org/3/library/stdtypes.html#printf-style-string-formatting)
continues to have its uses, such as with byte-based protocols,
simplicity in simple cases, and familiarity to many programmers:
...
from](https://docs.python.org/3/library/string.html#template-strings)
`292`{.interpreted-text role="pep"} (Simpler String Substitutions) is a
purposely simplified design, using familiar shell interpolation syntax,
with [safe-substitution
feature](https://docs.python.org/3/library/string.html#string.Template.safe_substitute),
...
is limited, it meets its requirements well. It isn\'t powerful enough
for many cases, and that helps keep inexperienced users out of trouble,
...
library](http://pythonhosted.org/flufl.i18n/) such as `flufl.i18n`.

### PEP 215 - String Interpolation

`215`{.interpreted-text role="pep"} was a former proposal of which this
one shares a lot in common. Apparently, the world was not ready for it
at the time, but considering recent support in a number of other
languages, its day may have come.

The large number of dollar sign (`$`) characters it included may have
led it to resemble Python\'s arch-nemesis Perl, and likely contributed
...
of](https://docs.python.org/3/library/string.html#format-string-syntax)
`3101`{.interpreted-text role="pep"} is the most recent and modern of
the existing options. It is also more powerful and usually easier to
...
`498`{.interpreted-text role="pep"} defines and discusses format
strings, as also described in the [Abstract](#abstract) above.
...
under [Rejected Ideas](#rejected-ideas).

### PEP 501 \-- Translation ready string interpolation

The complimentary `501`{.interpreted-text role="pep"} brings
internationalization into the discussion as a first-class concern, with
its proposal of the i-prefix, `string.Template` syntax integration
compatible with ES6 (Javascript), deferred rendering, and an object
return value.
...
In the [Motivation](#motivation) section it was shown how convenient
interpolation syntax existed in Bash, Perl, and Ruby. Let\'s take a look
at their expression support.
...
is directed through string prefixes. Each prefix has a different result:

    s"Hello, $name ${1 + 1}"                    # arbitrary
    f"$name%s is $height%2.2f meters tall"      # printf-style
    raw"a\nb"                                   # raw, like r''

These prefixes may also be implemented by the user, by extending
Scala\'s `StringContext` class.
...
Designers of [Template
strings](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/template_strings)
faced the same issue as Python where single and double quotes were
taken. Unlike Python however, \"backticks\" were not. Despite [their
issues](https://mail.python.org/pipermail/python-ideas/2007-January/000054.html),
...
C# has a useful new [interpolation
feature](https://msdn.microsoft.com/en-us/library/Dn961160.aspx) as
well, with some ability to [customize
interpolation](http://www.thomaslevesque.com/2015/02/24/customizing-string-interpolation-in-c-6/)
via the `IFormattable` interface:
...
Arbitrary [interpolation under
Swift](https://developer.apple.com/library/ios/documentation/Swift/Conceptual/Swift_Programming_Language/StringsAndCharacters.html#//apple_ref/doc/uid/TP40014097-CH7-ID292)
is available on all strings:
...
Wikipedia](https://en.wikipedia.org/wiki/String_interpolation#Examples).

Now that background and history have been covered, let\'s continue on
for a solution.
...
delves into the mechanics and implementation of this design.
...
1.  Only string literals have been considered for format-strings, not
    ...
    `str.format()` and alternatives [already
    handle](https://mail.python.org/pipermail/python-ideas/2015-July/034729.html)
    this use-case.
...
The [say module on PyPI](https://pypi.python.org/pypi/say/) implements
string interpolation as described here with the small burden of a
callable interface:
...
A Python implementation of Ruby interpolation [is also
available](https://github.com/syrusakbary/interpy). It uses the codecs
...
(see `501`{.interpreted-text role="pep"}), the finer details diverge at
almost every point, making a common solution unlikely:[^12]
...
against](https://mail.python.org/pipermail/python-ideas/2015-August/034913.html)
support of arbitrary expressions were:

1.  [YAGNI](https://en.wikipedia.org/wiki/You_aren't_gonna_need_it),
...
It can be seen in the [Implementations in Other
Languages](#implementations-in-other-languages) section that the
developer community at large tends to agree. String interpolation with
arbitrary expressions is becoming an industry standard in modern
languages due to its utility.

### Additional/Custom String-Prefixes

As seen in the [Implementations in Other
Languages](#implementations-in-other-languages) section, many modern
...
- Eric V. Smith for the authoring and implementation of
  `498`{.interpreted-text role="pep"}.
