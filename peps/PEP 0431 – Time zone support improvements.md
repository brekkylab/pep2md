...
**UPDATE**: The `615`{.interpreted-text role="pep"} \"Support for the
IANA Time Zone Database in the Standard Library\" added the `zoneinfo`
module to Python 3.9 and superseded this PEP.
...
the \"zoneinfo\" database. This database, also called \"tz\" or \"The
Olsen database\", is the de facto standard time zone database over time
zones, and it is included in most Unix and Unix-like operating systems,
including OS X.
...
zoneinfo database isn\'t included, such as Windows, or on platforms
where OS updates are no longer provided.
...
- `pytz` has the functions `localize()` and `normalize()` to work around
  that `tzinfo` doesn\'t have is_dst. When `is_dst` is implemented
  directly in `datetime.tzinfo` they are no longer needed.
