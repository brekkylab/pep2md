...
python_version: '3.12'
post_history:
- '`07-Dec-2021 <https://mail.python.org/archives/list/python-dev@python.org/thread/VNSD4TSAM2BM64FJNIQPAOPNEGNX4MDX/>`__'
...
source_path: https://github.com/python/peps/blob/main/peps/pep-0669.rst
source_commit: 83f27ec0f71408855cafd876838ea5052adbc094
---

::: canonical-doc
`python:sys.monitoring`{.interpreted-text role="mod"}
:::
...
that it will be implemented using the quickening step of
`659`{.interpreted-text role="pep"}.
...
The quickening mechanism provided by `659`{.interpreted-text role="pep"}
provides a way to dynamically modify executing Python bytecode. These
modifications have little cost beyond the parts of the code that are
modified and a relatively low cost to those parts that are modified. We
can leverage this to provide an efficient mechanism for monitoring that
was not possible in 3.10 or earlier.
...
- STOP_ITERATION \-- An artificial `StopIteration` is raised; see [the
  STOP_ITERATION event](#the-stop_iteration-event).

More events may be added in the future.

All events will be attributes of the `events` namespace in
`sys.monitoring`. All events will represented by a power of two integer,
so that they can be combined with the `|` operator.
...
`PEP 380 <380#use-of-stopiteration-to-return-values>`{.interpreted-text
role="pep"} specifies that a `StopIteration` exception is raised when
returning a value from a generator or coroutine. However, this is a very
inefficient way to return a value, so some Python implementations,
notably CPython 3.12+, do not raise an exception unless it would be
...
effect on the others. However, the `CALL`, `C_RAISE` and `C_RETURN`
events form a group. If any of those events are set or disabled, then
all events in the group are. Disabling a `CALL` event will not disable
the matching `C_RAISE` or `C_RETURN`, but will disable all subsequent
events.
...
There are some compatibility issues with `523`{.interpreted-text
role="pep"}, as the behavior of `523`{.interpreted-text role="pep"}
plugins is outside of the VM\'s control. It is up to
`523`{.interpreted-text role="pep"} plugins to ensure that they respect
the semantics of this PEP. Simple plugins that do not change the state
of the VM, and defer execution to `_PyEval_EvalFrameDefault()` should
continue to work.

`sys.settrace`{.interpreted-text role="func"} and
`sys.setprofile`{.interpreted-text role="func"} will act as if they were
tools 6 and 7 respectively, so can be used alongside this PEP.

This means that `sys.settrace`{.interpreted-text role="func"} and
`sys.setprofile`{.interpreted-text role="func"} may not work correctly
with all `523`{.interpreted-text role="pep"} plugins. Although, simple
`523`{.interpreted-text role="pep"} plugins, as described above, should
be fine.
...
supporting `sys.settrace`{.interpreted-text role="func"} directly.

The performance of `sys.settrace`{.interpreted-text role="func"} will be
about the same. The performance of `sys.setprofile`{.interpreted-text
role="func"} should be better. However, tools relying on
`sys.settrace`{.interpreted-text role="func"} and
`sys.setprofile`{.interpreted-text role="func"} can be made a lot faster
...
`sys.settrace`{.interpreted-text role="func"} and much cheaper than
using `523`{.interpreted-text role="pep"}.
...
`PEP 659 <659#quickening>`{.interpreted-text role="pep"}.
...
and `INSTRUCTION` events require additional information, as they need to
store the original instruction, or even the instrumented instruction if
they overlap other instrumentation.
...
when a code object is first encountered, so that any necessary
...
of profilers should be aware that instrumenting `LINE` events will have
