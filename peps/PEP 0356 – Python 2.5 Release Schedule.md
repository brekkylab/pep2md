...
source_path: https://github.com/python/peps/blob/main/peps/pep-0356.rst
source_commit: f2e3f90392558c9b7a924b5f0cad320a46221119
---
...
- `308`{.interpreted-text role="pep"}: Conditional Expressions
- `309`{.interpreted-text role="pep"}: Partial Function Application
- `314`{.interpreted-text role="pep"}: Metadata for Python Software
  Packages v1.1
- `328`{.interpreted-text role="pep"}: Absolute/Relative Imports
- `338`{.interpreted-text role="pep"}: Executing Modules as Scripts
- `341`{.interpreted-text role="pep"}: Unified try-except/try-finally to
  try-except-finally
- `342`{.interpreted-text role="pep"}: Coroutines via Enhanced
  Generators
- `343`{.interpreted-text role="pep"}: The \"with\" Statement (still
  need updates in Doc/ref and for the `contextlib` module)
- `352`{.interpreted-text role="pep"}: Required Superclass for
  Exceptions
- `353`{.interpreted-text role="pep"}: Using `ssize_t` as the index type
- `357`{.interpreted-text role="pep"}: Allowing Any Object to be Used
  for Slicing
...
- `hashlib` \-- adds support for SHA-224, -256, -384, and -512 (replaces
  old `md5` and `sha` modules)
...
- Added `302`{.interpreted-text role="pep"} `zipfile`/`__loader__`
  support to the following modules: `warnings`, `linecache`, `inspect`,
  `traceback`, `site`, and `doctest`
...
- Add write support for mailboxes from the code in sandbox/mailbox.
  (Owner: A.M. Kuchling. It would still be good if another person would
...
- Bugs deferred until 2.5.1 (or later):
  ...
  - <https://bugs.python.org/issue1544279> - `Socket` module is not
    thread-safe
  - <https://bugs.python.org/issue1541420> - tools and demo missing from
    windows
  ...
  - <https://bugs.python.org/issue1446043> - `unicode()` does not raise
    `LookupError`

- The `302`{.interpreted-text role="pep"} changes to (at least)
  `pkgutil`, `runpy` and `pydoc` must be documented.
...
[^2]: Joe Smith, [bdist](##REF##bdist)\* to stdlib?
