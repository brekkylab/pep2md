...
This PEP is a successor to `472`{.interpreted-text role="pep"}, which
was rejected due to lack of interest in 2019. Since then there\'s been
renewed interest in the feature.

# Overview

## Background

`472`{.interpreted-text role="pep"} was opened in 2014. The PEP detailed
various use cases and was created by extracting implementation
strategies from a broad discussion on the python-ideas mailing list,
although no clear consensus was reached on which strategy should be
used. Many corner cases have been examined more closely and felt
...
However, with the introduction of type hints in `484`{.interpreted-text
role="pep"} the square bracket notation has been used consistently to
enrich the typing annotations, e.g. to specify a list of integers as
...
different threads on python-ideas, recently by Caleb Donovick[^2] in
2019 and Andras Tantos[^3] in 2020. These requests prompted a strong
...
7.  Availability of star arguments would benefit `646`{.interpreted-text
    role="pep"} Variadic Generics, especially in the forms `a[*x]` and
    `a[*x, *y, p, q, *z]`. The PEP details exactly this notation in its
    \"Unpacking: Star Operator\" section.
...
5.  Keyword subscripts, if any, will be handled like they are in
    ...
    - a single positional index will not turn into a tuple just because
      one adds a keyword value.
...
6.  The same rules apply with respect to keyword subscripts as for
    ...
    - if there are any named parameters left over (without a value) when
      the keywords are all used, they are assigned their default value
      (if any);
...
7.  Sequence unpacking is allowed inside subscripts:
    ...
    treated as `[(slice(None), *args, slice(None))]`. Multiple star
    unpacking are allowed:
...
12. The same semantics given above must be extended to
    `__class__getitem__`: Since `560`{.interpreted-text role="pep"},
    type hints are dispatched so that for `x[y]`, if no `__getitem__`
    method is found, and `x` is a type (class) object, and `x` has a
    class method `__class_getitem__`, that method is called. The same
    changes should be applied to this method as well, so that a writing
    like `list[T=int]` can be accepted.
...
arguments. The same holds for all other classes (list, dict, etc.)
...
4.  As we saw, a single value followed by a keyword argument will not be
    ...
    `__getitem__(d, 1, a=3)`, NOT `__getitem__(d, (1,), a=3)`. It would
    be extremely confusing if adding keyword arguments were to change
    the type of the passed index. In other words, adding a keyword to a
...
compiler will have to generate these new opcodes. The old C
implementations will call the extended methods passing `NULL` as kwargs.
...
problem that the PEP solves" <1#what-belongs-in-a-successful-pep>`{.interpreted-text
role="pep"}.
...
`472`{.interpreted-text role="pep"} presents a good amount of ideas that
are now all to be considered Rejected. A personal email from D\'Aprano
to the author specifically said:
...
for all options presented in `472`{.interpreted-text role="pep"}.
Suffice to say that they were discussed, and each proposed alternative
had one or few dealbreakers.
...
- It will slow down subscripting. For every subscript access, this new
  ...
  only at class instantiation time, to add a bit flag to signal the
  availability of these methods. Regardess of the solution, the new
  dunder would be effective only if added at class creation time, not if
  it\'s added later. This would be unusual and would disallow (and
  behave unexpectedly) monkeypatching of the methods for whatever reason
  it might be needed.
...
- the solution relies on the assumption that all keyword indices
  ...
  the underlying numpy array, and those have no position to match up to.
  In other words, anonymous indexes are a plausible use case that this
  solution would remove, although it could be argued that using `*args`
...
in `472`{.interpreted-text role="pep"}:
...
[^2]: \"Allow kwargs in \_\_{get[\|set\|](##SUBST##|set|)del}item\_\_\"
