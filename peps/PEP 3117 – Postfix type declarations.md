...
source_path: https://github.com/python/peps/blob/main/peps/pep-3117.rst
source_commit: ad4365534c4f7fc09361980205421b0377c77cef
---
...
members of the PSU (for a few examples, see [\[EX1\]](#EX1){.citation},
[\[EX2\]](#EX2){.citation} or [\[EX3\]](#EX3){.citation}), and it also
made it a large-scale enterprise success unlikely.
...
(e.g., a certain language uses completely unrelated[^1] adjectives like
`dim` for type declaration).
...
characters, which is in accordance with the `Zen <20>`{.interpreted-text
role="pep"} (\"Readability counts.\"). Additionally, it eliminates the
need for a separate type declaration statement, and last but not least,
it makes Python measure up to Perl 6, which already uses Unicode for its
operators.[^2]
...
which enables Unicode parsing of the source[^4], makes `typedef` a
...
[^1]: Though, if you know the language in question, it may not be *that*
    unrelated.

[^2]: Well, it would, if there was a Perl 6.

[^3]: Since the name `TypeError` is already in use, this name has been
    chosen for obvious reasons.

[^4]: The encoding in which the code is written is read from a standard
    coding cookie. There will also be an autodetection mechanism,
