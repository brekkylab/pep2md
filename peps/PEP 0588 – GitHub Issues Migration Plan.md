...
::: canonical-doc
[psf/gh-migration#13](https://github.com/psf/gh-migration/issues/13)
...
tracker on Roundup to GitHub issues. See `581`{.interpreted-text
role="pep"} for rationale and background. `588`{.interpreted-text
role="pep"} also describes the detailed timeline for the migration.
...
Is the current \"priority\" list useful? Alyssa Coghlan noted that
perhaps only `release blocker` and `deferred blocker` are useful.
