...
source_path: https://github.com/python/peps/blob/main/peps/pep-0523.rst
source_commit: f9cd7a96808cff8afedfc31056b26631f0cd71e1
---
...
via `PyEval_EvalFrameEx()`[^5], control over the execution of Python
code comes down to individual objects instead of a holistic control of
...
The Pyjion project[^11] has used this proposed API to implement a JIT
for CPython using the CoreCLR\'s JIT[^12]. Each code object has its
`co_extra` field set to a `PyjionJittedCode` object which stores four
...
It should be mentioned that the Pyston[^13] team was consulted on an
earlier version of this PEP that was more JIT-specific and they were not
...
itself. An informal discussion with a developer on the PyPy[^14] team
led to a similar comment.

Numba[^15], on the other hand, suggested that they would be interested
in the proposed change in a post-1.0 future for themselves[^16].

The experimental Coconut JIT[^17] could have benefitted from this PEP.
In private conversations with Coconut\'s creator we were told that our
API was probably superior to the one they developed for Coconut to add
JIT support to CPython.

## Debugging

In conversations with the Python Tools for Visual Studio team (PTVS)
[^18], they thought they would find these API changes useful for
implementing more performant debugging. As mentioned in the
[Rationale](#rationale) section, this API would allow for switching on
debugging functionality only in frames where it is needed. This could
allow for either skipping information that `sys.settrace()` normally
provides and even go as far as to dynamically rewrite bytecode prior to
execution to inject e.g. breakpoints in the bytecode.
...
Pyjion project[^19]. In its current form it has more changes to CPython
...
team[^20], though, it became clear that the API was unnecessarily large.
...
[^13]: Pyston (<http://pyston.org>)

[^14]: PyPy (<http://pypy.org/>)

[^15]: Numba (<http://numba.pydata.org/>)

[^16]: numba-users mailing list: \"Would the C API for a JIT entrypoint
    being proposed by Pyjion help out Numba?\"
    (<https://groups.google.com/a/continuum.io/forum/#!topic/numba-users/yRl_0t8-m1g>)

[^17]: Coconut (<https://github.com/davidmalcolm/coconut>)

[^18]: Python Tools for Visual Studio
    (<http://microsoft.github.io/PTVS/>)

[^19]: Pyjion project (<https://github.com/microsoft/pyjion>)

[^20]: Numba (<http://numba.pydata.org/>)
