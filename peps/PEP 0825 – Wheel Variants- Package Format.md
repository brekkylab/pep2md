...
source_commit: b2d15ea40b1d5c5f6548cadf1e0f1d1fea08e632
---
...
role="ref"} of the Binary Distribution Format specification. The exact
URL where the file is hosted is insignificant, but a link to it MUST be
...
4.  For every compatible variant, determine the most preferred value
    ...
    After this step, a list of features along with their best values is
...
to express multidimensional wheel compatibility matrices. For example,
it permits expressing that a single variant requires certain CPU and GPU
features independently. It can express both AND-style dependencies (such
as different CPU instruction sets) and OR-style dependencies (such as
...
To facilitate variant selection while installing from remote index, the
variant metadata is mirrored in a JSON file published on the index. This
enables installers to obtain variant property mapping without having to
fetch individual wheels.
...
providing example solutions to [open issues](#open-issues).
