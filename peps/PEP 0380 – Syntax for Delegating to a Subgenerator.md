...
source_path: https://github.com/python/peps/blob/main/peps/pep-0380.rst
source_commit: aaa25d64c756a30cd16334439bf62fbb692d6393
---
...
Guido officially [accepted the
PEP](https://mail.python.org/pipermail/python-dev/2011-June/112010.html)
on 26th June, 2011.
...
- If a GeneratorExit exception is thrown into the delegating generator,
  ...
  call results in an exception, it is propagated to the delegating
...
- A block of code that catches GeneratorExit without subsequently
  re-raising it cannot be factored out while retaining exactly the same
  behaviour.
...
- It simplifies the implementation, because the point at which the
  return value from the subgenerator becomes available is the same point
  at which the exception is raised. Delaying until any later time would
  require storing the return value somewhere.
...
\_\_next\_\_(), or substitute it with a send() call with a specified
value, the intention being to support the use of generators wrapped so
that the initial \_\_next\_\_() is performed automatically.
...
[Examples and
Implementation](http://www.cosc.canterbury.ac.nz/greg.ewing/python/yield-from/)
