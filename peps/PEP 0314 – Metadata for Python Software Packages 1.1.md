...
source_commit: a2cf3c11df409915cb3e3c76ff0683a097b23915
---
...
is specified in `241`{.interpreted-text role="pep"}.
...
The PKG-INFO file format is a single set of `822`{.interpreted-text
role="rfc"} headers parseable by the rfc822.py module. The field names
listed in the following section are used as the header names.
...
Version of the file format; currently \"1.0\" and \"1.1\" are the only
legal values here.
...
\"\.../package-latest.tgz\", but instead must be
\"../package-0.45.tgz\".)
...
role="rfc"} \'From:\' header. It\'s not optional because cataloging
systems can use the e-mail portion of this field as a unique key
representing the author. A catalog might provide authors the ability to
store their GPG key, personal home page, and other additional metadata
*about the author*, and optionally the ability to associate several
e-mail addresses with the same person. Author-related metadata fields
are not covered by this PEP.
...
package. Classifiers are described in `301`{.interpreted-text
role="pep"}.
...
package name usable with the \'import\' statement, optionally followed
...
\"\<\", \"\>\", \"\<=\", \"\>=\", \"==\", and \"!=\". Version numbers
must be in the format accepted by the distutils.version.StrictVersion
class: two or three dot-separated numeric components, with an optional
\"pre-release\" tag on the end consisting of the letter \'a\' or \'b\'
followed by a number. Example version numbers are \"1.0\", \"2.3a2\",
\"1.3.99\",
...
- Added the Classifiers field from `301`{.interpreted-text role="pep"}.
- The License and Platform files should now only be used if the platform
  or license can\'t be handled by an appropriate Classifier value.
