...
source_commit: 299ddeb68b31006df4f0895f3f954d2dc6620acc
---
...
- gives guidance for static type checkers (and one small addition to the
  `typing` module)
- discusses the main objections and alternatives that have been brought
  up during extensive discussion of the proposal, both within the group
  of authors and in the python-dev community
...
like [Scala](https://docs.scala-lang.org/tour/pattern-matching.html) and
[Rust](https://doc.rust-lang.org/reference/patterns.html), to dynamic
languages like Elixir and Ruby, and is under consideration for
JavaScript. We are indebted to these languages for guiding the way to
Pythonic pattern matching, as Python is indebted to so many other
languages for many of its features: many basic syntactic features were
inherited from C, exceptions from Modula-3, classes were inspired by
C++, slicing came from Icon, regular expressions from Perl, decorators
resemble Java annotations, and so on.
...
See the [syntax](#syntax-and-semantics) sections below for a more
...
named tuples and dataclasses). See the [runtime](#runtime-specification)
section below.
...
types](https://en.wikipedia.org/wiki/Algebraic_data_type). See the
[static checkers](#static-checkers-specification) section for more
details.
...
- A **literal pattern** is useful to filter constant values in a
  ...
  `True`, `False` and `None`). It only matches objects equal to the
...
- A **constant value pattern** works like the literal but for certain
  named constants. Note that it must be a qualified (dotted) name, given
  the possible ambiguity with a capture pattern. It looks like
...
- A **mapping pattern** looks like `{"user": u, "emails": [*es]}`. It
  ...
  the sub-patterns bind while matching with the values corresponding to
  the keys. Adding `**rest` at the end of the pattern to capture extra
  items is allowed.
...
- A **walrus pattern** looks like `d := datetime(year=2020, month=m)`.
  It matches only if its sub-pattern also matches. It binds whatever the
...
See [Appendix A](#appendix-a----full-grammar) for the full, unabridged
grammar. The simplified grammars in this section are there for helping
the reader, not as a full specification.
...
described in detail below in the [patterns](#allowed-patterns)
subsection.
...
can be used (see the section on [capture_pattern](#capture-patterns)) as
...
Otherwise, it continues (see details in the
[runtime](#runtime-specification) section).
...
produce less efficient code. Essentially, most of the arguments in
`572`{.interpreted-text role="pep"} apply here equally.
...
- The class object for `Class` in `Class(<sub-patterns>)` is looked up
  and `isinstance(obj, Class)` is called, where `obj` is the value being
  matched. If false, the match fails.
- Otherwise, if any sub-patterns are given in the form of positional or
  keyword arguments, these are matched from left to right, as follows.
  The match fails as soon as a sub-pattern fails; if all sub-patterns
  succeed, the overall class pattern match succeeds.
- If there are match-by-position items and the class has a
  ...
  `Point2d.__match_args__ == ["x", "y"]`, is translated (approximately)
  into `obj.x == 5 and obj.y == 8`.
...
`484`{.interpreted-text role="pep"} specifies that static type checkers
should support exhaustiveness in conditional checks with respect to enum
values. `586`{.interpreted-text role="pep"} later generalized this
...
Obviously, no `Matchable` protocol (in terms of `544`{.interpreted-text
role="pep"}) is needed, since every class is matchable and therefore is
subject to the checks specified above.
...
We propose to add a special decorator class `@sealed` to the
`typing`{.interpreted-text role="py:mod"} module, that will have no
effect at runtime, but will indicate to static type checkers that all
subclasses (direct and indirect) of this class should be defined in the
...
with builtin generic classes in the recently accepted
`585`{.interpreted-text role="pep"}, because they prohibit `isinstance`
checks.
...
`TypedDict` type as specified by `589`{.interpreted-text role="pep"}.
For example:
...
in context. Since `617`{.interpreted-text role="pep"} the parser
backtracks, that means that on different attempts to parse a code
fragment it could interpret a soft keyword differently.
...
this is the auto-formatter
[Black](https://black.readthedocs.io/en/stable/). A particular
requirement with these kinds of tools is that they not only need to
understand the syntax of the current version of Python, but older
...
(such as [parso](https://github.com/davidhalter/parso) and
[libCST](https://github.com/Instagram/LibCST)) and upgrade them to be
PEG compatible.
...
based on the above implementation was created using
[Binder](https://mybinder.org) and [Jupyter](https://jupyter.org).

# Example Code

A small [collection of example
...
- Algorithms which cut across traditional lines of data encapsulation.
  ...
  types (such as evaluating or transforming an abstract syntax tree, or
  doing algebraic manipulation of mathematical symbols), forcing the
  user to implement the algorithm as individual methods on each element
  type results in logic that is smeared across the entire codebase
  instead of being neatly localized in one place.
- Program architectures where the set of possible data types is
  relatively stable, but there is an ever-expanding set of operations to
  be performed on those data types. Doing this in a strict OOP fashion
  requires constantly adding new methods to both the base class and
  subclasses to support the new methods, \"polluting\" the base class
  with lots of very specialized method definitions, and causing
  widespread disruption and churn in the code. By contrast, in a
  `match`-based dispatch, adding a new behavior merely involves writing
  a new `match` statement.
- OOP also does not handle dispatching based on the *shape* of an
  object, such as the length of a tuple, or the presence of an attribute
  \-- instead any such dispatching decision must be encoded into the
  object\'s type. Shape-based dispatching is particularly interesting
  when it comes to handling \"duck\"-typed objects.
...
- Use some implicit rules. For example, if a name was defined in the
  global scope, then it refers to a constant, rather than representing a
  capture pattern:
...
- Use a rule based on the case of a name. In particular, if the name
  ...
  `8`{.interpreted-text role="pep"}. The main objection is that there\'s
  no other part of core Python where the case of a name is semantically
  significant. In addition, Python allows identifiers to use different
  scripts, many of which (e.g. CJK) don\'t have a case distinction.
...
- There was also an idea to make lookup semantics the default, and
  ...
  - The authors are not aware of any other language that adorns captures
    in this way.
...
This was rejected because there is [documented
evidence](https://dl.acm.org/doi/abs/10.1145/2480360.2384582) that this
...
checkers](##REF##static checkers specification) can warn about missing
patterns.
...
C#, Elixir, Erlang, F#, Haskell, Mathematica, OCaml, Ruby, Rust, Scala,
and Swift. Now, in general, we should not be concerned too much with
what another language does, since Python is clearly different from all
these languages. However, if there is such an overwhelming and strong
...
- Allow stacked cases:
  ...
  for cases. However, we don\'t want to mislead people into thinking
  that `match`/`case` uses fall-through semantics (which are a common
  source of bugs in C). Also, this would be a novel indentation pattern,
  which might make it harder to support in IDEs and such (it would break
  the simple rule \"add an indentation level after a line ending in a
  colon\"). Finally, this wouldn\'t support OR patterns nested inside
  other patterns.
...
- In order for built-in types and standard library classes to be able to
  support matching in a reasonable and intuitive way, it was believed
...
- A \'full-featured\' match protocol which would pass in not only the
  subject to be matched, but detailed information about which attributes
  the specified pattern was interested in.
...
2.  Substantial rewrite, including:
    ...
    - Drop the `__match__` protocol (moved to [deferred
