...
source_path: https://github.com/python/peps/blob/main/peps/pep-0682.rst
source_commit: 560a5f9dd3917ef41692864ea370ca3db8d73247
---

# Abstract

Though `float` and `Decimal` types can represent [signed
zero](https://en.wikipedia.org/wiki/Signed_zero), in many fields of
mathematics negative zero is surprising or unwanted \-- especially in
the context of displaying an (often rounded) numerical result. This PEP
proposes an extension to the [string format
specification](https://docs.python.org/3/library/string.html#formatstrings)
allowing negative zero to be normalized to positive zero.
...
- [How to have negative zero always formatted as positive zero in a
  python
  string?](https://stackoverflow.com/questions/11010683/how-to-have-negative-zero-always-formatted-as-positive-zero-in-a-python-string/36604981#36604981)
  (Python, post-regex)
- [(Iron)Python formatting issue with modulo operator & \"negative
  zero\"](https://stackoverflow.com/questions/41564311/ironpython-formatting-issue-with-modulo-operator-negative-zero/41564834#41564834)
  (Python, pre-round)
- [Negative sign in case of zero in
  java](https://stackoverflow.com/questions/11929096/negative-sign-in-case-of-zero-in-java)
  (Java, post-regex)
- [Prevent small negative numbers printing as
  \"-0\"](https://stackoverflow.com/questions/10969399/prevent-small-negative-numbers-printing-as-0)
  (Objective-C, custom number formatter)
...
for C++
std::format()](http://www.open-std.org/jtc1/sc22/wg21/docs/papers/2020/p1496r2.pdf).
While the proposal was withdrawn for C++20, a consensus proposal is
promised for C++23. (The original [feature
request](https://bugs.python.org/issue45995) prompting this PEP was
argued without knowledge of the C++ proposal.)

When Rust developers debated whether to suppress negative zero in
`print` output, they took a small [survey of other
languages](https://github.com/rust-lang/rfcs/issues/1074#issuecomment-718243936).
Notably, it didn\'t mention any language providing an option for
...
option (`378`{.interpreted-text role="pep"}).
...
in someone else\'s code, reference to the [Format Specification
Mini-Language](https://docs.python.org/3/library/string.html#format-specification-mini-language)
section of the library reference manual should suffice.

# Reference Implementation

A reference implementation exists at [pull request
#30049](https://github.com/python/cpython/pull/30049).
