...
source_path: https://github.com/python/peps/blob/main/peps/pep-3127.rst
source_commit: d3b753f76947b2e65a265351ad3e21ed5f6b0705
---
...
a)  octal literals must now be specified with a leading \"0o\" or \"0O\"
    instead of \"0\";
b)  binary literals are now supported via a leading \"0b\" or \"0B\";
    and
...
- The default octal representation of integers is silently confusing to
  ...
  \'013\' means \'decimal 11\', not \'decimal 13\', to the Python
...
For Python 3.0, \"oldoctinteger\" will not be supported, and an
exception will be raised if a literal has a leading \"0\" and a second
character which is a digit.

For both versions, this will require changes to PyLong_FromString as
well as the grammar.
...
`306`{.interpreted-text role="pep"} should be checked for other issues,
and that PEP should be updated if the procedure described therein is
insufficient.
...
If an invalid token contains a leading \"0\", the exception error
message should be more informative than the current \"SyntaxError:
invalid token\". It should explain that decimal numbers may not have a
leading zero, and that octal numbers require an \"o\" after the leading
zero.
...
oct() should be updated to output \'0o\' in front of the octal digits
(for 3.0, and 2.6 compatibility mode).
...
alternate octal formatting will continue to add only \'0\'. In neither
2.6 nor 3.0 will the % operator support binary output. This is because
binary output is already supported by `3101`{.interpreted-text
role="pep"} (str.format), which is the preferred string formatting
method.
...
borrows heavily from C.[^1][^2] Usage has shown that the historical
method of specifying an octal number is confusing, and also that it
would be nice to have additional support for binary literals.
...
- Literal integer tokens, as used by normal module compilation, by
  eval(), and by int(token, 0). (int(token) and int(token, 2-36) are not
  modified by this proposal.)
  - Under 2.6, long() is treated the same as int()
- Formatting of integers into strings, either via the % string operator
  or the new `3101`{.interpreted-text role="pep"} advanced string
  formatting method.
...
have a very valid point \-- as discussed in [Supported
radices](#supported-radices), below, the entire non-computer world uses
decimal numbers almost exclusively. There is ample anecdotal evidence
that many people are dismayed and confused if they are confronted with
non-decimal radices.
...
attempt is being made to line up columns of numbers. But since
`8`{.interpreted-text role="pep"} specifically discourages the use of
spaces to try to align Python code, one would suspect the same argument
should apply to the use of leading zeros for the same purpose.
...
c)  let them continue to think that computers are happy with multi-digit
...
experience to the JavaScript interpreter in the browser, and let them
try to debug their issue there.

## Supported radices

This PEP proposes that the supported radices for the Python language
...
communicate with humans. One of the first modern computers, ENIAC[^4]
...
\"binary coded decimal\" (BCD)[^5], a representation which devotes 4
...
Even if the ability of Python to communicate binary information to
...
separate subject for a different PEP, as \'j\' for complex numbers,
\'e\' for exponent, and \'r\' for raw string (to name a few) already
support uppercase.
...
- It should be as compatible with other languages and previous versions
  of Python as is reasonable, both for the input syntax and for the
  output (e.g. string % operator) syntax.
...
Binary formatting is not being added to the % operator because
`3101`{.interpreted-text role="pep"} (Advanced String Formatting)
already supports binary, % formatting will be deprecated in the future.
...
numeric and string special modifiers, such as \'x\' for hexadecimal,
\'r\' for raw strings, \'e\' for exponentiation, and \'j\' for complex
numbers. This is an issue for a separate PEP.
...
\"alternate form\" of the string % operator to support uppercase \'B\'
or \'O\' characters for binary or octal output. Currently,
`3101`{.interpreted-text role="pep"} does not even support this
alternate capability, and the hex() function does not allow the
programmer to specify the case of the \'x\' character.

There are still some strong feelings that \'0123\' should be allowed as
a literal decimal in Python 3.0. If this is the right thing to do, this
