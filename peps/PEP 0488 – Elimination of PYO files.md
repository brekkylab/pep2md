...
expression defined by `3147`{.interpreted-text role="pep"}[^3],[^4]:
...
single interpreter. The \"opt-\" prefix was chosen over \"o\" so as to
be somewhat self-documenting. The \"opt-\" prefix was chosen over \"O\"
so as to not have any confusion in case \"0\" was the leading prefix of
...
never look at the contents of the \_\_pycache\_\_ directory and so this
won\'t be an issue.
...
bytecode file name semantics[^8],[^9]. The CLI for the `compileall`
...
Python 3.2 \-- including all of Python 2 \-- there was no
\_\_pycache\_\_ which already necessitates bifurcating bytecode file
handling support). If code was setting the `debug_override` argument to
...
Python 3.5 to support `465`{.interpreted-text role="pep"} there is no
need to support pre-existing `.pyo` files[^10].
