...
source_path: https://github.com/python/peps/blob/main/peps/pep-0498.rst
source_commit: 2e33996f7fd17ee6b02a74476b9b8e0deea6ef72
---

# Abstract

Python supports multiple ways to format text strings. These include
%-formatting[^1], `str.format()`[^2], and `string.Template` [^3]. Each
of these methods have their advantages, but in addition have
...
code, an f-string is a literal string, prefixed with \'f\', which
...
A similar feature was proposed in `215`{.interpreted-text role="pep"}.
`215`{.interpreted-text role="pep"} proposed to support a subset of
Python expressions, and did not support the type-specific string
formatting (the `__format__()` method) which was introduced with
`3101`{.interpreted-text role="pep"}.
...
`3101`{.interpreted-text role="pep"} for a detailed rationale. This PEP
reuses much of the `str.format()` syntax and machinery, in order to
provide continuity with an existing Python string formatting mechanism.
...
used. \'f\' may be combined with \'r\' or \'R\', in either order, to
produce raw f-string literals. \'f\' may not be combined with \'b\':
this PEP does not propose to add binary f-strings. \'f\' may not be
combined with \'u\'.
...
into their associated Unicode characters[^6].
...
A quick search of Python\'s standard library shows only a handful of
...
This idea has been proposed in the past, most recently in
`461#proposed-variations`{.interpreted-text role="pep"}. The discussions
of such a feature usually suggest either
...
The \'u\' prefix was added to Python 3.3 in `414`{.interpreted-text
role="pep"} as a means to ease source compatibility with Python 2.7.
Because Python 2.7 will never support f-strings, there is nothing to be
gained by being able to combine the \'f\' prefix with \'u\'.
