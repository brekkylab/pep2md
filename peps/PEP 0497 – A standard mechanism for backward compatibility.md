...
support of `5`{.interpreted-text role="pep"}, \"Guidelines for Language
Evolution\". `236`{.interpreted-text role="pep"}, \"Back to the
`__future__`\", introduced a mechanism for forward compatibility in
support of `5`{.interpreted-text role="pep"} but noted that a new
mechanism for backward compatibility was outside the scope of that PEP.
A related PEP (in progress) introduces such a mechanism for backward
compatibility.

`5`{.interpreted-text role="pep"}, \"Guidelines for Language
Evolution\", notes that \"This PEP \[`5`{.interpreted-text role="pep"}\]
does not replace or preclude other compatibility strategies such as
dynamic loading of backwards-compatible parsers.\"

# Context

From `236`{.interpreted-text role="pep"}: \"From time to time, Python
makes an incompatible change to the advertised semantics of core
language constructs, or changes their accidental
...
disrupting. `5`{.interpreted-text role="pep"}, Guidelines for Language
Evolution, suggests ways to ease the pain, and this PEP
\[`236`{.interpreted-text role="pep"}\] introduces some machinery in
support of that.\"

Also from `236`{.interpreted-text role="pep"}: \"The purpose of
future_statement is to make life easier for people who keep current with
the latest release in a timely fashion. We don\'t hate you if you
don\'t, but your problems are much harder to solve, and somebody with
those problems will need to write a PEP addressing them.
future_statement is aimed at a different audience.\"

# The current situation

When an incompatible change to core language syntax or semantics is
being made, Python currently provides the future_statement mechanism for
providing forward compatibility until the release that enforces the new
syntax or semantics, but provides no corresponding standard mechanism
...
in `238`{.interpreted-text role="pep"} in 2001, soon after
`236`{.interpreted-text role="pep"} introduced the future_statement
mechanism. `238`{.interpreted-text role="pep"} outlines a suite of
useful forward-compatibility mechanisms for \"true division\" in the
Python 2.x series but omits to include any backward-compatibility
mechanisms for after \"true division\" was first enforced in Python 3.0.
Python versions since 3.0 do not provide a backward compatibility
mechanism such as `from __past__ import division` for code that expects
the old \"classic division\" semantics, whereas Python versions prior to
3.0 do support both \"classic division\" code and also forward
compatibility with code expecting \"true division\". A further
consequence of this is that the \"most compatible\" interpreter with
respect to the variety of division-related Python code in the wild is
Python 2.7, the version before the breaking change was first enforced.
...
> `5`{.interpreted-text role="pep"} be augmented with a 6th step in the
> section \"Steps for Introducing Backwards-Incompatible Features\" to
> indicate that, when an incompatible change to core language syntax or
> semantics is being made, Python-dev\'s policy is to prefer and expect
> that, wherever possible, a mechanism for backward compatibility be
...
> for forward compatibility such as new future_statements. Furthermore,
> `387`{.interpreted-text role="pep"}, \"Backwards Compatibility
...
considered incomplete. `238`{.interpreted-text role="pep"} notes the
\"severe backwards compatibility issues\" raised by the proposal and
describes several measures for forward compatibility in the Abstract and
API Changes sections. It also mentions some backward compatibility ideas
raised on c.l.py, including \"Use `from __past__ import division` to use
...
`238`{.interpreted-text role="pep"}, because of its large-scale
compatibility implications, would also be accompanied by a backward
compatibility plan that enables users of future Python versions after
the breaking change has come into effect to re-enable the classic
division behaviour easily in their code.

# Proposal - part 2

The second proposal is that:

> Python provide a standard backward compatibility mechanism in parallel
> to the `__future__` module mechanism for forward compatibility.
...
those outlined in `296`{.interpreted-text role="pep"} for `__future__`.
Specifically:
...
knowledge of the specific `__past__` features invoked, or even that the
`__past__` mechanism for backward-compatibility exists.
