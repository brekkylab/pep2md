...
source_path: https://github.com/python/peps/blob/main/peps/pep-0575.rst
source_commit: ada8c4cce348e46acbcb401a46aa96331acf9d77
---

# Withdrawal notice

See `580`{.interpreted-text role="pep"} for a better solution to
allowing fast calling of custom classes.

See `579`{.interpreted-text role="pep"} for a broader discussion of some
of the other issues from this PEP.
...
way for `573`{.interpreted-text role="pep"}.
...
4.  The field `ml_doc` and the attributes `__doc__` and
    `__text_signature__` (see `Argument Clinic <436>`{.interpreted-text
    role="pep"}) are not supported.
...
1.  `m_ml` points to a `PyMethodDef` structure, extending
    ...
    Note that `PyMethodDef` is part of the
    `Python Stable ABI <384>`{.interpreted-text role="pep"} and it is
    used by practically all extension modules, so we absolutely cannot
    change this structure.

2.  `Argument Clinic <436>`{.interpreted-text role="pep"} is supported.
...
- For methods which bind instances of `base_function` (more precisely,
  ...
  slicing](#self-slicing), we instead use the `PyCFunctionDef` from the
  original function. This way, we don\'t lose any performance when
  calling bound methods. In this case, the `__func__` attribute is only
  used to implement various attributes but not for calling the method.
...
python-ideas[^5] the consensus was to keep this misfeature of built-in
functions.
...
- `int PyBaseFunction_CheckFast(PyObject *op)`: return true if `op` is
  an instance of a class with the `Py_TPFLAGS_BASEFUNCTION` set. This is
...
- For many existing `PyCFunction_...` and `PyMethod_` functions, we
  ...
  instances. The old functions are kept as aliases of the new functions.
...
Apart from that, no changes to the `types` module are made. In
...
## Why \_\_text_signature\_\_ is not a solution
...
for example needed to support `573`{.interpreted-text role="pep"}.
Converting existing methods to use `METH_PASS_FUNCTION` is trivial: it
only requires adding an extra argument to the C function.
...
These are of type
[PyCFunction_Type](https://github.com/python/cpython/blob/2cb4661707818cfd92556e7fdf9068a993577002/Objects/methodobject.c#L271)
with structure
...
These are of type
[PyMethodDescr_Type](https://github.com/python/cpython/blob/2cb4661707818cfd92556e7fdf9068a993577002/Objects/descrobject.c#L538)
with structure
...
These are of type
[PyFunction_Type](https://github.com/python/cpython/blob/2cb4661707818cfd92556e7fdf9068a993577002/Objects/funcobject.c#L592)
with structure
...
These are of type
[PyMethod_Type](https://github.com/python/cpython/blob/2cb4661707818cfd92556e7fdf9068a993577002/Objects/classobject.c#L329)
with structure
...
[^6]: Python bug 33261, inspect.isgeneratorfunction fails on
    hand-created methods (<https://bugs.python.org/issue33261> and
    <https://github.com/python/cpython/pull/6448>)

[^7]: Python bug 33265, contextlib.ExitStack abuses \_\_self\_\_
