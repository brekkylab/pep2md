...
python_version: '3.13'
post_history:
- '`01-Jan-2023 <https://mail.python.org/archives/list/typing-sig@python.org/thread/AKTFUYW3WDT7R7PGRIJQZMYHMDJNE4QH/>`__'
...
`typing:deprecated`{.interpreted-text role="ref"} and
`@warnings.deprecated<warnings.deprecated>`{.interpreted-text
role="external+py3.13:func"}
:::

# Abstract

This PEP adds an `@warnings.deprecated()` decorator that marks a class
or function as deprecated, enabling static checkers to warn when it is
...
`DeprecationWarning`{.interpreted-text role="exc"} warning class, which
is used to show warnings when deprecated functionality is used. This
mechanism is widely used: as of the writing of this PEP, the CPython
main branch contains about 150 distinct code paths that raise
`!DeprecationWarning`{.interpreted-text role="exc"}. Many third-party
libraries also use `!DeprecationWarning`{.interpreted-text role="exc"}
to mark deprecations. In the [top 5000 PyPI
packages](https://dev.to/hugovk/how-to-search-5000-python-projects-31gk),
there are:

- 1911 matches for the regex `warnings\.warn.*\bDeprecationWarning\b`,
  indicating use of `!DeprecationWarning`{.interpreted-text role="exc"}
  (not including cases where the warning is split over multiple lines);
...
example, the removal of various long-deprecated
`unittest`{.interpreted-text role="mod"} features had to be
[reverted](https://github.com/python/cpython/commit/b50322d20337ca468f2070eedb051a16ee1eba94)
...
and report deprecations that a single `grep` invocation could not find.
In addition, many type checkers integrate with IDEs, enabling users to
see deprecation warnings right in their editors.
...
- GCC supports a `deprecated`
  [attribute](https://gcc.gnu.org/onlinedocs/gcc-3.1.1/gcc/Type-Attributes.html)
  on function declarations. This powers CPython\'s `Py_DEPRECATED`
  macro.
- GraphQL
  [supports](https://spec.graphql.org/June2018/#sec-Field-Deprecation)
  marking fields as `@deprecated`.
- Kotlin
  [supports](https://kotlinlang.org/api/latest/jvm/stdlib/kotlin/-deprecated/)
  a `Deprecated` annotation.
- Scala
  [supports](https://www.scala-lang.org/api/2.12.5/scala/deprecated.html)
  an `@deprecated` annotation.
- Swift
  [supports](https://docs.swift.org/swift-book/ReferenceManual/Attributes.html)
  using the `@available` attribute to mark APIs as deprecated.
- TypeScript
  [uses](https://www.typescriptlang.org/docs/handbook/jsdoc-supported-types.html#deprecated)
  the `@deprecated` JSDoc tag to issue a hint marking use of deprecated
  functionality.

Several users have requested support for such a feature:

- [typing-sig
  thread](https://mail.python.org/archives/list/typing-sig@python.org/thread/E24WTMQUTGKPFKEXVCGGEFFMG7LDF3WT/)
- [Pyright feature
  request](https://github.com/microsoft/pyright/discussions/2300)
- [mypy feature request](https://github.com/python/mypy/issues/11439)

There are similar existing third-party tools:

- [Deprecated](https://pypi.org/project/Deprecated/) provides a
  decorator to mark classes, functions, or methods as deprecated. Access
  to decorated objects raises a runtime warning, but is not detected by
  type checkers.
- [flake8-deprecated](https://pypi.org/project/flake8-deprecated/) is a
  linter plugin that warns about use of deprecated features. However, it
  is limited to a short, hard-coded list of deprecations.

# Specification

A new decorator `@deprecated()` is added to the
`warnings`{.interpreted-text role="mod"} module. This decorator can be
used on a class, function or method to mark it as deprecated. This
includes `typing.TypedDict`{.interpreted-text role="class"} and
`typing.NamedTuple`{.interpreted-text role="class"} definitions. With
overloaded functions, the decorator may be applied to individual
overloads, indicating that the particular overload is deprecated. The
decorator may also be applied to the overload implementation function,
indicating that the entire function is deprecated.
...
The positional-only argument is of type `str` and contains a message
that should be shown by the type checker when it encounters a usage of
the decorated object. Tools may clean up the deprecation message for
display, for example by using `inspect.cleandoc`{.interpreted-text
role="func"} or equivalent logic. The message must be a string literal.
The content of deprecation messages is up to the user, but it may
include the version in which the deprecated object is to be removed, and
information about suggested replacement APIs.
...
- Any syntax that indirectly triggers a call to the function. For
  example, if the `__add__` method of a class `C` is deprecated, then
  the code `C() + C()` should trigger a diagnostic. Similarly, if the
  setter of a property is marked deprecated, attempts to set the
  property should trigger a diagnostic.

If a method is marked with the `typing.override`{.interpreted-text
role="func"} decorator from `698`{.interpreted-text role="pep"} and the
base class method it overrides is deprecated, the type checker should
produce a diagnostic.

There are additional scenarios where deprecations could come into play.
For example, an object may implement a
`typing.Protocol`{.interpreted-text role="class"}, but one of the
...
- `category`: A warning class. Defaults to
  `DeprecationWarning`{.interpreted-text role="exc"}. If this is set to
  `None`, no warning is issued at runtime and the decorator returns the
...
- `stacklevel`: The number of stack frames to skip when issuing the
  ...
  implementation will add the number of stack frames it uses in wrapper
  code.
...
methods. Type checkers may show a warning if `@deprecated` is used
without `category=None` in these cases.
...
For compatibility with `typing.get_overloads`{.interpreted-text
role="func"}, the `@deprecated` decorator should be placed after the
`@overload` decorator.
...
We propose that CPython\'s deprecation policy (`387`{.interpreted-text
role="pep"}) is updated to require that new deprecations use the
functionality in this PEP to alert users about the deprecation, if
possible. Concretely, this means that new deprecations should be
accompanied by a change to the `typeshed` repo to add the `@deprecated`
decorator in the appropriate place. This requirement does not apply to
deprecations that cannot be expressed using this PEP\'s functionality.
...
self-explanatory. Usage of the `@deprecated` decorator will be an
advanced feature mostly relevant to library authors. The decorator
should be mentioned in relevant documentation (e.g.,
`387`{.interpreted-text role="pep"} and the
`DeprecationWarning`{.interpreted-text role="exc"} documentation) as an
additional way to mark deprecated functionality.

# Reference implementation

A runtime implementation of the `@deprecated` decorator is available in
the [typing-extensions](https://pypi.org/project/typing-extensions/)
library since version 4.5.0. The `pyanalyze` type checker has [prototype
support](https://github.com/quora/pyanalyze/pull/578) for emitting
deprecation errors, as does
[Pyright](https://github.com/microsoft/pyright/issues/4456).
...
[examined](https://gist.github.com/JelleZijlstra/ff459edc5ff0918e22b56740bb28eb8b)
...
- 74 deprecations of functions, methods and classes (supported by this
  PEP)
- 28 deprecations of whole modules (largely due to
  `594`{.interpreted-text role="pep"})
...
- 38 deprecations that are not easily detectable in the type system (for
  example, for calling `asyncio.get_event_loop`{.interpreted-text
  role="func"} without an active event loop)
...
decorator in the `typing`{.interpreted-text role="mod"} module. However,
there was feedback that it would be unexpected for a decorator in the
`typing`{.interpreted-text role="mod"} module to have runtime behavior.
Therefore, the PEP now proposes adding the decorator the
`warnings`{.interpreted-text role="mod"} module instead.
