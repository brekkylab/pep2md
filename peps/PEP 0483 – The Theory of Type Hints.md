...
source_path: https://github.com/python/peps/blob/main/peps/pep-0483.rst
source_commit: 3f37c585414f04895e0695c00de880097d78d6ff
---

# Abstract

This PEP lays out the theory referenced by `484`{.interpreted-text
role="pep"}.
...
- Objects, classes defined with a class statement, and instances are
  denoted using standard `8`{.interpreted-text role="pep"} conventions.
- the symbol `==` applied to types in the context of this PEP means that
  two expressions represent the same type.
- Note that `484`{.interpreted-text role="pep"} makes a distinction
  between types and classes (a type is a concept for the type checker,
  while a class is a runtime concept). In this PEP we clarify this
  distinction but avoid unnecessary strictness to allow more flexibility
...
variables and functions using `3107`{.interpreted-text role="pep"}
syntax. These annotations can be used to avoid many kind of bugs, for
documentation purposes, or maybe even to increase speed of program
execution. Here we only focus on avoiding bugs by using a static type
checker.
...
number only works with `List[float]` so that the second condition fails:
...
# Summary of gradual typing {#pep-483-gradual-typing}
...
Typing](http://wphomes.soic.indiana.edu/jsiek/what-is-gradual-typing/)
...
this to `object` \-- it is not consistent with most types (e.g. you
can\'t use an `object()` instance where an `int` is expected). IOW both
`Any` and `object` mean \"any type is allowed\" when used to annotate an
...
type, and it is not a goal of `484`{.interpreted-text role="pep"}. *The
static types described in* `484`{.interpreted-text role="pep"} *should
not be confused with the runtime classes.* Examples:
...
- **Union\[t1, t2, \...\]**. Types that are subtype of at least one of
  ...
  - Unions whose components are all subtypes of `t1` etc. are subtypes
    of this. Example: `Union[int, str]` is a subtype of
    `Union[int, float, str]`.
...
- **Optional\[t1\]**. Alias for `Union[t1, None]`, i.e.
  `Union[t1, type(None)]`.
- **Tuple\[t1, t2, \..., tn\]**. A tuple whose items are instances of
  ...
  - A variadic homogeneous tuple type can be written `Tuple[t1, ...]`.
    (That\'s three dots, a literal ellipsis; and yes, that\'s a valid
    token in Python\'s syntax.)
- **Callable\[\[t1, t2, \..., tn\], tr\]**. A function with positional
  argument types `t1` etc., and return type `tr`. The argument list may
  be empty `n==0`. There is no way to indicate optional or keyword
...
- **Intersection\[t1, t2, \...\]**. Types that are subtype of *each* of
  ...
  - An intersection of fewer types is a supertype of an intersection of
    more types, e.g. `Intersection[int, str]` is a supertype of
  ...
  - The interaction between `Intersection` and `Union` is complex but
    ...
    intersections and unions of regular sets (note that sets of types
    can be infinite in size, since there is no limit on the number of
    new subclasses).
...
- The following function can take two arguments of type `int` and return
  an `int`, or take two arguments of type `float` and return a `float`,
  etc.:
...
`484`{.interpreted-text role="pep"}, the [mypy docs on
generics](http://mypy.readthedocs.io/en/latest/generics.html), and
...
- Other things, e.g. overloading and stub modules, see
  `484`{.interpreted-text role="pep"}.

## Predefined generic types and Protocols in typing.py

(See also the [typing.py
module](https://github.com/python/typing/blob/master/src/typing.py).)
