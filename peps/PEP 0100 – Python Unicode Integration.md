...
source_path: https://github.com/python/peps/blob/main/peps/pep-0100.rst
source_commit: 325d63e4fb8d21f5121b0d4515b1c5c0b475c308
---
...
fundamental objects in Python \-- we expect this proposal to undergo
...
- Unicode encoding names should be lower case on output and
  case-insensitive on input (they will be converted to lower case by all
  APIs taking an encoding name as input).
...
\<default encoding\> can be set and queried using the two sys module
APIs:
...
:   Sets the \<default encoding\> used by the Unicode implementation.
    ...
    Note: This API is only available in site.py! It is removed from the
    sys module by site.py after usage.
...
- all other characters represent themselves as Unicode ordinal (e.g.
  \'b\' -\> U+0062)
...
In containment tests (\'a\' in u\'abc\' and u\'a\' in \'abc\') both
sides should be coerced to Unicode before applying the test. Errors
occurring during coercion (e.g. None in u\'abc\') should not be masked.
...
They provide not only templates for use by encoding module implementors,
but also define the interface which is expected by the Unicode
implementation.
...
accidentally writing binary data to an output stream which the other end
might not recognize.
...
:   For Unicode objects: return a pointer to the object\'s \<defenc\>
    buffer (which uses the \<default encoding\>).

\"s#\"

:   Access to the default encoded version of the Unicode object (see
...
:   Takes two parameters: encoding (`const char *`) and buffer
    ...
    The input object is first coerced to Unicode in the usual way and
    then encoded into a string using the given encoding.

    On output, a buffer of the needed size is allocated and returned
    through `*buffer` as NULL-terminated string. The encoded may not
    contain embedded NULL characters. The caller is responsible for
...
:   Takes three parameters: encoding (`const char *`), buffer
    ...
    written (excluding the trailing NULL-byte). The output buffer is
    assured to be NULL-terminated.
...
- Encodings:
  ...
  - UTF-7: Defined in `2152`{.interpreted-text role="rfc"}
  - UTF-8: Defined in `2279`{.interpreted-text role="rfc"}
...
- changed encode Codec method to be symmetric to the decode method (they
  both return (object, data consumed) now and thus become
  interchangeable);
...
- made the Codec design more generic w/r to type of input and output
  objects;
- changed `StreamWriter.flush` to `StreamWriter.reset` in order to avoid
  overriding the stream\'s `.flush()` method;
...
- added Whitespace section and included references for Unicode
  characters that have the whitespace and the line break characteristic;
