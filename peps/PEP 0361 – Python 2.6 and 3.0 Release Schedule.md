...
See `3000`{.interpreted-text role="pep"} and `3100`{.interpreted-text
role="pep"} for details on the Python 3.0 project.

# Completed features for 2.6

PEPs:

- `352`{.interpreted-text role="pep"}: Raising a string exception now
  triggers a `TypeError`. Attempting to catch a string exception raises
  `DeprecationWarning`. `BaseException.message` has been deprecated.
- `358`{.interpreted-text role="pep"}: The \"bytes\" Object
- `366`{.interpreted-text role="pep"}: Main module explicit relative
  imports
...
- `3127`{.interpreted-text role="pep"}: Integer Literal Support and
  Syntax
- `371`{.interpreted-text role="pep"}: Addition of the multiprocessing
  package
...
- removal of modules because of `4`{.interpreted-text
  role="pep"}/`3100`{.interpreted-text
  role="pep"}/`3108`{.interpreted-text role="pep"}
...
[^1]: Adding a \_\_dir\_\_() magic method
