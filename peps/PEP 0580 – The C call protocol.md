...
This PEP is rejected in favor of `590`{.interpreted-text role="pep"},
which proposes a simpler public C API for callable objects.
...
For more background and motivation, see `579`{.interpreted-text
role="pep"}.
...
the rationale of `573`{.interpreted-text role="pep"} for details.
...
`stable ABI <384>`{.interpreted-text role="pep"}). Therefore, changing
the `tp_print` field from a `printfunc` (a function pointer) to a
`Py_ssize_t` should not be a problem, even if this changes the memory
layout of the `PyTypeObject` structure. Moreover, on all systems for
which binaries are commonly built (Windows, Linux, macOS), the size of
`printfunc` and `Py_ssize_t` are the same, so the issue of binary
compatibility will not come up anyway.
...
- If `cr_self` is NULL, then `func.__get__(obj, cls)(*args, **kwds)`
  ...
  slicing](#self-slicing): `obj` may be passed as `self` argument to the
...
The following function is added (also to the
`stable ABI <384>`{.interpreted-text role="pep"}):
...
part of the `stable ABI <384>`{.interpreted-text role="pep"}, it is
highly unlikely to be used that way: first of all, it is not even
documented. Second, the flag `METH_FASTCALL` is not part of the stable
ABI but it is very common (because of Argument Clinic). So, if one
cannot support `METH_FASTCALL`, it is hard to imagine a use case for
...
The function `PyCFunction_ClsNew` is added to the
`stable ABI <384>`{.interpreted-text role="pep"}.
...
Given that this is not even part of the public API (see also
`579`{.interpreted-text role="pep"}, issue 6), it would be strange to
add anything else from the C call protocol to the stable ABI.
...
One of the major complaints of `575`{.interpreted-text role="pep"} was
that is was coupling functionality (the calling and introspection
protocol) with the class hierarchy: a class could only benefit from the
new features if it was a subclass of `base_function`. It may be
difficult for existing classes to do that because they may have other
constraints on the layout of the C object structure, coming from an
existing base class or implementation details. For example,
`functools.lru_cache` cannot implement `575`{.interpreted-text
role="pep"} as-is.
...
`576`{.interpreted-text role="pep"} is an alternative approach to
solving the same problem as this PEP. See
<https://mail.python.org/pipermail/python-dev/2018-July/154238.html> for
comments on the difference between `576`{.interpreted-text role="pep"}
and `580`{.interpreted-text role="pep"}.
...
implements `functools.lru_cache` using `580`{.interpreted-text
role="pep"}: <https://github.com/jdemeyer/cpython/tree/lru580>
...
[^2]: Add tp_fastcall to PyTypeObject: support FASTCALL calling
