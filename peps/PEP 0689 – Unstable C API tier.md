...
python_version: '3.12'
post_history:
- '`27-Apr-2022 <https://mail.python.org/archives/list/python-dev@python.org/thread/PQXSP7E2B6KNXTJ2AERWMKKX42YP5D6O/>`__'
...
source_path: https://github.com/python/peps/blob/main/peps/pep-0689.rst
source_commit: 980e18de902655a3581c8ee53681943029e680db
---

::: canonical-doc
`devguide:c-api`{.interpreted-text role="ref"}

User-facing documentation is at
`py3.12:unstable-c-api`{.interpreted-text role="ref"}.
:::
...
The Python C-API is currently divided into [three stability
tiers](https://devguide.python.org/developer-workflow/c-api/index.html):

- Limited API, with high compatibility expectations
- Public API, which follows the `backwards compatibility policy
  <387>`{.interpreted-text role="pep"}, and requires deprecation
...
and also often change in practice. The unstable tier should make their
status obvious even to people who don\'t read the docs carefully enough,
making them hard to use accidentally.
...
- API that may change between minor releases, as in the Unstable tier
  proposed here (e.g. functions introduced in `523`{.interpreted-text
  role="pep"}).
...
The C API is divided by stability expectations into [three
"sections"](https://devguide.python.org/developer-workflow/c-api/index.html)
(internal, public, and limited). We\'ll now call these *stability
tiers*, or *tiers* for short.
...
- Unstable API should have no backwards-incompatible changes across
  patch releases, but may change or be removed in minor releases (3.x.0,
...
- Backwards-incompatible changes to these APIs should be made so that
  ...
  version (e.g. arguments should be added/removed, or a function should
  be renamed, but the semantic meaning of an argument should not
  change).
...
- To move an API from the public tier to the unstable tier, it should be
  ...
  API. Per Python\'s backwards compatibility policy
  (`387`{.interpreted-text role="pep"}), this deprecation needs to last
  *at least* two releases (without an SC exceptions). But it can also
  last indefinitely \-- for example, if `590`{.interpreted-text
  role="pep"}\'s
  `“provisional” <590#finalizing-the-api>`{.interpreted-text role="pep"}
  ...
  deprecated name) is allowed without an SC exception, as if the
  ...
  For examples, see the [initial unstable API](#initial-unstable-api)
...
These rules will be documented in the
[devguide](https://devguide.python.org/developer-workflow/c-api/index.html),
and [user documentation](https://docs.python.org/3/c-api/stable.html)
...
- API introduced before Python 3.12 that is documented or widely used
  ...
  few years core devs should do some research before changing
...
a CPython issue to request exposing public API for their use case, and
...
Code extra information (`523`{.interpreted-text role="pep"}):
...
`PyUnstable` prefix. Instead, defining `Py_USING_UNSTABLE_API` made the
API available in a given source file, signifying acknowledgement that
