...
bdfl_delegate: Mark Mangoba <mmangoba@python.org>
...
source_path: https://github.com/python/peps/blob/main/peps/pep-0541.rst
source_commit: d980971c881cce699c74e388fb2bbc07bb1281cb
---

# Abstract

This PEP proposes an extension to the Terms of Use[^1] of the Package
Index[^2], clarifying expectations of package owners regarding ownership
of a package name on the Package Index, specifically with regards to
conflict resolution.

Existing package repositories such as CPAN[^3], NPM[^4], and GitHub[^5]
...
## Continued maintenance of an abandoned project {#continue-maintenance}
...
## Removal of an abandoned project {#reclaim-name}
...
2.  User A owns a project X outside the Package Index. User B creates a
    ...
    is true even if User A\'s project X gains notability and the User
    B\'s project X is not notable.
...
- project is malware (designed to exploit or harm systems or users
  directly, to facilitate command-and-control attacks, or perform data
  exfiltration);
...
appropriate response. A package can be removed or transferred to a new
owner at the sole discretion of the Python Software Foundation to
...
2.  Check the criteria above to see when a transfer is allowed. In
    particular, the criteria for [reusing a name for a different
    project](#reclaim-name) are more stringent than for [continuing
    maintenance of the same project](#continue-maintenance) - although
    it\'s not easy to get a name transferred in either case.
3.  Search the [PyPI Support
    issues](https://github.com/pypa/pypi-support/issues) to see if
    anyone else is already requesting the same name.
4.  If all the criteria are met to transfer ownership of the name, [open
    a new
    issue](https://github.com/pypa/pypi-support/issues/new?labels=PEP+541&template=pep541-request.yml&title=PEP+541+Request%3A+PROJECT_NAME)
...
[Package Name Disputes](https://www.npmjs.com/policies/disputes). It is
described as a \"living document\", as of January 2017 its contents
