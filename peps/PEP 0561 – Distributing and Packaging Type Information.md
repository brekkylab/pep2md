...
source_path: https://github.com/python/peps/blob/main/peps/pep-0561.rst
source_commit: 97ced2cbb1051a52e62d4a2f2c77ddff3a17aac1
---
...
`484`{.interpreted-text role="pep"} introduced type hinting to Python,
with goals of making typing gradual and easy to adopt. Currently, typing
information must be distributed manually. This PEP provides a
standardized means to leverage existing tooling to package and
distribute type information with minimal work and an ordering for type
checkers to resolve modules and collect this information for type
checking.
...
`484`{.interpreted-text role="pep"} has a brief section on distributing
typing information. In this
`section <484#storing-and-distributing-stub-files>`{.interpreted-text
role="pep"} the PEP recommends using `shared/typehints/pythonX.Y/` for
...
\"inline\" - the types are part of the runtime code using
`526`{.interpreted-text role="pep"} and `3107`{.interpreted-text
role="pep"} syntax (the filename ends in `.py`).
...
distribute a release. (`426`{.interpreted-text role="pep"})
...
`spec of PEP 484 <484#storing-and-distributing-stub-files>`{.interpreted-text
role="pep"}.
...
For namespace packages (see `420`{.interpreted-text role="pep"}), the
`py.typed` file should be in the submodules of the namespace, to avoid
conflicts and for clarity.
...
For namespace packages (see `420`{.interpreted-text role="pep"}),
stub-only packages should use the `-stubs` suffix on only the root
namespace package. All stub-only namespace packages should omit
`__init__.pyi` files. `py.typed` marker files are not necessary for
stub-only packages, but similarly to packages with inline types, if
used, they should be in submodules of the namespace to avoid conflicts
and for clarity.
...
## Type Checker Module Resolution Order {#mro}
...
a `py.typed` file. For stub-packages distributing within a namespace
package (see `420`{.interpreted-text role="pep"}), the `py.typed` file
should be in the submodules of the namespace.
...
[\[typed_package\]](#typed_package){.citation}, as well as a
[\[stub_package\]](#stub_package){.citation}. A sample package checker
[\[pkg_checker\]](#pkg_checker){.citation} which reads the metadata of
installed packages and reports on their status as either not typed,
inline typed, or a stub package.

The mypy type checker has an implementation of `561`{.interpreted-text
role="pep"} searching which can be read about in the mypy docs[^2].
...
- 2023-01-13
  ...
  > - Clarify that the 4th step of the
  >   `Module Resolution Order <mro>`{.interpreted-text role="ref"}
...
- 2018-02-02
  ...
  > - Change stub-only package suffix to be -stubs not [stubs]{#stubs}.
...
- 2017-11-10
  ...
  > - Removed stub-only packages and merged into third party packages
  >   spec.
  > - Removed suggestion for typecheckers to consider checking runtime
  >   versions
