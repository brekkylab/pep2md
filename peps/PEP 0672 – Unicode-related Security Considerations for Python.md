...
source_path: https://github.com/python/peps/blob/main/peps/pep-0672.rst
source_commit: 5beaf9ff5cdb4b4f34e9e7cfc9165606c3f59060
---
...
[Unicode](https://home.unicode.org/) is a system for handling all kinds
of written language. It aims to allow any character from any human
language to be used. Python code may consist of almost all valid Unicode
characters. While this allows programmers from all around the world to
express themselves, it also allows writing code that is potentially
confusing to readers.

It is possible to misuse Python\'s Unicode-related features to write
code that *appears* to do something else than what it does. Evildoers
could take advantage of this to trick code reviewers into accepting
malicious code.
...
in Unicode text and source code, see Unicode technical reports
[\[tr36\]](#tr36){.citation}, [\[tr39\]](#tr39){.citation}, and
[\[tr55\]](#tr55){.citation}. (Note that Python does not necessarily
conform to these specifications.)

# Acknowledgement

Investigation for this document was prompted by
...
pairs (`\r\n`) as an end of line characters. Most code editors do as
well, but there are editors that display "non-native" line endings as
unknown characters (or nothing at all), rather than ending the line,
...
variable names). See `3131`{.interpreted-text role="pep"} for details
and rationale. Only "letters and numbers" are allowed, so while `γάτα`
is a valid Python identifier, `🐱` is not. (See [Identifiers and
...
"confusables". For example, the uppercase versions of the Latin `b`,
Greek `β` (Beta), and Cyrillic `в` (Ve) often look identical: `B`, `Β`
and `В`, respectively.
...
digit can be used. For example `߅` (`NKO DIGIT FIVE`) or `௫`
...
- In the statement `قيمة = ערך`, the variable `قيمة` is set to the value
  of `ערך`.
- In the statement `قيمة - (ערך ** 2)`, the value of `ערך` is squared
  and then subtracted from `قيمة`. The *opening* parenthesis is
  displayed as `)`.
...
`x` interspersed with 100 invisible marks), but under Unicode rules for
general text, it is rendered as `s = "x"` followed by an ASCII-only
comment:
...
either ended by a dedicated character, or until the end of line.
(Unicode specifies the effect to last until the end of a "paragraph"
(see [Unicode Bidirectional
Algorithm](http://www.unicode.org/reports/tr9/)), but allows tools to
interpret newline characters as paragraph ends (see Unicode [Newline
Guidelines](http://www.unicode.org/versions/Unicode14.0.0/ch05.pdf#G10213)).
...
Python strings are collections of *Unicode codepoints*, not
"characters".
...
This normalization applies *only* to identifiers, however. Functions
that treat strings as identifiers, such as `getattr`, do not perform
normalization:
...
first two lines of a file, as per [Encoding
...
encodings (see [Text
Encodings](https://docs.python.org/3/library/codecs.html#text-encodings)).
For example, with `encoding: unicode_escape`, characters like quotes or
braces can be hidden in an (f-)string, with many tools (syntax
highlighters, linters, etc.) considering them part of the string. For
example:
...
[tr36]{#tr36 .citation-label}

:   Unicode Technical Report #36: Unicode Security Considerations
    <http://www.unicode.org/reports/tr36/>

[tr39]{#tr39 .citation-label}

:   Unicode® Technical Standard #39: Unicode Security Mechanisms
    <http://www.unicode.org/reports/tr39/>

[tr55]{#tr55 .citation-label}

:   Unicode Technical Report #55: Unicode Source Code Handling
