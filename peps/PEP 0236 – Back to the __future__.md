...
source_path: https://github.com/python/peps/blob/main/peps/pep-0236.rst
source_commit: b4cc6c46e5c655fba0d4d6783400f4e8a2718c76
---
...
`5`{.interpreted-text role="pep"}, Guidelines for Language Evolution
suggests ways to ease the pain, and this PEP introduces some machinery
in support of that.

`227`{.interpreted-text role="pep"}, Statically Nested Scopes is the
first application, and will be used as an example here.

# Intent

\[Note: This is policy, and so should eventually move into
`5`{.interpreted-text role="pep"}\]
...
3.  The mechanisms described in `230`{.interpreted-text role="pep"},
    Warning Framework are used to generate warnings, whenever possible,
    about constructs or operations whose meaning may[^1] change in
    release R.
...
Nested scopes (`227`{.interpreted-text role="pep"}) are being introduced
in 2.1. But under 2.1, it still prints:
...
# Standard Module \_\_future\_\_.py
...
working around this. NOTE: `264`{.interpreted-text role="pep"} later
addressed this in a flexible way, by adding optional arguments to
`compile()`.
...
future_statement is not an expression.
...
`execfile()`. A future_statement typed at an interactive shell applies
to the rest of the shell session\'s life, as if the future_statement had
appeared at the top of a module.

# Resolved Problem: Simulated Interactive Shells

Interactive shells \"built by hand\" (by tools such as IDLE and the
Emacs Python-mode) should behave like native interactive shells (see
above). However, the machinery used internally by native interactive
shells has not been exposed, and there isn\'t a clear way for tools
building their own interactive shells to achieve the desired behavior.

NOTE: `264`{.interpreted-text role="pep"} later addressed this, by
adding intelligence to the standard `codeop.py`. Simulated shells that
don\'t use the standard library shell helpers can get a similar effect
by exploiting the new optional arguments to `compile()` added by
`264`{.interpreted-text role="pep"}.

# Questions and Answers

## What about a \"from \_\_past\_\_\" version, to get back *old* behavior?
...
Outside the scope of this PEP, although `5`{.interpreted-text
role="pep"} suggests a grace period there too, and the future_statement
may also have a role to play there.
...
[^2]: This ensures that a future_statement run under a release prior to
    ...
    transported to a release prior to 2.1, a runtime error will be
    raised because of the failure to import `__future__` (no such module
    existed in the standard distribution before the 2.1 release, and the
    double underscores make it a reserved name).
