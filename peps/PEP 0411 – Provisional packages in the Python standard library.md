...
source_path: https://github.com/python/peps/blob/main/peps/pep-0411.rst
source_commit: a0669db3d2c30769f89358fcd8130f0b8c94672f
---
...
development team explicitly states that no promises are made with
regards to the stability of the package\'s API, which may change for the
next release. While it is considered an unlikely outcome, such packages
...
provisional state, or be rejected and removed entirely from the Python
source tree. If the package ends up graduating into the stable state
after being provisional, its API may be changed according to accumulated
feedback. The core development team explicitly makes no guarantees about
API stability and backward compatibility of provisional packages.
...
The phrase \"provisional basis\" will then be a link to the glossary
term \"provisional package\", defined as:
...
> major changes to such packages are not expected, as long as they are
...
> will occur only if serious flaws are uncovered that were missed prior
> to the inclusion of the package.
>
> This process allows the standard library to continue to evolve over
> time, without locking in problematic design errors for extended
> periods of time. See `411`{.interpreted-text role="pep"} for more
> details.
...
acceptance conditions set by `2`{.interpreted-text role="pep"}.
...
inclusion in the standard library as \"provisional\" in some release
does not guarantee it will continue being part of Python in the next
release. At the same time, the bar for making changes in a provisional
package is quite high. We expect that most of the API of most
provisional packages will be unchanged at graduation. Withdrawals are
expected to be rare.
...
\"out-of-the-box\" experience - rather than being told \"oh, the
standard library tools for task X are horrible, download this 3rd party
library instead\", those superior tools are more likely to be just be an
import away.
...
- `daemon` (`3143`{.interpreted-text role="pep"})
- `ipaddr` (`3144`{.interpreted-text role="pep"})
...
- A standard image API (`368`{.interpreted-text role="pep"})
- Improved encapsulation of import state (`406`{.interpreted-text
  role="pep"})
- Standard event loop API (`3153`{.interpreted-text role="pep"})
- A binary version of WSGI for Python 3 (e.g. `444`{.interpreted-text
  role="pep"})
- Generic function support (e.g. `simplegeneric`)

# Rejected alternatives and variations

See `408`{.interpreted-text role="pep"}.
...
[^1]: <https://mail.python.org/pipermail/python-dev/2012-January/115962.html>
