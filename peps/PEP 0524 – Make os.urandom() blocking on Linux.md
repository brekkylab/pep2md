...
[#25420](http://bugs.python.org/issue25420) and
[#26839](http://bugs.python.org/issue26839).
...
platform blocking at `import random`. The issue #26839 describes a short
Python script used to compute a MD5 hash, systemd-cron, script called
very early in the init process. The system initialization blocks on this
...
- [Issue #13703: Hash collision security
  issue](http://bugs.python.org/issue13703)
- `PEP 456: Secure and interchangeable hash algorithm
  <456>`{.interpreted-text role="pep"}
...
[virtio-rng device](https://fedoraproject.org/wiki/Features/Virtio_RNG)
...
default. See: [Hardware RNG on Raspberry
Pi](http://fios.sector16.net/hardware-rng-on-raspberry-pi/).
...
`/dev/random`. See [Myths about
/dev/urandom](http://www.2uo.de/myths-about-urandom/) by Thomas Hühn:
\"Fact: /dev/urandom is the preferred source of cryptographic randomness
on UNIX-like systems\"

## getrandom(size, 0) can block forever on Linux

The origin of the Python issue #26839 is the [Debian bug report
#822431](https://bugs.debian.org/cgi-bin/bugreport.cgi?bug=822431): in
fact, `getrandom(size, 0)` blocks forever on the virtual machine. The
system succeeded to boot because systemd killed the blocked process
after 90 seconds.

Solutions like [Load entropy from disk at
boot](#load-entropy-from-disk-at-boot) reduces the risk of this bug.
...
<522>`{.interpreted-text role="pep"}.

Python should not decide for the developer how to handle [The
bug](#the-bug): raising immediately a `BlockingIOError` if
`os.urandom()` is going to block allows developers to choose how to
handle this case:
...
must be modified to handle `BlockingIOError` even if [The bug](#the-bug)
is unlikely.
...
case 2 described above: [Use Case 2: Web
server](#use-case-2-web-server). If a developer doesn\'t want to drop
`os.urandom()`, the code should be modified. Example:
...
The question is if [The bug](#the-bug) is common enough to require that
so many applications have to be modified.
...
See the [issue #27250: Add
os.urandom_block()](http://bugs.python.org/issue27250).
...
os.urandom()](#raise-blockingioerror-in-os.urandom), it doesn\'t seem
worth it to make the API more complex for a theoretical (or at least
very rare) use case.

As [Leave os.urandom() unchanged, add
os.getrandom()](#leave-os.urandom-unchanged-add-os.getrandom), the
problem is that it makes the API more complex and so more error-prone.

# Acceptance

The PEP was [accepted on 2016-08-08 by Guido van
Rossum](https://mail.python.org/pipermail/security-sig/2016-August/000101.html).
...
- [Linux:
  ...
  (Linux 3.17) \-- see also [A system call for random numbers:
  getrandom()](https://lwn.net/Articles/606141/)
- Solaris:
...
- Windows:
  [CryptGenRandom()](https://msdn.microsoft.com/en-us/library/windows/desktop/aa379942%28v=vs.85%29.aspx)
  (Windows XP)
