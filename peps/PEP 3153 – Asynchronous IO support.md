...
source_path: https://github.com/python/peps/blob/main/peps/pep-3153.rst
source_commit: eca11bc61beb7bdc641b21c0f359aade233281ef
---
...
- using a third party library, such as
  [Twisted](http://www.twistedmatrix.com/) or
...
descriptor on POSIX platforms or a Handle on Windows or some other data
structure appropriate to a particular platform. It encapsulates all of
the particular implementation details of using that platform data
structure and presents a uniform interface for application developers.
...
Specifically, this allows transports to use
[writev](http://pubs.opengroup.org/onlinepubs/009695399/functions/writev.html)
instead of
...
also known as scatter/gather IO.
...
HTTP[^1]. In practice, it tends to be primarily encapsulated in HTTP.
The protocol-transport abstraction allows us to build a stack of
...
Producers are modeled after the
[IPushProducer](http://twistedmatrix.com/documents/current/api/twisted.internet.interfaces.IPushProducer.html)
interface found in Twisted. Although there is an
[IPullProducer](http://twistedmatrix.com/documents/current/api/twisted.internet.interfaces.IPullProducer.html)
as well, it is on the whole far less interesting and therefore probably
...
In addition to the Twisted
[IPushProducer](http://twistedmatrix.com/documents/current/api/twisted.internet.interfaces.IPushProducer.html)
interface, producers have a `half_register` method which is called with
the consumer when the consumer tries to register that producer. In most
cases, this will just be a case of setting `self.consumer = consumer`,
but some producers may require more complex preconditions or behavior
when a consumer is registered. End-users are not supposed to call this
method directly.
...
\"passive\". It needs to be told, through a method call, to take action.
A producer is \"active\": it initiates those method calls. A real
producer has a symmetric relationship with its consumer. In the case of
a generator-turned-producer, only the consumer would have a reference,
and the producer is blissfully unaware of the consumer\'s existence.
...
[^1]: Sections
    [2.1](http://json-rpc.org/wiki/specification#a2.1JSON-RPCoverstreamconnections)
    and
    [2.2](http://json-rpc.org/wiki/specification#a2.2JSON-RPCoverHTTP) .
