...
python_version: '2.7'
post_history:
- '`22-Dec-2009 <https://mail.python.org/archives/list/python-dev@python.org/thread/5QWS2FXKZNNPKNZO6RLPGT5NORS5ZQ2V/>`__'
replaces: '314'
superseded_by: '566'
...
specified in `314`{.interpreted-text role="pep"}.
...
fields are \"Requires-Python\", \"Requires-External\",
\"Requires-Dist\", \"Provides-Dist\", and \"Obsoletes-Dist\". This
version also changes the \"Platform\" field. Three new fields were also
added: \"Maintainer\", \"Maintainer-email\" and \"Project-URL\".
...
`822`{.interpreted-text role="rfc"} format, any CRLF character has to be
suffixed by 7 spaces followed by a pipe (\"\|\") char. As a result, the
Description field is encoded into a folded field that can be interpreted
by `822#section-3.1.1`{.interpreted-text role="rfc"} parser.
...
field is unfolded using a `822`{.interpreted-text role="rfc"} reader.
...
and e-mail address in the legal forms for a `822`{.interpreted-text
role="rfc"} `From:` header.
...
name and e-mail address in the legal forms for a `822`{.interpreted-text
role="rfc"} `From:` header.
...
Version declarations must follow the rules described in [Version
...
described in [Version Specifiers](#version-specifiers). The
...
format specified in [Version Specifiers](#version-specifiers).
...
Version numbers must be in the format specified in [Version
Specifiers](#version-specifiers).
...
Each version number must be in the format specified in
...
include versions like \"2.5.2\" or \"2.5.3\". Pre and post releases in
that case are excluded. So in our example, versions like \"2.5a1\" are
...
Notice that some projects might omit the \".0\" suffix for the first
...
confusion between the \"2.5\" notation that represents the full range.
It is a recommended practice to use schemes of the same length for a
series to completely avoid this problem.
...
- `Requires-Dist: zope.interface (3.1.0)`: any version that starts with
  ...
  doesn\'t use more than 3 digits, it also means \"only the 3.1.0
  release\".
...
- `Requires-Dist: zope.interface (3.1,!=3.1.3)`: any version that starts
  with 3.1, excluding post or pre-releases of 3.1 *and* excluding any
  version that starts with \"3.1.3\". For this particular project, this
  means: \"any version of the 3.1 series but not 3.1.3\". This is
  equivalent to: \"\>=3.1,!=3.1.3,\<3.2\".
