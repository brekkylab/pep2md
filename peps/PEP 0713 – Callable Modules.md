...
python_version: '3.12'
post_history:
- '`23-Apr-2023 <https://discuss.python.org/t/pep-713-callable-modules/26127>`__'
resolution: https://discuss.python.org/t/26127/86
python_status: Rejected
url: https://peps.python.org/pep-0713/
source_path: https://github.com/python/peps/blob/main/peps/pep-0713.rst
source_commit: d34c75c64aadd6e613b7ca6fbaae2d4d6aef5114
---
...
as `module.__call__`. `562`{.interpreted-text role="pep"} added support
for `~object.__getattr__`{.interpreted-text role="meth"} and
`~object.__dir__`{.interpreted-text role="meth"} for modules, but
...
`from module import member`. It also results in a \"module\" object that
is missing all of the special module attributes, including `__doc__`,
...
The documentation for `callable types <types>`{.interpreted-text
role="external+python:ref"} will include modules in the list, with a
link to `~object.__call__`{.interpreted-text role="meth"}. The
`callable-types`{.interpreted-text role="external+python:ref"}
documentation will include a section covering callable modules, with
example code, similar to the section for [customizing module attribute
...
PR #103742](https://github.com/python/cpython/pull/103742).

# Rejected Ideas

Given the introduction of `__getattr__` and `__dir__`, and the proposal
to enable use of `__call__`, it was considered if it was worth allowing
use of *all* `specialnames`{.interpreted-text
role="external+python:ref"} for modules, such as `__or__` and
`__iter__`. While this would not be completely undesired, it increases
the potential for backward compatibility concerns, and these other
special methods are likely to provide less utility to library authors in
comparison to `__call__`.
