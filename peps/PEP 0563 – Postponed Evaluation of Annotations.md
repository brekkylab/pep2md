...
and have been replaced with deferred evaluation of annotations, as
proposed by `649`{.interpreted-text role="pep"} and
`749`{.interpreted-text role="pep"}.

# Abstract

`3107`{.interpreted-text role="pep"} introduced syntax for function
annotations, but the semantics were deliberately left undefined.
`484`{.interpreted-text role="pep"} introduced a standard meaning to
annotations: type hints. `526`{.interpreted-text role="pep"} defined
variable annotations, explicitly tying them with the type hinting use
case.
...
annotations on parts of a function definition. Just like default values,
annotations are evaluated at function definition time. This creates a
number of issues for the type hinting use case:
...
Postponing the evaluation of annotations solves both problems. NOTE:
`649`{.interpreted-text role="pep"} proposes an alternative solution to
the above issues, putting this PEP in danger of being superseded.

## Non-goals

Just like in `484`{.interpreted-text role="pep"} and
`526`{.interpreted-text role="pep"}, it should be emphasized that
**Python will remain a dynamically typed language, and the authors have
...
its precursors (`484`{.interpreted-text role="pep"} and
`526`{.interpreted-text role="pep"}), is predominantly motivated by the
type hinting use case.

In Python 3.8 `484`{.interpreted-text role="pep"} will graduate from
provisional status. Other enhancements to the Python programming
language like `544`{.interpreted-text role="pep"},
`557`{.interpreted-text role="pep"}, or `560`{.interpreted-text
role="pep"}, are already being built on this basis as they depend on
type annotations and the `typing` module as defined by
`484`{.interpreted-text role="pep"}. In fact, the reason
`484`{.interpreted-text role="pep"} is staying provisional in Python 3.7
is to enable rapid evolution for another release cycle that some of the
...
Note that as per `526`{.interpreted-text role="pep"}, local variable
annotations are not evaluated at all since they are not accessible
...
currently unclear pending decision on `649`{.interpreted-text
role="pep"}. In any case, use of annotations that depend upon their
eager evaluation is incompatible with both proposals and is no longer
supported.
...
annotation use can be performed. `557`{.interpreted-text role="pep"} is
one example of a recent development where evaluating type annotations at
runtime is useful.
...
The forward reference problem was discussed when `484`{.interpreted-text
role="pep"} was originally drafted, leading to the following statement
in the document:
...
in `484`{.interpreted-text role="pep"} will require their own respective
PEPs and approval.
...
up by a number of commenters. Alyssa (Nick) Coghlan benchmarked turning
...
Alyssa Coghlan got convinced to `563`{.interpreted-text role="pep"},
too, promptly beginning the mandatory bike shedding session on the name
of the `__future__` import. Many debaters agreed that `annotations`
seems like an overly broad name for the feature name. Guido van Rossum
briefly decided to call it `string_annotations` but then changed his
mind, arguing that `division` is a precedent of a broad name with a
clear meaning.
